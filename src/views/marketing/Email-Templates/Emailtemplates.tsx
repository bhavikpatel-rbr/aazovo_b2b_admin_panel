...
const initialDummyTemplates: EmailTemplateItem[] = [
    {
        ...
        categoryId: 'cat1', categoryName: 'Onboarding',
        subCategoryId: 'sub1', subCategoryName: 'Welcome Email',
        brandId: 'brandA', brandName: 'Brand A',
        variables: [ { variableName: 'firstName', variableType: 'string' }, { variableName: 'signupDate', variableType: 'date' } ],
    ...
    },
    {
        ...
        categoryId: 'cat2', categoryName: 'Transactional',
        subCategoryId: 'sub2', subCategoryName: 'Password Reset',
        variables: [ { variableName: 'resetLink', variableType: 'string' }, { variableName: 'userName', variableType: 'string' } ],
    ...
    },
     {
        ...
        categoryId: 'cat3', categoryName: 'Marketing',
        subCategoryId: 'sub4', subCategoryName: 'Newsletter', // Example
        brandId: 'brandB', brandName: 'Brand B',
        variables: [ { variableName: 'discountCode', variableType: 'string' }, { variableName: 'saleEndDate', variableType: 'date' } ],
...
const Emailtemplates = () => {
    ...
    const [allEmailTemplates, setAllEmailTemplates] =
        useState<EmailTemplateItem[]>(initialDummyTemplates)
    ...
    const nameOptions = useMemo(() => {
        ...
        return [...new Set(allEmailTemplates.map((t) => t.name))]
            .map((name) => ({ value: name, label: name }))
    }, [allEmailTemplates])

    const templateIdOptions = useMemo(() => {
         if (!Array.isArray(allEmailTemplates)) return []
        return [...new Set(allEmailTemplates.map((t) => t.templateId))]
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterTemplateIds && filterCriteria.filterTemplateIds.length > 0) {
            const ids = new Set(filterCriteria.filterTemplateIds.map(opt => opt.value));
        ...
        if (filterCriteria.filterCategories && filterCriteria.filterCategories.length > 0) {
            ...
            processedData = processedData.filter(item => item.categoryId && cats.has(item.categoryId));
        }
         if (filterCriteria.filterSubCategories && filterCriteria.filterSubCategories.length > 0) {
            const subCats = new Set(filterCriteria.filterSubCategories.map(opt => opt.value));
            processedData = processedData.filter(item => item.subCategoryId && subCats.has(item.subCategoryId));
        }
        if (filterCriteria.filterBrands && filterCriteria.filterBrands.length > 0) {
            ...
            processedData = processedData.filter(item => item.brandId && brands.has(item.brandId));
        }
        if (filterCriteria.filterStatus && filterCriteria.filterStatus.length > 0) {
            const statuses = new Set(filterCriteria.filterStatus.map(opt => opt.value as EmailTemplateItem['status']));
        ...
        if (filterCriteria.filterRoles && filterCriteria.filterRoles.length > 0) {
            ...
            processedData = processedData.filter(item => item.roles?.some(r => roles.has(r.value)));
        }
         if (filterCriteria.filterDepartments && filterCriteria.filterDepartments.length > 0) {
            const depts = new Set(filterCriteria.filterDepartments.map(opt => opt.value));
            processedData = processedData.filter(item => item.departments?.some(d => depts.has(d.value)));
        }
        if (filterCriteria.filterDesignations && filterCriteria.filterDesignations.length > 0) {
            const desigs = new Set(filterCriteria.filterDesignations.map(opt => opt.value));
            processedData = processedData.filter(item => item.designations?.some(d => desigs.has(d.value)));
        }
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter((item) => {
                return (
                    ...
                    (item.categoryId && mockCategoryOptions.find(c=>c.value === item.categoryId)?.label.toLowerCase().includes(query)) ||
                    (item.subCategoryId && mockSubCategoryOptions.find(c=>c.value === item.subCategoryId)?.label.toLowerCase().includes(query)) ||
                    (item.brandId && mockBrandOptions.find(c=>c.value === item.brandId)?.label.toLowerCase().includes(query)) ||
                    item.roles?.some((r) => r.label.toLowerCase().includes(query)) ||
                    item.departments?.some((d) => d.label.toLowerCase().includes(query)) ||
                    item.designations?.some((d) => d.label.toLowerCase().includes(query)) ||
                    item.variables?.some((v) => v.variableName.toLowerCase().includes(query))
                )
            })
        }
        ...
        if (order && key && processedData.length > 0) {
            processedData.sort((a, b) => {
                ...
                if (key === 'roles' || key === 'departments' || key === 'designations') {
    ...
    const columns: ColumnDef<EmailTemplateItem>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                enableSorting: true,
                cell: (props) => mockCategoryOptions.find(c => c.value === props.row.original.categoryId)?.label || '-',
            },
            {
                header: 'SubCategory',
                accessorKey: 'subCategoryId',
                enableSorting: true,
                cell: (props) => mockSubCategoryOptions.find(c => c.value === props.row.original.subCategoryId)?.label || '-',
            },
            {
                header: 'Brand',
                accessorKey: 'brandId',
                enableSorting: true,
                cell: (props) => mockBrandOptions.find(c => c.value === props.row.original.brandId)?.label || '-',
            },
            {
                header: 'Roles',
                accessorKey: 'roles',
                enableSorting: true, // Sorting by first role label
                cell: (props) => props.row.original.roles?.map(r => r.label).join(', ') || '-',
            },
            {
                header: 'Departments',
                accessorKey: 'departments',
                enableSorting: true,
                cell: (props) => props.row.original.departments?.map(d => d.label).join(', ') || '-',
            },
            {
                header: 'Designations',
                accessorKey: 'designations',
                enableSorting: true,
                cell: (props) => props.row.original.designations?.map(d => d.label).join(', ') || '-',
            },
            {
                header: 'Variables',
                ...
                cell: (props) => {
                    ...
                    return vars && vars.length > 0 ? (
                        <Tooltip title={vars.map(v => `${v.variableName} (${v.variableType})`).join('\n')}>
                ...
                {fieldArray.fields.map((field: any, index: number) => (
                    ...
                    >
                        <FormItem
                            ...
                            className="flex-grow"
                            invalid={
                                !!formMethods.formState.errors.variables?.[index]
                                    ?.variableName
                            }
                            errorMessage={
                                formMethods.formState.errors.variables?.[index]
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Select
                                        ...
                                        value={variableTypeOptions.find(opt => opt.value === field.value)}
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select
                                    ...
                                    value={mockCategoryOptions.find(opt => opt.value === field.value)}
                                    onChange={(option) => field.onChange(option?.value)}
                                />
                            )}
                        />
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select
                                    ...
                                    value={mockSubCategoryOptions.find(opt => opt.value === field.value)}
                                    onChange={(option) => field.onChange(option?.value)}
                                />
                            )}
                        />
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select
                                    ...
                                    value={mockBrandOptions.find(opt => opt.value === field.value)}
                                    onChange={(option) => field.onChange(option?.value)}
                                />
                            )}
                        />
