...
const CSV_KEYS: (keyof MessageTemplateItem)[] = [ // 'body' is often too long for CSV, so excluded.
    'id',
    'name',
    'type',
    'triggerEventExample',
...
function exportToCsv(filename: string, rows: MessageTemplateItem[]) {
    ...
    const blob = new Blob(['\ufeff' + csvContent], { // \ufeff for BOM
...
const initialDummyMessageTemplates: MessageTemplateItem[] = [
    { id: 'TPL001', name: 'SMS - Appointment Reminder', type: 'SMS', triggerEventExample: 'appointment_scheduled', status: 'active', body: 'Hi {{name}}, your appointment is scheduled for {{datetime}}.', variables: 'name, datetime', createdDate: new Date(2023, 9, 10)},
    { id: 'TPL002', name: 'Push - Order Shipped', type: 'PushNotification', triggerEventExample: 'order_shipped', status: 'active', subject: 'Your Order #{{orderId}} Has Shipped!', body: 'Track your package: {{trackingLink}}', variables: 'orderId, trackingLink', createdDate: new Date(2023, 8, 15)},
    { id: 'TPL003', name: 'InApp - Welcome Greeting', type: 'InApp', triggerEventExample: 'user_registered', status: 'draft', body: '<h1>Welcome, {{firstName}}!</h1><p>Explore our features.</p>', variables: 'firstName', createdDate: new Date(2023, 10, 1)},
    { id: 'TPL004', name: 'Webhook - New Lead Data', type: 'Webhook', triggerEventExample: 'lead_created', status: 'active', body: '{"lead_id": "{{leadId}}", "email": "{{email}}", "source": "{{source}}"}', variables: 'leadId, email, source', createdDate: new Date(2023, 7, 25)},
]
                ...
                <div className="flex items-center justify-between w-full px-4 sm:px-8">
                    <span className="flex items-center gap-2">
                        ...
                        <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
                            ...
                            <span>
                                Template{selectedItems.length > 1 ? 's' : ''}{' '}
                                selected
                            </span>
                        </span>
                    </span>
...
const MessageTemplates = () => { // Renamed from AutoMessages
    ...
    const addFormMethods = useForm<MessageTemplateFormData>({
        ...
        defaultValues: { name: '', type: 'SMS', triggerEventExample: '', subject: '', body: '', variables: '', status: 'draft' },
        mode: 'onChange',
    })
    ...
    const openAddDrawer = useCallback(() => {
        addFormMethods.reset({ name: '', type: 'SMS', triggerEventExample: '', subject: '', body: '', variables: '', status: 'draft' })
    ...
    const onEditTemplateSubmit = useCallback(async (data: MessageTemplateFormData) => {
     ...
     const handleChangeStatus = useCallback(async (template: MessageTemplateItem) => {
        const statusesCycle: MessageTemplateItem['status'][] = ['active', 'inactive', 'draft'];
  ...
  const [selectedItems, setSelectedItems] = useState<MessageTemplateItem[]>([])
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterTriggerEvents && filterCriteria.filterTriggerEvents.length > 0) {
            ...
            processedData = processedData.filter(item => events.has(item.triggerEventExample));
        }
    ...
    const columns: ColumnDef<MessageTemplateItem>[] = useMemo(
        () => [
            { header: 'Template Name', accessorKey: 'name', enableSorting: true, size: 250 },
            {
                header: 'Type',
                accessorKey: 'type',
                enableSorting: true,
            ...
            },
            {
                ...
                enableSorting: true,
                size: 200,
                cell: props => props.row.original.triggerEventExample.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
            },
            {
                header: 'Status',
                accessorKey: 'status',
            ...
            >
                ...
                >
                    ...
                    <FormItem
                        ...
                        errorMessage={
                            addFormMethods.formState.errors.body?.message
                        }
                    >
                        <Controller
                            name="body"
                            control={addFormMethods.control}
                            render={({ field }) => (
                                <Textarea {...field} rows={5} placeholder="Enter message body. Use {{variable_name}} for placeholders." />
                            )}
                        />
                    </FormItem>
                     <FormItem label="Example Variables (Optional)">
                        <Controller
                            ...
                            control={addFormMethods.control}
                            render={({ field }) => (
                                <Input {...field} placeholder="e.g., userName, orderId, productLink" />
                            )}
                        />
                    </FormItem>
                </Form>
            ...
            >
                ...
                >
                    ...
                    <FormItem
                        ...
                        errorMessage={
                            editFormMethods.formState.errors.body?.message
                        }
                    >
                        <Controller
                            name="body"
                            control={editFormMethods.control}
                            render={({ field }) => (
                                <Textarea {...field} rows={5} placeholder="Enter message body" />
                            )}
                        />
                    </FormItem>
                     <FormItem label="Example Variables (Optional)">
