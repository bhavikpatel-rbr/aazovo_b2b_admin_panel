...
const CSV_HEADERS = ['ID', 'Title', 'Slug', 'Status', 'Meta Title', 'Meta Description', 'Meta Keywords', 'Created Date', 'Icon Full Path'];
const CSV_KEYS: (keyof BlogItem | 'icon_full_path')[] = ['id', 'title', 'slug', 'status', 'meta_title', 'meta_descr', 'meta_keyword', 'created_at', 'icon_full_path'];
...
function exportToCsv(filename: string, rows: BlogItem[]) {
    ...
    const separator = ',';
    const csvContent = CSV_HEADERS.join(separator) + '\n' + rows .map((row) => { return CSV_KEYS.map((k) => { let cell: any = row[k as keyof BlogItem]; if (cell === null || cell === undefined) { cell = ''; } else if (k === 'created_at' || k === 'updated_at') { cell = new Date(cell).toLocaleDateString(); } else { cell = String(cell).replace(/"/g, '""'); } if (String(cell).search(/("|,|\n)/g) >= 0) { cell = `"${cell}"`; } return cell; }).join(separator); }) .join('\n');
...
// --- ActionColumn ---
const ActionColumn = ({ onEdit, onDelete, onClone }: { onEdit: () => void; onDelete: () => void; onClone: () => void; }) => {
    ...
    return (
        <div className="flex items-center justify-center gap-2">
            <Tooltip title="Clone Blog"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400')} role="button" tabIndex={0} onClick={onClone}><TbCopy /></div></Tooltip>
...
// --- BlogsTableTools ---
const BlogsTableTools = ({ onSearchChange, onFilter, onExport, onImport }: { onSearchChange: (query: string) => void; onFilter: () => void; onExport: () => void; onImport: () => void; }) => {
    return (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                ...
                <Button icon={<TbCloudDownload />} onClick={onImport} className="w-full sm:w-auto">Import</Button> {/* Added Import */}
...
const Blogs = () => {
    ...
    const tableLoading = masterLoading === true || masterLoading === 'loading' || isSubmitting || isProcessing;
    ...
    useEffect(() => {
        if (isAddDrawerOpen) {
            const subscription = addFormMethods.watch((value, { name, type }) => {
                console.log("--- ADD FORM WATCH --- Field:", name, "Type:", type, "Full Value:", value, "Icon:", value.icon, "File?:", value.icon instanceof File, "Null?:", value.icon === null, "Errors:", addFormMethods.formState.errors, "Valid:", addFormMethods.formState.isValid);
            });
    ...
    useEffect(() => {
        if (isEditDrawerOpen) {
             const subscription = editFormMethods.watch((value, { name, type }) => {
                console.log("--- EDIT FORM WATCH --- Field:", name, "Type:", type, "Full Value:", value, "Icon:", value.icon, "File?:", value.icon instanceof File, "Null?:", value.icon === null, "Errors:", editFormMethods.formState.errors, "Valid:", editFormMethods.formState.isValid);
            });
            return () => subscription.unsubscribe();
        }
    ...
    const onAddBlogSubmit: SubmitHandler<BlogFormData> = async (data) => {
        console.log("ADD BLOG SUBMIT - Data:", data, "isValid:", addFormMethods.formState.isValid);
        if (!addFormMethods.formState.isValid) {
            toast.push(<Notification title="Validation Error" type="danger">Please correct the form errors.</Notification>);
            return;
        }
    ...
    const openEditDrawer = (blog: BlogItem) => {
        ...
        editFormMethods.reset({
            title: blog.title, slug: blog.slug || null, blog_descr: blog.blog_descr || null,
            icon: null, // Icon is handled by preview logic, not pre-filled with file path
            status: blog.status, meta_title: blog.meta_title || null, meta_descr: blog.meta_descr || null,
    ...
    const onEditBlogSubmit = async (data: BlogFormData) => {
            if (!editingBlog || editingBlog.id === undefined || editingBlog.id === null) {
            ...
            try {
                ...
                toast.push(<Notification title="Brand Updated" type="success" duration={2000}>Brand "{data.title}" updated.</Notification>);
                closeEditDrawer();
                dispatch(getBlogsAction());
            } catch (error: any) {
    ...
    const handleCloneBlog = (blog: BlogItem) => {
        ...
        setAddDrawerOpen(true);
        toast.push(<Notification title="Clone Blog" type="info">Cloning "{blog.title}". Please review, upload an icon if needed, and save.</Notification>);
    };
    ...
    const openImageViewer = (src: string | null) => { // Renamed
        if (src) { 
            setImageToView(src); 
            setIsImageViewerOpen(true); 
        }
    };
    const closeImageViewer = () => { // Added
    ...
    const columns: ColumnDef<BlogItem>[] = useMemo( () => [
            ...
            { header: 'Actions', id: 'action', meta: { headerClass: 'text-center', cellClass: 'text-center' }, size: 120, cell: (props) => (<ActionColumn onEdit={() => openEditDrawer(props.row.original)} onDelete={() => handleDeleteClick(props.row.original)} onClone={() => handleCloneBlog(props.row.original)}/>),
            },
        ],
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [blogStatusColor, openImageViewer, openEditDrawer, handleDeleteClick, handleCloneBlog], 
    );
    ...
    return (
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4"> 
                        ...
                        <Button variant="solid" icon={<TbPlus />} onClick={openAddDrawer}>Add New Blog</Button> 
                    </div>
                    <BlogsTableTools onSearchChange={handleSearchChange} onFilter={openFilterDrawer} onExport={handleExportData} onImport={handleImportData} />
                    <div className="mt-4 flex-grow overflow-y-auto"> 
                        <BlogsTable columns={columns} data={pageData} loading={tableLoading} 
            ...
            {[ { type: 'add', title: 'Add New Blog', isOpen: isAddDrawerOpen, closeFn: closeAddDrawer, formId: 'addBlogForm', methods: addFormMethods, onSubmit: onAddBlogSubmit, submitButtonText: 'Add Blog', submittingText: 'Adding...' }, { type: 'edit', title: 'Edit Blog', isOpen: isEditDrawerOpen, closeFn: closeEditDrawer, formId: 'editBlogForm', methods: editFormMethods, onSubmit: onEditBlogSubmit, submitButtonText: 'Save Changes', submittingText: 'Saving...' } ].map(drawer => (
                <Drawer key={drawer.formId} title={drawer.title} isOpen={drawer.isOpen} onClose={drawer.closeFn} onRequestClose={drawer.closeFn}
                    footer={ <div className="text-right w-full">
                                 ...
                                 <Button size="sm" variant="solid" form={drawer.formId} type="submit" loading={isSubmitting}
                    ...
                    <Form id={drawer.formId} onSubmit={drawer.methods.handleSubmit(drawer.onSubmit as any)} className="flex flex-col gap-4">
                        ...
                        <FormItem label="Description (Optional)" invalid={!!drawer.methods.formState.errors.blog_descr} errorMessage={drawer.methods.formState.errors.blog_descr?.message as string | undefined}>
                            <Controller name="blog_descr" control={drawer.methods.control} render={({ field }) => <Textarea {...field} value={field.value ?? ''} placeholder="Enter main blog content..." rows={5} />} />
                        </FormItem>
                        <FormItem label="Status" invalid={!!drawer.methods.formState.errors.status} errorMessage={drawer.methods.formState.errors.status?.message as string | undefined} isRequired>
                             <Controller name="status" control={drawer.methods.control} render={({ field }) => (
                                <Select placeholder="Select Status" options={blogStatusOptions} value={blogStatusOptions.find(opt => opt.value === field.value)} onChange={(opt) => field.onChange(opt?.value)} />
                            )} />
                        </FormItem>
                        ...
                        <FormItem label="Meta Description" invalid={!!drawer.methods.formState.errors.meta_descr} errorMessage={drawer.methods.formState.errors.meta_descr?.message as string | undefined}>
                             <Controller name="meta_descr" control={drawer.methods.control} render={({ field }) => <Textarea {...field} value={field.value ?? ''} placeholder="SEO Description (max 160 chars)" rows={3} />} />
                        </FormItem>
                        ...
                        <FormItem label={`Icon${drawer.type === 'edit' && editingBlog?.icon_full_path && iconPreview === editingBlog.icon_full_path ? ' (Current icon below. Select new to change)' : ' (Optional)'}`} invalid={!!drawer.methods.formState.errors.icon} errorMessage={drawer.methods.formState.errors.icon?.message as string | undefined}>
                            ...
                            {drawer.type === 'edit' && iconPreview && editingBlog?.icon_full_path === iconPreview && (
                                <div className="mb-2 flex items-center gap-2">
                                    ...
                                    <Button icon={<TbX/>} variant='twoTone' size="xs" color="red" type="button" onClick={() => {
                                        if (iconPreview) URL.revokeObjectURL(iconPreview);
                                        setIconPreview(null); 
                                        drawer.methods.setValue('icon', null, {shouldValidate: true}); 
                            ...
                            {iconPreview && (drawer.methods.watch('icon') instanceof File || (drawer.type === 'edit' && editingBlog?.icon_full_path !== iconPreview)) && (
                             ...
                             {!iconPreview && drawer.type === 'edit' && editingBlog?.icon_full_path && !drawer.methods.watch('icon') && (
                                 <p className='text-xs text-gray-500 mb-1'>Current icon will be removed as "Remove Current Icon" was clicked.</p>
                             )}
                            <Input
                                type="file"
                                accept="image/*"
            ...
            <Drawer title="Import Blogs" isOpen={importDialogOpen} onClose={() => setImportDialogOpen(false)} onRequestClose={() => setImportDialogOpen(false)}>
                 <div className="p-4">
                    <p className="mb-4">Upload a CSV file to import blogs. Ensure the CSV format matches the export structure (see Export button).</p>
            ...
            <ConfirmDialog isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Blog" 
                           onClose={() => { setSingleDeleteConfirmOpen(false); setBlogToDelete(null); }} 
                           onRequestClose={() => { setSingleDeleteConfirmOpen(false); setBlogToDelete(null); }} 
                           onCancel={() => { setSingleDeleteConfirmOpen(false); setBlogToDelete(null); }} 
                           confirmButtonColor="red-600" onConfirm={onConfirmSingleDelete} 
                           loading={isProcessing || (masterLoading === 'loading' && !!itemBeingDeleted && blogToDelete?.id === itemBeingDeleted)} > 
