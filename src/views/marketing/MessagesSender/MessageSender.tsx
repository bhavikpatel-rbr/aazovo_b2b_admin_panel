...
const mockMessageTemplateOptions = [
    ...
    { value: 'AMT004', label: 'Webhook - New Lead Notification', type: 'Webhook'},
...
const initialDummyAutoMessages: AutoMessageItem[] = [
    { id: 'AMR001', name: '24hr Appointment Reminder', templateId: 'AMT001', triggerEvent: 'appointment_scheduled', status: 'active', delay: '24 hours before', conditions: 'Type = consultation', createdDate: new Date(2023, 9, 12), templateName: 'SMS - Appointment Reminder (24hr)', templateType: 'SMS' },
    { id: 'AMR002', name: 'Order Shipped Notification', templateId: 'AMT002', triggerEvent: 'order_shipped', status: 'active', delay: 'Immediately', createdDate: new Date(2023, 8, 16), templateName: 'Push - Order Shipped', templateType: 'PushNotification' },
    { id: 'AMR003', name: 'Welcome Tour Start', templateId: 'AMT003', triggerEvent: 'user_registered', status: 'draft', delay: '5 minutes after registration', createdDate: new Date(2023, 10, 2), templateName: 'InApp - Welcome Tour Step 1', templateType: 'InApp'},
    { id: 'AMR004', name: 'Abandoned Cart Email (Tier 1)', templateId: 'AMT005', triggerEvent: 'cart_abandoned', status: 'inactive', delay: '1 hour after abandonment', conditions: 'CartValue > $50', createdDate: new Date(2023, 9, 7), templateName: 'SMS - Password Reset Code', templateType: 'SMS'}, // Example, template might not match name
]
...
type AutoMessagesSearchProps = { // Renamed from AutoMessagesSearchProps to avoid conflict if in same scope
    onInputChange: (value: string) => void
    ref?: Ref<HTMLInputElement>
}
const RuleSearch = React.forwardRef< // Renamed component
    HTMLInputElement,
...
const RuleTableTools = ({ // Renamed component
    onSearchChange,
    onFilter,
    onExport,
}: {
...
type RuleTableProps = { // Renamed props type
...
const RuleTable = ({ // Renamed component
    columns,
    data,
    loading,
    pagingData,
...
type RuleSelectedFooterProps = { // Renamed props type
    selectedItems: AutoMessageItem[]
    onDeleteSelected: () => void
}
const RuleSelectedFooter = ({ // Renamed component
    selectedItems,
    onDeleteSelected,
                ...
                <div className="flex items-center justify-between w-full px-4 sm:px-8">
                    <span className="flex items-center gap-2">
                        ...
                        <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
                            ...
                            <span>
                                Rule{selectedItems.length > 1 ? 's' : ''}{' '} {/* Changed from Item to Rule */}
                                selected
                            </span>
                        </span>
                    </span>
...
const AutoMessages = () => {
    const [allAutoMessages, setAllAutoMessages] =
        useState<AutoMessageItem[]>(initialDummyAutoMessages)
            ...
            switch (action.type) {
                ...
                case 'autoMessages/add':
                    const newRule = {
                        ...
                        templateName: mockMessageTemplateOptions.find(t => t.value === action.payload.templateId)?.label,
                        templateType: mockMessageTemplateOptions.find(t => t.value === action.payload.templateId)?.type,
                    };
    ...
    const [editingRule, setEditingRule] =
        useState<AutoMessageItem | null>(null) // Renamed from editingTemplate
    ...
    const addFormMethods = useForm<AutoMessageFormData>({
        ...
        defaultValues: { name: '', templateId: '', triggerEvent: '', delay: '', conditions: '', status: 'draft' },
        mode: 'onChange',
    })
    ...
    const openAddDrawer = useCallback(() => {
        addFormMethods.reset({ name: '', templateId: '', triggerEvent: '', delay: '', conditions: '', status: 'draft' })
    ...
    const onAddRuleSubmit = useCallback(async (data: AutoMessageFormData) => { // Renamed
    ...
    const openEditDrawer = useCallback((rule: AutoMessageItem) => { // Renamed
        setEditingRule(rule)
    ...
    const onEditRuleSubmit = useCallback(async (data: AutoMessageFormData) => { // Renamed
    ...
    const handleChangeStatus = useCallback(async (rule: AutoMessageItem) => {
        const newStatus = rule.status === 'active' ? 'inactive' : rule.status === 'inactive' ? 'draft' : 'active';
        ...
        try {
            ...
            toast.push(
                <Notification title="Failed to Change Status" type="danger" duration={3000}>
    ...
    const handleDeleteClick = useCallback((rule: AutoMessageItem) => { // Renamed
        setRuleToDelete(rule)
        setSingleDeleteConfirmOpen(true);
    }, []);

    const onConfirmSingleDelete = useCallback(async () => { // Renamed
 ...
 const [selectedItems, setSelectedItems] = useState<AutoMessageItem[]>([])
    const handleDeleteSelected = useCallback(async () => { // Renamed
    ...
    const templateIdOptions = useMemo(() => {
        return mockMessageTemplateOptions.map(t => ({value: t.value, label: t.label}))
    }, [])
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterTemplateIds && filterCriteria.filterTemplateIds.length > 0) {
            const tIds = new Set(filterCriteria.filterTemplateIds.map(opt => opt.value));
        ...
        if (filterCriteria.filterStatus && filterCriteria.filterStatus.length > 0) {
            const statuses = new Set(filterCriteria.filterStatus.map(opt => opt.value as AutoMessageItem['status']));
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter((item) => {
                return (
                    ...
                    (item.templateName && item.templateName.toLowerCase().includes(query)) ||
    ...
    const columns: ColumnDef<AutoMessageItem>[] = useMemo(
        () => [
            { header: 'Rule Name', accessorKey: 'name', enableSorting: true, size: 200 },
            {
                ...
                cell: (props) => {
                    ...
                    const templateTypeDisplay = rule.templateType ? (
                         <Tag className={classNames(messageTypeColor[rule.templateType] || 'bg-gray-100 text-gray-600', 'ml-2 text-xs')}>
                            {rule.templateType}
                        </Tag>
                    ) : null;
                    return (
            ...
            },
            {
                ...
                size: 100,
                cell: (props) => (
                    <Tag className={classNames(ruleStatusColor[props.row.original.status] || 'bg-gray-100 text-gray-600', 'capitalize')}>
            ...
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        <h5 className="mb-2 sm:mb-0">Auto Message Rules</h5> {/* Changed title */}
                        <Button
                            variant="solid"
                            icon={<TbPlus />}
                ...
                >
                    ...
                    >
                        <Controller
                            ...
                            render={({ field }) => (
                                <Input {...field} placeholder="Enter rule name (e.g., Welcome Email Rule)" />
                            )}
                        />
                    </FormItem>
                    <FormItem
                        ...
                        errorMessage={
                            addFormMethods.formState.errors.templateId?.message
                        }
                    >
                        <Controller
                            name="templateId"
                            ...
                            render={({ field }) => (
                                <Select
                                    ...
                                    value={mockMessageTemplateOptions.find(opt => opt.value === field.value)}
                    ...
                    >
                        <Controller
                            ...
                            control={addFormMethods.control}
                            render={({ field }) => (
                                <Input {...field} placeholder="e.g., user_registered, order_placed" />
                            )}
                        />
                    </FormItem>
                    ...
                    <FormItem label="Delay (Optional)">
                        <Controller
                            ...
                            control={addFormMethods.control}
                            render={({ field }) => (
                                <Input {...field} placeholder="e.g., 1 hour after event, Immediately" />
                            )}
                        />
                    </FormItem>
                    <FormItem label="Conditions (Optional)">
                        <Controller
                            ...
                            control={addFormMethods.control}
                            render={({ field }) => (
                                <Textarea {...field} rows={3} placeholder="Describe conditions, e.g., user_role = 'premium' AND order_total > 100" />
                            )}
                        />
                    </FormItem>
                </Form>
            ...
            >
                ...
                >
                    ...
                    <FormItem
                        ...
                        errorMessage={
                            editFormMethods.formState.errors.templateId?.message
                        }
                    >
                        <Controller
                            name="templateId"
                    ...
                    <FormItem label="Conditions (Optional)">
                        <Controller
                            ...
                            control={editFormMethods.control}
                            render={({ field }) => (
                                <Textarea {...field} rows={3} placeholder="Describe conditions" />
                            )}
                        />
                    </FormItem>
                </Form>
