...
import {
  ...
  TbPhoto,
  TbPhone,
  TbFileDescription, // Added TbFileDescription for consistency
  TbForms,
} from "react-icons/tb";
...
const templateValues = AVAILABLE_TEMPLATES.map((t) => t.value) as [
  string,
  ...string[]
];
...
const campaignCreationFormSchema = z
  .object({
    ...
    text1: z
      .string()
      .max(1000, "Text 1 is too long.")
      .optional()
      .or(z.literal("")),
    image1Url: z
      .string()
      .url("Image 1 must be a valid URL.")
      .optional()
      .or(z.literal("")),
    text2: z
      .string()
    ...
    whatsappNo: z
      ...
      .optional()
      .or(z.literal("")),
    recipientSelectionMode: z.enum(["database", "import"], {
      required_error: "Please choose a recipient selection mode.",
    }),
    recipientUserIds: z.array(z.string()).optional(),
    importedFile: z.instanceof(File).optional().nullable(),
    campaignNameInternal: z
      .string()
      .min(1, "Campaign Name is required for tracking.")
      .max(150),
  ...
  .superRefine((data, ctx) => {
    if (
      data.recipientSelectionMode === "database" &&
      (!data.recipientUserIds || data.recipientUserIds.length === 0)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Please select recipients or a segment.",
    ...
    if (data.recipientSelectionMode === "import" && !data.importedFile) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Please upload a file for import.",
        path: ["importedFile"],
      });
    }
...
const initialDummyCampaigns: EmailCampaignItem[] = [
  ...
  },
  {
    ...
    details: "Triggered on registration.",
    status: "sent",
  },
  {
    id: "CAMP003",
    campaignName: "October Newsletter W4",
    dateTime: new Date(2023, 9, 28, 14, 30),
    templateId: "T002",
    templateName: "Weekly Newsletter - Oct Week 4",
    details: "To Newsletter segment.",
    status: "scheduled",
  },
];

// --- CSV Exporter ---
const CSV_HEADERS_CAMPAIGN = [
  "ID",
  "Campaign Name",
  "Sent/Scheduled DateTime",
  "Template ID",
  "Template Name",
  "Details",
  "Status",
];
const CSV_KEYS_CAMPAIGN: (keyof EmailCampaignItem)[] = [
  "id",
  "campaignName",
  "dateTime",
  "templateId",
...
function exportCampaignsToCsv(filename: string, rows: EmailCampaignItem[]) {
  ...
  const preparedRows = rows.map((row) => ({
    ...
    status:
      CAMPAIGN_STATUS_OPTIONS.find((s) => s.value === row.status)?.label ||
      row.status,
  }));
  const separator = ",";
  const csvContent =
...
const ActionColumn = ({ onViewDetails, onEdit }: { onViewDetails: () => void; onEdit: () => void; }) => {
...
const CampaignTableTools = ({
  ...
  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
    ...
    <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
      ...
      {/* Filter for listing not fully implemented */}
      <Button
        icon={<TbCloudUpload />}
        onClick={onExport}
        className="w-full sm:w-auto"
...
const EmailCampaignListing = () => {
  const [campaignsData, setCampaignsData] = useState<EmailCampaignItem[]>(
  ...
  const campaignFormMethods = useForm<CampaignCreationFormData>({
    ...
    defaultValues: {
      ...
      textWithWhatsappNo: "",
      whatsappNo: "",
      recipientSelectionMode: "database",
      recipientUserIds: [],
      importedFile: null,
  ...
  const nextStep = useCallback(async () => {
    ...
    if (currentWizardStep === 1) fieldsToValidate = ["templateId"];
      ...
      fieldsToValidate = []; // Optional fields, or validate if filled
    ...
    if (fieldsToValidate.length > 0) {
      ...
      if (!isValid) {
        toast.push(
          <Notification title="Validation Error" type="danger" duration={3000}>
            Please correct the errors on this step.
          </Notification>
        );
        return;
      }
  ...
  const openCreateDrawer = useCallback(() => {
    campaignFormMethods.reset({
      // Reset to initial defaults for a new campaign
      templateId: undefined,
      templateName: "",
      text1: "",
      image1Url: "",
    ...
    async (data: CampaignCreationFormData) => {
      ...
      try {
        ...
        const newCampaignEntry: EmailCampaignItem = {
          ...
          dateTime:
            data.sendOption === "schedule" && data.scheduledAt
              ? data.scheduledAt
              : new Date(),
          templateId: data.templateId || "N/A",
          templateName:
            AVAILABLE_TEMPLATES.find((t) => t.value === data.templateId)
              ?.label ||
            data.templateName ||
            "Custom/Unknown",
          details: `Recipients: ${
            data.recipientSelectionMode === "database"
              ? `${data.recipientUserIds?.length || 0} selected`
              : data.importedFile
              ? `File: ${data.importedFile.name}`
              : "None"
          }. Values: Text1 - ${data.text1 ? "Set" : "N/A"} etc.`,
        ...
        toast.push(
          <Notification title="Campaign Failed" type="danger" duration={4000}>
            {(e as Error).message || "Could not process campaign."}
          </Notification>
        );
      } finally {
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    ...
    if (tableData.query && tableData.query.trim() !== "") {
      ...
      processedData = processedData.filter(
        (c) =>
          ...
          (c.campaignName && c.campaignName.toLowerCase().includes(query)) ||
    ...
    if (order && key) {
      processedData.sort((a, b) => {
        ...
        const aVal = a[key as keyof EmailCampaignItem];
        const bVal = b[key as keyof EmailCampaignItem];
  ...
  const handleExportData = useCallback(() => {
    exportCampaignsToCsv("email_campaigns_log.csv", allFilteredAndSortedData);
        ...
        cell: (props) => (
          <div>
            <span className="font-semibold">
              {props.row.original.templateName}
            </span>{" "}
            <span className="text-xs text-gray-500">
              ({props.row.original.templateId})
            </span>
          </div>
        ),
      },
          ...
          props.row.original.status ? (
            <Tag
              className={classNames(
                ...
                campaignStatusColor[props.row.original.status]
              )}
            >
              {
                CAMPAIGN_STATUS_OPTIONS.find(
                  (s) => s.value === props.row.original.status
                )?.label
              }
            </Tag>
          ) : (
          ...
          <Card
            header={
              <div className="flex items-center gap-2 font-semibold">
                <TbTemplate /> Step 1: Choose Template
              </div>
            }
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    value={AVAILABLE_TEMPLATES.find(
                      (o) => o.value === field.value
                    )}
                    onChange={(opt) => {
                      field.onChange(opt?.value);
                      setValue("templateName", opt?.label || "", {
                        shouldValidate: true,
                      });
                    }}
                  />
            ...
            <FormItem label="Selected Template Name" className="mt-4">
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    {...field}
                    placeholder="Template name (auto-filled or custom)"
                    disabled
                  />
                )}
              />
              <p className="text-xs text-gray-500 mt-1">
                This name is for reference, based on the selected template ID.
              </p>
            </FormItem>
          </Card>
        );
      case 2: // Step 2: Add Dynamic Values
        return (
          <Card
            header={
              <div className="flex items-center gap-2 font-semibold">
                <TbForms /> Step 2: Add Dynamic Values
              </div>
            }
            bodyClass="p-4 md:p-6"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <FormItem
                label="Text Block 1 (e.g., for {{text1}})"
                className="md:col-span-2"
                invalid={!!errors.text1}
                errorMessage={errors.text1?.message}
              >
                <Controller
                  name="text1"
                  control={control}
              ...
              <FormItem
                label="Image 1 URL (e.g., for {{image1Url}})"
                invalid={!!errors.image1Url}
                errorMessage={errors.image1Url?.message}
              >
                <Controller
                  name="image1Url"
                  control={control}
              ...
              <div /> {/* Spacer */}
              <FormItem
                label="Text Block 2 (e.g., for {{text2}})"
                className="md:col-span-2"
                invalid={!!errors.text2}
                errorMessage={errors.text2?.message}
              >
                <Controller
                  name="text2"
                  control={control}
              ...
              <FormItem
                label="Text with WhatsApp Link (e.g., for {{textWithWhatsappNo}})"
                invalid={!!errors.textWithWhatsappNo}
                errorMessage={errors.textWithWhatsappNo?.message}
              >
                <Controller
                  name="textWithWhatsappNo"
                  control={control}
                  render={({ field }) => (
                    <Input
                      {...field}
        ...
        return (
          ...
          >
            <FormItem
              ...
              className="mb-6"
              invalid={!!errors.recipientSelectionMode}
              errorMessage={errors.recipientSelectionMode?.message}
            >
              <Controller
                ...
                render={({ field }) => (
                  <Radio.Group
                    ...
                    onChange={(val) => {
                      field.onChange(val);
                      if (val === "database") setValue("importedFile", null);
                      if (val === "import") setValue("recipientUserIds", []);
                    }}
                  >
            ...
            {watch("recipientSelectionMode") === "database" && (
              <FormItem
                label="Select Users or Segments"
                invalid={!!errors.recipientUserIds}
                errorMessage={errors.recipientUserIds?.message as string}
              >
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      isMulti
                      placeholder="Select users or predefined segments..."
                      options={AVAILABLE_USERS_FOR_SELECTION}
                      value={AVAILABLE_USERS_FOR_SELECTION.filter((o) =>
            ...
            {watch("recipientSelectionMode") === "import" && (
              <FormItem
                label="Upload Excel/CSV File (with 'email' column)"
                invalid={!!errors.importedFile}
                errorMessage={errors.importedFile?.message as string}
              >
                <Controller
                  name="importedFile"
                  control={control}
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Note: File will be processed on submission. Client-side
                  parsing not implemented here.
                </p>
              </FormItem>
            )}
          </Card>
      ...
      case 4: // Step 4: Review & Send/Schedule
        ...
        return (
          <Card
            header={
              <div className="flex items-center gap-2 font-semibold">
                <TbCircleCheck /> Step 4: Review & Send/Schedule
              </div>
            }
            bodyClass="p-4 md:p-6"
          >
            <FormItem
              label="Campaign Name (for internal tracking)"
              invalid={!!errors.campaignNameInternal}
              errorMessage={errors.campaignNameInternal?.message}
              className="mb-4"
            >
              <Controller
                name="campaignNameInternal"
                control={control}
                render={({ field }) => (
                  <Input
                    {...field}
                    prefix={<TbClipboardText />}
                    placeholder="e.g., Q4 Holiday Promo - US/CA Users"
                  />
                )}
              />
            </FormItem>
            <div className="space-y-2 mb-6 text-sm border-t border-b py-4 dark:border-gray-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1">
                ...
                <div>
                  ...
                  {formData.recipientSelectionMode === "database" ? (
                    `${formData.recipientUserIds?.length || 0} selected`
                  ) : formData.importedFile ? (
                    `File: ${formData.importedFile.name}`
                  ) : (
                    <span className="italic text-gray-500">Not Set</span>
                  )}
                </div>
              ...
              <p className="mt-2">
                ...
                {formData.text1 ? (
                  <span className="line-clamp-2" title={formData.text1}>
                    {formData.text1}
                  </span>
                ) : (
                  <span className="italic text-gray-500">Not Set</span>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Radio.Group
                    ...
                    onChange={(val) => {
                      field.onChange(val);
                      if (val === "now") setValue("scheduledAt", null);
                    }}
                  >
  ...
  return (
    <>
        ...
        <AdaptiveCard className="h-full" bodyClass="h-full">
            ...
            onFilter={() =>
              toast.push(
                <Notification title="Info">
                  Filter for listing page not fully implemented yet.
                </Notification>
              )
            }
          ...
          <div className="mt-4">
            <CampaignTable
              ...
              data={pageData}
              loading={
                masterLoadingStatus === "loading" || isSubmittingCampaign
              }
              pagingData={{
                total,
      ...
      >
        <h5 className="mb-4">
          Campaign Details:{" "}
          {viewingItem?.campaignName || viewingItem?.templateName}
        </h5>
        {viewingItem && (
          <div className="space-y-3 text-sm max-h-[70vh] overflow-y-auto pr-2">
            ...
            <p>
              ...
              {viewingItem.campaignName || (
                <span className="italic text-gray-500">N/A</span>
              )}
            </p>
            <p>
              <strong>Sent/Scheduled:</strong>{" "}
              {viewingItem.dateTime.toLocaleString()}
            </p>
            <p>
              <strong>Template:</strong> {viewingItem.templateName} (
              {viewingItem.templateId})
            </p>
            <p>
              ...
              {viewingItem.status ? (
                <Tag
                  className={classNames(
                    "capitalize whitespace-nowrap",
                    campaignStatusColor[viewingItem.status]
                  )}
                >
                  {
                    CAMPAIGN_STATUS_OPTIONS.find(
            ...
            <div>
              ...
              <pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-1 whitespace-pre-wrap">
                {viewingItem.details}
              </pre>
            </div>
          </div>
