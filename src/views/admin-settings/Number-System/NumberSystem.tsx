...
function exportNumberSystemsToCsv(filename: string, rows: NumberSystemItem[]) {
  ...
  const preparedRows: NumberSystemExportItem[] = rows.map((row) => ({ ...row, status: row.status || "Inactive", prefix: row.prefix || " ", country_ids: row.country_ids || " ", customer_code_starting: row.customer_code_starting || "0", current_customer_code: row.current_customer_code || "0", non_kyc_customer_code_starting: row.non_kyc_customer_code_starting || "0", non_kyc_current_customer_code: row.non_kyc_current_customer_code || "0", company_code_starting: row.company_code_starting || "0", current_company_code: row.current_company_code || "0", non_kyc_company_code_starting: row.non_kyc_company_code_starting || "0", non_kyc_current_company_code: row.non_kyc_current_company_code || "0", created_at_formatted: row.created_at ? new Date(row.created_at).toLocaleString() : " ", updated_by_name: row.updated_by_name || " ", updated_by_role: row.updated_by_role || " ", updated_at_formatted: row.updated_at ? new Date(row.updated_at).toLocaleString() : " " }));
...
const ActionColumn = ({ onEdit, onDelete }: { onEdit: () => void, onDelete: () => void; }) => (
  <>
    {getMenuRights("number_system")?.is_edit && <div className="flex items-center justify-center gap-1"><Tooltip title="Edit"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400" role="button" onClick={onEdit}><TbPencil /></div></Tooltip></div>}
  </>
);
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-1 w-full sm:w-auto">
        ...
        <Button icon={<TbCloudUpload />} menuName="number_system" isExport={true} onClick={onExport} className="w-full sm:w-auto" disabled={!isDataReady}>Export</Button>
      </div>
    </div>
  )
};
...
const NumberSystems = () => {
  ...
  const baseColumns: ColumnDef<NumberSystemItem>[] = useMemo(() => [
    ...
    { header: "Countries", accessorKey: "country_ids", id: "countriesCount", enableSorting: true, size: 360, cell: (props) => { const ids = props.row.original.country_ids?.split(',').map(id => id.trim()).filter(Boolean) || []; if (ids.length === 0) return <Tag> </Tag>; const names = ids.map(id => countryOptions.find(c => c.value === id)?.label || `ID:${id}`); return (<div className="flex flex-wrap gap-1">{names.slice(0, 2).map((n, i) => <Tag key={i} className="bg-gray-100 text-gray-600 dark:bg-gray-500/20 dark:text-gray-100 text-[11px] border-b border-emerald-300 dark:border-emerald-700">{n}</Tag>)}{names.length > 2 && <Tooltip title={names.join(', ')}><Tag className="bg-gray-200 dark:bg-gray-500">+{names.length - 2} more</Tag></Tooltip>}</div>); } },
      ...
      <Container className="h-auto">
        <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
            ...
            <Button variant="solid" menuName="number_system" isAdd={true} icon={<TbPlus />} onClick={openAddDrawer} disabled={!isDataReady}>Add New</Button>
          </div>
          ...
          <div className="mt-2 flex-grow overflow-y-auto">
            <DataTable menuName="number_system" selectable columns={visibleColumns} data={pageData} noData={!tableLoading && pageData.length === 0} loading={tableLoading} pagingData={{ total: visibleColumns?.length ? total : 0, pageIndex: tableData.pageIndex, pageSize: tableData.pageSize }} onPaginationChange={(p) => handleSetTableData({ pageIndex: p })} onSelectChange={(s) => handleSetTableData({ pageSize: s, pageIndex: 1 })} onSort={(s) => handleSetTableData({ sort: s })} checkboxChecked={(row) => selectedItems.some(s => s.id === row.id)} onCheckBoxChange={(c, r) => setSelectedItems(p => c ? [...p, r] : p.filter(i => i.id !== r.id))} onIndeterminateCheckBoxChange={(c, rs) => { const rIds = new Set(rs.map(r => r.original.id)); setSelectedItems(p => c ? [...p, ...rs.map(r => r.original).filter(r => !p.some(i => i.id === r.id))] : p.filter(i => !rIds.has(i.id))) }} />
          </div>
        </AdaptiveCard>
      </Container>
