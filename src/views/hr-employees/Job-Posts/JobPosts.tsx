...
export type JobPlatform = {
  id?: string | number | null; // MODIFIED: Add optional id, can be null
  portal: string;
  link: string;
...
// --- Zod Schema for Job Platform ---
const jobPlatformSchema = z.object({
  id: z.union([z.string(), z.number(), z.null()]).optional(), // MODIFIED: Add optional id schema
  portal: z.string().min(1, "Portal selection is required."),
  link: z.string().url("Must be a valid URL (e.g., https://...)."),
...
const jobPostFormSchema = z.object({
  ...
  description: z
    ...
    .max(10000, "Description too long (max 10000 chars).")
    .optional()
    .or(z.literal("")),
  ...
  vacancies: z.string()
    .min(1, "Vacancies information is required.")
    .max(50, "Vacancies information too long (max 50 chars)."),
...
const CSV_HEADERS_JOB = [
  "ID", "Job Title", "Status", "Department Name", "Location", "Experience",
  "Vacancies", "Created At", "Updated At", "Updated By",
];

type JobPostExportItem = Omit<JobPostItem, 'created_at' | 'updated_at' | 'description' | 'job_plateforms' | 'created_by_user' | 'updated_by_user'> & {
...
const CSV_KEYS_JOB: (keyof JobPostExportItem)[] = [
  "id", "job_title", "status", "job_department_name", "location", "experience",
  "vacancies", "created_at_formatted", "updated_at_formatted", "updated_by_name"
];
...
function exportJobPostsToCsv(filename: string, rows: JobPostItem[], departmentOptions: JobDepartmentOption[]) {
  ...
  const transformedRows: JobPostExportItem[] = rows.map((row) => ({
    id: row.id,
    job_title: row.job_title,
    status: row.status,
    job_department_name: departmentOptions.find(d => String(d.value) === String(row.job_department_id))?.label || String(row.job_department_id),
  ...
  const csvContent =
    ...
    transformedRows
      .map((row) => {
        return CSV_KEYS_JOB.map((k) => {
          let cell = row[k as keyof JobPostExportItem];
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      <Tooltip title="Edit Job Post">
        <div
          className={classNames(iconButtonClass, hoverBgClass, "text-gray-500 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400")}
          role="button"
          onClick={() => onEdit(item)}
        ><TbPencil /></div>
      </Tooltip>
      <Tooltip title="Delete Job Post">
        <div
          className={classNames( iconButtonClass, hoverBgClass, "text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400")}
          role="button"
          onClick={() => onDelete(item)}
        ><TbTrash /></div>
      </Tooltip>
      <Dropdown renderTitle={<BsThreeDotsVertical className="ml-0.5 mr-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md text-lg" />}>
...
type JobPostsTableProps = {
  ...
  selectedItems: JobPostItem[]; onPaginationChange: (page: number) => void;
...
const JobPostsTable = ({ columns, data, loading, pagingData, selectedItems, onPaginationChange, onSelectChange, onSort, onRowSelect, onAllRowSelect }: JobPostsTableProps) => (
  <DataTable
    selectable
...
const JobPostsSelectedFooter = ({ selectedItems, onDeleteSelected, isDeleting }: JobPostsSelectedFooterProps) => {
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  if (selectedItems.length === 0) return null;
  const handleDeleteClick = () => setDeleteConfirmOpen(true);
  const handleCancelDelete = () => setDeleteConfirmOpen(false);
  const handleConfirmDelete = () => { onDeleteSelected(); setDeleteConfirmOpen(false); };
  return (
    <>
        ...
        <div className="flex items-center justify-between w-full px-4 sm:px-8">
          <span className="flex items-center gap-2">
            ...
            <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
              ...
              <span>Job Post{selectedItems.length > 1 ? "s" : ""} selected</span>
            </span>
          </span>
      ...
      <ConfirmDialog isOpen={deleteConfirmOpen} type="danger" title={`Delete ${selectedItems.length} Job Post${selectedItems.length > 1 ? "s" : ""}`}
        onClose={handleCancelDelete} onRequestClose={handleCancelDelete} onCancel={handleCancelDelete} onConfirm={handleConfirmDelete} loading={isDeleting}>
        <p>Are you sure you want to delete the selected job post{selectedItems.length > 1 ? "s" : ""}? This action cannot be undone.</p>
      </ConfirmDialog>
    </>
  );
};
...
const JobPostsListing = () => {
  ...
  const {
    jobPostsData = { data: [], counts: {} },
    departmentsData = { data: [] }, 
    status: masterLoadingStatus = "idle",
  } = useSelector(masterSelector, shallowEqual);

  const departmentOptions = useMemo(() => {
    if (Array.isArray(departmentsData?.data) && departmentsData?.data.length > 0) {
      return departmentsData?.data.map((dept: JobDepartmentListItem) => ({
  ...
  const defaultFormValues: JobPostFormData = useMemo(() => ({
    ...
    vacancies: "1", 
    status: "Active",
    job_plateforms: [{ portal: "", link: "", application_count: 0, id: null }], // MODIFIED: Add id: null
  }), [departmentOptions]);
  ...
  useEffect(() => {
    if ((isAddDrawerOpen || isEditDrawerOpen) && departmentOptions.length > 0 && !formMethods.getValues('job_department_id')) {
        formMethods.setValue('job_department_id', defaultFormValues.job_department_id, { shouldValidate: true });
    }
  }, [departmentOptions, isAddDrawerOpen, isEditDrawerOpen, formMethods, defaultFormValues.job_department_id]);
  ...
  const openEditDrawer = useCallback(
    (item: JobPostItem) => {
      ...
      if (item.job_plateforms) {
        ...
        if (typeof item.job_plateforms === 'string') {
          try {
            tempPlatforms = JSON.parse(item.job_plateforms);
          } catch (e) {
            console.error("Failed to parse job_plateforms string:", e);
          }
        } else if (Array.isArray(item.job_plateforms)) {
          tempPlatforms = item.job_plateforms;
        }

        if (Array.isArray(tempPlatforms) && tempPlatforms.length > 0) {
          parsedPlatforms = tempPlatforms.map(p => ({
            id: p.id || null, 
            portal: p.portal || "",
            link: p.link || "",
          ...
          parsedPlatforms = [{ portal: "", link: "", application_count: 0, id: null }];
        }
      }
      ...
      formMethods.reset({
        ...
        status: (item.status as JobPostStatusForm) || "Active",
        job_plateforms: parsedPlatforms,
      });
  ...
  const onSubmitHandler = async (data: JobPostFormData) => {
    ...
    const loggedInUserId = "1"; // Replace with actual user ID logic

    const jobPlatformsString = JSON.stringify(data.job_plateforms);

    const baseApiPayload = {
        status: data.status as JobPostStatusApi,
        job_title: data.job_title,
        description: data.description,
        location: data.location,
        vacancies: data.vacancies,
        experience: data.experience,
        job_plateforms: jobPlatformsString, 
        job_department_id: parseInt(data.job_department_id, 10),
    };
    ...
    try {
      if (editingItem && editingItem.id) {
        const editPayload = {
            ...
            id: editingItem.id, 
            _method: 'PUT',     
            created_by: editingItem.created_by || loggedInUserId, 
        };
  ...
  const onConfirmSingleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.push(<Notification title="Job Post Deleted" type="success" duration={2000}>{`Job post "${itemToDelete.job_title || itemToDelete.id}" deleted.`}</Notification>);
      setSelectedItems((prev) => prev.filter((d) => d.id !== itemToDelete!.id));
      dispatch(getJobPostsAction());
    } catch (error: any) {
      toast.push(<Notification title="Delete Failed" type="danger" duration={3000}>{error.message || "Failed to delete job post."}</Notification>);
    } finally {
      setIsDeleting(false);
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    setIsDeleting(true);
    const idsToDelete = selectedItems.map((item) => String(item.id)).filter(id => id);
    if (idsToDelete.length === 0) {
        setIsDeleting(false);
        return;
    }
    try {
      await dispatch(deleteAllJobPostsAction({ ids: idsToDelete.join(",") })).unwrap();
      toast.push(<Notification title="Deletion Successful" type="success" duration={2000}>{`${idsToDelete.length} job post(s) deleted.`}</Notification>);
      setSelectedItems([]);
      dispatch(getJobPostsAction());
    } catch (error: any)      {
      toast.push(<Notification title="Deletion Failed" type="danger" duration={3000}>{error.message || "Failed to delete selected job posts."}</Notification>);
    } finally {
      setIsDeleting(false);
    }
  ...
  const onApplyFiltersSubmit = useCallback((data: FilterFormData) => {
    setFilterCriteria({ filterStatus: data.filterStatus || [], filterDepartment: data.filterDepartment || [] });
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    ...
    if (filterCriteria.filterDepartment?.length) {
      ...
      processedData = processedData.filter((item) => deptValues.includes(String(item.job_department_id)));
    }
    ...
    if (order && key && processedData.length > 0 && typeof key === 'string') {
  ...
  }, [jobPostsData?.data, tableData, filterCriteria]);
  ...
  const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
    ...
    const moduleName = "JobPosts";
    const timestamp = new Date().toISOString().split("T")[0];
    const fileName = `job_posts_export_${timestamp}.csv`;
    try {
      await dispatch(submitExportReasonAction({ reason: data.reason, module: moduleName, file_name: fileName })).unwrap();
      toast.push(<Notification title="Export Reason Submitted" type="success" />);
      const exportSuccess = exportJobPostsToCsv(fileName, allFilteredAndSortedData, departmentOptions);
      if (exportSuccess) {
    ...
    () => [
      ...
      },
      {
        ...
        cell: props => {
          ...
          return <Tag className={classNames("capitalize whitespace-nowrap min-w-[70px] text-center", jobStatusColor[statusVal] || "bg-gray-100 text-gray-600")}>{statusVal}</Tag>;
        },
      },
      {
        header: "Department", accessorKey: "job_department_id", size: 160, enableSorting: true,
        cell: props => {
          const deptId = String(props.getValue());
          const department = departmentOptions.find(opt => opt.value === deptId);
          return department ? department.label : deptId;
        },
      },
      ...
      },
      {
        ...
        cell: props => <ActionColumn item={props.row.original} onEdit={openEditDrawer} onDelete={handleDeleteClick} />,
      },
    ],
  ...
  const renderDrawerForm = (
    ...
    currentEditingItem: JobPostItem | null 
  ) => {
    ...
    return (
      <>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <FormItem label={<div>Job Title<span className="text-red-500"> *</span></div>} className="md:col-span-2" invalid={!!currentFormMethods.formState.errors.job_title} errorMessage={currentFormMethods.formState.errors.job_title?.message}>
            <Controller name="job_title" control={currentFormMethods.control} render={({ field }) => <Input {...field} prefix={<TbBriefcase />} placeholder="e.g., Software Engineer" />} />
          </FormItem>
          <FormItem label={<div>Job Department<span className="text-red-500"> *</span></div>} invalid={!!currentFormMethods.formState.errors.job_department_id} errorMessage={currentFormMethods.formState.errors.job_department_id?.message}>
            <Controller name="job_department_id" control={currentFormMethods.control} render={({ field }) => (
              <Select placeholder={departmentOptions.length > 0 ? "Select Department" : "Loading..."} options={departmentOptions} value={departmentOptions.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} disabled={departmentOptions.length === 0 && masterLoadingStatus === "loading"} />
            )} />
          </FormItem>
          <FormItem label={<div>Status<span className="text-red-500"> *</span></div>} invalid={!!currentFormMethods.formState.errors.status} errorMessage={currentFormMethods.formState.errors.status?.message}>
            <Controller name="status" control={currentFormMethods.control} render={({ field }) => (
              <Select placeholder="Select Status" options={JOB_POST_STATUS_OPTIONS_FORM} value={JOB_POST_STATUS_OPTIONS_FORM.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} />
            )} />
          </FormItem>
          <FormItem label="Location" className="md:col-span-2" invalid={!!currentFormMethods.formState.errors.location} errorMessage={currentFormMethods.formState.errors.location?.message}>
          ...
          <FormItem label="Total Vacancies" invalid={!!currentFormMethods.formState.errors.vacancies} errorMessage={currentFormMethods.formState.errors.vacancies?.message}>
            <Controller name="vacancies" control={currentFormMethods.control} render={({ field }) => <Input {...field} type="text" prefix={<TbUsers />} placeholder="e.g., 5 or 'Multiple'" />} />
          </FormItem>
        ...
        <div className="md:col-span-2 mt-4"> 
          <div className="flex justify-between items-center mb-2">
            <h6 className="text-gray-700 dark:text-gray-200">Job Platforms<span className="text-red-500"> *</span></h6>
            <Button type="button" size="sm" variant="outline" onClick={() => append({ portal: "", link: "", application_count: 0, id: null })}>Add Platform</Button>
          </div>
          {currentFormMethods.formState.errors.job_plateforms && !currentFormMethods.formState.errors.job_plateforms.root && !Array.isArray(currentFormMethods.formState.errors.job_plateforms) && (
            <div className="text-red-500 text-xs mb-2">{currentFormMethods.formState.errors.job_plateforms.message}</div>
          )}
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto_auto] gap-x-3 gap-y-2 border p-3 rounded-md mb-3 items-end">
              <FormItem label="Portal" className="mb-0" invalid={!!currentFormMethods.formState.errors.job_plateforms?.[index]?.portal} errorMessage={currentFormMethods.formState.errors.job_plateforms?.[index]?.portal?.message}>
                <Controller name={`job_plateforms.${index}.portal`} control={currentFormMethods.control} render={({ field: controllerField }) => (
                  <Select {...controllerField} placeholder="Select Portal" options={PORTAL_OPTIONS} value={PORTAL_OPTIONS.find(o => o.value === controllerField.value)} onChange={opt => controllerField.onChange(opt?.value)} />
                )} />
              </FormItem>
              <FormItem label="Link" className="mb-0" invalid={!!currentFormMethods.formState.errors.job_plateforms?.[index]?.link} errorMessage={currentFormMethods.formState.errors.job_plateforms?.[index]?.link?.message}>
                <Controller name={`job_plateforms.${index}.link`} control={currentFormMethods.control} render={({ field: controllerField }) => <Input {...controllerField} placeholder="https://linkedin.com/jobs/..." />} />
              </FormItem>
              <FormItem label="App. Count" className="mb-0 md:w-32" invalid={!!currentFormMethods.formState.errors.job_plateforms?.[index]?.application_count} errorMessage={currentFormMethods.formState.errors.job_plateforms?.[index]?.application_count?.message}>
                <Controller name={`job_plateforms.${index}.application_count`} control={currentFormMethods.control} render={({ field: controllerField }) => <Input {...controllerField} type="number" min={0} placeholder="0" />} />
              </FormItem>
              <Button type="button" size="sm" variant="plain" icon={<TbTrash />} onClick={() => remove(index)} className="text-red-500 self-center mb-1 md:mb-0" disabled={fields.length <= 1}>
              </Button>
            </div>
          ))}
        </div>
        ...
        {currentEditingItem && (
            <div className="mt-6"> 
              <div className="grid grid-cols-[2fr_3fr] text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded">
                <div>
                  <b className="font-semibold text-primary">Latest Update:</b>
                  <p className="text-sm font-semibold mt-1">{currentEditingItem.updated_by_user?.name || "N/A"}</p>
                  <p>{currentEditingItem.updated_by_user?.roles?.[0]?.display_name || "N/A"}</p>
                </div>
                <div>
                  <br />
  ...
  return (
    <>
        ...
        <AdaptiveCard className="h-full" bodyClass="h-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 mb-4 gap-4">
            <Card bodyClass="flex gap-2 p-3" className="rounded-md border border-blue-200 dark:border-blue-700">
              <div className="h-12 w-12 rounded-md flex items-center justify-center bg-blue-100 dark:bg-blue-500/20 text-blue-500 dark:text-blue-200"><TbBriefcase size={24} /></div>
              <div><h6 className="text-blue-500 dark:text-blue-200">{jobPostsData?.counts?.total || 0}</h6><span className="font-semibold text-xs">Total</span></div>
            </Card>
            <Card bodyClass="flex gap-2 p-3" className="rounded-md border border-violet-200 dark:border-violet-700">
              <div className="h-12 w-12 rounded-md flex items-center justify-center bg-violet-100 dark:bg-violet-500/20 text-violet-500 dark:text-violet-200"><TbFileCheck size={24} /></div>
              <div><h6 className="text-violet-500 dark:text-violet-200">{jobPostsData?.counts?.active || 0}</h6><span className="font-semibold text-xs">Active</span></div>
            </Card>
            <Card bodyClass="flex gap-2 p-3" className="rounded-md border border-red-200 dark:border-red-700">
              <div className="h-12 w-12 rounded-md flex items-center justify-center bg-red-100 dark:bg-red-500/20 text-red-500 dark:text-red-200"><TbFileExcel size={24} /></div>
              <div><h6 className="text-red-500 dark:text-red-200">{jobPostsData?.counts?.expired || 0}</h6><span className="font-semibold text-xs">Expired</span></div>
            </Card>
             <Card bodyClass="flex gap-2 p-3" className="rounded-md border border-orange-200 dark:border-orange-700">
              <div className="h-12 w-12 rounded-md flex items-center justify-center bg-orange-100 dark:bg-orange-500/20 text-orange-500 dark:text-orange-200"><TbFileSmile size={24} /></div>
              <div><h6 className="text-orange-500 dark:text-orange-200">{jobPostsData?.counts?.views || 0}</h6><span className="font-semibold text-xs">Total Views</span></div>
            </Card>
            <Card bodyClass="flex gap-2 p-3" className="rounded-md border border-green-200 dark:border-green-700">
              <div className="h-12 w-12 rounded-md flex items-center justify-center bg-green-100 dark:bg-green-500/20 text-green-500 dark:text-green-200"><TbFileLike size={24} /></div>
              <div><h6 className="text-green-500 dark:text-green-200">{jobPostsData?.counts?.applicants || 0}</h6><span className="font-semibold text-xs">Applicants</span></div>
            </Card>
          </div>
          ...
          <div className="mt-4">
            <JobPostsTable columns={columns} data={pageData} loading={masterLoadingStatus === "loading" || isSubmitting || isDeleting }
              pagingData={{ total, pageIndex: tableData.pageIndex as number, pageSize: tableData.pageSize as number }}
              selectedItems={selectedItems} onPaginationChange={handlePaginationChange} onSelectChange={handleSelectChange} onSort={handleSort}
      ...
      <Drawer title={editingItem ? "Edit Job Post" : "Add New Job Post"} isOpen={isAddDrawerOpen || isEditDrawerOpen}
        onClose={editingItem ? closeEditDrawer : closeAddDrawer} onRequestClose={editingItem ? closeEditDrawer : closeAddDrawer}
        width={700}
        footer={
      ...
      >
        <Form id="jobPostForm" onSubmit={formMethods.handleSubmit(onSubmitHandler)} className="flex flex-col gap-2 pb-20">
          {renderDrawerForm(formMethods, editingItem)}
        </Form>
      </Drawer>
      ...
      <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer}
        footer={
          <div className="text-right w-full flex justify-end gap-2">
            <Button size="sm" onClick={onClearFilters} type="button">Clear</Button>
            <Button size="sm" variant="solid" form="filterJobPostForm" type="submit">Apply</Button>
          </div>
        }
      >
        <Form id="filterJobPostForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
          <FormItem label="Filter by Status">
            <Controller name="filterStatus" control={filterFormMethods.control} render={({ field }) => (
              <Select isMulti placeholder="Any Status" options={JOB_POST_STATUS_OPTIONS_FORM.map(s => ({ value: s.value, label: s.label }))} value={field.value || []} onChange={val => field.onChange(val || [])} />
            )} />
          </FormItem>
          <FormItem label="Filter by Department">
            <Controller name="filterDepartment" control={filterFormMethods.control} render={({ field }) => (
              <Select isMulti placeholder={departmentOptions.length > 0 ? "Any Department" : "Loading..."} options={departmentOptions} value={field.value || []} onChange={val => field.onChange(val || [])} disabled={departmentOptions.length === 0 && masterLoadingStatus === "loading"} />
            )} />
          </FormItem>
        </Form>
      </Drawer>
      ...
      <ConfirmDialog isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Job Post"
        onClose={() => { setSingleDeleteConfirmOpen(false); setItemToDelete(null); }} onRequestClose={() => { setSingleDeleteConfirmOpen(false); setItemToDelete(null); }}
        onCancel={() => { setSingleDeleteConfirmOpen(false); setItemToDelete(null); }} onConfirm={onConfirmSingleDelete} loading={isDeleting}
      >
