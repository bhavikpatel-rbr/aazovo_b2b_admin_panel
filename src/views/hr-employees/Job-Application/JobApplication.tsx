...
const AddJobLinkDialog: React.FC<{ application: JobApplicationItemInternal; onClose: () => void; onLinkSubmit: (id: string, link: string) => void; }> = ({ application, onClose, onLinkSubmit }) => { const { control, handleSubmit } = useForm({ defaultValues: { link: application.jobApplicationLink || '' } }); const onSubmit = (data: { link: string }) => { onLinkSubmit(application.id, data.link) }; return ( <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}> <h5 className="mb-4">Generate Job Link for {application.name}</h5> <Form onSubmit={handleSubmit(onSubmit)}> <FormItem label="Job Application Link"><Controller name="link" control={control} render={({ field }) => (<Input {...field} placeholder="https://your-career-page.com/apply/..." />)} /></FormItem> <div className="text-right mt-6"><Button type="button" className="mr-2" onClick={onClose}>Cancel</Button><Button variant="solid" type="submit">Save</Button></div> </Form> </Dialog> ); };
const GenericActionDialog: React.FC<{ type: ModalType | null; application: JobApplicationItemInternal; onClose: () => void; }> = ({ type, application, onClose }) => { const [isLoading, setIsLoading] = useState(false); const title = type ? `Confirm: ${type.charAt(0).toUpperCase() + type.slice(1)}` : 'Confirm Action'; const handleConfirm = () => { setIsLoading(true); console.log(`Performing action '${type}' for application ${application.name}`); setTimeout(() => { toast.push(<Notification type="success" title="Action Completed" />); setIsLoading(false); onClose(); }, 1000) }; return ( <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}> <h5 className="mb-2">{title}</h5><p>Are you sure you want to perform this action for <span className="font-semibold">{application.name}</span>'s application?</p> <div className="text-right mt-6"><Button className="mr-2" onClick={onClose} disabled={isLoading}>Cancel</Button><Button variant="solid" onClick={handleConfirm} loading={isLoading}>Confirm</Button></div> </Dialog> ); };
const JobApplicationModals: React.FC<JobApplicationModalsProps> = ({ modalState, onClose, onLinkSubmit, }) => { const { type, data: application, isOpen } = modalState; if (!isOpen || !application) return null; const renderModalContent = () => { switch (type) { case 'link': return (<AddJobLinkDialog application={application} onClose={onClose} onLinkSubmit={onLinkSubmit} />); case 'convert': default: return (<GenericActionDialog type={type} application={application} onClose={onClose} />); } }; return <>{renderModalContent()}</>; };

// --- Other Components ---
const ActionColumn = ({ item, onView, onEdit, onDownloadResume, onOpenModal, onAddNotification, onAddSchedule, onAssignToTask, onMarkAsActive, onSendEmail, onSendWhatsapp, }: { item: JobApplicationItemInternal; onView: () => void; onEdit: () => void; onDownloadResume: () => void; onOpenModal: (type: ModalType, data: JobApplicationItemInternal) => void; onAddNotification: () => void; onAddSchedule: () => void; onAssignToTask: () => void; onMarkAsActive: () => void; onSendEmail: () => void; onSendWhatsapp: () => void; }) => { const iconButtonClass = 'text-lg p-0.5 rounded-md transition-colors duration-150 ease-in-out cursor-pointer select-none'; const hoverBgClass = 'hover:bg-gray-100 dark:hover:bg-gray-700'; return ( <div className="flex items-center justify-end"> <Tooltip title="View Details"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400')} role="button" onClick={onView}><TbEye /></div></Tooltip> <Tooltip title="Edit Application"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400')} role="button" onClick={onEdit}><TbPencil /></div></Tooltip> <Dropdown renderTitle={<BsThreeDotsVertical className="ml-0.5 mr-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md" />}> <Dropdown.Item onClick={onAddNotification} className="flex items-center gap-2"><TbBell size={18} />{' '}<span className="text-xs">Add as Notification</span></Dropdown.Item> <Dropdown.Item onClick={onMarkAsActive} className="flex items-center gap-2"><TbTagStarred size={18} />{' '}<span className="text-xs">Mark as Active</span></Dropdown.Item> <Dropdown.Item onClick={onAddSchedule} className="flex items-center gap-2"><TbCalendarEvent size={18} />{' '}<span className="text-xs">Schedule Interview</span></Dropdown.Item> <Dropdown.Item onClick={onAssignToTask} className="flex items-center gap-2"><TbUser size={18} />{' '}<span className="text-xs">Assign to Task</span></Dropdown.Item> <Dropdown.Item onClick={() => onOpenModal('link', item)} className="flex items-center gap-2"><TbLink size={18} />{' '}<span className="text-xs">Generate Job Link</span></Dropdown.Item> <Dropdown.Item onClick={onDownloadResume} className="flex items-center gap-2"><TbDownload size={18} />{' '}<span className="text-xs">Download Resume</span></Dropdown.Item> <Dropdown.Item onClick={() => onOpenModal('convert', item)} className="flex items-center gap-2"><TbUserShare size={18} />{' '}<span className="text-xs">Convert to Employee</span></Dropdown.Item> <Dropdown.Item onClick={onSendEmail} className="flex items-center gap-2"><TbMail size={18} /> <span className="text-xs"> Send Email</span></Dropdown.Item> <Dropdown.Item onClick={onSendWhatsapp} className="flex items-center gap-2"><TbBrandWhatsapp size={18} />{' '}<span className="text-xs">Send on Whatsapp</span></Dropdown.Item> </Dropdown> </div> ); };
    ...
    const userOptions: SelectOption[] = useMemo(
        () =>
            Array.isArray(getAllUserData)
                ? getAllUserData.map((user: UserItem) => ({
...
const { useEncryptApplicationStorage } = config;
    ...
    // --- MODIFIED: Added state for special filters ---
...
useEffect(() => {
    ...
    const processedAndSortedData = useMemo(() => {
        ...
        // --- MODIFIED: Apply special, exclusive filters first ---
        if (showOnlyScheduled) {
            processedDataResult = processedDataResult.filter(app => app.isSchedule === 1);
        } else if (showOnlyToday) {
            console.log("processedDataResult",processedDataResult);
            
            // This line will now work correctly
            processedDataResult = processedDataResult.filter(app => dayjs(app.applicationDate).isToday());
        }
    ...
    }, [transformedJobApplications, tableData.query, tableData.sort, filterCriteria, showOnlyScheduled, showOnlyToday])
    ...
    const onApplyFiltersSubmit = useCallback((data: FilterFormData) => { setFilterCriteria({ filterStatus: data.filterStatus || [], filterDepartment: data.filterDepartment || [] }); handleSetTableData({ pageIndex: 1 }); setShowOnlyScheduled(false); setShowOnlyToday(false); closeFilterDrawer(); }, [handleSetTableData, closeFilterDrawer]);
    ...
    const handleToggleScheduleView = useCallback(() => {
        clearSpecialFilters();
        setShowOnlyToday(false);
        setShowOnlyScheduled(prev => !prev);
    }, [filterFormMethods, handleSetTableData]);

    // --- MODIFIED: New handler for "Today" card ---
    ...
    const handleSendEmail = useCallback((application: JobApplicationItemInternal) => { const subject = `Regarding your application for ${application.jobTitle}`; const body = `Dear ${application.name},\n\nThank you for your interest in the ${application.jobTitle} position.\n\nWe are currently reviewing applications and will be in touch shortly.\n\nBest regards,\nThe Hiring Team`; window.open(`mailto:${application.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`); }, []);
    const handleSendWhatsapp = useCallback((application: JobApplicationItemInternal) => { const phone = application.mobileNo?.replace(/\D/g, ''); if (!phone) { toast.push(<Notification type="warning" title="No Mobile Number" children="This applicant does not have a mobile number." />); return; } const message = `Hi ${application.name}, this is a message regarding your application for the ${application.jobTitle} position.`; window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, '_blank'); }, []);
    ...
    const columns: ColumnDef<JobApplicationItemInternal>[] = useMemo( () => [ { header: 'Applicant', accessorKey: 'name', cell: (props) => ( <div className="flex items-center"> <Avatar size={28} shape="circle" src={props.row.original.avatar} icon={<TbUserCircle />} > {!props.row.original.avatar && props.row.original.name ? props.row.original.name.charAt(0).toUpperCase() : ''} </Avatar> <div className="ml-2"> <span className="font-semibold">{props.row.original.name}</span> <div className="text-xs text-gray-500">{props.row.original.email}</div> </div> </div> ), }, { header: 'Status', accessorKey: 'status', width: 120, cell: (props) => (<Tag className={`${applicationStatusColor[props.row.original.status]} text-white capitalize px-2 py-1 text-xs`}>{props.row.original.status}</Tag>), }, { header: 'Mobile', accessorKey: 'mobileNo', width: 140, cell: (props) => props.row.original.mobileNo || '-', }, { header: 'Department', accessorKey: 'departmentName', width: 160, cell: (props) => props.row.original.departmentName || '-', }, { header: 'Job Title', accessorKey: 'jobTitle', width: 200, cell: (props) => props.row.original.jobTitle || 'N/A', }, { header: 'Experience', accessorKey: 'workExperience', width: 150, cell: (props) => props.row.original.workExperience || 'N/A', }, { header: 'Applied Date', accessorKey: 'applicationDate', width: 160, cell: (props) => { const formattedDate = props.row.original.applicationDate ? dayjs(props.row.original.applicationDate).format('DD MMM YYYY, h:mm A') : 'N/A'; return (<div className="text-xs"><span>{formattedDate}</span></div>); }, }, { header: 'Action', id: 'action', width: 130, meta: { HeaderClass: 'text-center' }, cell: (props) => ( <ActionColumn item={props.row.original} onView={() => handleViewDetails(props.row.original)} onEdit={() => openEditDrawer(props.row.original)} onDownloadResume={() => handleDownloadResume(props.row.original)} onOpenModal={handleOpenModal} onAddNotification={() => openNotificationModal(props.row.original)} onAddSchedule={() => openScheduleModal(props.row.original)} onAssignToTask={() => openAssignToTaskModal(props.row.original)} onMarkAsActive={() => openActivityModal(props.row.original)} onSendEmail={() => handleSendEmail(props.row.original)} onSendWhatsapp={() => handleSendWhatsapp(props.row.original)} /> ), }, ], [navigate, handleViewDetails, openEditDrawer, handleDownloadResume, handleOpenModal, openNotificationModal, openScheduleModal, openAssignToTaskModal, openActivityModal, handleSendEmail, handleSendWhatsapp], )
            ...
            <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                <div className="lg:flex items-center justify-between mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3">
                        <Button icon={<TbCalendarTime />} className="w-full sm:w-auto" variant={showOnlyScheduled ? 'solid' : 'default'} onClick={handleToggleScheduleView} >
                            {showOnlyScheduled ? 'View All' : 'View Schedule'}
                        </Button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 mb-4 gap-2">
                   <Tooltip title="Click to show all applications"><div onClick={() => handleCardClick()} className={cardClass}><Card bodyClass="flex gap-2 p-2" className="rounded-md border-blue-200"><div className="h-12 w-12 rounded-md flex items-center justify-center bg-blue-100 text-blue-500"><TbMail size={24} /></div><div><h6 className="text-blue-500">{jobApplicationsData?.counts?.total || 0}</h6><span className="font-semibold text-xs">Total</span></div></Card></div></Tooltip>
                    ...
                    {/* --- MODIFIED: Added onClick and dynamic class to "Today" card --- */}
