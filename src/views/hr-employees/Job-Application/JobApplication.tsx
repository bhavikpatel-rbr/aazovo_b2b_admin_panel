...
const ApplicationDetailDialog = ({ isOpen, onClose, application, }: { isOpen: boolean; onClose: (e?: MouseEvent) => void; application: JobApplicationItemInternal | null; }) => { if (!application) return null; const detailItemClass = 'py-2 border-b border-gray-100 dark:border-gray-700 last:border-b-0'; const labelClass = 'text-xs font-semibold text-gray-500 dark:text-gray-400 mb-0.5'; const valueClass = 'text-sm text-gray-800 dark:text-gray-200'; return (<Dialog isOpen={isOpen} onClose={onClose} onRequestClose={onClose} title={`Application Details: ${application.name}`} width={600} > <div className="mt-4 space-y-3 max-h-[70vh] overflow-y-auto pr-2"> <div className={detailItemClass}><p className={labelClass}>Applicant Name:</p><p className={valueClass}>{application.name}</p></div> <div className={detailItemClass}><p className={labelClass}>Email:</p><p className={valueClass}>{application.email}</p></div> <div className={detailItemClass}><p className={labelClass}>Mobile No:</p><p className={valueClass}>{application.mobileNo || ' '}</p></div> <div className={detailItemClass}><p className={labelClass}>Status:</p><Tag className={`${applicationStatusColor[application.status]} text-white capitalize px-2 py-1 text-xs`}>{application.status.replace(/_/g, ' ')}</Tag></div> <div className={detailItemClass}><p className={labelClass}>Department:</p><p className={valueClass}>{application.departmentName || ' '}</p></div> <div className={detailItemClass}><p className={labelClass}>Job Title:</p><p className={valueClass}>{application.jobTitle || ' '}</p></div> <div className={detailItemClass}><p className={labelClass}>Work Experience:</p><p className={valueClass}>{application.workExperience || ' '}</p></div> <div className={detailItemClass}><p className={labelClass}>Application Date:</p><p className={valueClass}>{application.applicationDate ? dayjs(application.applicationDate).format(' D MMM YYYY, h:mm A') : ' '}</p></div> {application.resumeUrl && (<div className={detailItemClass}><p className={labelClass}>Resume URL:</p><a href={application.resumeUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">{application.resumeUrl}</a></div>)} <div className={detailItemClass}><p className={labelClass}>Notes:</p><p className={valueClass} style={{ whiteSpace: 'pre-wrap' }}>{application.notes || ' '}</p></div> </div> <div className="text-right mt-6"><Button variant="solid" onClick={onClose}>Close</Button></div> </Dialog>); };
...
const JobApplicationListing = () => {
    ...
    const userOptions = useMemo(() => Array.isArray(getAllUserData) ? getAllUserData.map((user: UserItem) => ({ value: String(user.id), label: `(${user.employee_id}) - ${user.name || ' '}` })) : [], [getAllUserData]);
    ...
    const closeScheduleModal = useCallback(async() => { setScheduleItem(null); setIsScheduleModalOpen(false); await dispatch(getJobApplicationsAction()); }, []);
    const openAssignToTaskModal = useCallback((item: JobApplicationItemInternal) => { setTaskTargetItem(item); setIsTaskModalOpen(true); }, []);
    const closeAssignToTaskModal = useCallback(async () => { setTaskTargetItem(null); setIsTaskModalOpen(false);  }, []);
    ...
    const columns: ColumnDef<JobApplicationItemInternal>[] = useMemo(() => [
        ...
        { header: 'Job Title', accessorKey: 'jobTitle', width: 200, cell: (props) => props.row.original.jobTitle || ' ' },
        { header: 'Experience', accessorKey: 'workExperience', width: 150, cell: (props) => props.row.original.workExperience || ' ' },
        { header: 'Applied Date', accessorKey: 'applicationDate', width: 160, cell: (props) => <div className="text-xs">{props.row.original.applicationDate ? dayjs(props.row.original.applicationDate).format('DD MMM YYYY, h:mm A') : ' '}</div> },
