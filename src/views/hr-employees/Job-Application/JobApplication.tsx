...
const JobApplicationListing = () => {
    ...
    // --- RESTORED: State for all action dialogs ---
    ...
    // --- RESTORED: Dialog open/close handlers ---
    ...
    // --- RESTORED: Action confirmation handlers ---
    ...
    return (
        <Container className="h-auto">
            <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                <div className="lg:flex items-center justify-between mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3">
                        <Button icon={<TbCalendarTime />} className="w-full sm:w-auto" variant={showOnlyScheduled ? 'solid' : 'default'} onClick={handleToggleScheduleView}> {showOnlyScheduled ? 'View All' : 'View Schedule'} </Button>
                ...
                <div className="mb-4">
                    <ApplicationTableTools onSearchChange={handleSearchChange} onFilterOpen={() => setIsFilterDrawerOpen(true)} onClearFilters={onClearFilters} onExport={handleOpenExportReasonModal} columns={columns} filteredColumns={filteredColumns} setFilteredColumns={setFilteredColumns} activeFilterCount={activeFilterCount} />
                </div>
            ...
            {/* RESTORED: All action dialogs are now rendered */}
            ...
            <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={() => setIsFilterDrawerOpen(false)} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={onClearFilters}>Clear</Button><Button size="sm" variant="solid" form="filterAppForm" type="submit">Apply</Button></div>}>
                 <Form id="filterAppForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    ...
                    <FormItem label="Department"><Controller name="filterDepartment" control={filterFormMethods.control} render={({ field }) => <Select isMulti placeholder="Select department(s)..." options={departmentOptionsForFilter} value={field.value || []} onChange={val => field.onChange(val || [])} />} /></FormItem>
                </Form>
            </Drawer>
        </Container>
    );
