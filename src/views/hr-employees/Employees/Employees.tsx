...
const EmployeesListing = () => {
    ...
    const columns: ColumnDef<EmployeeItem>[] = useMemo(() => [
        { header: "Status", accessorKey: "status", cell: (props) => { const { status } = props.row.original || {}; const validStatus: EmployeeStatus = EMPLOYEE_STATUS_OPTIONS.some(o => o.value === status) ? status : 'Blocked'; const statusLabel = EMPLOYEE_STATUS_OPTIONS.find(o => o.value === validStatus)?.label || 'Blocked'; return (<Tag className={`${employeeStatusColor[validStatus]} capitalize font-semibold border-0`}>{statusLabel}</Tag>); } },
