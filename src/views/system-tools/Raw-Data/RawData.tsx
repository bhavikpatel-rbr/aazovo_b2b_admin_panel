...
import {
  ...
  Card,
  Dropdown,
} from "@/components/ui"; // Removed Upload as not directly used in this pattern
...
// --- CSV Exporter ---
type RowDataExportItem = Omit<
  RowDataItem,
  "created_at" | "updated_at" | "country" | "category" | "brand"
> & {
...
function exportRowDataToCsvLogic(
  filename: string,
  rows: RowDataItem[],
  ...
  if (!rows || !rows.length) {
    return false;
  } // Caller handles toast for no data
  const preparedRows: RowDataExportItem[] = rows.map((row) => ({
    ...
    countryNameDisplay:
      countryOptions.find((c) => c.value === String(row.country_id))?.label ||
      String(row.country_id),
    categoryNameDisplay:
      categoryOptions.find((c) => c.value === String(row.category_id))?.label ||
      String(row.category_id),
    brandNameDisplay:
      brandOptions.find((b) => b.value === String(row.brand_id))?.label ||
      String(row.brand_id),
    statusDisplay:
      STATUS_OPTIONS_UI.find((s) => s.value === row.status)?.label ||
      row.status,
    qualityDisplay:
      QUALITY_LEVELS_UI.find((q) => q.value === row.quality)?.label ||
      row.quality,
  ...
  const separator = ",";
  const csvContent =
    CSV_HEADERS_ROW_DATA.join(separator) +
    "\n" +
    preparedRows
      .map((row) =>
        CSV_KEYS_ROW_DATA_EXPORT.map((k) => {
          let cell = row[k as keyof RowDataExportItem];
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      {/* <Tooltip title="Convert to Member">
        <Link to={`/business-entities/member-create?rowDataId=${item.id}`} state={{ rowData?.data: item }} className={`text-xl cursor-pointer select-none text-gray-500 hover:text-violet-600 dark:text-gray-400 dark:hover:text-violet-400`} role="button"><TbUserShare /></Link>
      </Tooltip> */}
      ...
      >
        <Dropdown.Item className="flex items-center gap-2">
          <TbTrash size={18} onClick={onDelete} />{" "}
          <span className="text-xs">Delete</span>
        </Dropdown.Item>
        <Dropdown.Item className="flex items-center gap-2">
          <TbUserShare size={18} />
          <Link
            to={`/business-entities/member-create?rowDataId=${item.id}`}
            state={{ rowData: item }}
          >
            {" "}
            <span className="text-xs">Convert to Member</span>
          </Link>
        </Dropdown.Item>
        {item.status !== "Blacklist" && (
          <Dropdown.Item className="flex items-center gap-2">
            <TbCancel size={18} onClick={onBlacklist} />{" "}
            <span className="text-xs">Add As Blacklist</span>
          </Dropdown.Item>
        )}
      </Dropdown>
    </div>
...
type RowDataSelectedFooterProps = {
  selectedItems: RowDataItem[];
  onDeleteSelected: () => void;
  isDeleting: boolean;
};
const RowDataSelectedFooter = ({
  selectedItems,
  onDeleteSelected,
  isDeleting,
}: RowDataSelectedFooterProps) => {
  ...
  return (
    <>
      ...
      >
        <p>
          Are you sure you want to delete the selected row data item
...
const RowDataListing = () => {
  ...
  const [itemToBlacklist, setItemToBlacklist] = useState<RowDataItem | null>(
    null
  );
  ...
  useEffect(() => {
    setCategoryOptions(
      Array.isArray(CategoriesData)
        ? CategoriesData.map((c: CategoryListItem) => ({
            value: String(c.id),
            label: c.name,
          }))
        : []
  ...
  const openViewDialog = useCallback(
    (item: RowDataItem) => setViewingItem(item),
    []
  );
  ...
  const onSubmitHandler = async (data: RowDataFormData) => {
    ...
    try {
      if (editingItem) {
        await dispatch(
          editRowDataAction({ id: editingItem.id, ...apiPayload })
        ).unwrap();
        toast.push(
          <Notification
        ...
        toast.push(
          <Notification title="Raw Data Added" type="success" duration={2000} />
        );
        closeAddDrawer();
        dispatch(getRowDataAction());
      }
      ...
      toast.push(
        <Notification
          title={(editingItem ? "Update" : "Add") + " Failed"}
          type="danger"
          duration={3000}
        >
  ...
  const handleDeleteClick = useCallback((item: RowDataItem) => {
    if (!item.id) return;
  ...
  const onConfirmSingleDelete = useCallback(async () => {
    ...
    try {
      await dispatch(
        deleteRowDataAction({ id: String(itemToDelete.id) })
      ).unwrap();
      toast.push(
        <Notification
          title="Raw Data Deleted"
          type="success"
          duration={2000}
        >{`Entry "${
          itemToDelete.name || itemToDelete.mobile_no
      ...
      toast.push(
        <Notification title="Delete Failed" type="danger" duration={3000}>
          {(e as Error).message || "Could not delete item."}
        </Notification>
      );
    } finally {
      setIsDeleting(false);
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      await dispatch(
        deleteAllRowDataAction({ ids: idsToDelete.join(",") })
      ).unwrap();
      toast.push(
        <Notification
          title="Raw Data Deleted"
          type="success"
          duration={2000}
        >{`${validItems.length} item(s) deleted.`}</Notification>
      );
      setSelectedItems([]);
      dispatch(getRowDataAction());
    } catch (e: any) {
      toast.push(
        <Notification title="Delete Failed" type="danger" duration={3000}>
          {(e as Error).message || "Failed to delete selected items."}
        </Notification>
      );
    } finally {
      setIsDeleting(false);
  ...
  const handleBlacklistClick = useCallback((item: RowDataItem) => {
    setItemToBlacklist(item);
  ...
  const onConfirmBlacklist = async () => {
    ...
    const payload: any = {
      ...itemToBlacklist,
      status: "Blacklist",
      country_id: String(itemToBlacklist.country_id),
      category_id: String(itemToBlacklist.category_id),
      brand_id: String(itemToBlacklist.brand_id),
    };
    ...
    try {
      ...
      toast.push(
        <Notification
          title="Raw Data Blacklisted"
          type="warning"
          duration={2000}
        >{`Entry "${
          itemToBlacklist.name || itemToBlacklist.mobile_no
        }" blacklisted.`}</Notification>
      );
      dispatch(getRowDataAction());
    } catch (e: any) {
      toast.push(
        <Notification title="Blacklist Failed" type="danger" duration={3000}>
          {(e as Error).message}
        </Notification>
      );
    } finally {
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    const sourceData: RowDataItem[] = Array.isArray(rowData?.data)
      ? rowData?.data
      : [];
    let processedData: RowDataItem[] = cloneDeep(sourceData);

    if (filterCriteria.filterCountry?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterCountry!.some(
          (fc) => fc.value === String(item.country_id)
        )
      );
    if (filterCriteria.filterCategory?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterCategory!.some(
          (fc) => fc.value === String(item.category_id)
        )
      );
    if (filterCriteria.filterBrand?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterBrand!.some(
          (fb) => fb.value === String(item.brand_id)
        )
      );
    if (filterCriteria.filterStatus?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterStatus!.some((fs) => fs.value === item.status)
      );
    if (filterCriteria.filterQuality?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterQuality!.some((fq) => fq.value === item.quality)
      );
    ...
    if (tableData.query && tableData.query.trim() !== "") {
      ...
      processedData = processedData.filter(
        (item) =>
          ...
          (
            item.country?.name?.toLowerCase() || String(item.country_id)
          ).includes(query) || // Added null check for country.name
          (
            item.category?.name?.toLowerCase() || String(item.category_id)
          ).includes(query) || // Added null check for category.name
          (item.brand?.name?.toLowerCase() || String(item.brand_id)).includes(
            query
          ) || // Added null check for brand.name
    ...
    if (order && key && validSortKeys.includes(String(key) as any)) {
      processedData.sort((a, b) => {
        ...
        if (key === "created_at" || key === "updated_at") {
          ...
          aVal = a[key as keyof RowDataItem] ?? "";
          bVal = b[key as keyof RowDataItem] ?? "";
        }
  ...
  const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
    ...
    const fileName = `raw-data_${date}.csv`; // Dynamic filename
    try {
  ...
  const handleRowSelect = useCallback((checked: boolean, row: RowDataItem) => {
  ...
  const handleAllRowSelect = useCallback(
    (checked: boolean, currentRows: Row<RowDataItem>[]) => {
    ...
    () => [
      // { header: 'ID', accessorKey: 'id', enableSorting: true, size: 60, meta: { tdClass: "text-center", thClass: "text-center" } },
      {
        ...
        size: 160,
          ...
          return (
            <>
            <span className="">{props.row.original.id} - {props.row.original.name}</span>
      ...
      // },
      {
        ...
        size: 160,
        cell: (props) =>
          props.row.original.country?.name || String(props.getValue()),
      },
      {
        header: "Category",
        ...
        size: 170,
        cell: (props) =>
          props.row.original.category?.name || String(props.getValue()),
      },
      {
        header: "Brand",
        ...
        size: 160,
        cell: (props) =>
          props.row.original.brand?.name || String(props.getValue()),
      },
      {
        header: "Quality",
        accessorKey: "quality",
        ...
        size: 100,
        cell: (props) => {
          const qVal = props.getValue<string>();
          const qOpt = QUALITY_LEVELS_UI.find((q) => q.value === qVal);
          return (
            <Tag
              className={classNames(
                "capitalize",
                qVal === "A" &&
                  "bg-green-100 text-green-600 dark:bg-green-500/20 dark:text-green-100",
                qVal === "B" &&
      ...
      },
      {
        ...
        size: 110,
        cell: (props) => {
          const sVal = props.getValue<string>();
          const sOpt = STATUS_OPTIONS_UI.find((s) => s.value === sVal);
          return (
            <Tag className={classNames("capitalize", statusColors[sVal])}>
              {sOpt?.label || sVal}
            </Tag>
          );
        },
      },
      {
        ...
        enableSorting: true,
        size: 170, // Matched size with Domain module
        cell: (props) => {
      ...
      },
      {
        ...
        cell: (props) => (
          <ActionColumn
            ...
            onBlacklist={() => handleBlacklistClick(props.row.original)}
          />
        ),
      },
    ...
    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-2">
        ...
        <Controller
          ...
          render={({ field }) => (
            <Input
              {...field}
              prefix={<TbUser />}
              placeholder="Contact Name / Lead Name"
            />
          )}
        />
      </FormItem>
      ...
      >
        <Controller
          ...
          control={formMethods.control}
          render={({ field }) => (
            <Input {...field} prefix={<TbBuilding />} placeholder="ABC Corp" />
          )}
        />
      </FormItem>
      <FormItem
        ...
        errorMessage={formMethods.formState.errors.country_id?.message}
      >
        <Controller
          name="country_id"
          control={formMethods.control}
      ...
      <FormItem
        ...
        errorMessage={formMethods.formState.errors.category_id?.message}
      >
        <Controller
          name="category_id"
          control={formMethods.control}
      ...
      <FormItem
        ...
        errorMessage={formMethods.formState.errors.brand_id?.message}
      >
        <Controller
          name="brand_id"
          control={formMethods.control}
      ...
      <FormItem
        ...
        invalid={!!formMethods.formState.errors.mobile_no}
        errorMessage={formMethods.formState.errors.mobile_no?.message}
      >
        <Controller
          name="mobile_no"
          control={formMethods.control}
      ...
      <FormItem
        label="Quality"
        invalid={!!formMethods.formState.errors.quality}
        errorMessage={formMethods.formState.errors.quality?.message}
      >
        <Controller
          name="quality"
          control={formMethods.control}
          render={({ field }) => (
            <Select
              ...
              options={QUALITY_LEVELS_UI}
              value={QUALITY_LEVELS_UI.find((o) => o.value === field.value)}
              onChange={(opt) => field.onChange(opt?.value)}
            />
          )}
        />
      ...
      <FormItem
        label="City"
        invalid={!!formMethods.formState.errors.city}
      ...
      >
        <Controller
          ...
          control={formMethods.control}
          render={({ field }) => (
            <Input {...field} prefix={<TbMapPin />} placeholder="City Name" />
          )}
        />
      </FormItem>
      <FormItem
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select
              ...
              options={
                editingItem?.status === "Blacklist"
                  ? STATUS_OPTIONS_UI
                  : STATUS_OPTIONS_UI.filter((s) => s.value !== "Blacklist")
              }
              value={STATUS_OPTIONS_UI.find((o) => o.value === field.value)}
              onChange={(opt) => field.onChange(opt?.value)}
              disabled={editingItem?.status === "Blacklist"}
            />
          )}
        />
      </FormItem>
      <FormItem
        ...
        invalid={!!formMethods.formState.errors.remarks}
        errorMessage={formMethods.formState.errors.remarks?.message}
      >
        <Controller
          name="remarks"
          control={formMethods.control}
    ...
    try {
      ...
      toast.push(
        <Notification title="Import Initiated" type="success" duration={2000}>
          File uploaded. Processing will continue.
        </Notification>
      );
      dispatch(getRowDataAction());
      closeImportModal();
      ...
      toast.push(
        <Notification title="Import Failed" type="danger" duration={3000}>
          {apiError.message || "Failed to import data."}
        </Notification>
      );
    } finally {
  ...
  return (
    <>
        ...
        <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
          ...
          <div className="mt-4 flex-grow overflow-auto w-full">
            <DataTable
              ...
              selectable
              checkboxChecked={(row: RowDataItem) =>
      ...
      <RowDataSelectedFooter
        selectedItems={selectedItems}
        onDeleteSelected={handleDeleteSelected}
        isDeleting={isDeleting && selectedItems.length > 0}
      />

      <Drawer
        title={editingItem ? "Edit Raw Data" : "Add New Raw Data"}
...
>
  <h5 className="mb-4 text-lg font-semibold">
    Raw Data Details - {viewingItem?.name || viewingItem?.mobile_no}
  </h5>

  {viewingItem && (
    <div className="space-y-3 text-sm max-h-[60vh] overflow-y-auto pr-1" style={{
    scrollbarWidth: "thin",
    scrollbarColor: "#cbd5e0 transparent", // Light gray thumb, transparent track
  }}>
      {(Object.keys(viewingItem) as Array<keyof RowDataItem>)
        .filter(
          (key) =>
            key !== "country" && key !== "category" && key !== "brand"
        )
        .map((key) => {
          let label = key
            .replace(/_id$/, "")
          ...
          if (key === "country_id")
            value =
              countryOptions.find((c) => c.value === String(value))?.label ||
              String(value);
          else if (key === "category_id")
            value =
              categoryOptions.find((c) => c.value === String(value))?.label ||
              String(value);
          else if (key === "brand_id")
            value =
              brandOptions.find((b) => b.value === String(value))?.label ||
              String(value);
          else if (key === "quality")
            value =
              QUALITY_LEVELS_UI.find((q) => q.value === value)?.label ||
              String(value);
          else if (key === "status")
            value =
              STATUS_OPTIONS_UI.find((s) => s.value === value)?.label ||
              String(value);
          else if (
            (key === "created_at" || key === "updated_at") &&
            value
          ) {
            value = `${new Date(value).getDate()} ${new Date(value).toLocaleString("en-US", {
              month: "short",
            })} ${new Date(value).getFullYear()}, ${new Date(value).toLocaleTimeString("en-US", {
              hour: "numeric",
              minute: "2-digit",
              hour12: true,
            })}`;
          ...
          return (
            ...
            >
              ...
              <span className="w-2/3 text-gray-900 dark:text-gray-100">
                {value}
              </span>
            </div>
          );
  ...
  <div className="text-right mt-6">
    <Button variant="solid" onClick={closeViewDialog}>
      ...
      <ConfirmDialog
        ...
        loading={isDeleting && !!itemToDelete}
      >
        <p>
          Are you sure you want to delete entry for "
          <strong>{itemToDelete?.name || itemToDelete?.mobile_no}</strong>"?
      ...
      >
        <p>
          Are you sure you want to blacklist entry for "
          <strong>{itemToBlacklist?.name || itemToBlacklist?.mobile_no}</strong>
          "? This will change status to 'Blacklist'.
        </p>
      </ConfirmDialog>
      ...
      >
        <div className="py-4">
          ...
          <p className="mb-4 text-xs text-gray-500 dark:text-gray-400">
            ...
            <code>country_id, category_id, brand_id, mobile_no, name, ...</code>
          </p>
