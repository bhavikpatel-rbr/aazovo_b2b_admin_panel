...
// DatePicker is removed based on your provided add/edit fields
...
function exportRowDataToCsv(filename: string, rows: RowDataItem[], countryOptions: SelectOption[], categoryOptions: SelectOption[], brandOptions: SelectOption[]) {
  ...
  const preparedRows = rows.map((row) => ({
    ...row,
    countryNameDisplay: countryOptions.find(c => c.value === row.country_id)?.label || row.country_id,
    categoryNameDisplay: categoryOptions.find(c => c.value === row.category_id)?.label || row.category_id,
    brandNameDisplay: brandOptions.find(b => b.value === row.brand_id)?.label || row.brand_id,
...
const RowDataListing = () => {
  ...
  const openAddDrawer = useCallback(() => {
    formMethods.reset({
        ...
        city: "", status: STATUS_OPTIONS_UI[0]?.value || "", remarks: "",
    });
  ...
  const openEditDrawer = useCallback((item: RowDataItem) => {
    setEditingItem(item);
    formMethods.reset({
        country_id: String(item.country_id), category_id: String(item.category_id), brand_id: String(item.brand_id),
        mobile_no: item.mobile_no, email: item.email || "", name: item.name, company_name: item.company_name || "",
        quality: item.quality, city: item.city || "", status: item.status, remarks: item.remarks || "",
    });
    setIsEditDrawerOpen(true);
  }, [formMethods]);

  const closeEditDrawer = () => { setIsEditDrawerOpen(false); setEditingItem(null); };
  const openViewDialog = (item: RowDataItem) => setViewingItem(item);
  ...
  const onSubmitHandler = async (data: RowDataFormData) => {
    ...
    try {
      if (editingItem) {
        ...
        toast.push(<Notification title="Row Data Updated" type="success" />);
        closeEditDrawer();
      } else {
        await dispatch(addRowDataAction(apiPayload)).unwrap();
        toast.push(<Notification title="Row Data Added" type="success" />);
        closeAddDrawer();
      }
      dispatch(getRowDataAction());
    } catch (e: any) {
      toast.push(<Notification title={editingItem ? "Update" : "Add" + " Failed"} type="danger">{(e as Error).message}</Notification>);
  ...
  const onConfirmSingleDelete = async () => {
    ...
    try {
      await dispatch(deleteAllRowDataAction({ ids: itemToDelete.id })).unwrap();
      toast.push(<Notification title="Row Data Deleted" type="success">{`Entry "${itemToDelete.name || itemToDelete.mobile_no}" deleted.`}</Notification>);
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      toast.push(<Notification title="Row Data Deleted" type="success">{selectedItems.length} entr(ies) deleted.</Notification>);
  ...
  const onConfirmBlacklist = async () => {
    ...
    const payload = { ...itemToBlacklist, status: "Blacklist", country_id: String(itemToBlacklist.country_id), category_id: String(itemToBlacklist.category_id), brand_id: String(itemToBlacklist.brand_id) };
    delete payload.country; delete payload.category; delete payload.brand;
    try {
      await dispatch(editRowDataAction({ id: itemToBlacklist.id, ...payload })).unwrap();
      toast.push(<Notification title="Row Data Blacklisted" type="warning">{`Entry "${itemToBlacklist.name || itemToBlacklist.mobile_no}" blacklisted.`}</Notification>);
      dispatch(getRowDataAction());
    } catch (e: any) { toast.push(<Notification title="Blacklist Failed" type="danger">{(e as Error).message}</Notification>);
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    const sourceDataWithDisplayNames: (RowDataItem & { countryNameDisplay?: string, categoryNameDisplay?: string, brandNameDisplay?: string })[] = Array.isArray(rowData) ? rowData.map(item => ({
        ...item,
        countryNameDisplay: item.country?.name || String(item.country_id),
        categoryNameDisplay: item.category?.name || String(item.category_id),
        brandNameDisplay: item.brand?.name || String(item.brand_id),
    })) : [];

    let processedData = cloneDeep(sourceDataWithDisplayNames);
    if (filterCriteria.filterCountry?.length) processedData = processedData.filter(item => filterCriteria.filterCountry!.some((fc: SelectOption) => fc.value === String(item.country_id)));
    if (filterCriteria.filterCategory?.length) processedData = processedData.filter(item => filterCriteria.filterCategory!.some((fc: SelectOption) => fc.value === String(item.category_id)));
    if (filterCriteria.filterBrand?.length) processedData = processedData.filter(item => filterCriteria.filterBrand!.some((fb: SelectOption) => fb.value === String(item.brand_id)));
    if (filterCriteria.filterStatus?.length) processedData = processedData.filter(item => filterCriteria.filterStatus!.some((fs: SelectOption) => fs.value === item.status));
    if (filterCriteria.filterQuality?.length) processedData = processedData.filter(item => filterCriteria.filterQuality!.some((fq: SelectOption) => fq.value === item.quality));
    ...
    if (tableData.query && tableData.query.trim() !== "") {
      ...
      processedData = processedData.filter(item =>
          ...
          (item.countryNameDisplay && item.countryNameDisplay.toLowerCase().includes(query)) ||
          (item.categoryNameDisplay && item.categoryNameDisplay.toLowerCase().includes(query)) ||
          (item.brandNameDisplay && item.brandNameDisplay.toLowerCase().includes(query))
      );
    }
    ...
    if (order && key) {
      processedData.sort((a, b) => {
        ...
        if (key === 'country_id') { aVal = a.countryNameDisplay; bVal = b.countryNameDisplay; }
        else if (key === 'category_id') { aVal = a.categoryNameDisplay; bVal = b.categoryNameDisplay; }
        else if (key === 'brand_id') { aVal = a.brandNameDisplay; bVal = b.brandNameDisplay; }
...
const closeFilterDrawer = useCallback(() => {
  ...
  const columns: ColumnDef<RowDataItem>[] = useMemo(() => [
    ...
    { header: 'Quality', accessorKey: 'quality', size: 100, cell: props => <Tag className={classNames("capitalize", props.getValue() === "A" && "bg-green-100 text-green-600", props.getValue() === "B" && "bg-yellow-100 text-yellow-600", props.getValue() === "C" && "bg-red-100 text-red-600")}>{QUALITY_LEVELS_UI.find(q => q.value === props.getValue())?.label || props.getValue()}</Tag> },
    { header: 'Status', accessorKey: 'status', size: 110, cell: props => <Tag className={classNames("capitalize", statusColors[props.getValue<string>()])}>{STATUS_OPTIONS_UI.find(s => s.value === props.getValue())?.label || props.getValue()}</Tag> },
    { header: 'Remarks', accessorKey: 'remarks', size: 150, cell: props => <div className="truncate w-36" title={props.getValue() as string}>{props.getValue() as string}</div> },
  ...
  ], [countryOptions, categoryOptions, brandOptions, openEditDrawer, openViewDialog, handleDeleteClick, handleBlacklistClick]);

  const renderDrawerForm = (currentFormMethods: typeof formMethods) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
      <FormItem label="Country" invalid={!!currentFormMethods.formState.errors.country_id} errorMessage={currentFormMethods.formState.errors.country_id?.message}>
        <Controller name="country_id" control={currentFormMethods.control} render={({ field }) => (<Select placeholder="Select country" options={countryOptions} value={countryOptions.find(o => o.value === field.value)} onChange={(opt) => field.onChange(opt?.value)} prefix={<TbWorld />} />)} />
      </FormItem>
      ...
      <FormItem label="Brand" invalid={!!currentFormMethods.formState.errors.brand_id} errorMessage={currentFormMethods.formState.errors.brand_id?.message}>
        <Controller name="brand_id" control={currentFormMethods.control} render={({ field }) => (<Select placeholder="Select brand" options={brandOptions} value={brandOptions.find(o => o.value === field.value)} onChange={(opt) => field.onChange(opt?.value)} prefix={<TbBuildingArch />} />)} />
      </FormItem>
      <FormItem label="Mobile No." invalid={!!currentFormMethods.formState.errors.mobile_no} errorMessage={currentFormMethods.formState.errors.mobile_no?.message}>
        <Controller name="mobile_no" control={currentFormMethods.control} render={({ field }) => (<Input {...field} prefix={<TbPhone />} placeholder="+XX-XXXXXXXXXX" />)} />
      </FormItem>
      <FormItem label="Email (Optional)" invalid={!!currentFormMethods.formState.errors.email} errorMessage={currentFormMethods.formState.errors.email?.message}>
      ...
      <FormItem label="Name" invalid={!!currentFormMethods.formState.errors.name} errorMessage={currentFormMethods.formState.errors.name?.message}>
        <Controller name="name" control={currentFormMethods.control} render={({ field }) => (<Input {...field} prefix={<TbUser />} placeholder="Contact Name / Lead Name" />)} />
      </FormItem>
      <FormItem label="Company Name (Optional)" invalid={!!currentFormMethods.formState.errors.company_name} errorMessage={currentFormMethods.formState.errors.company_name?.message}>
        <Controller name="company_name" control={currentFormMethods.control} render={({ field }) => (<Input {...field} prefix={<TbBuilding />} placeholder="ABC Corp" />)} />
      </FormItem>
      <FormItem label="Quality" invalid={!!currentFormMethods.formState.errors.quality} errorMessage={currentFormMethods.formState.errors.quality?.message}>
        <Controller name="quality" control={currentFormMethods.control} render={({ field }) => (<Select placeholder="Select quality" options={QUALITY_LEVELS_UI} value={QUALITY_LEVELS_UI.find(o => o.value === field.value)} onChange={(opt) => field.onChange(opt?.value)} />)} />
      </FormItem>
      <FormItem label="City (Optional)" invalid={!!currentFormMethods.formState.errors.city} errorMessage={currentFormMethods.formState.errors.city?.message}>
        <Controller name="city" control={currentFormMethods.control} render={({ field }) => (<Input {...field} prefix={<TbMapPin />} placeholder="City Name" />)} />
      </FormItem>
      <FormItem label="Status" invalid={!!currentFormMethods.formState.errors.status} errorMessage={currentFormMethods.formState.errors.status?.message}>
        <Controller name="status" control={currentFormMethods.control} render={({ field }) => (<Select placeholder="Select status" options={STATUS_OPTIONS_UI.filter(s => s.value !== "Blacklist")} value={STATUS_OPTIONS_UI.find(o => o.value === field.value)} onChange={(opt) => field.onChange(opt?.value)} />)} />
      </FormItem>
      <FormItem label="Remarks (Optional)" className="md:col-span-2 lg:col-span-3" invalid={!!currentFormMethods.formState.errors.remarks} errorMessage={currentFormMethods.formState.errors.remarks?.message}>
        <Controller name="remarks" control={currentFormMethods.control} render={({ field }) => (<Input textArea {...field} rows={3} placeholder="Add any relevant notes or comments..." />)} />
      </FormItem>
    </div>
  );
  ...
  return (
    <>
      ...
      <Drawer
        title={editingItem ? "Edit Row Data" : "Add New Row Data"}
      ...
      <Dialog isOpen={!!viewingItem} onClose={closeViewDialog} onRequestClose={closeViewDialog} width={700}>
        <h5 className="mb-4">Row Data Details - {viewingItem?.name || viewingItem?.mobile_no}</h5>
        {viewingItem && ( <div className="space-y-3"> {(Object.keys(viewingItem) as Array<keyof RowDataItem>).map((key) => { if (key === 'country' || key === 'category' || key === 'brand') return null; let label = key.replace(/_id$/, '').replace(/_/g, ' ').replace(/^./, str => str.toUpperCase()); let value: any = viewingItem[key]; if (key === 'country_id') value = viewingItem.country?.name || value; else if (key === 'category_id') value = viewingItem.category?.name || value; else if (key === 'brand_id') value = viewingItem.brand?.name || value; else if (key === 'quality') value = QUALITY_LEVELS_UI.find(q => q.value === value)?.label || value; else if (key === 'status') value = STATUS_OPTIONS_UI.find(s => s.value === value)?.label || value; else if (key === 'created_at' && value) value = new Date(value).toLocaleDateString(); return (<div key={key} className="flex"><span className="font-semibold w-1/3">{label}:</span><span className="w-2/3">{value || "-"}</span></div>); })} </div> )}
        <div className="text-right mt-6"><Button variant="plain" onClick={closeViewDialog}>Close</Button></div>
      </Dialog>

      <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer}
        footer={ <div className="text-right w-full"> <div> <Button size="sm" className="mr-2" onClick={onClearFilters}>Clear</Button> <Button size="sm" variant="solid" form="filterRowDataForm" type="submit">Apply</Button> </div> </div> } >
