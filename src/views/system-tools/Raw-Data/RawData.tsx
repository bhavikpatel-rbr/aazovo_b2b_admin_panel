...
const ALL_AVAILABLE_COUNTRIES = [
  ...
  { value: "GB", label: "United Kingdom" } /* ...more */,
];
const countryValues = ALL_AVAILABLE_COUNTRIES.map((c) => c.value) as [
  string,
  ...string[]
];
...
const CATEGORIES_LIST = [
  ...
  { value: "retail", label: "Retail" } /* ...more */,
];
const categoryValues = CATEGORIES_LIST.map((c) => c.value) as [
  string,
  ...string[]
];
...
const qualityValues = QUALITY_LEVELS.map((q) => q.value) as [
  string,
  ...string[]
];
...
const statusValues = STATUS_OPTIONS.map((s) => s.value) as [
  string,
  ...string[]
];
...
const leadFormSchema = z.object({
  ...
  brandName: z.enum(brandValues, {
    errorMap: () => ({ message: "Brand is required." }),
  }),
  mobileNo: z
    .string()
    .min(1, "Mobile No. is required.")
    .max(20, "Mobile No. too long."), // Add regex for specific formats if needed
...
const initialDummyLeads: LeadItem[] = [
  {
    ...
    city: "Bangalore",
    status: "verified",
    remarks: "Followed up, interested.",
    date: "2024-07-29",
  },
  {
    id: "LEAD002",
...
function exportLeadsToCsv(filename: string, rows: LeadItem[]) {
  ...
  const preparedRows = rows.map((row) => ({
    ...row,
    country:
      ALL_AVAILABLE_COUNTRIES.find((c) => c.value === row.country)?.label ||
      row.country,
    categoryName:
      CATEGORIES_LIST.find((c) => c.value === row.categoryName)?.label ||
      row.categoryName,
    brandName:
      BRANDS_LIST.find((b) => b.value === row.brandName)?.label ||
      row.brandName,
    quality:
      QUALITY_LEVELS.find((q) => q.value === row.quality)?.label || row.quality,
    status:
...
const LeadManagement = () => {
  ...
  const formMethods = useForm<LeadFormData>({
    ...
    defaultValues: {
      // Sensible defaults
      country: countryValues[0] || "",
      categoryName: categoryValues[0] || "",
      brandName: brandValues[0] || "",
      mobileNo: "",
  ...
  const openEditDrawer = (item: LeadItem) => {
    setEditingItem(item);
  ...
  const onConfirmSingleDelete = async () => {
    ...
    try {
      ...
      toast.push(
        <Notification title="Lead Deleted" type="success">{`Lead "${
          itemToDelete.contactName || itemToDelete.mobileNo
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      toast.push(
        <Notification title="Leads Deleted" type="success">
          {selectedItems.length} lead(s) deleted.
        </Notification>
      );
      setSelectedItems([]);
    } catch (e: any) {
  ...
  const handleBlacklistClick = (item: LeadItem) => {
  ...
  const onConfirmBlacklist = async () => {
    ...
    try {
      ...
      toast.push(
        <Notification title="Lead Blacklisted" type="warning">{`Lead "${
          itemToBlacklist.contactName || itemToBlacklist.mobileNo
        }" has been blacklisted.`}</Notification>
      );
    } catch (e: any) {
      toast.push(
        <Notification title="Blacklist Failed" type="danger">
          {e.message}
        </Notification>
      );
    } finally {
    ...
    if (filterCriteria.filterCountry?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterCountry!.some(
          (fc: any) => fc.value === item.country
        )
      );
    if (filterCriteria.filterCategory?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterCategory!.some(
          (fc: any) => fc.value === item.categoryName
        )
      );
    if (filterCriteria.filterBrand?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterBrand!.some(
          (fb: any) => fb.value === item.brandName
        )
      );
    if (filterCriteria.filterStatus?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterStatus!.some((fs: any) => fs.value === item.status)
      );
    if (filterCriteria.filterQuality?.length)
      processedData = processedData.filter((item) =>
        filterCriteria.filterQuality!.some(
          (fq: any) => fq.value === item.quality
        )
      );
  ...
  const columns: ColumnDef<LeadItem>[] = useMemo(
    () => [
      // { header: 'ID', accessorKey: 'id', size: 80, enableSorting: true }, // Optional
      {
        ...
        size: 100,
        cell: (props) =>
          ALL_AVAILABLE_COUNTRIES.find((c) => c.value === props.getValue())
      ...
      },
      {
        ...
        size: 120,
        cell: (props) =>
          CATEGORIES_LIST.find((c) => c.value === props.getValue())?.label ||
          props.getValue(),
      },
      {
        header: "Brand",
        ...
        size: 100,
        cell: (props) =>
          BRANDS_LIST.find((b) => b.value === props.getValue())?.label ||
          props.getValue(),
      },
      ...
      // { header: 'Company', accessorKey: 'companyName', size: 150 },
      {
        ...
        cell: (props) => (
          ...
          >
            {QUALITY_LEVELS.find((q) => q.value === props.getValue())?.label ||
              props.getValue()}
          </Tag>
        ),
      },
      // { header: 'City', accessorKey: 'city', size: 100 },
      {
        ...
        cell: (props) => (
          ...
          >
            {STATUS_OPTIONS.find((s) => s.value === props.getValue())?.label ||
              props.getValue()}
          </Tag>
        ),
      },
  ...
  const renderDrawerForm = (
    formInstance: typeof formMethods // Pass formInstance
  ) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
      <FormItem
        label="Country"
        invalid={!!formInstance.formState.errors.country}
        errorMessage={formInstance.formState.errors.country?.message}
      >
        <Controller
          name="country"
          ...
          render={({ field }) => (
            <Select
              ...
              value={ALL_AVAILABLE_COUNTRIES.find(
      ...
      <FormItem
        label="Category"
        invalid={!!formInstance.formState.errors.categoryName}
        errorMessage={formInstance.formState.errors.categoryName?.message}
      >
        <Controller
          name="categoryName"
          control={formInstance.control}
          render={({ field }) => (
            <Select
              ...
              value={CATEGORIES_LIST.find((o) => o.value === field.value)}
              onChange={(opt) => field.onChange(opt?.value)}
            />
          )}
        />
      ...
      <FormItem
        label="Brand"
        invalid={!!formInstance.formState.errors.brandName}
        errorMessage={formInstance.formState.errors.brandName?.message}
      >
        <Controller
          name="brandName"
          control={formInstance.control}
          render={({ field }) => (
            <Select
              ...
              options={BRANDS_LIST}
              value={BRANDS_LIST.find((o) => o.value === field.value)}
              onChange={(opt) => field.onChange(opt?.value)}
            />
          )}
        />
      ...
      <FormItem
        label="Mobile No."
        invalid={!!formInstance.formState.errors.mobileNo}
        errorMessage={formInstance.formState.errors.mobileNo?.message}
      >
        <Controller
          name="mobileNo"
          control={formInstance.control}
      ...
      <FormItem
        ...
        invalid={!!formInstance.formState.errors.email}
        errorMessage={formInstance.formState.errors.email?.message}
      >
        <Controller
          name="email"
          control={formInstance.control}
      ...
      <FormItem
        ...
        invalid={!!formInstance.formState.errors.contactName}
        errorMessage={formInstance.formState.errors.contactName?.message}
      >
        <Controller
          name="contactName"
          control={formInstance.control}
          render={({ field }) => (
            <Input {...field} prefix={<TbUser />} placeholder="John Doe" />
          )}
        />
      </FormItem>
      <FormItem
        label="Company Name (Optional)"
        invalid={!!formInstance.formState.errors.companyName}
        errorMessage={formInstance.formState.errors.companyName?.message}
      >
        <Controller
          name="companyName"
          control={formInstance.control}
      ...
      <FormItem
        label="Quality"
        invalid={!!formInstance.formState.errors.quality}
        errorMessage={formInstance.formState.errors.quality?.message}
      >
        <Controller
          name="quality"
          control={formInstance.control}
          render={({ field }) => (
            <Select
              ...
              value={QUALITY_LEVELS.find((o) => o.value === field.value)}
              onChange={(opt) => field.onChange(opt?.value)}
            />
          )}
        />
      ...
      <FormItem
        label="City (Optional)"
        invalid={!!formInstance.formState.errors.city}
        errorMessage={formInstance.formState.errors.city?.message}
      >
        <Controller
          name="city"
          control={formInstance.control}
      ...
      <FormItem
        label="Status"
        invalid={!!formInstance.formState.errors.status}
        errorMessage={formInstance.formState.errors.status?.message}
      >
        <Controller
          name="status"
          control={formInstance.control}
          render={({ field }) => (
            <Select
              ...
              options={STATUS_OPTIONS.filter(
                (s) => s.value !== "blacklist"
              )} /* Don't allow setting blacklist via form */
      ...
      <FormItem
        ...
        invalid={!!formInstance.formState.errors.remarks}
        errorMessage={formInstance.formState.errors.remarks?.message}
      >
        <Controller
          name="remarks"
          control={formInstance.control}
  ...
  return (
    <>
      ...
      >
        <h5 className="mb-4">
          Lead Details - {viewingItem?.contactName || viewingItem?.mobileNo}
        </h5>
        {viewingItem && (
          <div className="space-y-3">
            {(Object.keys(viewingItem) as Array<keyof LeadItem>).map((key) => {
              ...
              if (key === "country")
                value =
                  ALL_AVAILABLE_COUNTRIES.find((c) => c.value === value)
                    ?.label || value;
              else if (key === "categoryName")
                value =
                  CATEGORIES_LIST.find((c) => c.value === value)?.label ||
                  value;
              else if (key === "brandName")
                value =
                  BRANDS_LIST.find((b) => b.value === value)?.label || value;
              else if (key === "quality")
                value =
                  QUALITY_LEVELS.find((q) => q.value === value)?.label || value;
              else if (key === "status")
                value =
        ...
        <div className="text-right mt-6">
          <Button variant="plain" onClick={closeViewDialog}>
            Close
          </Button>
        </div>
      </Dialog>
      ...
      >
        <p>
          ...
          <strong>{itemToDelete?.contactName || itemToDelete?.mobileNo}</strong>
          "?
        </p>
      </ConfirmDialog>
      ...
      >
        <p>
          Are you sure you want to blacklist lead for "
          <strong>
            {itemToBlacklist?.contactName || itemToBlacklist?.mobileNo}
          </strong>
          "? This will change their status to 'Blacklisted'.
        </p>
      </ConfirmDialog>
    </>
  );
