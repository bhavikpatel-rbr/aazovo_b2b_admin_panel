  ...
  if (globalFormsData === undefined) {
    ...
    if (typeof originalInitialDummyForms === "undefined") {
      ...
      try {
        ...
        console.error(
          "Error parsing or stringifying originalInitialDummyForms:",
          error
        );
...
export const getFormByIdFromStore = (
  id: string | number
): FormBuilderItem | undefined => {
  initializeGlobalFormsData();
...
export const deleteFormFromStore = (formId: string | number): boolean => {
  ...
  globalFormsData = globalFormsData!.filter(
...
export const cloneFormInStore = (
  itemToClone: FormBuilderItem
): FormBuilderItem | undefined => {
  initializeGlobalFormsData();
...
export const changeFormStatusInStore = (
...
export const resetFormsInStore = () => {
  if (typeof originalInitialDummyForms === "undefined") {
    console.error(
      "CRITICAL ERROR during resetFormsInStore: originalInitialDummyForms is undefined."
    );
    globalFormsData = [];
  } else {
    try {
      ...
      console.error(
        "Error parsing or stringifying originalInitialDummyForms during reset:",
        error
      );
      globalFormsData = [];
    }
