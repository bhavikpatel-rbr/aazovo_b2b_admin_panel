...
import {
  ...
  // submitExportReasonAction is already imported and used below
...
export const FORM_STATUS_OPTIONS = [ // Export for use in FormBuilderFormPage
  { value: "draft", label: "Draft" },
  { value: "published", label: "Published" },
  { value: "archived", label: "Archived" },
];
export const formStatusValues = FORM_STATUS_OPTIONS.map((s) => s.value) as [string,...string[]]; // Export

export const DEPARTMENT_OPTIONS = [ // Export
  { value: "eng", label: "Engineering" },
  { value: "mkt", label: "Marketing" },
  { value: "hr", label: "Human Resources" },
  { value: "sales", label: "Sales" },
];
export const departmentValues = DEPARTMENT_OPTIONS.map((d) => d.value) as [string,...string[]]; // Export

export const CATEGORY_OPTIONS = [ // Export
  { value: "feedback", label: "Feedback" },
  { value: "survey", label: "Survey" },
  { value: "application", label: "Application" },
  { value: "support", label: "Support Ticket" },
];
export const categoryValues = CATEGORY_OPTIONS.map((c) => c.value) as [string,...string[]]; // Export

export const QUESTION_TYPE_OPTIONS = [ // Export
...
export const questionTypeValues = QUESTION_TYPE_OPTIONS.map((q) => q.value) as [string,...string[]]; // Export
...
const FormBuilder = () => {
  ...
  const onConfirmSingleDelete = async () => {
    ...
    try {
      ...
      toast.push(<Notification title="Delete Failed" type="danger" duration={3000}>{e.message || "Could not delete form."}</Notification>);
    } finally {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      await dispatch(deleteAllFormBuildersAction({ ids: idsToDelete })).unwrap(); // Redux action
      ...
      toast.push(<Notification title="Delete Failed" type="danger" duration={3000}>{e.message || "Could not delete selected forms."}</Notification>);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleChangeStatus = async (item: FormBuilderItem) => {
    ...
    try {
      ...
      toast.push(<Notification title="Status Change Failed" type="danger" duration={3000}>{e.message || "Could not change form status."}</Notification>);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleCloneForm = async (itemToClone: FormBuilderItem) => {
    ...
    try {
      ...
      toast.push(<Notification title="Clone Failed" type="danger" duration={3000}>{e.message || "Could not clone form."}</Notification>);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
    ...
    try {
        ...
        toast.push(<Notification title="Operation Failed" type="danger" message={error.message || "Could not complete export reason submission."} />);
    } finally {
    ...
    <>
      <Container className="h-auto">
        <AdaptiveCard className="h-full" bodyClass="h-full">
          ...
          <div className="mt-4">
            <DataTable
              columns={columns} data={pageData} loading={tableLoading} // Use masterLoadingStatus for overall table load
        ...
        <Dialog.Body className="p-4 max-h-[70vh] overflow-y-auto">
            {viewingItem && (
                <div className="space-y-4">
                    ...
                    <p><strong>Updated By:</strong> {viewingItem.updated_by_name || 'N/A'} ({viewingItem.updated_by_role || 'N/A'})</p>
                    
                    <h6 className="font-semibold mt-3">Questions ({viewingItem.questions.length}):</h6>
                    {viewingItem.questions.length > 0 ? (
                        <ul className="list-disc pl-5 space-y-2">
                            {viewingItem.questions.map((q, idx) => (
                                <li key={q.id || idx}>
                                    {q.questionSectionTitle && <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Section: {q.questionSectionTitle}</p>}
                                    {q.questionSectionDescription && <p className="text-xs text-gray-500 dark:text-gray-400">Desc: {q.questionSectionDescription}</p>}
                                    <p>{q.questionText} <Tag className="ml-2 text-xs">{QUESTION_TYPE_OPTIONS.find(qt => qt.value === q.questionType)?.label || q.questionType}</Tag></p>
                                    {q.isRequired && <span className="text-red-500 text-xs ml-1">(Required)</span>}
                                    {q.options && q.options.length > 0 && (
                                        <ul className="list-circle pl-5 text-xs">
                                            {q.options.map(opt => <li key={opt.value}>{opt.label}</li>)}
                                        </ul>
                                    )}
                                </li>
                            ))}
