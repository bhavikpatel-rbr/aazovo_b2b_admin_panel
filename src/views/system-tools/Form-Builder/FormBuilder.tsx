...
export type FormBuilderItem = {
  ...
  created_by_user: {
    // Assuming this structure is always present, but properties might be null/missing
    id: number;
    name: string;
    ...
    if (Array.isArray(row.departments) && row.departments.length > 0) {
      ...
      departmentNamesDisplay =
        deptIds
          .map(
            (id) =>
              allDepartmentsMasterList.find((d) => String(d.id) === id)?.name
          )
          .filter(Boolean)
          .join(", ") || "N/A";
    }
    ...
    if (Array.isArray(row.categories) && row.categories.length > 0) {
      ...
      categoryNamesDisplay =
        catIds
          .map(
            (id) =>
              allCategoriesMasterList.find((c) => String(c.id) === id)?.name
          )
          .filter(Boolean)
          .join(", ") || "N/A";
    }
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      >
        ...
        >
          <TbCopy size={18} /> <span className="text-xs">Clone Form</span>
        </Dropdown.Item>
...
const FormBuilder = () => {
  ...
  const categoryFilterOptions = useMemo(
    () =>
      (CategoriesData || []).map((cat: GeneralCategoryListItem) => ({
        value: String(cat.id),
        label: cat.name,
      })),
    [CategoriesData]
  ...
  const departmentFilterOptions = useMemo(
    () =>
      (departmentsData?.data || []).map((dept: DepartmentListItem) => ({
  ...
  const openViewDialog = (item: FormBuilderItem) =>
    navigate(`/system-tools/formbuilder-edit/${item.id}?preview=true`);
    ...
    try {
      ...
      toast.push(
        <Notification
          title="Forms Deleted"
          type="success"
        >{`${selectedItems.length} form(s) deleted.`}</Notification>
      );
      setSelectedItems([]);
    } catch (e: any) {
      toast.push(
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    const processedFormsData: FormBuilderItem[] = (rawFormsData || []).map(
      (form) => {
        let qCount = 0;
        let parsedSecs: FormSection[] = [];
        try {
          if (form.section) {
    ...
    if (filterCriteria.filterDepartmentIds?.length) {
      const selectedDeptIds = filterCriteria.filterDepartmentIds.map(
        (d) => d.value
      );
      filteredData = filteredData.filter((item) =>
        item.department_ids_array?.some((id) =>
          selectedDeptIds.includes(String(id))
        )
      );
    }
    if (filterCriteria.filterCategoryIds?.length) {
      const selectedCatIds = filterCriteria.filterCategoryIds.map(
        (c) => c.value
      );
      filteredData = filteredData.filter((item) =>
        item.category_ids_array?.some((id) =>
          selectedCatIds.includes(String(id))
        )
      );
    }
    if (tableData.query) {
      ...
      filteredData = filteredData.filter(
        (item) =>
          ...
          (Array.isArray(item.departments) &&
            item.departments.join(", ").toLowerCase().includes(query)) ||
    ...
    if (order && key) {
      filteredData?.sort((a, b) => {
        ...
        if (aVal === null || aVal === undefined)
          aVal = order === "asc" ? Infinity : -Infinity; // Push nulls to end/start
        if (bVal === null || bVal === undefined)
          bVal = order === "asc" ? Infinity : -Infinity;
  ...
  const columns: ColumnDef<FormBuilderItem>[] = useMemo(
    () => [
      {
        header: "Form Name / Title",
        accessorKey: "form_name",
        size: 260,
        enableSorting: true,
        cell: (props) => (
          <div
            onClick={() => openViewDialog(props.row.original)}
            className="cursor-pointer"
          >
            <span className="font-semibold text-blue-600 dark:text-blue-400 hover:underline">
              {props.row.original.form_name || "N/A"}
            </span>
            <br />
        ...
        cell: (props) => (
          <Tag
            className={classNames(
              "capitalize",
              statusColors[props.row.original.status] || statusColors.Draft
            )}
          >
            {props.row.original.status || "N/A"}
          </Tag>
        ),
      },
      {
        ...
        cell: (props) => {
          ...
          return (
            <div className="flex flex-wrap gap-1">
              {displayNames.slice(0, 2).map((name) => (
                <Tag key={name}>{name}</Tag>
              ))}
              {displayNames.length > 2 && <Tag>+{displayNames.length - 2}</Tag>}
            </div>
          );
        },
      },
      {
        ...
        cell: (props) => {
          ...
          if (Array.isArray(item.categories) && item.categories.length > 0) {
            ...
            displayNames = ids.map(
              (id) =>
                (CategoriesData || []).find((c) => String(c.id) === String(id))
                  ?.name || `ID:${id}`
            );
          }
          if (!displayNames.length) return <Tag>N/A</Tag>;
          ...
          return (
            <div className="text-xs">
              <span>
                ...
                <br />
                <b>{updated_by_user?.roles?.[0]?.display_name || "N/A"}</b>
              </span>
              <br />
    ...
    */}
      ...
      >
        <p>
          ...
          <strong>{itemToDelete?.form_name || "this form"}</strong>"?
        </p>
      </ConfirmDialog>
      <ConfirmDialog
