...
const ActionColumn = ({
  ...
  <div className="flex items-center justify-center gap-1">
    ...
    <Tooltip title="Block User/IP">
      <button
        className="text-xl p-1 rounded-md text-gray-500 hover:text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700"
        onClick={onBlock}
      >
        <TbCancel />
      </button>
...
const ActiveFiltersDisplay = ({
  ...
  const filters = Object.entries(filterData).flatMap(([key, values]) => {
    if (key === "filterDateRange" && values && (values[0] || values[1])) {
      const start = values[0]
        ? new Date(values[0]).toLocaleDateString()
        : "...";
      const end = values[1] ? new Date(values[1]).toLocaleDateString() : "...";
      return [{ key, value: "date-range", label: `${start} to ${end}` }];
    }
    if (key === "filterUserName" && typeof values === "string" && values)
      return [{ key, value: values, label: values }];
    if (Array.isArray(values))
      return values.map((v) => ({ key: key as keyof FilterFormData, ...v }));
    return [];
  });
