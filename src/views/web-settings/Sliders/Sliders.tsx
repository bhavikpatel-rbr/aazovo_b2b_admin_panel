...
const Sliders = () => {
  ...
  const { PageData: pageName = [], slidersData: rawSlidersData = [] } = useSelector(masterSelector);
  const displayPageOptionsConst = useMemo(() => Array.from(new Set((Array.isArray(pageName?.data) ? pageName?.data : []).map(p => p.name))).map(name => ({ value: name, label: name })), [pageName?.data]);
  ...
  return (<> <Container className="h-auto"> <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4"> <h5 className="mb-2 sm:mb-0">Sliders</h5> <div> <Link to="/task/task-list/create"><Button className="mr-2" icon={<TbUser />} clickFeedback={false}>Assign to Task</Button></Link> <Button menuName="sliders" isAdd={true} variant="solid" icon={<TbPlus />} onClick={openAddDrawer} disabled={!isDataReady}>Add New</Button> </div> </div>
