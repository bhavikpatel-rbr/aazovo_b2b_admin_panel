...
import {
  ...
  Select as UiSelect,
  DatePicker,
} from "@/components/ui"; // Renamed Select
...
const filterFormSchema = z.object({
  ...
  filterExportFrom: z
    ...
    .optional(),
  filterDateRange: z
    .custom<[Date | null, Date | null] | null>((val) => {
      if (val === null) return true; // Allow null
      if (Array.isArray(val) && val.length === 2) {
        return (
          (val[0] === null || val[0] instanceof Date) &&
          (val[1] === null || val[1] instanceof Date)
        );
      }
      return false;
    }, "Invalid date range")
...
const CSV_HEADERS_EXPORT_MAPPING = [
  "Record ID",
  "User Name",
  "User Role",
  "Exported From",
...
function exportToCsvExportMapping(filename: string, rows: ExportMappingItem[]) {
  ...
  const transformedRows: ExportMappingCsvItem[] = rows.map((item) => ({
    ...
    userRole:
      item.user.roles.map((role) => role.display_name).join(", ") || "N/A",
  ...
  const csvKeys: (keyof ExportMappingCsvItem)[] = [
    "id",
    "userName",
    "userRole",
    "export_from",
...
const ExportMapping = () => {
  ...
  const userNameOptions = useMemo(() => {
    ...
    const uniqueNames = new Set(
      ExportMappingData.map((item) => item.user.name)
    );
  ...
  const exportFromOptions = useMemo(() => {
    ...
    const uniqueSources = new Set(
      ExportMappingData.map((item) => item.export_from)
    );
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    ...
    // Apply filters
    if (
      filterCriteria.filterUserNames &&
      filterCriteria.filterUserNames.length > 0
    ) {
      const selectedNames = filterCriteria.filterUserNames.map((opt) =>
        opt.value.toLowerCase()
      );
      processedData = processedData.filter((item) =>
        selectedNames.includes(item.user.name.toLowerCase())
      );
    }
    if (
      filterCriteria.filterUserRoles &&
      filterCriteria.filterUserRoles.length > 0
    ) {
      const selectedRoles = filterCriteria.filterUserRoles.map((opt) =>
        opt.value.toLowerCase()
      );
      processedData = processedData.filter((item) =>
        item.user.roles.some((role) =>
          selectedRoles.includes(role.display_name.toLowerCase())
        )
      );
    }
    if (
      filterCriteria.filterExportFrom &&
      filterCriteria.filterExportFrom.length > 0
    ) {
      const selectedSources = filterCriteria.filterExportFrom.map((opt) =>
        opt.value.toLowerCase()
      );
      processedData = processedData.filter((item) =>
        selectedSources.includes(item.export_from.toLowerCase())
      );
    }
    if (
      filterCriteria.filterDateRange &&
      filterCriteria.filterDateRange[0] &&
      filterCriteria.filterDateRange[1]
    ) {
    ...
    if (tableData.query && tableData.query.trim() !== "") {
      ...
      processedData = processedData.filter((item: ExportMappingItem) => {
        ...
        const userRoleLower =
          item.user.roles.map((r) => r.display_name.toLowerCase()).join(" ") ??
          "";
    ...
    if (order && key && processedData.length > 0) {
      ...
      sortedData.sort((a, b) => {
        ...
        if (key === "user.name") {
          aValue = a.user.name;
          bValue = b.user.name;
        } else if (key === "user.roles") {
          // Simplified role sort (e.g., by first role)
