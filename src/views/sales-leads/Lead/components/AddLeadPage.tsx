...
// Redux (If fetching dropdown data or submitting form via Redux)
// import { useAppDispatch } from '@/reduxtool/store';
// import { addLeadAction, getMembersForFormAction, getSalesPersonsForFormAction, ... } from '@/reduxtool/master/middleware';

// Mock Options (Replace with fetched data)
...
const dummyProductSpecs = [{ value: "India", label: "India" }, { value: "China", label: "China" }];
...
const AddLeadPage = () => {
    ...
    // --- States for dropdown options (replace with Redux selectors or API calls) ---
    // const [memberOptions, setMemberOptions] = useState(dummyMembers);
    // const [salesPersonOptions, setSalesPersonOptions] = useState(dummySalesPersons);
    ...
    const { control, handleSubmit, reset, formState: { errors, isDirty, isValid } } = useForm<LeadFormData>({
        ...
        defaultValues: {
            memberId: "", enquiryType: "", leadIntent: null, productName: null, qty: null,
            targetPrice: null, leadStatus: "New", assignedSalesPersonId: null,
            sourcing_supplierId: null, sourcing_productId: null, sourcing_productSpecId: null,
            ...
            sourcing_cartoonTypeId: null, sourcing_dispatchStatus: null, sourcing_paymentTermId: null,
    ...
    //     // dispatch(getMembersForFormAction());
    //     // dispatch(getSalesPersonsForFormAction());
    //     // ...
    // }, [dispatch]);
    ...
    const onSubmit = async (data: LeadFormData) => {
        ...
        const payload = {
            ...data,
            sourcing_eta: data.sourcing_eta ? dayjs(data.sourcing_eta).toISOString() : null, // Ensure ISO string for API
        };
        console.log("Submitting New Lead:", payload);
        try {
            // await dispatch(addLeadAction(payload)).unwrap(); // Example Redux
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <AdaptableCard className="mb-4">
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2">
                            <FormItem label="Lead Member (Supplier/Buyer) *" error={errors.memberId?.message}><Controller name="memberId" control={control} render={({ field }) => <UiSelect placeholder="Select Member" options={dummyMembers} value={dummyMembers.find(o=>o.value===field.value)} onChange={opt=>field.onChange(opt?.value)} {...field} />}/></FormItem>
                            ...
                            <FormItem label="Lead Status*" className='gr' error={errors.leadStatus?.message}><Controller name="leadStatus" control={control} render={({ field }) => <UiSelect placeholder="Select Status" options={leadStatusOptions} value={leadStatusOptions.find(o=>o.value===field.value)} onChange={opt=>field.onChange(opt?.value)} {...field} />}/></FormItem>
                    ...
                    <AdaptableCard>
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2">
                            ...
                            <FormItem label="Location" error={errors.sourcing_location?.message}><Controller name="sourcing_location" control={control} render={({ field }) => <Input placeholder="e.g., Warehouse A" {...field} />}/></FormItem>
                            <FormItem label="Internal Remarks" error={errors.sourcing_internalRemarks?.message} className="md:col-span-2 lg:col-span-3"><Controller name="sourcing_internalRemarks" control={control} render={({ field }) => <Input textArea rows={3} placeholder="Internal notes..." {...field} />}/></FormItem>
                        </div>
                    </AdaptableCard>

                    <div className="mt-6 flex justify-end gap-2">
                        <Button type="button" onClick={handleCancel} disabled={isSubmitting}>Cancel</Button>
                        <Button type="submit" variant="solid" loading={isSubmitting} disabled={!isValid && !isSubmitting}>Create Lead</Button>
                    </div>
                </form>
            </FormContainer>
