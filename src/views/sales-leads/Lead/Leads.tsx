...
const LeadsListing = () => {
    ...
    return LeadsData.map(
      (apiLead: any): LeadListItem => ({
        ...
        updatedAt: apiLead.updated_at
          ...
          : undefined,
        sourcingDetails: apiLead.sourcing_details
          ? typeof apiLead.sourcing_details === "string"
            ? JSON.parse(apiLead.sourcing_details)
            : apiLead.sourcing_details
          : undefined,
      })
    );
  }, [LeadsData]);
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo((): {
    ...
    if (filterCriteria.filterSalesPersonIds?.length) {
      ...
      processedData = processedData.filter(
        (item) => !!item.salesPersonId && spIds.has(String(item.salesPersonId))
      );
    }
    if (tableData.query) {
  ...
  const onConfirmSingleDelete = useCallback(async () => {
    /* ... (Keep your existing logic, ensure dispatch(getLeadAction()) is called on success) ... */
  ...
  const onAssignSubmit = useCallback(
    async (data: { salesPersonId: string | number | null }) => {
      ...
      try {
        /* await dispatch(assignLeadAction({ leadId: editingLeadForDrawer.id, salesPersonId: data.salesPersonId })).unwrap(); */ console.log(
          "Simulating assign..."
        );
  ...
  const onChangeStatusSubmit = useCallback(
    async (data: { newStatus: LeadStatus }) => {
      ...
      setIsSubmittingDrawer(true);
      try {
        /* await dispatch(changeLeadStatusAction({ leadId: editingLeadForDrawer.id, newStatus: data.newStatus })).unwrap(); */ console.log(
  ...
  const handleConvertToOpportunity = useCallback((lead: LeadListItem) => {
    /* ... (Keep) ... */ toast.push(
      <Notification title="Info" type="info">
        Convert to Opportunity: Not Implemented
      </Notification>
    );
  }, []);
  const openViewDialog = useCallback((lead: LeadListItem) => {
    setLeadToView(lead);
  ...
  const columns: ColumnDef<LeadListItem>[] = useMemo(
    () => [
        ...
        cell: (props) => {
          return (
            <div className="flex flex-col gap-0.5 text-xs">
              ...
              <span>
                Sales Person : {props.row.original.salesPersonName || "Mukesh"}
              </span>
              <b>
      ...
      //   header: "Actions",
      //   id: "action",
      //   meta : { HeaderClass : "text-center" }, size: 120,
      //   cell: (props: CellContext<LeadListItem, any>) => (
      //     <LeadActionColumn
  ...
  return (
    <>
      ...
      <Dialog
        ...
        size="max-w-2xl"
        title=""
        contentClassName="!p-0 bg-slate-50 dark:bg-slate-800 rounded-xl shadow-2xl overflow-hidden"
      >
        {leadToView ? (
              ...
              <div className="flex justify-between items-center">
                ...
                <Tooltip title="Edit Lead">
                  <Button
                    ...
                    size="sm"
                    icon={
                      <TbPencil className="text-slate-500 hover:text-blue-600" />
                    }
                    onClick={() => {
                      closeViewDialog();
                      handleOpenEditLeadPage(leadToView);
                    }}
                  />
                </Tooltip>
              </div>
              ...
              <div className="pr-4 md:pr-6 space-y-5">
                <div>
                  ...
                  <div className="p-4 bg-white dark:bg-slate-700/60 rounded-lg shadow-sm space-y-2.5">
                    <DialogDetailRow
                      label="Lead ID"
                      value={String(leadToView.id)}
                    />
                    <DialogDetailRow
                      label="Member"
                      value={leadToView.memberName || leadToView.memberId}
                    />
                    <DialogDetailRow
                      label="Status"
                      value={
                        <Tag
                          className={`${
                            leadStatusColor[leadToView.status] ||
                            leadStatusColor.default
                          } capitalize font-semibold text-[10px] px-2 py-0.5 rounded-full`}
                        >
                          {leadToView.status}
                        </Tag>
                      }
                    ...
                    <DialogDetailRow
                      ...
                      value={
                        <Tag
                          className={`${
                            enquiryTypeColor[leadToView.enquiryType] ||
                            enquiryTypeColor.default
                          } capitalize text-[10px] px-2 py-0.5 rounded-md`}
                        >
                          {leadToView.enquiryType}
                        </Tag>
                      }
                    />
                    <DialogDetailRow
                      label="Intent"
                      value={leadToView.intent || "-"}
                    />
                    <DialogDetailRow
                      label="Product Interest"
                      value={leadToView.productName || "-"}
                    />
                    <DialogDetailRow
                      label="Quantity"
                      value={leadToView.qty?.toString() || "-"}
                    />
                    <DialogDetailRow
                      label="Target Price"
                      value={
                        leadToView.targetPrice
                          ? `$${leadToView.targetPrice.toFixed(2)}`
                          : "-"
                      }
                    />
                    <DialogDetailRow
                      label="Assigned To"
                      value={leadToView.salesPersonName || "Unassigned"}
                    />
                  </div>
                </div>
                {leadToView.sourcingDetails && (
                  <div>
                    {" "}
                    {/* Sourcing Specifics Section */}
                    <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 mt-4">
                      Sourcing Specifics
                    </h6>
                    <div className="p-4 bg-white dark:bg-slate-700/60 rounded-lg shadow-sm space-y-2.5">
                      {leadToView.sourcingDetails.supplierId && (
                        <DialogDetailRow
                          label="Supplier"
                          value={
                            dummySuppliers.find(
                              (s) =>
                                s.id === leadToView.sourcingDetails?.supplierId
                            )?.name ||
                            String(leadToView.sourcingDetails.supplierId)
                          }
                        />
                      )}
                      {leadToView.sourcingDetails.productId && (
                        <DialogDetailRow
                          label="Sourced Product"
                          value={
                            dummyProducts.find(
                              (p) =>
                                p.id === leadToView.sourcingDetails?.productId
                            )?.name ||
                            `ID: ${leadToView.sourcingDetails.productId}`
                          }
                        />
                      )}
                      {/* ... Add all other sourcing details from your original dialog ... */}
                      {leadToView.sourcingDetails.qty && (
                        <DialogDetailRow
                          label="Sourced Qty"
                          value={String(leadToView.sourcingDetails.qty)}
                        />
                      )}
                      {leadToView.sourcingDetails.price && (
                        <DialogDetailRow
                          label="Sourced Price"
                          value={`$${leadToView.sourcingDetails.price.toFixed(
                            2
                          )}`}
                        />
                      )}
                      {leadToView.sourcingDetails.productStatus && (
                        <DialogDetailRow
                          label="Product Status"
                          value={leadToView.sourcingDetails.productStatus}
                        />
                      )}
                      {leadToView.sourcingDetails.deviceCondition && (
                        <DialogDetailRow
                          label="Device Condition"
                          value={leadToView.sourcingDetails.deviceCondition}
                        />
                      )}
                      {leadToView.sourcingDetails.color && (
                        <DialogDetailRow
                          label="Color"
                          value={leadToView.sourcingDetails.color}
                        />
                      )}
                      {leadToView.sourcingDetails.cartoonTypeId && (
                        <DialogDetailRow
                          label="Cartoon Type"
                          value={
                            dummyCartoonTypes.find(
                              (c) =>
                                c.id ===
                                leadToView.sourcingDetails?.cartoonTypeId
                            )?.name || "-"
                      ...
                      {leadToView.sourcingDetails.dispatchStatus && (
                        <DialogDetailRow
                          label="Dispatch Status"
                          value={leadToView.sourcingDetails.dispatchStatus}
                        />
                      )}
                      {leadToView.sourcingDetails.paymentTermId && (
                        <DialogDetailRow
                          label="Payment Term"
                          value={
                            dummyPaymentTerms.find(
                              (p) =>
                                p.id ===
                                leadToView.sourcingDetails?.paymentTermId
                            )?.name || "-"
                      ...
                      {leadToView.sourcingDetails.eta && (
                        <DialogDetailRow
                          label="ETA"
                          value={
                            leadToView.sourcingDetails.eta
                              ? dayjs(leadToView.sourcingDetails.eta).isValid()
                                ? dayjs(leadToView.sourcingDetails.eta).format(
                                    "MMM D, YYYY"
                                  )
                                : String(leadToView.sourcingDetails.eta)
                              : "-"
                          }
                        />
                      )}
                      {leadToView.sourcingDetails.location && (
                        <DialogDetailRow
                          label="Location"
                          value={leadToView.sourcingDetails.location}
                        />
                      )}
                      {leadToView.sourcingDetails.deviceType && (
                        <DialogDetailRow
                          label="Device Type"
                          value={leadToView.sourcingDetails.deviceType}
                        />
                      )}
                      {leadToView.sourcingDetails.internalRemarks && (
                        <DialogDetailRow
                          label="Internal Remarks"
                          value={leadToView.sourcingDetails.internalRemarks}
                          preWrap
                        />
                      )}
                    </div>
                ...
                <AdaptiveCard className="!mt-5 bg-white dark:bg-slate-700/60 border-slate-200 dark:border-slate-600 p-4 shadow-sm">
                  <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">
                    Activity Log
                  </h6>
                      ...
                      value={
                        <>
                          <span className="text-sm">
                            {dayjs(leadToView.createdAt).format("MMM D, YYYY")}
                          </span>
                          <span className="text-slate-500 dark:text-slate-400 text-[10px] ml-1">
                            {dayjs(leadToView.createdAt).format("h:mm A")}
                          </span>
                        </>
                      }
                    />
                    {leadToView.updatedAt && (
                      <DialogDetailRow
                        label="Last Updated"
                        value={
                          <>
                            <span className="text-sm">
                              {dayjs(leadToView.updatedAt).format(
                                "MMM D, YYYY"
                              )}
                            </span>
                            <span className="text-slate-500 dark:text-slate-400 text-[10px] ml-1">
                              {dayjs(leadToView.updatedAt).format("h:mm A")}
                            </span>
                          </>
                        }
                      />
      ...
      >
        ...
        >
          <p className="mb-4">
            Assign lead <strong>{editingLeadForDrawer?.leadNumber}</strong>.
          </p>
          <FormItem
            label="Sales Person"
      ...
      >
        ...
        >
          <p className="mb-4">
            Change status for{" "}
            <strong>{editingLeadForDrawer?.leadNumber}</strong>.
          </p>
          <FormItem
            label="New Status"
      ...
      >
        <p>
          Delete <strong>{itemToDelete?.leadNumber}</strong>?
        </p>
      </ConfirmDialog>
    </>
  );
