...
// --- MODALS SECTION ---
// ============================================================================
export type LeadModalType = | "email" | "whatsapp" | "notification" | "task" | "active" | "calendar" | "alert" | "document" | "feedback" | "convertToDeal" | "viewOpportunities" | "viewLeadForm" | "viewDeal" | "addAccountDocuments";
export interface LeadModalState { isOpen: boolean; type: LeadModalType | null; data: LeadListItem | null; }
interface LeadModalsProps { modalState: LeadModalState; onClose: () => void; getAllUserDataOptions: SelectOption[] }
...
const LeadModals: React.FC<LeadModalsProps> = ({ modalState, onClose, getAllUserDataOptions }) => {
  ...
  const renderModalContent = () => {
    switch (type) {
      ...
      case "notification": return <AddNotificationDialog lead={lead} onClose={onClose} getAllUserDataOptions={getAllUserDataOptions} />;
      ...
      case "convertToDeal": return <ConvertLeadToDealDialog lead={lead} onClose={onClose} />;
      case "viewOpportunities": return <ViewOpportunitiesDialog lead={lead} onClose={onClose} />;
      case "viewLeadForm": return <ViewLeadFormDialog lead={lead} onClose={onClose} />;
      case "viewDeal": return <ViewDealDialog lead={lead} onClose={onClose} />;
      case "addAccountDocuments": return <AddAccountDocumentsDialog lead={lead} onClose={onClose} />;
...
const ConvertLeadToDealDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose }) => {
  ...
  const { control, handleSubmit } = useForm({ defaultValues: { dealValue: lead.target_price || 0, dealName: `${lead.productName} for ${lead.customerName}` } });
  const onConvert = (data: any) => {
    setIsLoading(true); console.log(`Converting lead ${lead.lead_number} to deal with data:`, data);
    setTimeout(() => { toast.push(<Notification type="success" title="Lead Converted">Successfully converted to a new deal.</Notification>); setIsLoading(false); onClose(); }, 1000);
  };
  return (
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}>
      ...
      <form onSubmit={handleSubmit(onConvert)}>
        <FormItem label="Deal Name"><Controller name="dealName" control={control} render={({ field }) => <Input {...field} />} /></FormItem>
        <FormItem label="Estimated Deal Value ($)"><Controller name="dealValue" control={control} render={({ field }) => <Input type="number" {...field} />} /></FormItem>
        <div className="text-right mt-6"><Button type="button" className="mr-2" onClick={onClose} disabled={isLoading}>Cancel</Button><Button variant="solid" type="submit" loading={isLoading}>Confirm & Convert</Button></div>
      </form>
    </Dialog>
  );
};
  ...
  return (
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose} width={700}>
      ...
      <div className="max-h-[400px] overflow-y-auto">
        <Table>
          <Table.THead><Table.Tr><Table.Th>ID</Table.Th><Table.Th>Name</Table.Th><Table.Th>Stage</Table.Th><Table.Th>Value</Table.Th><Table.Th>Close Date</Table.Th></Table.Tr></Table.THead>
          <Table.TBody>{dummyOpportunities.map(op => (<Table.Tr key={op.id}><Table.Td>{op.id}</Table.Td><Table.Td>{op.name}</Table.Td><Table.Td><Tag className="bg-purple-100 text-purple-700">{op.stage}</Tag></Table.Td><Table.Td>${op.value.toLocaleString()}</Table.Td><Table.Td>{dayjs(op.closeDate).format('DD MMM, YYYY')}</Table.Td></Table.Tr>))}</Table.TBody>
        </Table>
      </div>
...
const ViewDealDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose }) => (
  <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}>
    ...
    <Card bordered className="mt-4">
      <p><strong>Deal ID:</strong> {dummyDeal.id}</p><p><strong>Value:</strong> ${dummyDeal.value.toLocaleString()}</p><p><strong>Pipeline:</strong> {dummyDeal.pipeline}</p><p><strong>Stage:</strong> <Tag className="bg-emerald-100 text-emerald-700">{dummyDeal.stage}</Tag></p><p><strong>Close Date:</strong> {dayjs(dummyDeal.closeDate).format('DD MMM, YYYY')}</p>
    </Card>
    <div className="text-right mt-6"><Button variant="solid" onClick={onClose}>Close</Button></div>
  </Dialog>
);
const AddAccountDocumentsDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose }) => {
  ...
  const onUpload = (data: any) => {
    setIsLoading(true); console.log(`Uploading document for lead ${lead.lead_number} with data:`, data);
    setTimeout(() => { toast.push(<Notification type="success" title="Document Uploaded"></Notification>); setIsLoading(false); onClose(); }, 1200);
  };
  return (
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}>
      ...
      <form onSubmit={handleSubmit(onUpload)}>
        <FormItem label="Document File"><Controller name="document" control={control} render={({ field }) => <Input type="file" onChange={(e: any) => field.onChange(e.target.files?.[0])} />} /></FormItem>
        <FormItem label="Description"><Controller name="description" control={control} render={({ field }) => <Input textArea {...field} />} /></FormItem>
        <div className="text-right mt-6"><Button type="button" className="mr-2" onClick={onClose} disabled={isLoading}>Cancel</Button><Button variant="solid" type="submit" loading={isLoading}>Upload</Button></div>
      </form>
    </Dialog>
  );
};
const SendEmailDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose }) => {
  ...
  const onSendEmail = (data: { subject: string; message: string }) => {
    setIsLoading(true); console.log("Simulating email to member of", lead.lead_number, "with data:", data);
...
const AssignTaskDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose, }) => {
  ...
  const onAssignTask = (data: any) => {
    setIsLoading(true); console.log("Assigning task for", lead.lead_number, "with data:", data);
...
const AddScheduleDialog: React.FC<{ lead: LeadListItem; onClose: () => void }> = ({ lead, onClose, }) => {
  const dispatch = useAppDispatch();
  const [isLoading, setIsLoading] = useState(false);
  const { control, handleSubmit, formState: { errors, isValid }, } = useForm<ScheduleFormData>({ resolver: zodResolver(scheduleSchema), defaultValues: { event_title: `Follow up on lead ${lead.lead_number}`, event_type: undefined, date_time: null as any, remind_from: null, notes: `Regarding lead for "${lead.productName}" from customer "${lead.customerName}".`, }, mode: "onChange", });
  const onAddEvent = async (data: ScheduleFormData) => {
    setIsLoading(true);
    const payload = { module_id: Number(lead.id), module_name: "Member", event_title: data.event_title, event_type: data.event_type, date_time: dayjs(data.date_time).format("YYYY-MM-DDTHH:mm:ss"), ...(data.remind_from && { remind_from: dayjs(data.remind_from).format("YYYY-MM-DDTHH:mm:ss"), }), notes: data.notes || "", };
    try {
    ...
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose} width={700}>
      ...
      <div className="max-h-[400px] overflow-y-auto">
        <Table>
          ...
          <Table.TBody>{dummyFeedback.map((fb) => (<Table.Tr key={fb.id}><Table.Td>{fb.type}</Table.Td><Table.Td>{fb.subject}</Table.Td><Table.Td><Tag className="text-white" prefix prefixClass={statusColors[fb.status]}>{fb.status}</Tag></Table.Td></Table.Tr>))}</Table.TBody>
        </Table>
      </div>
      <div className="text-right mt-6"><Button variant="solid" onClick={onClose}>Close</Button></div>
    </Dialog>
...
const GenericActionDialog: React.FC<{ type: LeadModalType | null; lead: LeadListItem; onClose: () => void; }> = ({ type, lead, onClose }) => {
  ...
  const handleConfirm = () => {
    setIsLoading(true); console.log(`Action '${type}' for ${lead.lead_number}`);
    setTimeout(() => { toast.push(<Notification type="success" title="Action Completed" />); setIsLoading(false); onClose(); }, 1000);
  };
  return (
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}>
      <h5 className="mb-2">{title}</h5><p>Perform this action for <span className="font-semibold">{lead.lead_number}</span>?</p>
...
const LeadActionColumn = ({ onViewDetail, onEdit, onDelete, onAssign, onChangeStatus, onOpenModal, }: { onViewDetail: () => void; onEdit: () => void; onDelete: () => void; onAssign: () => void; onChangeStatus: () => void; onOpenModal: (type: LeadModalType) => void; }) => {
  ...
  return (
    <div className="flex items-center justify-center">
      <Tooltip title="Edit Lead"><div className={classNames(iconButtonClass, hoverBgClass, "text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400")} role="button" onClick={onEdit}><TbPencil /></div></Tooltip>
      <Tooltip title="View Lead"><div className={classNames(iconButtonClass, hoverBgClass, "text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400")} role="button" onClick={onViewDetail}><TbEye /></div></Tooltip>
      <Tooltip title="Delete Lead"><div className={classNames(iconButtonClass, hoverBgClass, "text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400")} role="button" onClick={onDelete}><TbTrash /></div></Tooltip>
      <Dropdown renderTitle={<BsThreeDotsVertical className="ml-0.5 mr-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md" />}>
        ...
        <Dropdown.Item onClick={() => onOpenModal("whatsapp")} className="flex items-center gap-2 text-xs"><TbBrandWhatsapp size={18} /> Send Whatsapp</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("notification")} className="flex items-center gap-2 text-xs"><TbBell size={18} /> Add Notification</Dropdown.Item>
        ...
        <Dropdown.Item onClick={() => onOpenModal("calendar")} className="flex items-center gap-2 text-xs"><TbCalendarEvent size={18} /> Add Schedule</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("active")} className="flex items-center gap-2 text-xs"><TbTagStarred size={18} /> Add Active</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("convertToDeal")} className="flex items-center gap-2 text-xs"><TbRocket size={18} /> Convert to Deal</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("viewOpportunities")} className="flex items-center gap-2 text-xs"><TbTrophy size={18} /> View Opportunities</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("viewLeadForm")} className="flex items-center gap-2 text-xs"><TbFileDescription size={18} /> View Lead Form</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("viewDeal")} className="flex items-center gap-2 text-xs"><TbPennant size={18} /> View Deal</Dropdown.Item>
        <Dropdown.Item onClick={() => onOpenModal("addAccountDocuments")} className="flex items-center gap-2 text-xs"><TbFileInvoice size={18} /> Add Account Documents</Dropdown.Item>
      </Dropdown>
    </div>
  );
};
...
const LeadsListing = ({ isDashboard }) => {
  ...
  const handleOpenModal = useCallback((type: LeadModalType, leadData: LeadListItem) => setModalState({ isOpen: true, type, data: leadData }), []);
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo((): { pageData: LeadListItem[]; total: number; allFilteredAndSortedData: LeadListItem[]; } => {
    ...
    if (filterCriteria.filterStatuses?.length) { const statuses = new Set(filterCriteria.filterStatuses); processedData = processedData.filter((item) => statuses.has(item.lead_status)); }
    if (filterCriteria.filterEnquiryTypes?.length) { const types = new Set(filterCriteria.filterEnquiryTypes); processedData = processedData.filter((item) => types.has(item.enquiry_type)); }
    if (filterCriteria.filterIntents?.length) { const intents = new Set(filterCriteria.filterIntents); processedData = processedData.filter((item) => !!item.lead_intent && intents.has(item.lead_intent)); }
    if (filterCriteria.filterProductIds?.length) { const productIds = new Set(filterCriteria.filterProductIds); processedData = processedData.filter((item) => !!item.productId && productIds.has(item.productId)); }
    if (filterCriteria.filterSalesPersonIds?.length) { const spIds = new Set(filterCriteria.filterSalesPersonIds); processedData = processedData.filter((item) => !!item.assigned_sales_person_id && spIds.has(String(item.assigned_sales_person_id))); }
    if (tableData.query && tableData.query.trim() !== "") {
      const query = tableData.query.toLowerCase().trim();
      processedData = processedData.filter((item) => String(item.id).toLowerCase().includes(query) || String(item.lead_number).toLowerCase().includes(query) || String(item.productName).toLowerCase().includes(query) || String(item.customerName).toLowerCase().includes(query) || String(item.customerId).toLowerCase().includes(query) || String(item.salesPersonName).toLowerCase().includes(query) || String(item.lead_status).toLowerCase().includes(query) || String(item.enquiry_type).toLowerCase().includes(query));
    }
    const { order, key } = tableData.sort as OnSortParam;
    if (order && key) {
      processedData.sort((a, b) => {
        let aVal = a[key as keyof LeadListItem] as any; let bVal = b[key as keyof LeadListItem] as any;
        if (key === "createdAt" || key === "updatedAt") { const dateA = aVal ? dayjs(aVal).valueOf() : 0; const dateB = bVal ? dayjs(bVal).valueOf() : 0; return order === "asc" ? dateA - dateB : dateB - dateA; }
  ...
  const onConfirmSingleDelete = useCallback(async () => {
    ...
    try {
      ...
      dispatch(getLeadAction()); setSelectedItems((p) => p.filter((i) => i.id !== itemToDelete.id));
  ...
  const onDeleteSelected = useCallback(async () => {
    ...
    try {
      ...
      dispatch(getLeadAction()); setSelectedItems([]);
  ...
  const openAssignDrawer = useCallback((lead: LeadListItem) => { setEditingLeadForDrawer(lead); assignFormMethods.reset({ salesPersonId: lead.assigned_sales_person_id || null, }); setIsAssignDrawerOpen(true); }, [assignFormMethods]);
  ...
  const onAssignSubmit = useCallback(async () => {
    ...
    try {
      console.log("Simulating assign..."); await new Promise((r) => setTimeout(r, 500));
      toast.push(<Notification title="Success" type="success">Lead assigned.</Notification>);
      closeAssignDrawer(); dispatch(getLeadAction());
  ...
  const openChangeStatusDrawer = useCallback((lead: LeadListItem) => { setEditingLeadForDrawer(lead); statusFormMethods.reset({ newStatus: lead.lead_status }); setIsChangeStatusDrawerOpen(true); }, [statusFormMethods]);
  ...
  const onChangeStatusSubmit = useCallback(async () => {
    if (!editingLeadForDrawer) return; setIsSubmittingDrawer(true);
    try {
      console.log("Simulating status change..."); await new Promise((r) => setTimeout(r, 500));
      toast.push(<Notification title="Success" type="success">Status updated.</Notification>);
      closeChangeStatusDrawer(); dispatch(getLeadAction());
  ...
  const handleOpenExportModal = useCallback(() => {
    if (!allFilteredAndSortedData || allFilteredAndSortedData.length === 0) { toast.push(<Notification title="No Data" type="info">Nothing to export.</Notification>); return; }
  ...
  const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
    setIsSubmittingExportReason(true); const moduleName = "Leads"; const date = new Date().toISOString().split("T")[0]; const fileName = `leads_export_${date}.csv`;
    try {
  ...
  return (
    <>
      <Container className="h-auto">
        <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
          ...
          <div className="flex-grow overflow-auto">
            <LeadTable
              columns={filteredColumns} data={pageData} loading={tableIsLoading}
              pagingData={{ total, pageIndex: tableData.pageIndex as number, pageSize: tableData.pageSize as number, }}
              selectable noData={!tableIsLoading && pageData.length === 0}
      ...
      <Dialog isOpen={!!leadToView} onClose={closeViewDialog} onRequestClose={closeViewDialog} width={700}>
        <h5 className="mb-4 text-lg font-semibold">Lead Details - {leadToView?.lead_number}</h5>
        {leadToView ? (
