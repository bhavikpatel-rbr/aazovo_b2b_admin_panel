  ...
  useEffect(() => {
    ...
    if (prefillData && prefillData.productId && initialDataLoaded && !isEdit) {
        const prefillForm = async () => {
            setValue('name', `Demand for ${prefillData.ListData?.product_name || 'Product'}`);
            if (prefillData.buyer_ids) setValue('product_data.0.buyer_ids', prefillData.buyer_ids.map(Number));
            if (prefillData.seller_ids) setValue('product_data.0.seller_ids', prefillData.seller_ids.map(Number));

            setValue('product_data.0.spec_id', prefillData.productSpecId || null);
            setValue('product_data.0.product_status', prefillData.ListData?.product_status?.toLowerCase() === 'non-active' ? 'non-active' : 'active');
