...
const initialDummyForms: FormItem[] = [
  {
    ...
    inquiry_description:
      "We are interested in getting a quotation for your enterprise software solution. This is a detailed description that might be long and require truncation.",
  ...
  },
  {
    ...
    inquiry_description:
      "We are facing issues with our integration and need technical assistance. Another long description to test the line clamp feature.",
  ...
  },
  {
    ...
    inquiry_description:
      "We are exploring potential partnership opportunities with your company. This inquiry has been resolved and further details are available in the resolution notes.",
    ...
    inquiry_resolution:
      "Provided details and scheduled a follow-up call. The partnership proposal was shared and accepted.",
    inquiry_attachments: [
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        type="danger"
        title={`Delete ${selectedItems.length} Inquir${
      ...
      >
        <p>
          ...
          {selectedItems.length > 1 ? "inquiries" : "inquiry"}? This action
          cannot be undone.
        </p>
      </ConfirmDialog>
    </>
      ...
      <div className="p-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
          <DetailSection title="Inquiry Info">
            ...
            <p>
              <strong>Subject:</strong> {item.inquiry_subject}
            </p>
            <p>
              <strong>Type:</strong> {item.inquiry_type}
            </p>
            <p>
              <strong>Record Status:</strong>{" "}
              <Tag className={`${recordStatusColor[item.status]} capitalize`}>
                {item.status}
              </Tag>
            </p>
          </DetailSection>
          ...
          <DetailSection title="Company & Contact">
            ...
            <p>
              <strong>Contact:</strong> {item.contact_person_name}
            </p>
            <p>
              <strong>Email:</strong>{" "}
              <a
                href={`mailto:${item.contact_person_email}`}
            ...
            <p>
              <strong>Phone:</strong> {item.contact_person_phone || "N/A"}
            </p>
          </DetailSection>
          ...
          <DetailSection title="Status & Assignment">
            <p>
              ...
              <Tag
                className={`${
                  priorityColors[item.inquiry_priority] ||
                  priorityColors.default
                } capitalize`}
              >
                {item.inquiry_priority}
              </Tag>
            </p>
            <p>
              ...
              <Tag
                className={`${
                  inquiryStatusColors[item.inquiry_status] ||
                  inquiryStatusColors.default
                } capitalize`}
              >
                {item.inquiry_status}
              </Tag>
            ...
            <p>
              <strong>Feedback Status:</strong> {item.feedback_status}
            </p>
          </DetailSection>
          ...
          <DetailSection title="Dates">
            <FormattedDate
              label="Inquiry Date"
              dateString={item.inquiry_date}
            />
            <FormattedDate
              label="Response Date"
              dateString={item.response_date}
            />
            <FormattedDate
              label="Resolution Date"
              dateString={item.resolution_date}
            />
            <FormattedDate
              label="Follow-up Date"
              dateString={item.follow_up_date}
            />
          </DetailSection>
        </div>

        <DetailSection title="Description">
          <p className="whitespace-pre-wrap p-2 bg-gray-50 dark:bg-gray-700 rounded">
            {item.inquiry_description || "N/A"}
          </p>
        </DetailSection>

        <DetailSection title="Resolution Notes">
          <p className="whitespace-pre-wrap p-2 bg-gray-50 dark:bg-gray-700 rounded">
            {item.inquiry_resolution || "N/A"}
          </p>
        </DetailSection>
...
const InquiriesListing = () => {
  ...
  const [currentItemForView, setCurrentItemForView] = useState<FormItem | null>(
    null
  );
  ...
  const handleSelectChange = useCallback(
    (value: number) => {
      handleSetTableData(
        cloneDeep({ ...tableData, pageSize: Number(value), pageIndex: 1 })
      );
      setSelectedForms([]);
    },
    [tableData, handleSetTableData]
  ...
  const handleSearchChange = useCallback(
    (query: string) => {
      handleSetTableData(
        cloneDeep({ ...tableData, query: query, pageIndex: 1 })
      );
    },
    [tableData, handleSetTableData]
  );
  ...
  const handleAllRowSelect = useCallback(
    (checked: boolean, rows: Row<FormItem>[]) => {
      ...
      if (checked) {
        ...
        setSelectedForms((prev) =>
          prev.filter((item) => !currentDataIds.has(item.id))
        );
      }
    },
    [pageData]
  ...
  const handleEdit = useCallback(
    (form: FormItem) => {
      ...
      toast.push(
        <Notification title="Action" type="info">
          Edit for {form.inquiry_id} clicked.
        </Notification>
      );
    },
    [navigate]
  ...
  const handleCloneForm = useCallback((formToClone: FormItem) => {
    ...
    toast.push(
      <Notification
        title="Inquiry Cloned"
        type="success"
        duration={2500}
      >{`New inquiry ${clonedForm.inquiry_id} created.`}</Notification>
    );
  }, []);
  ...
  const handleChangeStatus = useCallback((formToChange: FormItem) => {
    ...
    toast.push(
      <Notification
        ...
        type="success"
        duration={2500}
      >{`Status of ${formToChange.inquiry_id} changed to ${newStatus}.`}</Notification>
    );
  }, []);
    ...
    if (itemToDelete) {
      ...
      toast.push(
        <Notification
          ...
          type="success"
          duration={2500}
        >{`Inquiry ${itemToDelete.inquiry_id} deleted.`}</Notification>
      );
    }
  ...
  const columns: ColumnDef<FormItem>[] = useMemo(
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col gap-0.5">
              <span
                className="font-semibold text-sm text-gray-800 dark:text-gray-100 hover:text-blue-600 cursor-pointer"
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Tag
                  className={`${recordStatusColor[status]} capitalize text-[10px] px-1.5 py-0.5`}
                >
                  {status}
                </Tag>
              </div>
          ...
          return (
            <div className="flex flex-col gap-0.5">
              ...
              <InfoLine
                ...
                title={contact_person_email}
              />
              <InfoLine
                icon={<TbPhone size={14} />}
                text={contact_person_phone}
                title={contact_person_phone}
              />
            </div>
          );
        },
      },
      {
        ...
        size: 200,
        cell: ({ row }) => {
          const { inquiry_priority, inquiry_status, assigned_to } =
            row.original;
          return (
            <div className="flex flex-col gap-1 text-xs">
              <div className="flex items-center gap-1">
                ...
                <Tag
                  className={`${
                    priorityColors[inquiry_priority] || priorityColors.default
              ...
              <div className="flex items-center gap-1">
                ...
                <Tag
                  className={`${
                    inquiryStatusColors[inquiry_status] ||
                    inquiryStatusColors.default
                  } capitalize text-[10px] px-1.5 py-0.5`}
                >
                  {inquiry_status}
    ...
    <Container className="h-full">
      ...
      >
        <p>
          ...
          <strong>{itemToDelete?.inquiry_id}</strong>? This action cannot be
          undone.
        </p>
      </ConfirmDialog>
    </Container>
