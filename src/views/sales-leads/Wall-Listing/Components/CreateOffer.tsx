...
import {
    ...
    getUsersAction, // Added API action
...
const offerFormSchema = z.object({
    ...
    assign_user: z.number({ required_error: "Assigned User is required."}).nullable(), // Added assign_user
...
const CreateOffer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            await Promise.all([
                dispatch(getUsersAction()), // Added API call
    ...
    const dispatchStatusOptions: OptionType[] = [{ value: "Booking", label: "Booking" }, { value: "Ready Stock", label: "Ready Stock" }]; // Updated options
    ...
    const { control, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<OfferFormData>({
        ...
        defaultValues: { name: "", assign_user: null, product_data: [defaultProductGroup], groupA: "", groupB: "" },
    });

    const { fields, append, remove } = useFieldArray({ control, name: "product_data" });
    const watchedProductGroups = watch("product_data");
    const watchedItems = watch('product_data.0.items'); // Restored for pre-fill logic

    // Effect for setting form data in EDIT mode
    useEffect(() => {
        const offerDataFromState = location.state?.originalApiItem;
        if (isEdit && offerDataFromState && initialDataLoaded) {
    ...
    const handleProductChange = useCallback(async (groupIndex: number, productId: number | null) => {
        ...
        try {
            ...
            productPrice = (result.status && result.sales_price) ? parseFloat(result.sales_price) : undefined;
        } catch (error) {
    ...
    // Effect for pre-filling data in CREATE mode (Restored)
    useEffect(() => {
    ...
    // Effect for pre-filling item details in CREATE mode (Restored)
    useEffect(() => {
        const prefillData = location.state;
        if (prefillData && watchedItems && watchedItems.length > 0 && !isEdit) {
            ...
            } else if (watchedItems.length > 0) { // Fallback to first item if color doesn't match
    ...
    }, [watchedItems, location.state, setValue, isEdit]);
    ...
    const onFormSubmit = useCallback(async (data: OfferFormData) => {
        ...
        const apiPayload = {
            ...
            name: data.name,
            assign_user: data.assign_user, // Added to payload
            groupA: data.groupA,
            groupB: data.groupB,
        ...
        try {
            const action = isEdit ? editOfferAction(apiPayload) : addOfferAction(apiPayload);
            await dispatch(action).unwrap();
            const actionType = isEdit ? 'Updated' : 'Created';
            toast.push(<Notification title={`Offer ${actionType}`} type="success">{`Offer has been successfully ${actionType.toLowerCase()}.`}</Notification>);
        ...
        <Form id="offerForm" onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
                    ...
                    <div className="p-4">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <FormItem label="Assign User" invalid={!!errors.assign_user} errorMessage={errors.assign_user?.message}>
                                <Controller name="assign_user" control={control} render={({ field }) => <UiSelect placeholder="Select a user" options={userOptions} value={userOptions.find(o => o.value === field.value)} onChange={(option) => field.onChange(option ? option.value : null)} isLoading={isLoading} isClearable />} />
                            </FormItem>
                        </div>
                    </div>
                )}
            ...
            {fields.map((field, index) => (
                <Card key={field.id} bodyClass="p-4">
                    <div className="flex justify-end mb-2"><Button size="sm" type="button" icon={<TbPlus />} onClick={() => append(defaultProductGroup)}>Add Product Group</Button></div>
