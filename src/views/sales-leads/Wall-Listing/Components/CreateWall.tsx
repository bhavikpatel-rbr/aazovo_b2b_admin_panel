...
const singleWallItemSchema = z.object({
  ...
  activeHours: z
    .string({
      required_error: "Active Hours is required.",
      invalid_type_error: "Active Hours must be a string.",
    })
    .regex(/^([01]\d|2[0-3]):([0-5]\d)$/, "Active Hours must be in HH:mm format."),
...
const WallItemForm = () => {
  ...
  const memberOptions: OptionType<number>[] = useMemo(() => Array.isArray(memberData) ? memberData.map((c: any) => ({ value: c.id, label: `(${c.customer_code}) - ${c.name || ' '}`, })) : [], [memberData]);
