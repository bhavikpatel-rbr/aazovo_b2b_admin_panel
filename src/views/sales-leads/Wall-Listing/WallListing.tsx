...
import {
  ...
  TbEye,
  TbLink,
  TbCloudDownload, // Added TbCloudDownload for import
} from "react-icons/tb";
...
const wallItemFormSchema = z.object({
  ...
  productSpecId: z
    .number({ required_error: "Product Specification is required." })
    .min(1, "Product Spec is required."),
  companyId: z
    .string({ required_error: "Company is required." })
    .min(1, "Company is required."),
  customerId: z
    .number({ required_error: "Member/Customer is required." })
  ...
  intent: z.enum(
    ["Buy", "Sell", "Exchange"] as const, // Use as const for stricter typing
...
const ActionColumn = ({
  ...
  const iconBtnClass =
    "text-lg p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors";
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      <Tooltip title="Wall Link">
        <Button
          ...
          icon={<TbLink />}
          onClick={onWallLink}
          className={`${iconBtnClass} hover:text-green-500`}
        />
      </Tooltip>
...
const WallSearch = React.forwardRef<HTMLInputElement, WallSearchProps>(
  ({ onInputChange }, ref) => (
    <DebouceInput
      ref={ref}
      className="w-full"
      placeholder="Search wall listings..."
      suffix={<TbSearch />}
...
const WallListing = () => {
              ...
              prev.map((item) =>
                item.id === action.payload.id
                  ? {
                      ...item,
                      ...action.payload.data,
                      status: action.payload.data.recordStatus,
                      want_to: action.payload.data.intent,
                      updated_at: new Date().toISOString(),
                    }
                  : item
              )
  ...
  const openAddDrawer = useCallback(() => {
    ...
    formMethods.reset({
      ...
      companyId: "",
      customerId: undefined, // Ensure companyId is string
      qty: 1,
      ...
      dispatchStatus: null,
      paymentTermId: null,
      deviceCondition: deviceConditionOptions[0]?.value,
      eta: null,
      location: null,
      ...
      try {
        if (editingItem) {
          await dispatchSimulated({
            type: "wall/edit",
            payload: { id: editingItem.id, data: apiPayload },
          }).unwrap();
          toast.push(
  ...
  const { pageData, total, allFilteredAndSortedData } =
    useMemo((): ProcessedDataType => {
      ...
      if (
        filterCriteria.filterProductSpecIds &&
        filterCriteria.filterProductSpecIds.length > 0
      ) {
        const specIds = new Set(
          filterCriteria.filterProductSpecIds.map((opt) => opt.value as number)
        );
        processedData = processedData.filter((item) =>
          specIds.has(item.productSpecId)
        );
      }
      if (
        ...
        const companyIds = new Set(
          filterCriteria.filterCompanyIds.map((opt) => opt.value as string)
        );
        processedData = processedData.filter((item) =>
      ...
      if (
        filterCriteria.filterCustomerIds &&
        filterCriteria.filterCustomerIds.length > 0
      ) {
        const customerIds = new Set(
          filterCriteria.filterCustomerIds.map((opt) => opt.value as number)
        );
        processedData = processedData.filter((item) =>
  ...
  const handleImportData = useCallback(() => {
    // Define handleImportData
    setImportDialogOpen(true);
  }, []);

  const handleImportFileSelect = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      // Define handleImportFileSelect
  ...
  const columns: ColumnDef<WallItem>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.contact_person_name ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.contact_person_email ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.contact_person_phone ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.product_category ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.product_subcategory ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.product_description ?? "-"}</span>
        ),
      },
      {
      ...
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.product_status ?? "-"}</span>
        ),
      },
      {
      ...
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.shipping_options ?? "-"}</span>
        ),
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.payment_method ?? "-"}</span>
        ),
      },
      {
        header: "Warranty",
      ...
      },
      {
        ...
        cell: (props) => (
          <span>
            {props.row.original.created_date
              ? new Date(props.row.original.created_date).toLocaleDateString()
              : "-"}
          </span>
        ),
      },
        ...
        cell: (props) => (
          <span>
            {props.row.original.last_updated
              ? new Date(props.row.original.last_updated).toLocaleDateString()
              : "-"}
          </span>
        ),
      },
      ...
      },
      {
        ...
        enableSorting: true,
        cell: (props) => (
          <span>{props.row.original.interaction_type ?? "-"}</span>
        ),
      },
    ...
    <>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto p-1">
            ...
            <FormItem
              ...
              errorMessage={
                formMethods.formState.errors.product_images?.message
              }
            >
              <Controller
                name="product_images"
        ...
        >
          <div className="flex-grow overflow-y-auto p-1">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <UiSelect
                    ...
                    options={dummyProductSpecs.map((p) => ({
                      value: p.id,
                      label: p.name,
                    }))}
                    {...field}
            ...
            <FormItem label="Member/Customer">
              <Controller
                name="filterCustomerIds"
                control={filterFormMethods.control}
                render={({ field }) => (
                  <UiSelect
                    isMulti
                    placeholder="Select members..."
                    options={dummyCustomers.map((p) => ({
                      value: p.id,
                      label: p.name,
                    }))}
                    {...field}
      ...
      >
        <p>
          ...
          <strong>ID: {itemToDelete?.id}</strong> ({itemToDelete?.product_name}
          )?
        </p>
      </ConfirmDialog>
