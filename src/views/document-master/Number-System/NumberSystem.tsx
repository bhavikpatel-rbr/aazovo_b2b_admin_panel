...
}).superRefine((data, ctx) => { // Ensure current numbers are not less than start numbers
...
const CSV_HEADERS_NS = ['ID', 'Name', 'Prefix', 'Country IDs', 'KYC Start', 'KYC Current', 'Temp Start', 'Temp Current'];
const CSV_KEYS_NS: (keyof NumberSystemItem)[] = [
    'id', 'name', 'prefix', 'country_ids',
    'customer_code_starting', 'current_customer_code',
    'non_kyc_customer_code_starting', 'non_kyc_current_customer_code'
];
...
function exportNumberSystemsToCsv(filename: string, rows: NumberSystemItem[]) {
    ...
    const csvContent =
        ...
        rows.map((row) => CSV_KEYS_NS.map((k) => {
            let cell = row[k];
...
const ActionColumn = ({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void; }) => {
    ...
    return (
        <div className="flex items-center justify-center gap-3">
            <Tooltip title="Edit Number System"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400')} role="button" onClick={onEdit}><TbPencil /></div></Tooltip>
            <Tooltip title="Delete Number System"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400')} role="button" onClick={onDelete}><TbTrash /></div></Tooltip>
        </div>
    );
};
...
const ItemSearch = React.forwardRef<HTMLInputElement, ItemSearchProps>(
    ({ onInputChange }, ref) => (
        <DebouceInput ref={ref} className="w-full" placeholder="Search by name, ID, or prefix..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />
    )
);
...
const NumberSystemsSelectedFooter = ({ selectedItems, onDeleteSelected, isDeleting }: NumberSystemsSelectedFooterProps) => {
    ...
    return (
        <>
            ...
            <ConfirmDialog
                isOpen={deleteConfirmationOpen} type="danger" title={`Delete ${selectedItems.length} System${selectedItems.length > 1 ? 's' : ''}`}
                onClose={handleCancelDelete} onRequestClose={handleCancelDelete} onCancel={handleCancelDelete} onConfirm={handleConfirmDelete}
            >
                <p>Are you sure you want to delete the selected number system{selectedItems.length > 1 ? 's' : ''}? This action cannot be undone.</p>
            </ConfirmDialog>
        </>
    );
};
...
const NumberSystems = () => {
    ...
    useEffect(() => {
        if (Array.isArray(CountriesData) && CountriesData.length > 0) {
            ...
            setCountryOptions(prevOptions => {
                if (JSON.stringify(prevOptions) !== JSON.stringify(newOptions)) return newOptions;
                return prevOptions;
            });
    ...
    const openEditDrawer = useCallback((item: NumberSystemItem) => {
        setEditingItem(item);
    ...
    const onSubmitHandler = async (data: NumberSystemFormData) => {
        ...
        try {
            if (editingItem) {
                ...
                toast.push(<Notification title="Number System Updated" type="success" duration={2000}>System updated.</Notification>);
                closeEditDrawer();
            } else {
                await dispatch(addNumberSystemAction(apiPayload)).unwrap();
                toast.push(<Notification title="Number System Added" type="success" duration={2000}>New system added.</Notification>);
                closeAddDrawer();
            }
            dispatch(getNumberSystemsAction());
        } catch (error: any) {
    ...
    const handleDeleteClick = useCallback((item: NumberSystemItem) => { if (item.id === undefined || item.id === null) { toast.push(<Notification title="Error" type="danger">Cannot delete: ID is missing.</Notification>); return; } setItemToDelete(item); setSingleDeleteConfirmOpen(true); }, []);
    const onConfirmSingleDelete = useCallback(async () => { if (!itemToDelete?.id) { return; } setIsDeleting(true); setSingleDeleteConfirmOpen(false); try { await dispatch(deleteNumberSystemAction({ id: itemToDelete.id })).unwrap(); toast.push(<Notification title="Number System Deleted" type="success" duration={2000}>{`System "${itemToDelete.name}" deleted.`}</Notification>); setSelectedItems((prev) => prev.filter((item) => item.id !== itemToDelete!.id)); dispatch(getNumberSystemsAction()); } catch (error: any) { toast.push(<Notification title="Delete Failed" type="danger" duration={3000}>{error.message || 'Could not delete system.'}</Notification>); console.error("Delete Error:", error); } finally { setIsDeleting(false); setItemToDelete(null); } }, [dispatch, itemToDelete]);
    const handleDeleteSelected = useCallback(async () => { if (selectedItems.length === 0) { return; } setIsDeleting(true); const validItems = selectedItems.filter(item => item.id !== undefined && item.id !== null); if (validItems.length === 0) { setIsDeleting(false); return; } const idsToDelete = validItems.map(item => String(item.id)); try { await dispatch(deleteAllNumberSystemsAction({ ids: idsToDelete.join(',') })).unwrap(); toast.push(<Notification title="Deletion Successful" type="success" duration={2000}>{`${validItems.length} system(s) deleted.`}</Notification>); setSelectedItems([]); dispatch(getNumberSystemsAction()); } catch (error: any) { toast.push(<Notification title="Deletion Failed" type="danger" duration={3000}>{error.message || 'Failed to delete selected systems.'}</Notification>); console.error("Bulk Delete Error:", error); } finally { setIsDeleting(false); } }, [dispatch, selectedItems]);
    ...
    const onClearFilters = useCallback(() => { const defaultFilters = { filterCountryIds: [] }; filterFormMethods.reset(defaultFilters); setFilterCriteria(defaultFilters); setTableData(prev => ({ ...prev, pageIndex: 1 })); }, [filterFormMethods]);
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterCountryIds && filterCriteria.filterCountryIds.length > 0) {
            ...
            processedData = processedData.filter(item => {
                ...
                return itemCountryIds.some(id => selectedFilterCountryOptionValues.includes(id));
            });
        }
        ...
        if (order && key && ['id', 'name', 'prefix', 'countriesCount'].includes(String(key))) {
    ...
    const handleExportData = useCallback(() => { const success = exportNumberSystemsToCsv('number_systems_export.csv', allFilteredAndSortedData); if (success) toast.push(<Notification title="Export Successful" type="success" duration={2000}>Data exported.</Notification>); }, [allFilteredAndSortedData]);
    ...
    const handleRowSelect = useCallback((checked: boolean, row: NumberSystemItem) => { setSelectedItems((prev) => { if (checked) return prev.some((item) => item.id === row.id) ? prev : [...prev, row]; return prev.filter((item) => item.id !== row.id); }); }, []);
    const handleAllRowSelect = useCallback((checked: boolean, currentRows: Row<NumberSystemItem>[]) => { const cPOR = currentRows.map((r) => r.original); if (checked) { setSelectedItems((pS) => { const pSIds = new Set(pS.map((i) => i.id)); const nRTA = cPOR.filter((r) => !pSIds.has(r.id)); return [...pS, ...nRTA]; }); } else { const cPRIds = new Set(cPOR.map((r) => r.id)); setSelectedItems((pS) => pS.filter((i) => !cPRIds.has(i.id))); } }, []);
        ...
        () => [
            ...
            // { header: 'Prefix', accessorKey: 'prefix', enableSorting: true, size: 100 }, // Uncomment to display prefix
            {
                ...
                cell: (props) => {
                    ...
                    const ids = countryIdString.split(',').map(id => id.trim()).filter(id => id);
                    if (ids.length === 0) return <Tag>None</Tag>;

                    const names = ids.map(id => {
                        const country = CountriesData.find((c: CountryListItem) => String(c.id) === id);
                    ...
                    return (
                        <Tooltip title={names.join(', ')} wrapperClassName="whitespace-nowrap max-w-[200px] overflow-hidden text-ellipsis">
    ...
    const renderDrawerForm = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2"> {/* Adjusted gap */}
            ...
            <div className="md:col-span-2 border-t pt-3 mt-1"> {/* Adjusted padding/margin */}
                <h6 className="text-sm font-medium mb-2">Numbering System For KYC Member</h6>
            </div>
            <FormItem label="Member ID Starting No." invalid={!!formMethods.formState.errors.kycStartNumber} errorMessage={formMethods.formState.errors.kycStartNumber?.message}>
                <Controller name="kycStartNumber" control={formMethods.control} render={({ field }) => <Input {...field} type="number" placeholder="e.g., 10001" />} />
            </FormItem>
            <FormItem label="Member ID Current No." invalid={!!formMethods.formState.errors.kycCurrentNumber} errorMessage={formMethods.formState.errors.kycCurrentNumber?.message}>
            ...
            <FormItem label="Member ID Starting No." invalid={!!formMethods.formState.errors.tempStartNumber} errorMessage={formMethods.formState.errors.tempStartNumber?.message}>
                <Controller name="tempStartNumber" control={formMethods.control} render={({ field }) => <Input {...field} type="number" placeholder="e.g., 5001" />} />
            </FormItem>
            <FormItem label="Member ID Current No." invalid={!!formMethods.formState.errors.tempCurrentNumber} errorMessage={formMethods.formState.errors.tempCurrentNumber?.message}>
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <Select
                            ...
                            onChange={(selectedOpts) => field.onChange(selectedOpts ? selectedOpts.map(opt => opt.value) : [])}
                        />
                    )}
                />
            </FormItem>
    ...
    return (
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        <h3 className="mb-4 sm:mb-0 flex items-center gap-2"><TbSettingsCog /> Numbering Systems</h3>
            ...
            <NumberSystemsSelectedFooter selectedItems={selectedItems} onDeleteSelected={handleDeleteSelected} isDeleting={isDeleting} />
            ...
            >
                <Form id="numberSystemForm" onSubmit={formMethods.handleSubmit(onSubmitHandler)} className="flex flex-col gap-2"> {/* Reduced overall gap */}
                    {renderDrawerForm()}
                </Form>
            </Drawer>
            ...
            {/* Filter Drawer */}
            <Drawer
                title="Filter Number Systems" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer}
                footer={
            ...
            >
                <Form id="filterNumberSystemForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Filter by Countries">
                        <Controller name="filterCountryIds" control={filterFormMethods.control}
                            render={({ field }) => (
                                <Select
                                    isMulti
            ...
            {/* ConfirmDialog */}
            <ConfirmDialog
                isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Number System"
            ...
            >
                <p>Are you sure you want to delete the number system "<strong>{itemToDelete?.name}</strong>"? This action cannot be undone.</p>
            </ConfirmDialog>
        </>
    );
};
