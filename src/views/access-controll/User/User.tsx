...
const UserActionTools = ({ allUsers }: { allUsers: UserItem[] }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-3">
      {" "}
      {/* <CSVLink ... /> */}{" "}
      <Button variant="solid" icon={<TbUserPlus />} onClick={handleAdd} block>
        {" "}
        Add new User{" "}
      </Button>{" "}
    </div>
        ...
        <div className="flex items-center justify-between w-full px-4 sm:px-8">
          <span className="flex items-center gap-2">
            ...
            <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
              <span className="heading-text">{selectedUsers.length}</span>
              <span>User{selectedUsers.length > 1 ? "s" : ""} selected</span>
            </span>
          </span>
...
const UserListing = () => {
    ...
    if (tableData.query) {
      ...
      processedData = processedData.filter(
        (u) =>
          ...
          u.roles.some((role) => role.toLowerCase().includes(query)) ||
    ...
    if (order && key) {
      ...
      sortedData.sort((a, b) => {
        if (key === "createdDate" || key === "lastLogin") {
          const timeA =
            a[key]?.getTime() ?? (order === "asc" ? Infinity : -Infinity);
          const timeB =
            b[key]?.getTime() ?? (order === "asc" ? Infinity : -Infinity);
  ...
  const handleRowSelect = useCallback(
    (checked: boolean, row: UserItem) => {
      setSelectedUsers((prev) => {
        // Corrected setter
        if (checked) {
          return prev.some((u) => u.id === row.id) ? prev : [...prev, row];
        } else {
  ...
  const handleDelete = useCallback(
    (userToDelete: UserItem) => {
      ...
      setUsers((current) => current.filter((u) => u.id !== userToDelete.id)); // Use correct setter
      setSelectedUsers((prev) => prev.filter((u) => u.id !== userToDelete.id)); // Corrected setter
      toast.push(
        <Notification
          title="User Deleted"
          type="success"
    ...
    () => [
      {
        ...
        cell: (props) => {
          ...
          return (
            <Tag className={`${userStatusColor[status]} text-white capitalize`}>
              {displayStatus}
            </Tag>
          );
        },
      ...
      },
      {
        ...
        cell: (props) => (
          <ActionColumn
            ...
            onChangePassword={() => handleOpenResetPwd(props.row.original)}
          />
        ),
      },
    ],
