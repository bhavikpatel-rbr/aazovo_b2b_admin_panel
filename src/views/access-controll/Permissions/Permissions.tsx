...
const initialDummyPermissions: PermissionItem[] = [
  ...
  },
  {
    ...
    description: "Allows access to and modification of system-wide settings.",
    module: "System",
  },
...
type PermissionSelectedFooterProps = {
  selectedItems: PermissionItem[];
  onDeleteSelected: () => void;
};
const PermissionSelectedFooter = ({
  selectedItems,
  onDeleteSelected,
}: PermissionSelectedFooterProps) => {
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between w-full px-4 sm:px-8">
          <span className="flex items-center gap-2">
            ...
            <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
              ...
              <span>
                Permission{selectedItems.length > 1 ? "s" : ""} selected
              </span>
            </span>
          </span>
          <Button
      ...
      <ConfirmDialog
        ...
        type="danger"
        title={`Delete ${selectedItems.length} Permission${
...
function exportToCsvPermissions(filename: string, rows: PermissionItem[]) {
  ...
  const csvKeysPermissionExport: (keyof PermissionExportItem)[] = [
    "id",
    "name",
    "description",
    "module",
  ...
  const csvContent =
    ...
    transformedRows
      .map((row) => {
        return csvKeysPermissionExport
          .map((k) => {
            let cell = row[k];
...
const PermissionsListing = () => {
  ...
  const addFormMethods = useForm<PermissionFormData>({
    resolver: zodResolver(permissionFormSchema),
    mode: "onChange",
  });
  const editFormMethods = useForm<PermissionFormData>({
    resolver: zodResolver(permissionFormSchema),
    mode: "onChange",
  });
  const filterFormMethods = useForm<PermissionFilterFormData>({
    resolver: zodResolver(permissionFilterFormSchema),
    defaultValues: filterCriteria,
  });

  // --- CRUD Handlers ---
  const openAddDrawer = () => {
    addFormMethods.reset({ id: "", name: "", description: "", module: "" });
  ...
  const onAddPermissionSubmit = async (data: PermissionFormData) => {
    ...
    if (permissions.some((p) => p.id === data.id)) {
      addFormMethods.setError("id", {
        type: "manual",
        message: "This Permission ID already exists.",
      });
    ...
    toast.push(
      <Notification
        ...
        type="success"
      >{`Permission "${data.name}" added.`}</Notification>
    );
    closeAddDrawer();
    setIsSubmitting(false);
    setMasterLoadingStatus("idle");
  ...
  const onEditPermissionSubmit = async (data: PermissionFormData) => {
    ...
    const updatedPermission: PermissionItem = {
      ...editingPermission,
      ...data,
    ...
    toast.push(
      <Notification
        ...
        type="success"
      >{`Permission "${data.name}" updated.`}</Notification>
    );
    closeEditDrawer();
    setIsSubmitting(false);
    setMasterLoadingStatus("idle");
  ...
  const handleDeleteClick = (permission: PermissionItem) => {
  ...
  const onConfirmSingleDelete = async () => {
    ...
    toast.push(
      <Notification
        ...
        type="success"
      >{`Permission "${itemToDelete.name}" deleted.`}</Notification>
    );
    setIsDeleting(false);
    setMasterLoadingStatus("idle");
  ...
  const handleDeleteSelected = async () => {
    ...
    toast.push(
      <Notification
        ...
        type="success"
      >{`${selectedItems.length} permission(s) deleted.`}</Notification>
    );
    setSelectedItems([]);
    setIsDeleting(false);
    setMasterLoadingStatus("idle");
  ...
  const handleAllRowSelect = useCallback(
    (checked: boolean, currentRows: Row<PermissionItem>[]) => {
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    ...
    if (filterCriteria.filterModule?.length) {
      const v = filterCriteria.filterModule.map((o) => o.value.toLowerCase());
      processedData = processedData.filter(
        (item) => item.module && v.includes(item.module.toLowerCase())
      );
    }
  ...
  const handleExportData = () => {
    const success = exportToCsvPermissions(
      "system_permissions_export.csv",
  ...
  const moduleOptionsForFilter = useMemo(() => {
    const unique = new Set(
      permissions
        ...
        .filter(
          Boolean as any as (value: string | undefined) => value is string
        )
    );
    return Array.from(unique).map((m) => ({ value: m, label: m }));
  }, [permissions]);
    ...
    <>
      ...
      >
        <Form
          ...
          onSubmit={addFormMethods.handleSubmit(onAddPermissionSubmit)}
          className="flex flex-col gap-4 p-1"
        >
          <FormItem
            label="Permission ID (System Key)"
            invalid={!!addFormMethods.formState.errors.id}
            errorMessage={addFormMethods.formState.errors.id?.message}
          >
            <Controller
              name="id"
              ...
              render={({ field }) => (
                <Input
                  {...field}
                  prefix={<TbKey />}
                  placeholder="e.g., users:create_new"
                />
              )}
            />
          </FormItem>
          ...
          <FormItem
            label="Module/Group (Optional)"
            invalid={!!addFormMethods.formState.errors.module}
            errorMessage={addFormMethods.formState.errors.module?.message}
          >
            <Controller
              name="module"
              control={addFormMethods.control}
      ...
      >
        <Form
          ...
          onSubmit={editFormMethods.handleSubmit(onEditPermissionSubmit)}
          className="flex flex-col gap-4 p-1"
        >
          <FormItem
            label="Permission ID (System Key)"
            invalid={!!editFormMethods.formState.errors.id}
            errorMessage={editFormMethods.formState.errors.id?.message}
          >
            <Controller
              name="id"
              ...
              render={({ field }) => (
                <Input
                  {...field}
                  prefix={<TbKey />}
                  disabled={true /* ID is usually not editable */}
                />
              )}
            />
          </FormItem>
          ...
          >
            <Controller
              ...
              control={editFormMethods.control}
              render={({ field }) => (
                <Input {...field} prefix={<TbShieldLock />} />
              )}
            />
          </FormItem>
          <FormItem
            label="Module/Group (Optional)"
            invalid={!!editFormMethods.formState.errors.module}
            errorMessage={editFormMethods.formState.errors.module?.message}
          >
            <Controller
              name="module"
              control={editFormMethods.control}
        ...
        >
          <FormItem label="Name / ID">
            <Controller
              ...
              render={({ field }) => (
                <Input {...field} placeholder="Enter name or ID to filter..." />
              )}
            />
          </FormItem>
      ...
      >
        <p>
          ...
          <strong>{itemToDelete?.name}</strong>" (ID: {itemToDelete?.id})?
        </p>
      </ConfirmDialog>
    </>
  );
