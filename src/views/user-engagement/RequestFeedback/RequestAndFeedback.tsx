...
export type RequestFeedbackStatus = "Pending" | "In progress" | "Resolved" | "Rejected";
...
const statusColors: Record<RequestFeedbackStatus, string> = {
  ...
  "In progress": "bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-100",
  Resolved: "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-100",
...
const requestFeedbackFormSchema = z.object({
  ...
  type: z.enum(["Request", "Feedback"], { errorMap: () => ({ message: "Please select a type." }) }),
  category: z.string().min(1, "Category is required."),
  department_id: z.string().min(1, "Department is required."),
  feedback_details: z.string().min(10, "Description must be at least 10 characters.").max(5000),
  status: z.enum(["Pending", "In progress", "Resolved", "Rejected"], { errorMap: () => ({ message: "Please select a status." }), }),
...
const ItemActionColumn = ({ onEdit, onViewDetail, onSendEmail, onSendWhatsapp, onAddNotification, onAssignTask, onAddSchedule, }: {
  ...
  <div className="flex items-center justify-center gap-1">
    <Tooltip title="Edit"><div className="text-xl p-1 cursor-pointer text-gray-500 hover:text-emerald-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700" role="button" onClick={onEdit}><TbPencil /></div></Tooltip>
    <Tooltip title="View"><div className="text-xl p-1 cursor-pointer text-gray-500 hover:text-blue-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700" role="button" onClick={onViewDetail}><TbEye /></div></Tooltip>
...
const ItemTableTools = ({ onSearchChange, onFilter, onClearFilters, columns, filteredColumns, setFilteredColumns, activeFilterCount, isDataReady }: { onSearchChange: (query: string) => void; onFilter: () => void; onClearFilters: () => void; columns: ColumnDef<RequestFeedbackItem>[]; filteredColumns: ColumnDef<RequestFeedbackItem>[]; setFilteredColumns: React.Dispatch<React.SetStateAction<ColumnDef<RequestFeedbackItem>[]>>; activeFilterCount: number; isDataReady: boolean }) => {
  ...
  const toggleColumn = (checked: boolean, colId: string) => {
    if (checked) {
      ...
      if (originalColumn) setFilteredColumns(prev => [...prev, originalColumn].sort((a, b) => columns.findIndex(c => (c.id || c.accessorKey) === (a.id || a.accessorKey)) - columns.findIndex(c => (c.id || c.accessorKey) === (b.id || b.accessorKey))));
    } else {
...
const RequestAndFeedbackListing = () => {
  ...
  const defaultFormValues: RequestFeedbackFormData = useMemo(() => ({
    ...
    type: "Request", category: "", department_id: "",
    feedback_details: "", status: "Pending",
    rating: null, attachment: undefined,
  }), []);
  ...
  const openEditDrawer = useCallback((item: RequestFeedbackItem) => {
    ...
    reset({
      ...
      company_name: item.company_name || "", type: item.type, category: item.category,
      department_id: String(item.department_id), feedback_details: item.feedback_details,
      status: item.status, rating: item.rating ? String(item.rating) : null,
  ...
  const onSubmitHandler = async (data: RequestFeedbackFormData) => {
    ...
    if (editingItem) {
      ...
      else if (removeExistingAttachment && editingItem.attachment) formData.append("delete_attachment", "true");
    } else {
      if (selectedFile instanceof File) formData.append("attachment", selectedFile);
    }
  ...
  const handleSendEmail = useCallback((item: RequestFeedbackItem) => { window.open(`mailto:${item.email}`); }, []);
  ...
  const handleCardClick = useCallback((status?: RequestFeedbackStatus | 'all') => { onClearFilters(); if (status && status !== 'all') { const statusOption = STATUS_OPTIONS.find(opt => opt.value === status); if (statusOption) setFilterCriteria({ filterStatus: [statusOption] }); } }, [onClearFilters]);
  ...
  const { pageData, total } = useMemo(() => {
    ...
    if (filterCriteria.filterType?.length) processedData = processedData.filter((item) => filterCriteria.filterType!.some(f => f.value === item.type));
    if (filterCriteria.filterStatus?.length) processedData = processedData.filter((item) => filterCriteria.filterStatus!.some(f => f.value === item.status));
    if (filterCriteria.filterRating?.length) processedData = processedData.filter((item) => filterCriteria.filterRating!.some(f => f.value === String(item.rating)));
    if (tableData.query) {
  ...
  const columns: ColumnDef<RequestFeedbackItem>[] = useMemo(() => [
    { header: "Member Info", accessorKey: "name", size: 200, cell: (props: CellContext<RequestFeedbackItem, unknown>) => (<div className="flex items-center gap-2"><div className="flex flex-col gap-0.5 text-xs"><span className="font-semibold text-sm">{(props.row.original as any)?.customer_code || "N/A" + '|' + props.row.original.name || "N/A"}</span><span className="text-gray-600 dark:text-gray-400">{props.row.original.customer_id === "0" ? "Guest" : `ID: ${props.row.original.customer_id}`}</span><span className="text-gray-600 dark:text-gray-400">{props.row.original.email || "N/A"}</span><span className="text-gray-600 dark:text-gray-400">{props.row.original.mobile_no || "N/A"}</span></div></div>), },
  ...
  const DrawerFormContent = () => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <FormItem label="Company Name"><Controller name="company_name" control={control} render={({ field }) => <Input {...field} prefix={<TbBuilding />} placeholder="Your Company" />} /></FormItem>
        <FormItem label={<div>Type<span className="text-red-500"> *</span></div>} invalid={!!errors.type} errorMessage={errors.type?.message}><Controller name="type" control={control} render={({ field }) => <Select placeholder="Select Type" options={TYPE_OPTIONS} value={TYPE_OPTIONS.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} prefix={<TbMessageDots />} />} /></FormItem>
        <FormItem label={<div>Category<span className="text-red-500"> *</span></div>} invalid={!!errors.category} errorMessage={errors.category?.message}><Controller name="category" control={control} render={({ field }) => <Select placeholder="Select Category" options={categoryOptions} value={categoryOptions.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} prefix={<TbCategory />} />} /></FormItem>
        <FormItem label={<div>Department<span className="text-red-500"> *</span></div>} className="md:col-span-2" invalid={!!errors.department_id} errorMessage={errors.department_id?.message}><Controller name="department_id" control={control} render={({ field }) => <Select placeholder="Select Department" options={departmentOptions} value={departmentOptions.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} prefix={<TbBuildingStore />} />} /></FormItem>
        <FormItem label={<div>Description<span className="text-red-500"> *</span></div>} className="md:col-span-2" invalid={!!errors.feedback_details} errorMessage={errors.feedback_details?.message}><Controller name="feedback_details" control={control} render={({ field }) => <Input textArea {...field} rows={5} placeholder="Describe your feedback or request in detail..." />} /></FormItem>
        {typeValue === 'Feedback' && (<FormItem label="Rating" className="md:col-span-2" invalid={!!errors.rating} errorMessage={errors.rating?.message}><Controller name="rating" control={control} render={({ field }) => <Select placeholder="Select Rating (Optional)" options={RATING_OPTIONS} value={RATING_OPTIONS.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} isClearable prefix={<TbStar />} />} /></FormItem>)}
        <FormItem label="Attachment (Optional)" className="md:col-span-2"><Controller name="attachment" control={control} render={({ field: { onChange, name, ref, onBlur } }) => <Input type="file" name={name} ref={ref} onBlur={onBlur} onChange={(e) => { const file = e.target.files?.[0]; onChange(file); setSelectedFile(file || null); if (file) setRemoveExistingAttachment(false); }} prefix={<TbPaperclip />} />} />{editingItem?.attachment && !selectedFile && (<div className="mt-2 text-sm text-gray-500 flex items-center justify-between"><span className="truncate max-w-[calc(100%-100px)]">Current: <a href={itemPathUtil(editingItem.attachment)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{editingItem.attachment.split("/").pop()}</a></span>{!removeExistingAttachment && <Button size="xs" variant="plain" className="text-red-500" onClick={() => setRemoveExistingAttachment(true)}>Remove</Button>}</div>)}{selectedFile && <div className="mt-1 text-xs text-gray-500">New file: {selectedFile.name}</div>}</FormItem>
        {editingItem && <FormItem label={<div>Status<span className="text-red-500"> *</span></div>} className="md:col-span-2" invalid={!!errors.status} errorMessage={errors.status?.message}><Controller name="status" control={control} render={({ field }) => <Select placeholder="Select Status" options={STATUS_OPTIONS} value={STATUS_OPTIONS.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} prefix={<TbToggleRight />} />} /></FormItem>}
      </div>
    );
  };
  ...
  return (<>
        ...
        <div className="p-1">
            ...
            {viewingItem && (
                <div className="space-y-3 text-sm">
                    ...
                    <div className="flex"><span className="font-semibold w-1/3 text-gray-700">Category:</span><span className="w-2/3">{viewingItem.category || "N/A"}</span></div>
                    ...
                    {viewingItem.attachment && (<div className="flex items-start"><span className="font-semibold w-1/3 pt-1">Attachment:</span><span className="w-2/3"><a href={itemPathUtil(viewingItem.attachment)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{viewingItem.attachment.split("/").pop()}</a></span></div>)}
    ...
    <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={() => setIsFilterDrawerOpen(false)} width={400} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={onClearFilters}>Clear</Button><Button size="sm" variant="solid" form="filterReqFeedbackForm" type="submit">Apply</Button></div>}>
        <Form id="filterReqFeedbackForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
            ...
            <FormItem label="Rating"><Controller name="filterRating" control={filterFormMethods.control} render={({ field }) => <Select isMulti placeholder="Any Rating" options={RATING_OPTIONS} value={field.value || []} onChange={(val: any) => field.onChange(val || [])} />} /></FormItem>
        </Form>
    </Drawer>
