...
const RequestFeedbackModals: React.FC<RequestFeedbackModalsProps> = ({ modalState, onClose, getAllUserDataOptions, }) => {
  ...
  const renderModalContent = () => {
    switch (type) {
      ...
      case "task": return <AssignTaskDialog item={item} onClose={onClose} getAllUserDataOptions={getAllUserDataOptions} />;
      case "calendar": return <AddScheduleDialog item={item} onClose={onClose} />;
      case "active": return <AddActivityDialog item={item} onClose={onClose} />;
...
const AssignTaskDialog: React.FC<{ item: RequestFeedbackItem; onClose: () => void; getAllUserDataOptions: SelectOption[] }> = ({ item, onClose, getAllUserDataOptions }) => {
    const dispatch = useAppDispatch();
    const [isLoading, setIsLoading] = useState(false);
    const { control, handleSubmit, formState: { errors, isValid } } = useForm<TaskFormData>({
        resolver: zodResolver(taskValidationSchema),
        defaultValues: {
            task_title: `Follow up on ${item.type} from ${item.name}`,
            assign_to: [],
            priority: 'Medium',
            due_date: null,
    ...
    const onAssignTask = async (data: TaskFormData) => {
        ...
        try {
            ...
            toast.push(<Notification type="success" title="Task Assigned" children={`Successfully assigned task for item #${item.id}.`} />);
            onClose();
        } catch (error: any) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormItem label="Assign To" invalid={!!errors.assign_to} errorMessage={errors.assign_to?.message}>
                        <Controller name="assign_to" control={control} render={({ field }) => (
                            <Select isMulti placeholder="Select User(s)" options={getAllUserDataOptions} value={getAllUserDataOptions.filter(o => field.value?.includes(o.value))} onChange={(opts: any) => field.onChange(opts?.map((o: any) => o.value) || [])} />
                        )} />
                    </FormItem>
                    <FormItem label="Priority" invalid={!!errors.priority} errorMessage={errors.priority?.message}>
                        <Controller name="priority" control={control} render={({ field }) => (
                            <Select placeholder="Select Priority" options={taskPriorityOptions} value={taskPriorityOptions.find(p => p.value === field.value)} onChange={(opt: any) => field.onChange(opt?.value)} />
                        )} />
                    </FormItem>
                </div>
...
const AddActivityDialog: React.FC<{ item: RequestFeedbackItem; onClose: () => void; }> = ({ item, onClose }) => {
    const dispatch = useAppDispatch();
    ...
    const { control, handleSubmit, formState: { errors, isValid } } = useForm<ActivityFormData>({
        ...
        defaultValues: { item: `Follow-up on ${item.type} from ${item.name}`, notes: '' },
        mode: 'onChange',
    });
    ...
    const onAddActivity = async (data: ActivityFormData) => {
        ...
        const payload = {
            ...
            module_id: String(item.id),
            module_name: 'RequestFeedback',
            user_id: user.id, // Assuming user id is available in auth state
        };
        try {
    ...
    return (
        <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose}>
            <h5 className="mb-4">Add Activity Log for Item #{item.id}</h5>
            <Form onSubmit={handleSubmit(onAddActivity)}>
                <FormItem label="Activity" invalid={!!errors.item} errorMessage={errors.item?.message}>
                    <Controller name="item" control={control} render={({ field }) => <Input {...field} placeholder="e.g., Followed up with customer" />} />
                </FormItem>
...
const RequestAndFeedbackListing = () => {
  ...
  ], [openEditDrawer, openViewDialog, handleDeleteClick, handleOpenModal, openImageViewer]);
