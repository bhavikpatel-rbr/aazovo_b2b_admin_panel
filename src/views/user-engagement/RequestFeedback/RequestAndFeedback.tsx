...
const AddScheduleDialog: React.FC<{ item: RequestFeedbackItem; onClose: () => void; }> = ({ item, onClose }) => {
  ...
  const { control, handleSubmit, formState: { errors, isValid } } = useForm<ScheduleFormData>({
    ...
    defaultValues: {
      event_title: `Follow-up on ${item.type} from ${item.name}`,
      event_type: undefined,
      date_time: null as any,
      remind_from: null,
      notes: `Regarding ${item.type.toLowerCase()}: "${item.subject || 'General Inquiry'}" (ID: ${item.id})`,
    },
    mode: 'onChange',
  });
  ...
  const onAddEvent = async (data: ScheduleFormData) => {
    ...
    try {
      ...
      toast.push(<Notification type="success" title="Event Scheduled" children={`Successfully scheduled event for item #${item.id}.`} />);
      onClose();
    } catch (error: any) {
...
const ItemTableTools = ({ onSearchChange, onFilter, onExport, onClearFilters, }: ItemTableToolsProps) => (<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 w-full"><div className="flex-grow"><ItemSearch onInputChange={onSearchChange} /></div><div className="flex flex-col sm:flex-row gap-1 w-full sm:w-auto"><Tooltip title="Clear Filters"><Button icon={<TbReload />} onClick={onClearFilters}></Button></Tooltip><Button icon={<TbFilter />} onClick={onFilter} className="w-full sm:w-auto">Filter</Button><Button icon={<TbCloudUpload />} onClick={onExport} className="w-full sm:w-auto">Export</Button></div></div>);
