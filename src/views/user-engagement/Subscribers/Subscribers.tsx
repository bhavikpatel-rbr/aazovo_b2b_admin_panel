...
const AddNotificationDialog = ({ document, onClose, getAllUserDataOptions }: { document: SubscriberItem, onClose: () => void, getAllUserDataOptions: SelectOption[] }) => {
    ...
    const { control, handleSubmit, formState: { errors, isValid } } = useForm<NotificationFormData>({ resolver: zodResolver(notificationSchema), defaultValues: { notification_title: `Regarding Subscriber: ${document.name}`, send_users: [], message: `This is a notification regarding subscriber "${document.name}" (${document.email}).` }, mode: 'onChange' });
...
const AddScheduleDialog: React.FC<{ document: SubscriberItem; onClose: () => void; }> = ({ document, onClose }) => {
    ...
    const onAddEvent = async (data: ScheduleFormData) => {
        setIsLoading(true);
        const payload = { module_id: Number(document.id), module_name: 'Subscriber', event_title: data.event_title, event_type: data.event_type, date_time: dayjs(data.date_time).format('YYYY-MM-DDTHH:mm:ss'), ...(data.remind_from && { remind_from: dayjs(data.remind_from).format('YYYY-MM-DDTHH:mm:ss') }), notes: data.notes || '', };
        try { await dispatch(addScheduleAction(payload)).unwrap(); toast.push(<Notification type="success" title="Event Scheduled" children={`Successfully scheduled event for ${document.name}.`} />); onClose(); }
...
const SubscribersListing = () => {
    ...
    const onSubmitHandler = async (data: AddEditSubscriberFormData) => {
        ...
        if (editingItem) {
            apiPayload = { _method: "PUT", email: data.email, name: data.name, mobile: data.mobile_no, created_at: dayjs(data.subscribedDate).toISOString(), subscription_type: data.subscriptionType, source: data.source, status: data.status, rating: data.rating ? Number(data.rating) : null, note: data.note || null, reason: data.status === 'Unsubscribed' ? (data.unsubscribeReason || null) : null };
        } else {
            apiPayload = { email: data.email, name: data.name, mobile: data.mobile_no, created_at: dayjs(data.subscribedDate).toISOString(), subscription_type: data.subscriptionType, source: data.source, status: data.status, rating: data.rating ? Number(data.rating) : null, note: data.note || null, reason: data.status === 'Unsubscribed' ? (data.unsubscribeReason || null) : null };
        }
        try {
            if (editingItem) { await dispatch(editSubscriberAction({ id: editingItem.id, formData: apiPayload })).unwrap(); toast.push(<Notification title="Subscriber Updated" type="success" />); }
            else { await dispatch(addSubscriberAction(apiPayload)).unwrap(); toast.push(<Notification title="Subscriber Added" type="success" />); }
            closeAddEditDrawer(); dispatch(getSubscribersAction());
        } catch (e: any) { const errorMessage = e?.response?.data?.message || e?.message || (editingItem ? "Update Failed" : "Add Failed"); toast.push(<Notification title={editingItem ? "Update Failed" : "Add Failed"} type="danger">{errorMessage}</Notification>); }
    ...
    const onConfirmSingleDelete = useCallback(async () => {
        ...
        try { await new Promise(resolve => setTimeout(resolve, 1000)); toast.push(<Notification title="Subscriber Deleted" type="success" >{`Subscriber "${itemToDelete.name}" deleted.`}</Notification>); dispatch(getSubscribersAction()); }
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.dateRange && (filterCriteria.dateRange[0] || filterCriteria.dateRange[1])) {
            ...
            processedData = processedData.filter(item => { if (isNaN(item.subscribedDate.getTime())) return false; const itemDate = dayjs(item.subscribedDate); if (start && end) return itemDate.isBetween(start, end, 'day', '[]'); if (start) return itemDate.isSameOrAfter(start, 'day'); if (end) return itemDate.isSameOrBefore(end, 'day'); return true; });
        }
        ...
        if (order && key && processedData.length > 0 && processedData[0].hasOwnProperty(key)) {
            processedData.sort((a, b) => {
                let aVal = a[key as keyof SubscriberItem] as any; let bVal = b[key as keyof SubscriberItem] as any;
                if (aVal instanceof Date && bVal instanceof Date) { if (isNaN(aVal.getTime())) return order === 'asc' ? 1 : -1; if (isNaN(bVal.getTime())) return order === 'asc' ? -1 : 1; return order === 'asc' ? aVal.getTime() - bVal.getTime() : bVal.getTime() - aVal.getTime(); }
    ...
    const handleConfirmExportWithReason = useCallback(async (data: ExportReasonFormData) => {
        setIsSubmittingExportReason(true); const moduleName = "Subscribers"; const timestamp = dayjs().format('YYYYMMDD_HHmmss'); const fileName = `subscribers_export_${timestamp}.csv`;
        try { await dispatch(submitExportReasonAction({ reason: data.reason, module: moduleName, file_name: fileName })).unwrap(); toast.push(<Notification title="Export Reason Submitted" type="success" />); exportSubscribersToCsv(fileName, allFilteredAndSortedData); setIsExportReasonModalOpen(false); }
        catch (error: any) { toast.push(<Notification title="Operation Failed" type="danger" >{(error as Error).message || "Could not complete export."}</Notification>); }
    ...
    const columns: ColumnDef<SubscriberItem>[] = useMemo(
        () => [
            ...
            { header: 'Source', accessorKey: 'source', id: 'source', enableSorting: true, cell: (props) => <span>{props.getValue() as string || "N/A"}</span> },
            ...
            { header: "Rating", accessorKey: "rating", id: 'rating', enableSorting: true, size: 80, cell: (props) => props.getValue() ? <span className="flex items-center gap-1"><TbStar className="text-amber-500" />{`${props.getValue()}`}</span> : "N/A" },
            { header: "Action", id: "action", size: 120, meta: { HeaderClass: "text-center" }, cell: (props) => (<div className='flex gap-1 items-center pr-1.5'><Tooltip title="Edit Subscriber"><div className="text-xl cursor-pointer text-gray-500 hover:text-emerald-600" onClick={() => openEditDrawer(props.row.original)} role="button"><TbPencil /></div></Tooltip><Tooltip title="Send Test Email"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-orange-600" role="button"><TbMailForward size={18} /></div></Tooltip><Tooltip title="Add to Campaign"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-blue-600" role="button"><TbAlignBoxCenterBottom size={17} /></div></Tooltip><Tooltip title="Email Log"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-red-600" role="button"><TbMailbox size={18} /></div></Tooltip><Tooltip title="Send Now"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-red-600" role="button"><TbSend size={18} /></div></Tooltip></div>), },
    ...
    return (
        <>
            ...
            <Drawer title={editingItem ? "Edit Subscriber" : "Add New Subscriber"} width={520} isOpen={isAddEditDrawerOpen} onClose={closeAddEditDrawer} onRequestClose={closeAddEditDrawer} footer={
                <div className="w-full flex gap-2 justify-end">
                    <Button size="sm" type="button" onClick={closeAddEditDrawer} disabled={isSubmitting}>Cancel</Button>
                    <Button size="sm" variant="solid" type="submit" form="addEditSubscriberForm" loading={isSubmitting} disabled={!isValid || isSubmitting}>{isSubmitting ? (editingItem ? 'Saving...' : 'Adding...') : (editingItem ? 'Save Changes' : 'Add Subscriber')}</Button>
                </div>
            }>
                <Form id="addEditSubscriberForm" onSubmit={handleSubmit(onSubmitHandler)} className="flex flex-col gap-y-4">
                    ...
                    <div className='md:grid grid-cols-2 gap-3'>
                        <FormItem label={<div>Mobile No.<span className="text-red-500"> *</span></div>} invalid={!!errors.mobile_no} errorMessage={errors.mobile_no?.message}><Controller name="mobile_no" control={control} render={({ field }) => <Input {...field} type='tel' placeholder='Enter Mobile No.' prefix={<TbPhone />} />} /></FormItem>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        <Button size="sm" className="mr-2" type="button" onClick={onClearFilters}>
                            Clear
                        </Button>
                        <Button
                            size="sm"
