...
export type CategoryItem = {
    id: number; name: string; slug: string; icon: string | null; icon_full_path: string | null;
    parentId: number | null; parentCategoryName?: string;
    showHomePage: number; showHeader: number; showPageName: string | null;
    status: CategoryStatus; metaTitle: string | null; metaDescription: string | null;
    metaKeyword: string | null; comingSoon: number; createdAt: string; updatedAt: string;
}
...
const categoryFormSchema = z.object({
    ...
    parent_category: z.union([z.number().positive().nullable(), z.string().length(0)]).transform(val => val === "" ? null : (val === null ? null : Number(val))).optional().nullable(),
    icon: z.union([z.instanceof(File), z.null()]).optional().nullable(),
    show_home_page: z.enum(['0', '1'], { errorMap: () => ({ message: "Please select."}) }).transform(val => Number(val)),
    show_header: z.enum(['0', '1'], { errorMap: () => ({ message: "Please select."}) }).transform(val => Number(val)),
    ...
    comingsoon: z.enum(['0', '1'], { errorMap: () => ({ message: "Please select."}) }).transform(val => Number(val)),
    show_page_name: z.string().max(255).optional().nullable(),
});
type CategoryFormData = z.infer<typeof categoryFormSchema>;

const filterFormCategorySchema = z.object({ // Renamed
    ...
    filterParentIds: z.array(z.object({value: z.number(), label: z.string()})).optional(), // For filtering by parent ID
});
type FilterFormData = z.infer<typeof filterFormCategorySchema>; // Renamed

// --- CSV Export ---
const CSV_HEADERS_CATEGORY = ['ID', 'Name', 'Slug', 'Icon URL', 'Parent ID', 'Parent Name', 'Show Home', 'Show Header', 'Show Page Name', 'Coming Soon', 'Status', 'Meta Title', 'Meta Desc', 'Meta Keyword', 'Created At', 'Updated At'];
type CategoryCsvItem = {
    id: number; name: string; slug: string; icon_full_path: string | null; parentId: number | null; parentCategoryName?: string;
    showHomePage: number; showHeader: number; showPageName: string | null; comingSoon: number; status: CategoryStatus;
    metaTitle: string | null; metaDescription: string | null; metaKeyword: string | null; createdAt: string; updatedAt: string;
};
function exportToCsvCategory(filename: string, rows: CategoryItem[]) { // Renamed
    ...
    const transformedRows: CategoryCsvItem[] = rows.map(item => ({
        id: item.id, name: item.name, slug: item.slug, icon_full_path: item.icon_full_path, parentId: item.parentId, parentCategoryName: item.parentCategoryName,
        showHomePage: item.showHomePage, showHeader: item.showHeader, showPageName: item.showPageName, comingSoon: item.comingSoon, status: item.status,
        metaTitle: item.metaTitle, metaDescription: item.metaDescription, metaKeyword: item.metaKeyword,
        createdAt: new Date(item.createdAt).toLocaleString(), updatedAt: new Date(item.updatedAt).toLocaleString(),
    }));
    const csvKeys: (keyof CategoryCsvItem)[] = ['id', 'name', 'slug', 'icon_full_path', 'parentId', 'parentCategoryName', 'showHomePage', 'showHeader', 'showPageName', 'comingSoon', 'status', 'metaTitle', 'metaDescription', 'metaKeyword', 'createdAt', 'updatedAt'];
    const separator = ',';
    const csvContent = CSV_HEADERS_CATEGORY.join(separator) + '\n' + transformedRows.map(row =>
        csvKeys.map(k => {
            let cellValue = row[k];
            if (cellValue === null || cellValue === undefined) cellValue = ''; else cellValue = String(cellValue).replace(/"/g, '""');
            if (String(cellValue).search(/("|,|\n)/g) >= 0) cellValue = `"${cellValue}"`; return cellValue;
        }).join(separator)
    ).join('\n');
...
// --- Constants & Options ---
const CATEGORY_ICON_BASE_URL = import.meta.env.VITE_API_URL_STORAGE || 'https://your-api-domain.com/storage/category_icons/'; // MODIFY_PATH if different
...
const uiStatusOptions: { value: CategoryStatus; label: string }[] = [ { value: 'active', label: 'Active' }, { value: 'inactive', label: 'Inactive' }];
const apiStatusOptions: { value: "Active" | "Inactive"; label: string }[] = [ { value: 'Active', label: 'Active' }, { value: 'Inactive', label: 'Inactive' }];
const yesNoOptions: { value: '0' | '1'; label: string }[] = [ { value: '1', label: 'Yes' }, { value: '0', label: 'No' }];


// --- Helper Components ---
const ActionColumn = ({ onEdit, onClone, onChangeStatus, onDelete }: { onEdit: () => void; onClone: () => void; onChangeStatus: () => void; onDelete: () => void; }) => {
    ...
    return (
        <div className="flex items-center justify-center gap-2">
            <Tooltip title="Clone Category"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400')} role="button" tabIndex={0} onClick={onClone}><TbCopy /></div></Tooltip>
            <Tooltip title="Change Status"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400')} role="button" tabIndex={0} onClick={onChangeStatus}><TbSwitchHorizontal /></div></Tooltip>
            <Tooltip title="Edit Category"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400')} role="button" tabIndex={0} onClick={onEdit}><TbPencil /></div></Tooltip>
            <Tooltip title="Delete Category"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400')} role="button" tabIndex={0} onClick={onDelete}><TbTrash /></div></Tooltip>
        </div>
    );
};
const CategorySearch = React.forwardRef<HTMLInputElement, { onInputChange: (value: string) => void }>(({ onInputChange }, ref) => (<DebouceInput ref={ref} className="w-full" placeholder="Search categories..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />)); // Renamed
CategorySearch.displayName = 'CategorySearch';
const CategoryTableTools = ({ onSearchChange, onFilter, onExport, onImport }: { onSearchChange: (query: string) => void; onFilter: () => void; onExport: () => void; onImport: () => void; }) => ( // Renamed
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
        <div className="flex-grow"><CategorySearch onInputChange={onSearchChange} /></div>
...
type CategoryTableProps = { // Renamed
    columns: ColumnDef<CategoryItem>[]; data: CategoryItem[]; loading: boolean;
    pagingData: { total: number; pageIndex: number; pageSize: number }; selectedItems: CategoryItem[];
    onPaginationChange: (page: number) => void; onSelectChange: (value: number) => void; onSort: (sort: OnSortParam) => void;
    onRowSelect: (checked: boolean, row: CategoryItem) => void; onAllRowSelect: (checked: boolean, rows: Row<CategoryItem>[]) => void;
}
const CategoryTable = ({ columns, data, loading, pagingData, selectedItems, onPaginationChange, onSelectChange, onSort, onRowSelect, onAllRowSelect }: CategoryTableProps) => ( // Renamed
...
const CategorySelectedFooter = ({ selectedItems, onDeleteSelected }: { selectedItems: CategoryItem[]; onDeleteSelected: () => void; }) => { // Renamed
        ...
        <>
            <StickyFooter className="flex items-center justify-between py-4 bg-white dark:bg-gray-800" stickyClass="-mx-4 sm:-mx-8 border-t border-gray-200 dark:border-gray-700 px-8">
                <div className="flex items-center justify-between w-full px-4 sm:px-8">
                    <span className="flex items-center gap-2"><span className="text-lg text-primary-600 dark:text-primary-400"><TbChecks /></span><span className="font-semibold flex items-center gap-1 text-sm sm:text-base"><span className="heading-text">{selectedItems.length}</span><span>Category{selectedItems.length > 1 ? 'ies' : 'y'} selected</span></span></span>
            ...
            <ConfirmDialog isOpen={deleteOpen} type="danger" title={`Delete ${selectedItems.length} Categor${selectedItems.length > 1 ? 'ies' : 'y'}`}
...
const Categories = () => { // Renamed component
    ...
    const [editingCategory, setEditingCategory] = useState<CategoryItem | null>(null); // Renamed
    const [isFilterDrawerOpen, setFilterDrawerOpen] = useState(false);
    const [isSubmitting, setSubmitting] = useState(false);
    const [isProcessing, setIsProcessing] = useState(false);
    const [singleDeleteOpen, setSingleDeleteOpen] = useState(false);
    const [categoryToDelete, setCategoryToDelete] = useState<CategoryItem | null>(null); // Renamed
    const [statusChangeConfirmOpen, setStatusChangeConfirmOpen] = useState(false);
    const [categoryForStatusChange, setCategoryForStatusChange] = useState<CategoryItem | null>(null); // Renamed
    const [importDialogOpen, setImportDialogOpen] = useState(false);
    const [filterCriteria, setFilterCriteria] = useState<FilterFormData>({ filterNames: [], filterStatuses: [], filterParentIds: [] }); // Updated filter criteria
    ...
    useEffect(() => { dispatch(getCategoriesAction()) }, [dispatch]); // Use getCategoriesAction
    useEffect(() => {
        return () => {
    ...
    const defaultCategoryFormValues: Omit<CategoryFormData, 'show_home_page' | 'show_header' | 'comingsoon'> & {
        ...
        status: 'Active', meta_title: null, meta_descr: null, meta_keyword: null, comingsoon: '0', show_page_name: null,
    };
    ...
    const filterFormMethods = useForm<FilterFormData>({ resolver: zodResolver(filterFormCategorySchema), defaultValues: filterCriteria }); // Use category filter schema

    const parentCategoryOptions = useMemo(() => {
        if (!Array.isArray(CategoriesData)) return [{ value: null, label: 'No Parent Category' }];
        const options = CategoriesData
            .filter(cat => !editingCategory || cat.id !== editingCategory.id) // Prevent self-parenting during edit
            .map(cat => ({
                value: cat.id,
                label: cat.name,
            }));
    ...
    const mappedCategories: CategoryItem[] = useMemo(() => {
        ...
        return CategoriesData.map((apiItem: ApiCategoryItem): CategoryItem => {
            ...
            return {
                ...
                parentId: parentId, parentCategoryName: parentId ? (categoriesMap.get(parentId) || 'Unknown Parent') : undefined,
                showHomePage: Number(apiItem.show_home_page), showHeader: Number(apiItem.show_header), showPageName: apiItem.show_page_name,
                ...
                comingSoon: Number(apiItem.comingsoon || 0), createdAt: apiItem.created_at, updatedAt: apiItem.updated_at,
            };
        });
    }, [CategoriesData]);

    const openAddCategoryDrawer = () => { addFormMethods.reset(defaultCategoryFormValues); setAddFormPreviewUrl(null); setAddDrawerOpen(true); }; // Renamed
    const closeAddCategoryDrawer = () => { setAddDrawerOpen(false); if (addFormPreviewUrl) URL.revokeObjectURL(addFormPreviewUrl); setAddFormPreviewUrl(null); }; // Renamed
    const onAddCategorySubmit = async (data: CategoryFormData) => { // Renamed
    ...
    const openEditCategoryDrawer = (category: CategoryItem) => { // Renamed
        setEditingCategory(category);
        editFormMethods.reset({
            name: category.name, slug: category.slug, parent_category: category.parentId, icon: null,
            show_home_page: String(category.showHomePage) as '0' | '1', show_header: String(category.showHeader) as '0' | '1',
            status: category.status === 'active' ? 'Active' : 'Inactive',
            meta_title: category.metaTitle || null, meta_descr: category.metaDescription || null, meta_keyword: category.metaKeyword || null,
            comingsoon: String(category.comingSoon) as '0' | '1', show_page_name: category.showPageName || null,
        });
        setEditFormPreviewUrl(null); setEditDrawerOpen(true);
    };
    const closeEditCategoryDrawer = () => { setEditDrawerOpen(false); setEditingCategory(null); if (editFormPreviewUrl) URL.revokeObjectURL(editFormPreviewUrl); setEditFormPreviewUrl(null); }; // Renamed
    const onEditCategorySubmit = async (data: CategoryFormData) => { // Renamed
        if (!editingCategory) return;
        setSubmitting(true); const formData = new FormData(); formData.append('_method', 'PUT');
        (Object.keys(data) as Array<keyof CategoryFormData>).forEach((key) => {
            const value = data[key];
            if (key === 'icon') { if (value instanceof File) formData.append(key, value); }
            else { if (value === null && key !== 'parent_category') formData.append(key, ''); // Send empty for most nulls
                   else if (key === 'parent_category' && value === null) formData.append(key, ''); // Explicitly send parent_category as empty if null
        ...
        try {
            ...
            if (responseData) { if (responseData.message) errorMessage = responseData.message; if (responseData.errors) { const validationErrors = Object.values(responseData.errors).flat().join(' '); errorMessage += ` Details: ${validationErrors}`; }}
            else if (error.message) errorMessage = error.message;
            toast.push(<Notification title="Failed to Update" type="danger" duration={4000}>{errorMessage}</Notification>);
            console.error('Edit Category Error:', error.response || error, responseData);
        } finally { setSubmitting(false); }
    };

    const handleDeleteCategoryClick = (category: CategoryItem) => { setCategoryToDelete(category); setSingleDeleteOpen(true); }; // Renamed
    const onConfirmSingleDeleteCategory = async () => { // Renamed
        ...
        try {
            ...
            setSelectedItems(prev => prev.filter(item => item.id !== categoryToDelete!.id)); dispatch(getCategoriesAction());
        } catch (error: any) {
    ...
    const handleDeleteSelectedCategories = async () => { // Renamed
        if (selectedItems.length === 0) return; setIsProcessing(true); const idsToDelete = selectedItems.map(item => item.id);
        try {
    ...
    const openChangeCategoryStatusDialog = (category: CategoryItem) => { setCategoryForStatusChange(category); setStatusChangeConfirmOpen(true); }; // Renamed
    const onConfirmChangeCategoryStatus = async () => { // Renamed
        if (!categoryForStatusChange) return; setIsProcessing(true);
        ...
        try {
            ...
            toast.push(<Notification title="Status Update (Mock)" type="info" duration={2000}>Status update for "{categoryForStatusChange.name}" to be implemented.</Notification>);
        } catch (error: any) {
        ...
        } finally { setStatusChangeConfirmOpen(false); setIsProcessing(false); setCategoryForStatusChange(null); }
    };
    const handleCloneCategory = (category: CategoryItem) => { // Renamed
        addFormMethods.reset({
            name: `${category.name} (Copy)`, slug: `${category.slug}-copy`, parent_category: category.parentId, icon: null,
            show_home_page: String(category.showHomePage) as '0' | '1', show_header: String(category.showHeader) as '0' | '1',
            status: category.status === 'active' ? 'Active' : 'Inactive',
            meta_title: category.metaTitle, meta_descr: category.metaDescription, meta_keyword: category.metaKeyword,
            comingsoon: String(category.comingSoon) as '0' | '1', show_page_name: category.showPageName,
        });
        setAddFormPreviewUrl(null); setAddDrawerOpen(true);
        toast.push(<Notification title="Clone Category" type="info">Cloning "{category.name}". Review and save.</Notification>);
    };
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterParentIds && filterCriteria.filterParentIds.length > 0) { const selectedParentIds = filterCriteria.filterParentIds.map(opt => opt.value); processedData = processedData.filter(item => item.parentId !== null && selectedParentIds.includes(item.parentId)); }
        if (tableData.query && tableData.query.trim() !== '') { const query = tableData.query.toLowerCase().trim(); processedData = processedData.filter(item => item.name?.toLowerCase().includes(query) || item.slug?.toLowerCase().includes(query) || String(item.id).toLowerCase().includes(query) || item.status.toLowerCase().includes(query) || item.parentCategoryName?.toLowerCase().includes(query)); }
    ...
    const handleExportData = () => { const success = exportToCsvCategory('categories_export.csv', allFilteredAndSortedData); if (success) toast.push(<Notification title="Export Successful" type="success">Data exported.</Notification>); }; // Renamed
        ...
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        ...
                        <Button variant="solid" icon={<TbPlus />} onClick={openAddCategoryDrawer}>Add New Category</Button>
                    </div>
                    <CategoryTableTools onSearchChange={handleSearchChange} onFilter={openFilterDrawer} onExport={handleExportData} onImport={handleImportData} />
                    <div className="mt-4 flex-grow overflow-y-auto">
                        <CategoryTable columns={columns} data={pageData} loading={tableLoading}
            ...
            <Drawer title="Edit Category" isOpen={isEditDrawerOpen} onClose={closeEditCategoryDrawer} onRequestClose={closeEditCategoryDrawer}
                ...
                <Form id="editCategoryForm" onSubmit={editFormMethods.handleSubmit(onEditCategorySubmit)} className="flex flex-col gap-4">
                    ...
                    <FormItem label="Status" invalid={!!editFormMethods.formState.errors.status} errorMessage={editFormMethods.formState.errors.status?.message} isRequired> <Controller name="status" control={editFormMethods.control} render={({ field }) => ( <UiSelect options={apiStatusOptions} value={apiStatusOptions.find(opt => opt.value === field.value)} onChange={opt => field.onChange(opt ? opt.value : undefined)} /> )} /> </FormItem>
                    <h6 className="text-sm font-medium mt-2">Meta Options (Optional)</h6>
            ...
            <ConfirmDialog isOpen={singleDeleteOpen} type="danger" title="Delete Category" onClose={() => { setSingleDeleteOpen(false); setCategoryToDelete(null); }} onRequestClose={() => { setSingleDeleteOpen(false); setCategoryToDelete(null); }} onCancel={() => { setSingleDeleteOpen(false); setCategoryToDelete(null); }} confirmButtonColor="red-600" onConfirm={onConfirmSingleDeleteCategory} loading={isProcessing}><p>Are you sure you want to delete the category "<strong>{categoryToDelete?.name}</strong>"? This action cannot be undone.</p></ConfirmDialog>
            <ConfirmDialog isOpen={statusChangeConfirmOpen} type="warning" title="Change Category Status" onClose={() => { setStatusChangeConfirmOpen(false); setCategoryForStatusChange(null); }} onRequestClose={() => { setStatusChangeConfirmOpen(false); setCategoryForStatusChange(null); }} onCancel={() => { setStatusChangeConfirmOpen(false); setCategoryForStatusChange(null); }} confirmButtonColor="amber-600" onConfirm={onConfirmChangeCategoryStatus} loading={isProcessing}><p>Are you sure you want to change status for "<strong>{categoryForStatusChange?.name}</strong>" to <strong>{categoryForStatusChange?.status === 'active' ? 'Inactive' : 'Active'}</strong>?</p></ConfirmDialog>
