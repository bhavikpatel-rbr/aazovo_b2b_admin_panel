...
import { Drawer, Form, FormItem, Input, Select as UiSelect, Tag, Dialog } from '@/components/ui' // Added Dialog
...
const brandFormSchema = z.object({
    ...
    icon: z.union([z.instanceof(File), z.null()]).optional().nullable(), // For RHF, handles File or null
...
type BrandCsvItem = {
    ...
    status: BrandStatus; metaTitle: string | null; metaDescription: string | null; metaKeyword: string | null;
    mobileNo: string | null; createdAt: string; updatedAt: string;
};
...
function exportToCsvBrand(filename: string, rows: BrandItem[]) {
    ...
    const csvContent = CSV_HEADERS_BRAND.join(separator) + '\n' + transformedRows.map(row =>
        csvKeys.map(k => {
            ...
            if (String(cellValue).search(/("|,|\n)/g) >= 0) cellValue = `"${cellValue}"`; // Enclose in double quotes
            return cellValue;
        }).join(separator)
    ).join('\n');
...
const ActionColumn = ({ onEdit, onClone, onChangeStatus, onDelete }: {
    ...
    return (
        <div className="flex items-center justify-center gap-2">
            <Tooltip title="Clone Brand"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400')} role="button" tabIndex={0} onClick={onClone}><TbCopy /></div></Tooltip>
            ...
            <Tooltip title="Edit Brand"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400')} role="button" tabIndex={0} onClick={onEdit}><TbPencil /></div></Tooltip>
            <Tooltip title="Delete Brand"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400')} role="button" tabIndex={0} onClick={onDelete}><TbTrash /></div></Tooltip>
        </div>
    );
};
...
const Brands = () => {
    ...
    const defaultBrandFormValues: Omit<BrandFormData, 'show_header'> & { show_header: '0' | '1'; icon: null } = {
        name: '', slug: '', mobile_no: null, icon: null,
        show_header: '1',
        status: 'Active',
        meta_title: null, meta_descr: null, meta_keyword: null,
    };
    ...
    const openEditDrawer = (brand: BrandItem) => {
        ...
        editFormMethods.reset({
            ...
            meta_title: brand.metaTitle || null, meta_descr: brand.metaDescription || null, meta_keyword: brand.metaKeyword || null,
        });
    ...
    const onConfirmChangeStatus = async () => {
        ...
        finally { setStatusChangeConfirmOpen(false); setIsProcessing(false); setBrandForStatusChange(null); }
    };

    const handleClone = (brand: BrandItem) => {
        addFormMethods.reset({
            name: `${brand.name} (Copy)`, slug: `${brand.slug}-copy`, mobile_no: brand.mobileNo, icon: null,
            show_header: String(brand.showHeader) as '0' | '1',
            status: brand.status === 'active' ? 'Active' : 'Inactive',
    ...
    const onClearFilters = () => {
        ...
        filterFormMethods.reset(defaultFilters); setFilterCriteria(defaultFilters);
        handleSetTableData({ pageIndex: 1 });
    };
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter(item =>
                ...
                String(item.id).toLowerCase().includes(query) || item.mobileNo?.toLowerCase().includes(query) ||
        ...
        if (order && key && processedData.length > 0) {
            ...
            processedData.sort((a, b) => {
                ...
                else if (sortKey === 'id' || sortKey === 'showHeader') { aValue = Number(aValue); bValue = Number(bValue); }
        ...
        <>
            ...
            <Drawer title="Add Brand" isOpen={isAddDrawerOpen} onClose={closeAddDrawer} onRequestClose={closeAddDrawer}
                ...
                <Form id="addBrandForm" onSubmit={addFormMethods.handleSubmit(onAddBrandSubmit)} className="flex flex-col gap-4">
                   ...
                   <FormItem label="Icon" invalid={!!addFormMethods.formState.errors.icon} errorMessage={addFormMethods.formState.errors.icon?.message as string}>
                        <Controller name="icon" control={addFormMethods.control} render={({ field: { onChange, onBlur, name, ref } }) => (
                    ...
                    <FormItem label="Show in Header?" invalid={!!addFormMethods.formState.errors.show_header} errorMessage={addFormMethods.formState.errors.show_header?.message} isRequired>
                        <Controller name="show_header" control={addFormMethods.control} render={({ field }) => (
                            <UiSelect options={showHeaderOptions}
                                      value={showHeaderOptions.find(opt => opt.value === field.value)}
                ...
                <Form id="editBrandForm" onSubmit={editFormMethods.handleSubmit(onEditBrandSubmit)} className="flex flex-col gap-4">
                    {editingBrand?.icon_full_path && !editFormPreviewUrl && (
                        <FormItem label="Current Icon">
                            <Avatar size={80} src={editingBrand.icon_full_path} shape="circle" icon={<TbBuildingStore />} />
                        </FormItem>
                    )}
                    ...
                    <FormItem label="New Icon (Optional)" invalid={!!editFormMethods.formState.errors.icon} errorMessage={editFormMethods.formState.errors.icon?.message as string}>
                        <Controller name="icon" control={editFormMethods.control} render={({ field: { onChange, onBlur, name, ref } }) => (
                            <Input type="file" name={name} ref={ref} onBlur={onBlur}
                                onChange={e => {
                        ...
                        {editFormPreviewUrl && (
                             <div className="mt-2">
                                ...
                                <p className="text-xs text-gray-500 mt-1">Preview of new icon.</p>
                            </div>
                        )}
                ...
                <Form id="filterBrandForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Filter by Name(s)">
                        <Controller name="filterNames" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select names..." options={brandNameOptions}
            ...
            <ConfirmDialog isOpen={statusChangeConfirmOpen} type="warning" title="Change Brand Status"
                ...
                <p>Are you sure you want to change the status for "<strong>{brandForStatusChange?.name}</strong>" to <strong>{brandForStatusChange?.status === 'active' ? 'Inactive' : 'Active'}</strong>?</p>
            </ConfirmDialog>
