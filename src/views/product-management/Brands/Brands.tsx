        ...
        <Form id="editBrandForm" onSubmit={editFormMethods.handleSubmit(onEditBrandSubmit)} className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            <FormItem label="New Icon (Optional)" invalid={!!editFormMethods.formState.errors.icon} errorMessage={editFormMethods.formState.errors.icon?.message as string}>
              <Controller name="icon" control={editFormMethods.control} render={({ field: { onChange, onBlur, name, ref } }) => (<Input type="file" name={name} ref={ref} onBlur={onBlur} onChange={(e) => { const file = e.target.files ? e.target.files[0] : null; onChange(file); if (editFormPreviewUrl) URL.revokeObjectURL(editFormPreviewUrl); setEditFormPreviewUrl(file ? URL.createObjectURL(file) : null); }} accept=".svg, image/svg+xml, image/png, image/jpeg, image/gif, image/svg+xml, image/webp" />)} /><p className="text-xs text-gray-500 mt-1">Leave blank to keep current icon. Selecting a new file will replace it.</p>
            </FormItem>
