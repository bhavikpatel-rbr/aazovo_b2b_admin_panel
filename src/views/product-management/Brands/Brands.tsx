...
const brandFormSchema = z.object({
  name: z.string({
    required_error: "Brand name is required", // Custom message for when the field is missing
  })
    .trim() // Removes leading and trailing whitespace
    .min(1, "Brand name is required.") // Now validates the trimmed string
...
const ActionColumn = ({ onEdit, onViewDetail, onDelete, }: { onEdit: () => void; onViewDetail: () => void; onDelete: () => void; }) => {
  return (
    <div className="flex items-center justify-center gap-2">
      {getMenuRights("brands")?.is_edit && <Tooltip title="Edit"><div className={`text-xl cursor-pointer select-none text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400`} role="button" onClick={onEdit}><TbPencil /></div></Tooltip>}
...
const BrandTable = (props: any) => (<DataTable selectable {...props} />);

const BrandSelectedFooter = ({ selectedItems, onDeleteSelected }: any) => {
...
const BrandPageSkeleton = () => {
  const skeletonColumns = [
    { key: "id" }, { key: "name" }, { key: "mobileNo" }, { key: "status" }, { key: "action" }
  ];
  return (
...
const Brands = () => {
  ...
  const { BrandData = [] } = useSelector(masterSelector);
  const [masterLoadingStatus, setmasterLoadingStatus] = useState(false);
  useEffect(() => { setmasterLoadingStatus(true); dispatch(getBrandAction()).finally(() => { setmasterLoadingStatus(false) }); }, [dispatch]);
  ...
  const columns: ColumnDef<BrandItem>[] = useMemo(
    ...
    [openViewDetailModal] // Add dependencies if they use state/props from outside
  );
  ...
  const tableLoading = masterLoadingStatus;
    ...
    <>
      ...
      <Drawer title="Add Brand" isOpen={isAddDrawerOpen} onClose={closeAddDrawer} onRequestClose={closeAddDrawer} width={480} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={closeAddDrawer} disabled={isSubmitting}>Cancel</Button><Button size="sm" variant="solid" form="addBrandForm" type="submit" loading={isSubmitting} disabled={!addFormMethods.formState.isValid || isSubmitting}>{isSubmitting ? "Adding..." : "Save"}</Button></div>}>
        <Form id="addBrandForm" onSubmit={addFormMethods.handleSubmit(onAddBrandSubmit)} className="flex flex-col gap-2">
          ...
          <FormItem label={<div>Slug/URL</div>} invalid={!!addFormMethods.formState.errors.slug} errorMessage={addFormMethods.formState.errors.slug?.message} ><Controller disabled name="slug" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="Enter brand-slug" />} /></FormItem>
        ...
        {brandToView ? (
          <div className="flex flex-col max-h-[90vh]">
            <div className="p-4 space-y-3 overflow-y-auto">
              <div className="p-4 bg-white dark:bg-slate-700/60 rounded-lg space-y-3">
                <div className="flex items-center gap-4">
                  {brandToView.icon_full_path && (
                    <div
                      className="cursor-pointer"
                      onClick={() => openImageViewer(brandToView.icon_full_path)}
                    >
                      <Avatar
                        size={60}
                        shape="circle"
                        ...
                        className="border-2 border-white dark:border-slate-500 shadow-md hover:ring-2 hover:ring-indigo-400 transition-all"
                      />
                    </div>
                  )}
                  <div>
                    <h5 className="font-bold text-lg text-slate-800 dark:text-slate-100">{brandToView.name}</h5>
                    <p className="text-xs text-slate-600 dark:text-slate-400">ID: {String(brandToView.id).padStart(6, '0')}</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 pt-2 border-t border-slate-200 dark:border-slate-600 mt-3">
                  <DialogDetailRow
                    label="Status"
                    value={<Tag className={`${statusColor[brandToView.status]} capitalize font-semibold border-0 text-[11px] px-2 py-0.5 rounded-full`}>{brandToView.status}</Tag>}
                  />
                  <DialogDetailRow
                  ...
                  <DialogDetailRow
                    label="Last Updated"
                    value={new Date(brandToView.updatedAt).toLocaleString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  />
                </div>
              </div>
              {(brandToView.metaTitle || brandToView.metaDescription || brandToView.metaKeyword) && (
                <div className="p-4 bg-white dark:bg-slate-700/60 rounded-lg">
          ...
          <div className="p-8 text-center flex flex-col items-center justify-center min-h-[200px]">
            <p className="text-slate-600 dark:text-slate-400">Loading brand details...</p>
          </div>
        )}
      </Dialog>
