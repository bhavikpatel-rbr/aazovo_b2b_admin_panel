...
import {
  ...
  // clearSubcategoriesAction // Ensure this is correctly implemented in Redux if used
...
const Products = () => {
  ...
  const { watch: watchForm, setValue: setFormValue, reset: resetForm, getValues: getFormValues } = formMethods;

  const filterFormMethods = useForm<FilterFormData>({ resolver: zodResolver(filterFormSchema), defaultValues: filterCriteria });
  const { watch: watchFilter, reset: resetFilterForm, setValue: setFilterFormValue, getValues: getFilterValues } = filterFormMethods;
  ...
  // Effect to update local subcategoryOptions (used by both forms) when SubcategoriesData from Redux changes
  useEffect(() => {
  ...
  // --- Subcategory Logic for Main Form ---
  const watchedFormCategoryId = watchForm("category_id");
  useEffect(() => {
    const currentSubCategoryId = getFormValues("sub_category_id"); // Get current value before potential reset
    ...
    if (watchedFormCategoryId && typeof watchedFormCategoryId === 'number' && watchedFormCategoryId > 0) {
      ...
      // Also, only reset if there was a sub_category_id value previously.
      if (currentSubCategoryId && (!editingProduct || editingProduct.categoryId !== watchedFormCategoryId)) {
          setFormValue("sub_category_id", undefined, { shouldValidate: true });
      }
    } else if (!watchedFormCategoryId) {
      // If category is cleared in the form
      // dispatch(clearSubcategoriesAction()); // Optional: if you want to clear global SubcategoriesData
      if (currentSubCategoryId) { // Only reset if there was a value
        setFormValue("sub_category_id", undefined, { shouldValidate: true });
      }
    }
  }, [watchedFormCategoryId, dispatch, setFormValue, editingProduct, getFormValues]);


  // --- Subcategory Logic for Filter Drawer ---
  const watchedFilterCategoryIds = watchFilter("filterCategoryIds");
  useEffect(() => {
    if (isFilterDrawerOpen && watchedFilterCategoryIds && watchedFilterCategoryIds.length === 1) {
      dispatch(getSubcategoriesByCategoryIdAction(watchedFilterCategoryIds[0]));
      // Do not reset filterSubCategoryIds here, allow user to keep selection if subcategories are reloaded for the single selected category.
    } else if (isFilterDrawerOpen && (!watchedFilterCategoryIds || watchedFilterCategoryIds.length !== 1)) {
      // If multiple categories or no category selected in filter
      // dispatch(clearSubcategoriesAction()); // Optional: if you want to clear global SubcategoriesData
      // Or, clear local filter form's subcategory selection:
  ...
  }, [watchedFilterCategoryIds, isFilterDrawerOpen, dispatch, setFilterFormValue, getFilterValues]);


  useEffect(() => { // Cleanup for image previews
    const localThumbImagePreviewUrl = thumbImagePreviewUrl;
    const localGalleryImages = [...galleryImages];
    return () => {
      if (localThumbImagePreviewUrl && localThumbImagePreviewUrl.startsWith('blob:')) {
           URL.revokeObjectURL(localThumbImagePreviewUrl);
      }
  ...
  const mappedProducts: ProductItem[] = useMemo(() => {
    ...
    return ProductsData.map((apiItem: ApiProductItem): ProductItem => {
      ...
      const subCategoryNameFromOptions = subcategoryOptions.find(sc => sc.value === Number(apiItem.sub_category_id))?.label;
  ...
  }, [ProductsData, domainOptions, categoryOptions, subcategoryOptions, brandOptions, unitOptions, countryOptions]);

  const { pageData, total } = useMemo(() => { // Removed allFilteredAndSortedData as it's not used directly
  ...
  const openAddDrawer = useCallback(() => {
    ...
    // dispatch(clearSubcategoriesAction()); // Optional
  ...
  const onConfirmChangeStatus = async () => { if (!productForStatusChange || !selectedNewStatus) return; setIsProcessingStatusChange(true); const apiStatus = apiProductStatusOptions.find(opt => opt.value.toLowerCase() === selectedNewStatus.toLowerCase())?.value || selectedNewStatus; try { await dispatch(changeProductStatusAction({ productId: productForStatusChange.id, status: apiStatus })).unwrap(); toast.push(<Notification type="success" title="Status Updated" duration={2000}>Product status changed to {selectedNewStatus}.</Notification>); dispatch(getProductsAction()); } catch (error: any) { toast.push(<Notification type="danger" title="Status Update Failed">{error.message || "Could not update status."}</Notification>); } finally { setIsProcessingStatusChange(false); setIsChangeStatusDialogOpen(false); setProductForStatusChange(null); }};
  ...
  const onClearFilters = useCallback(() => { resetFilterForm({}); setFilterCriteria({}); setFilterFormValue("filterSubCategoryIds", []); /* dispatch(clearSubcategoriesAction()); */ handlePaginationChange(1); closeFilterDrawer(); }, [resetFilterForm, setFilterFormValue, handlePaginationChange, closeFilterDrawer]);
  ...
  ], [openImageViewer, openEditDrawer, openViewDetailModal, handleDeleteProductClick, handleChangeStatusClick]);

  const tableIsLoading = masterLoadingStatus === 'pending' || isSubmitting || isProcessingDelete || isProcessingStatusChange;
    ...
    <>
      ...
      <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={onClearFilters}>Clear</Button><Button size="sm" variant="solid" form="filterProductForm" type="submit">Apply</Button></div>}>
        <Form id="filterProductForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
          ...
          <FormItem label="Categories"><Controller name="filterCategoryIds" control={filterFormMethods.control} render={({ field }) => <UiSelect isMulti placeholder="Select Categories" options={categoryOptions} value={categoryOptions.filter(o => field.value?.includes(o.value))} onChange={opts => { field.onChange(opts?.map(o => o.value)); if (!opts || opts.length !== 1) { const currentFilterSubCatIds = getFilterValues("filterSubCategoryIds"); if (currentFilterSubCatIds && currentFilterSubCatIds.length > 0) setFilterFormValue("filterSubCategoryIds", []);} }} />} /></FormItem>
          <FormItem label="Sub Categories"><Controller name="filterSubCategoryIds" control={filterFormMethods.control} render={({ field }) => <UiSelect isMulti placeholder={watchedFilterCategoryIds?.length !== 1 ? "Select one category first" : (subcategoryOptions.length === 0 ? "No subcategories" : "Select Sub Categories")} options={subcategoryOptions} value={subcategoryOptions.filter(o => field.value?.includes(o.value))} onChange={opts => field.onChange(opts?.map(o => o.value))} isDisabled={watchedFilterCategoryIds?.length !== 1 || subcategoryOptions.length === 0} />} /></FormItem>
