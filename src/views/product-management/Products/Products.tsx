...
import {
    ...
    TbSwitchHorizontal, TbCopy, TbCloudDownload, TbEye, TbMailForward, TbChevronDown,
    TbPhoto, TbFileDescription, TbSettings, TbClipboardText, TbX
} from 'react-icons/tb';
...
import {
     getProductsAction, 
     addProductAction, editProductAction, deleteProductAction, deleteAllProductsAction,
...
// --- Type Definitions ---
type ApiProductItem = {
    id: number; category_id: string | number | null; sub_category_id: string | number | null; brand_id: string | number | null; sku_code: string | null; name: string; unit: string | number | null; country_id: string | number | null; color: string | null; hsn_code: string | null; shelf_life: string | null; packaging_size: string | null; packaging_type: string | null; tax_rate: string | number | null; procurement_lead_time: string | null; slug: string; description: string | null; short_description: string | null; payment_term: string | null; delivery_details: string | null; thumb_image: string | null; icon: string | null; product_images: string | null; /* JSON string of image filenames */ status: "Active" | "Inactive" | "Pending" | "Draft"; licence: string | null; currency_id: string | number | null; product_specification: string | null; meta_title: string | null; meta_descr: string | null; meta_keyword: string | null; domain_ids: string | null; /* comma-separated IDs */ created_at: string; updated_at: string; icon_full_path?: string; thumb_image_full_path?: string; product_images_array?: { id?: number; image: string; image_full_path: string }[]; category?: { id: number; name: string; } | null; sub_category?: { id: number; name: string; } | null; brand?: { id: number; name: string; } | null; unit_obj?: { id: number; name: string; } | null; country_obj?: { id: number; name: string; } | null;
};
...
export type ProductItem = {
    id: number; name: string; slug: string; skuCode: string | null; status: ProductStatus; categoryId: number | null; categoryName?: string; subCategoryId: number | null; subCategoryName?: string; brandId: number | null; brandName?: string; unitId: number | null; unitName?: string; countryId: number | null; countryName?: string; domainIds: number[]; color: string | null; hsnCode: string | null; shelfLife: string | null; packagingSize: string | null; packagingType: string | null; taxRate: string | number | null; procurementLeadTime: string | null; description: string | null; shortDescription: string | null; paymentTerm: string | null; deliveryDetails: string | null; productSpecification: string | null; icon: string | null; iconFullPath: string | null; thumbImage: string | null; thumbImageFullPath: string | null; productImages: ProductGalleryImageItem[]; metaTitle: string | null; metaDescription: string | null; metaKeyword: string | null; createdAt: string; updatedAt: string;
};
const productFormSchema = z.object({
    ...
    category_id: z.number({ invalid_type_error: "Category is required." }).positive().nullable(),
    sub_category_id: z.number().positive().nullable().optional(),
    brand_id: z.number({ invalid_type_error: "Brand is required." }).positive().nullable(),
    unit_id: z.number({ invalid_type_error: "Unit is required." }).positive().nullable(),
    country_id: z.number({ invalid_type_error: "Country is required." }).positive().nullable(),
...
const selectStringOptionSchema = z.object({ value: z.string(), label: z.string() });
...
// --- Constants & Options ---
const PRODUCT_THUMB_IMAGE_BASE_URL = import.meta.env.VITE_API_URL_STORAGE || 'https://your-api-domain.com/storage/product_thumbs/'; // MODIFY
const PRODUCT_IMAGES_BASE_URL = import.meta.env.VITE_API_URL_STORAGE || 'https://your-api-domain.com/storage/product_gallery/'; // MODIFY
...
const uiProductStatusOptions: { value: ProductStatus; label: string }[] = [
    { value: 'active', label: 'Active' }, { value: 'inactive', label: 'Inactive' }, { value: 'pending', label: 'Pending' }, { value: 'draft', label: 'Draft' },
];
const apiProductStatusOptions: { value: string; label: string }[] = [ // For form select, value is what API expects
    { value: 'Active', label: 'Active' }, { value: 'Inactive', label: 'Inactive' }, { value: 'Pending', label: 'Pending' }, { value: 'Draft', label: 'Draft' },
];
...
const ProductSearch = React.forwardRef<HTMLInputElement, { onInputChange: (value: string) => void }>(({ onInputChange }, ref) => (<DebouceInput ref={ref} className="w-full" placeholder="Search by Name, SKU..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />));
ProductSearch.displayName = 'ProductSearch';

const ProductTableTools = ({ onSearchChange, onFilter, onExportImport }: {
    onSearchChange: (query: string) => void; onFilter: () => void; onExportImport: () => void; /* Changed to single handler for dropdown */
}) => (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <Button icon={<TbCloudDownload />} onClick={onExportImport} className="w-full sm:w-auto">Export/Import</Button>
        </div>
    </div>
);
...
const Products = () => {
    ...
    const { ProductsData = [], status: masterLoadingStatus = 'idle' } = useSelector(masterSelector);
    ...
    const [viewingProduct, setViewingProduct] = useState<ProductItem | null>(null); // For View Drawer
    ...
    const [selectedCategoryIdForForm, setSelectedCategoryIdForForm] = useState<number | null>(null); // For dynamic subcategories
    ...
    useEffect(() => {
        ...
        setDomainOptions([{value: 1, label: "Domain A"}, {value: 2, label: "Domain B"}]);
        setCategoryOptions([{value: 1, label: "Electronics"}, {value: 2, label: "Clothing"}]);
        setBrandOptions([{value: 1, label: "BrandX"}, {value: 2, label: "BrandY"}]);
        setUnitOptions([{value: 1, label: "PCS"}, {value: 2, label: "Box"}]);
        setCountryOptions([{value: 1, label: "USA"}, {value: 2, label: "India"}]);
    }, [dispatch]);
    ...
    useEffect(() => { /* ... Cleanup image preview URLs ... */ }, [thumbImagePreviewUrl, galleryImages]);

    // --- Data Mapping & Filtering ---
    const [masterProducts, setMasterProducts] = useState<ProductItem[]>([]); // Store raw mapped products
     useEffect(() => { // Map API data once it arrives
        if (ProductsData && ProductsData.length > 0) {
            const mapped = ProductsData.map((apiItem: ApiProductItem): ProductItem => {
                let iconFullPath: string | null = null; if(apiItem.icon_full_path) iconFullPath = apiItem.icon_full_path; else if(apiItem.icon) iconFullPath = `${PRODUCT_IMAGES_BASE_URL}${apiItem.icon}`;
                let thumbImageFullPath: string | null = null; if(apiItem.thumb_image_full_path) thumbImageFullPath = apiItem.thumb_image_full_path; else if(apiItem.thumb_image) thumbImageFullPath = `${PRODUCT_THUMB_IMAGE_BASE_URL}${apiItem.thumb_image}`;
                const parsedDomainIds = apiItem.domain_ids ? apiItem.domain_ids.split(',').map(id => parseInt(id.trim(),10)).filter(id => !isNaN(id)) : [];
                return {
                    id: apiItem.id, name: apiItem.name, slug: apiItem.slug, skuCode: apiItem.sku_code, status: (apiItem.status?.toLowerCase() || 'draft') as ProductStatus,
                    categoryId: apiItem.category_id ? Number(apiItem.category_id) : null, categoryName: apiItem.category?.name || (categoryOptions.find(c=>c.value === Number(apiItem.category_id))?.label),
                    subCategoryId: apiItem.sub_category_id ? Number(apiItem.sub_category_id) : null, subCategoryName: apiItem.sub_category?.name || (subcategoryOptions.find(sc=>sc.value === Number(apiItem.sub_category_id))?.label),
                    brandId: apiItem.brand_id ? Number(apiItem.brand_id) : null, brandName: apiItem.brand?.name || (brandOptions.find(b=>b.value === Number(apiItem.brand_id))?.label),
                    unitId: apiItem.unit ? Number(apiItem.unit) : null, unitName: apiItem.unit_obj?.name || (unitOptions.find(u=>u.value === Number(apiItem.unit))?.label),
                    countryId: apiItem.country_id ? Number(apiItem.country_id) : null, countryName: apiItem.country_obj?.name || (countryOptions.find(c=>c.value === Number(apiItem.country_id))?.label),
                    domainIds: parsedDomainIds, color: apiItem.color, hsnCode: apiItem.hsn_code, shelfLife: apiItem.shelf_life, packagingSize: apiItem.packaging_size, packagingType: apiItem.packaging_type, taxRate: apiItem.tax_rate, procurementLeadTime: apiItem.procurement_lead_time,
                    description: apiItem.description, shortDescription: apiItem.short_description, paymentTerm: apiItem.payment_term, deliveryDetails: apiItem.delivery_details, productSpecification: apiItem.product_specification,
                    icon: apiItem.icon, iconFullPath, thumbImage: apiItem.thumb_image, thumbImageFullPath,
                    productImages: apiItem.product_images_array?.map(img => ({ id: img.id, serverPath: img.image, previewUrl: img.image_full_path, isNew: false, isDeleted: false })) || [],
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => { /* ... Filtering logic as before ... */
        ...
        if (order && key) { processedData.sort((a, b) => { let aVal = a[key as keyof ProductItem] as any; let bVal = b[key as keyof ProductItem] as any; if (typeof aVal === 'number' && typeof bVal === 'number') return order === 'asc' ? aVal - bVal : bVal - aVal; return order === 'asc' ? String(aVal ?? '').localeCompare(String(bVal ?? '')) : String(bVal ?? '').localeCompare(String(aVal ?? '')); });}
    ...
    const openAddDrawer = useCallback(() => {
        setEditingProduct(null); setViewingProduct(null);
    ...
    const openEditDrawer = useCallback((product: ProductItem) => {
        setEditingProduct(product); setViewingProduct(null);
        // formMethods.reset({
        ...
        setGalleryImages(product.productImages.map(img => ({ ...img, previewUrl: img.url || img.serverPath || '', isNew: false, isDeleted: false }))); // Ensure previewUrl
        setIsAddEditDrawerOpen(true);
    }, [formMethods]);
    ...
    const closeViewProductDrawer = useCallback(() => setIsViewDrawerOpen(false), []);
    ...
    const onProductFormSubmit = useCallback(async (data: ProductFormData) => {
        ...
        Object.entries(data).forEach(([key, value]) => {
            ...
            if (key === 'domain_ids' && Array.isArray(value)) { value.forEach(id => formData.append('domain_ids[]', String(id))); }
            else if (value !== null && value !== undefined && String(value).trim() !== '') { formData.append(key, String(value)); }
        });
        if (newThumbImageFile) formData.append('thumb_image', newThumbImageFile); // Use 'thumb_image' as key

        galleryImages.forEach((img, index) => {
            if (img.file && img.isNew && !img.isDeleted) formData.append(`product_images[${index}]`, img.file); // Key for new images
            else if (img.id && img.isDeleted) formData.append('deleted_image_ids[]', String(img.id)); // Key for deleted
        });
        // For debugging:
        ...
        try {
            if (editingProduct) {
                ...
                toast.push(<Notification type="success" title="Product Updated (Mock)" />);
            } else {
                // await dispatch(addProductAction(formData)).unwrap();
                toast.push(<Notification type="success" title="Product Added (Mock)" />);
            }
            closeAddEditDrawer();
            // dispatch(getProductsAction()); // Refresh list
        } catch (error: any) { toast.push(<Notification type="danger" title="Error" >{error.message || "Operation failed"}</Notification>); }
        finally { setIsSubmitting(false); }
    }, [editingProduct, dispatch, closeAddEditDrawer, newThumbImageFile, galleryImages /*, getProductsAction */]);
    ...
    const onConfirmSingleDelete = useCallback(async () => { if(!itemToDelete) return; setIsProcessing(true); /* ...dispatch deleteProductAction... */ toast.push(<Notification title="Deleted (Mock)" />); setIsProcessing(false); setSingleDeleteConfirmOpen(false); setItemToDelete(null);}, [dispatch, itemToDelete]);
    const onDeleteSelectedProducts = useCallback(async () => { if(selectedItems.length === 0) return; setIsProcessing(true); /* ...dispatch deleteAllProductsAction... */ toast.push(<Notification title="Bulk Deleted (Mock)" />); setIsProcessing(false); setSelectedItems([]);}, [dispatch, selectedItems]);
    const openChangeProductStatusDialog = useCallback((product: ProductItem) => { setItemForStatusChange(product); setStatusChangeConfirmOpen(true); }, []);
    const onConfirmChangeStatus = useCallback(async () => { if(!itemForStatusChange) return; setIsProcessing(true); /* ...dispatch changeProductStatusAction... */ toast.push(<Notification title="Status Changed (Mock)" />); setIsProcessing(false); setStatusChangeConfirmOpen(false); setItemForStatusChange(null);}, [dispatch, itemForStatusChange]);
    const handleCloneProduct = useCallback((product: ProductItem) => { /* ...logic to prefill add form... */ toast.push(<Notification title="Product Cloned (Mock)" />); openAddDrawer(); }, [formMethods, openAddDrawer]);
    const handleSendLaunchMail = useCallback((product: ProductItem) => { toast.push(<Notification title={`Launch Mail for ${product.name} (Mock)`} />); }, []);
    ...
    const handleExportProducts = useCallback(() => { toast.push(<Notification title="Export Products (Mock)" />); }, [allFilteredAndSortedData]);
    const handleImportProducts = useCallback(() => setImportDialogType('product'), []);
    const handleExportKeywords = useCallback(() => toast.push(<Notification title="Export Keywords (Mock)" />), []);
    const handleImportKeywords = useCallback(() => setImportDialogType('keyword'), []);
    ...
    const handleImportFileSelected = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file && importDialogType) {
            toast.push(<Notification title={`Importing ${importDialogType} (Mock)`} message={`File: ${file.name}`} />);
    ...
    const columns: ColumnDef<ProductItem>[] = useMemo(() => [
        ...
        { header: 'Product', id:'productInfo', size: 300, cell: (props: CellContext<ProductItem, any>) => (
            <div className="flex items-center gap-3">
                <Avatar size="lg" shape="circle" src={props.row.original.thumbImageFullPath || undefined} icon={<TbBox/>} className={props.row.original.thumbImageFullPath ? 'cursor-pointer' : ''} onClick={() => props.row.original.thumbImageFullPath && openImageViewer(props.row.original.thumbImageFullPath)} />
                <div> <span className="font-semibold hover:text-blue-600 cursor-pointer" onClick={() => openViewProductDrawer(props.row.original)}>{props.row.original.name}</span> <div className="text-xs text-gray-500">SKU: {props.row.original.skuCode || '-'}</div> </div>
            </div>
        )},
        { header: 'Category', accessorKey: 'categoryName', cell: props => props.row.original.categoryName || '-' },
        { header: 'Subcategory', accessorKey: 'subCategoryName', cell: props => props.row.original.subCategoryName || '-' },
        { header: 'Brand', accessorKey: 'brandName', cell: props => props.row.original.brandName || '-' },
        { header: 'Status', accessorKey: 'status', cell: (props: CellContext<ProductItem, any>) => <Tag className={`${statusColor[props.row.original.status] || 'bg-gray-200'} capitalize`}>{props.row.original.status}</Tag> },
        { header: 'Actions', id: 'actions', meta: { thClass: 'text-center', tdClass: 'text-center' }, cell: (props: CellContext<ProductItem, any>) => <ActionColumn onEdit={() => openEditDrawer(props.row.original)} onView={() => openViewProductDrawer(props.row.original)} onDelete={() => handleDeleteProductClick(props.row.original)} onChangeStatus={() => openChangeProductStatusDialog(props.row.original)} onSendLaunchMail={()=>handleSendLaunchMail(props.row.original)} onClone={() => handleCloneProduct(props.row.original)} /> },
    ], [ProductsData, openImageViewer, openEditDrawer, openViewProductDrawer, handleDeleteProductClick, openChangeProductStatusDialog, handleSendLaunchMail, handleCloneProduct]); // Added masterProducts
        ...
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    ...
                    <ProductTableTools onSearchChange={handleSearchChange} onFilter={openFilterDrawer} onExportImport={handleExportImportDropdown} />
                    <div className="mt-4 flex-grow overflow-y-auto">
                        <DataTable /* Adapt ProductTable props */
                            columns={columns} data={pageData} loading={tableIsLoading}
                            pagingData={{ total, pageIndex: tableData.pageIndex as number, pageSize: tableData.pageSize as number }}
                            selectable selectedItems={selectedItems} // Pass selectedItems if your DataTable handles selection
            ...
            {/* Add/Edit Drawer */}
            <Drawer
                title={editingProduct ? 'Edit Product' : (viewingProduct ? `View Product: ${viewingProduct.name}` : 'Add New Product')}
                ...
                footer={ !viewingProduct && // Only show footer for add/edit
                    <div className="text-right">
                        <Button size="sm" className="mr-2" type="button" onClick={closeAddEditDrawer} disabled={isSubmitting}>Cancel</Button>
                        <Button size="sm" variant="solid" form="productForm" type="submit" loading={isSubmitting} disabled={isSubmitting || (isAddEditDrawerOpen && !editingProduct && !formMethods.formState.isValid) || (isAddEditDrawerOpen && editingProduct && !formMethods.formState.isDirty && !formMethods.formState.isValid) }>
            ...
            >
                {isViewDrawerOpen && viewingProduct ? (
                    <div className="p-4"> {/* Basic View Product Layout - Enhance as needed */}
                        <h3 className="text-lg font-semibold mb-2">{viewingProduct.name}</h3>
                        {viewingProduct.thumbImageFullPath && <Avatar src={viewingProduct.thumbImageFullPath} size={120} shape="rounded" className="mb-2"/>}
                        <p><strong>SKU:</strong> {viewingProduct.skuCode || '-'}</p>
                        <p><strong>Category:</strong> {viewingProduct.categoryName || '-'}</p>
                        <p><strong>Brand:</strong> {viewingProduct.brandName || '-'}</p>
                        <p><strong>Status:</strong> <Tag className={`${statusColor[viewingProduct.status] || ''} capitalize`}>{viewingProduct.status}</Tag></p>
                        <div className="mt-2"><strong>Description:</strong><div dangerouslySetInnerHTML={{ __html: viewingProduct.description || '<p>-</p>' }} /></div>
                        {/* Add more fields to view */}
                    </div>
                ) : (
                <Form id="productForm" onSubmit={formMethods.handleSubmit(onProductFormSubmit)} className="flex flex-col gap-y-3 h-full">
                    {/* Tab Navigation */}
                        ...
                        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                            {[FORM_TABS.GENERAL, FORM_TABS.DESCRIPTION, FORM_TABS.MEDIA, FORM_TABS.META].map((tab) => (
                                <button key={tab} type="button" onClick={() => handleFormTabChange(tab)}
                                    ...
                                    {tab === FORM_TABS.GENERAL && <TbSettings />} {tab === FORM_TABS.DESCRIPTION && <TbFileDescription />}
                    ...
                    <div className="flex-grow overflow-y-auto pb-2 pr-1"> {/* Added padding-right for scrollbar */}
                        {currentFormTab === FORM_TABS.GENERAL && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                                ...
                                <FormItem label="SKU Code" invalid={!!formMethods.formState.errors.sku_code} errorMessage={formMethods.formState.errors.sku_code?.message}><Controller name="sku_code" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                ...
                                <FormItem label="Category" isRequired invalid={!!formMethods.formState.errors.category_id} errorMessage={formMethods.formState.errors.category_id?.message}><Controller name="category_id" control={formMethods.control} render={({ field }) => <UiSelect options={categoryOptions} value={categoryOptions.find(o=>o.value === field.value)} onChange={opt => { field.onChange(opt?.value); /* setSelectedCategoryIdForForm handled by watch */ }} isClearable />}/></FormItem>
                                ...
                                <FormItem label="Color" invalid={!!formMethods.formState.errors.color} errorMessage={formMethods.formState.errors.color?.message}><Controller name="color" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="HSN Code" invalid={!!formMethods.formState.errors.hsn_code} errorMessage={formMethods.formState.errors.hsn_code?.message}><Controller name="hsn_code" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Shelf Life" invalid={!!formMethods.formState.errors.shelf_life} errorMessage={formMethods.formState.errors.shelf_life?.message}><Controller name="shelf_life" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Packaging Size" invalid={!!formMethods.formState.errors.packaging_size} errorMessage={formMethods.formState.errors.packaging_size?.message}><Controller name="packaging_size" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Packaging Type" invalid={!!formMethods.formState.errors.packaging_type} errorMessage={formMethods.formState.errors.packaging_type?.message}><Controller name="packaging_type" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Tax Rate" invalid={!!formMethods.formState.errors.tax_rate} errorMessage={formMethods.formState.errors.tax_rate?.message}><Controller name="tax_rate" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Procurement Lead Time" invalid={!!formMethods.formState.errors.procurement_lead_time} errorMessage={formMethods.formState.errors.procurement_lead_time?.message}><Controller name="procurement_lead_time" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Thumbnail Image" className="md:col-span-2" invalid={!!formMethods.formState.errors.thumb_image_input} errorMessage={formMethods.formState.errors.thumb_image_input?.message as string}><Controller name="thumb_image_input" control={formMethods.control} render={({ field: { onChange, onBlur, name, ref } }) => (<Input type="file" name={name} ref={ref} onBlur={onBlur} onChange={(e) => { const file = e.target.files?.[0] || null; onChange(file); setNewThumbImageFile(file); if(thumbImagePreviewUrl) URL.revokeObjectURL(thumbImagePreviewUrl); setThumbImagePreviewUrl(file ? URL.createObjectURL(file) : null); }} accept="image/*" /> )} /> {(thumbImagePreviewUrl || (editingProduct && editingProduct.thumbImageFullPath && !newThumbImageFile)) && (<div className="mt-2"><Avatar src={thumbImagePreviewUrl || editingProduct?.thumbImageFullPath} size={100} shape="rounded" /> {newThumbImageFile && <Button size="xs" variant="plain" className="text-red-500 mt-1" onClick={() => { setNewThumbImageFile(null); setThumbImagePreviewUrl(null); formMethods.setValue('thumb_image_input', null);}}>Remove</Button>}</div>)} </FormItem>
                            </div>
                        )}
                        {currentFormTab === FORM_TABS.DESCRIPTION && (
                            <div className="flex flex-col gap-y-4">
                                <FormItem label="Description" invalid={!!formMethods.formState.errors.description} errorMessage={formMethods.formState.errors.description?.message}><Controller name="description" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={8} />}/></FormItem>
                                <FormItem label="Short Description" invalid={!!formMethods.formState.errors.short_description} errorMessage={formMethods.formState.errors.short_description?.message}><Controller name="short_description" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={4} />}/></FormItem>
                                <FormItem label="Payment Term" invalid={!!formMethods.formState.errors.payment_term} errorMessage={formMethods.formState.errors.payment_term?.message}><Controller name="payment_term" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={3} />}/></FormItem>
                                <FormItem label="Delivery Details" invalid={!!formMethods.formState.errors.delivery_details} errorMessage={formMethods.formState.errors.delivery_details?.message}><Controller name="delivery_details" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={3} />}/></FormItem>
                                <FormItem label="Product Specification" invalid={!!formMethods.formState.errors.product_specification} errorMessage={formMethods.formState.errors.product_specification?.message}><Controller name="product_specification" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={5} />}/></FormItem>
                            </div>
                        )}
                        {currentFormTab === FORM_TABS.MEDIA && (
                            <div> {/* Implement Multiple Image Upload UI for galleryImages state */}
                                <label className="form-label block mb-2">Product Gallery Images (Max 5, 1024x1024 recommended)</label>
                                <Input type="file" multiple accept="image/*" onChange={(e) => {
                                    const files = Array.from(e.target.files || []);
                                    const newImages = files.slice(0, 5 - galleryImages.filter(img => !img.isDeleted).length).map(file => ({ file, previewUrl: URL.createObjectURL(file), isNew: true, isDeleted: false }));
                                ...
                                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    {galleryImages.filter(img => !img.isDeleted).map((image, index) => (
                                        <div key={image.id || image.previewUrl} className="relative group">
                                            <Avatar src={image.previewUrl} size={120} shape="rounded" className="w-full"/>
                                            <Button shape="circle" size="xs" icon={<TbX/>} className="absolute top-1 right-1 bg-red-500 text-white opacity-75 group-hover:opacity-100"
                                                onClick={() => {
                                                    if (image.isNew) { // If it's a new image not yet uploaded
                                                        URL.revokeObjectURL(image.previewUrl);
                                                        setGalleryImages(prev => prev.filter(img => img.previewUrl !== image.previewUrl));
                                                    } else { // Mark existing image for deletion
                                                        setGalleryImages(prev => prev.map(img => img.id === image.id || img.serverPath === image.serverPath ? {...img, isDeleted: true} : img ));
                                                    }
                                                }}
                                            />
                                        </div>
                        ...
                        {currentFormTab === FORM_TABS.META && (
                            <div className="flex flex-col gap-y-4">
                                <FormItem label="Meta Tag Title" invalid={!!formMethods.formState.errors.meta_title} errorMessage={formMethods.formState.errors.meta_title?.message}><Controller name="meta_title" control={formMethods.control} render={({ field }) => <Input {...field} />}/></FormItem>
                                <FormItem label="Meta Tag Description" invalid={!!formMethods.formState.errors.meta_descr} errorMessage={formMethods.formState.errors.meta_descr?.message}><Controller name="meta_descr" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={4} />}/></FormItem>
                                <FormItem label="Meta Tag Keywords" invalid={!!formMethods.formState.errors.meta_keyword} errorMessage={formMethods.formState.errors.meta_keyword?.message}><Controller name="meta_keyword" control={formMethods.control} render={({ field }) => <Textarea {...field} rows={3} placeholder="keyword1, keyword2, keyword3" />}/></FormItem>
                            </div>
                        )}
                    </div>
                </Form>
            ...
            {/* Filter Drawer */}
            <Drawer title="Filter Products" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} /* ... footer ... */ >
            ...
            <ConfirmDialog isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Product" /* ... */ ><p>Delete <strong>{itemToDelete?.name}</strong>?</p></ConfirmDialog>
            <ConfirmDialog isOpen={statusChangeConfirmOpen} type="warning" title="Change Status" /* ... */ ><p>Change status for <strong>{itemForStatusChange?.name}</strong>?</p></ConfirmDialog>
            <Dialog isOpen={isImageViewerOpen} onClose={closeImageViewer} /* ... */ title="View Image" > <div className="p-4 flex justify-center items-center">{imageToView && <img src={imageToView} alt="Product view" className="max-w-full max-h-[80vh] object-contain"/>}</div> <Dialog.Footer><Button onClick={closeImageViewer}>Close</Button></Dialog.Footer> </Dialog>

            {/* Import Dialogs */}
            <Dialog isOpen={importDialogType === 'product'} onClose={closeImportDialog} title="Import Products">
                <div className="p-4"> <p>Upload CSV to import products.</p> <Input type="file" accept=".csv" onChange={handleImportFileSelected} /> <div className="mt-4 text-right"><Button onClick={closeImportDialog}>Cancel</Button></div> </div>
            </Dialog>
            <Dialog isOpen={importDialogType === 'keyword'} onClose={closeImportDialog} title="Import Product Keywords">
                 <div className="p-4"> <p>Upload CSV to import product keywords.</p> <Input type="file" accept=".csv" onChange={handleImportFileSelected} /> <div className="mt-4 text-right"><Button onClick={closeImportDialog}>Cancel</Button></div> </div>
            </Dialog>
...
const initialDummyProducts: ApiProductItem[] = [
    { id: 1, name: 'iPhone 15 Pro', slug: 'iphone-15-pro', sku_code: 'IP15PRO', status: 'Active', category_id: 1, sub_category_id: 11, brand_id: 1, unit: 1, country_id: 1, domain_ids: '1,2', color: 'Titanium Blue', hsn_code: '8517', shelf_life: null, packaging_size: null, packaging_type: 'Box', tax_rate: 18, procurement_lead_time: '3 days', description: '<p>Latest iPhone</p>', short_description: 'New iPhone', payment_term: 'COD', delivery_details: 'Express', product_specification: null, thumb_image: 'thumb_iphone.jpg', icon: null, product_images: JSON.stringify([{image:'gallery1.jpg'}, {image:'gallery2.jpg'}]), licence: null, currency_id: 1, meta_title: 'iPhone 15 Pro', meta_descr: 'Buy iPhone 15 Pro', meta_keyword: 'iphone, apple', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), category: {id: 1, name: "Electronics"}, brand: {id: 1, name: "Apple"}, sub_category: {id: 11, name: "Mobiles"}, thumb_image_full_path: 'https://via.placeholder.com/150/0000FF/808080?Text=iPhoneThumb', product_images_array: [{image:'g1.jpg', image_full_path:'https://via.placeholder.com/300/FF0000/FFFFFF?Text=Img1'}, {image:'g2.jpg', image_full_path:'https://via.placeholder.com/300/00FF00/FFFFFF?Text=Img2'}]},
    { id: 2, name: 'Galaxy S24 Ultra', slug: 'galaxy-s24-ultra', sku_code: 'GS24ULT', status: 'Pending', category_id: 1, sub_category_id: 11, brand_id: 2, unit: 1, country_id: 2, domain_ids: '1', color: 'Black', hsn_code: '8517', shelf_life: null, packaging_size: null, packaging_type: 'Box', tax_rate: 18, procurement_lead_time: '5 days', description: '<p>Samsung Flagship</p>', short_description: 'New Samsung', payment_term: 'Prepaid', delivery_details: 'Standard', product_specification: null, thumb_image: 'thumb_s24.jpg', icon: null, product_images: null, licence: null, currency_id: 1, meta_title: 'Galaxy S24 Ultra', meta_descr: 'Buy Galaxy S24', meta_keyword: 'samsung, galaxy', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), category: {id: 1, name: "Electronics"}, brand: {id: 2, name: "Samsung"}, sub_category: {id: 11, name: "Mobiles"}, thumb_image_full_path: 'https://via.placeholder.com/150/00FF00/808080?Text=S24Thumb'},
];