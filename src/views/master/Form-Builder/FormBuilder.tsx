...
import { Drawer, Form, FormItem, Input, Tag, Dialog, Card } from '@/components/ui';

// Icons
import {
    TbPencil, TbTrash, TbChecks, TbSearch, TbFilter, TbPlus, TbCloudUpload,
    TbCopy, TbSwitchHorizontal, TbEye, TbFileDescription, TbListDetails, TbQuestionMark, TbX,
} from 'react-icons/tb';
...
const DEPARTMENT_OPTIONS = [
    { value: 'eng', label: 'Engineering' }, { value: 'mkt', label: 'Marketing' },
    { value: 'hr', label: 'Human Resources' }, { value: 'sales', label: 'Sales' },
];
const departmentValues = DEPARTMENT_OPTIONS.map(d => d.value) as [string, ...string[]];

const CATEGORY_OPTIONS = [
    { value: 'feedback', label: 'Feedback' }, { value: 'survey', label: 'Survey' },
    { value: 'application', label: 'Application' }, { value: 'support', label: 'Support Ticket' },
];
...
const initialDummyForms: FormBuilderItem[] = [
    { id: 'FORM001', formName: 'Employee Feedback Survey Q3', status: 'published', departmentName: 'hr', categoryName: 'survey', questions: [{ questionSectionTitle: 'General Feedback', questionText: 'Overall satisfaction with Q3?', questionType: 'radio' }] },
    { id: 'FORM002', formName: 'New Feature Request Form', status: 'draft', departmentName: 'eng', categoryName: 'feedback', questions: [{ questionText: 'Please describe the new feature you would like to see.', questionType: 'textarea' }] },
    { id: 'FORM003', formName: 'Job Application - Senior Frontend Developer', status: 'published', departmentName: 'eng', categoryName: 'application', questions: [{ questionText: 'Upload Your Resume/CV', questionType: 'file' }, {questionText: 'Years of experience with React?', questionType: 'text'}] },
];


// --- CSV Exporter Utility ---
const CSV_HEADERS_FORM = ['ID', 'Form Name', 'Status', 'Department', 'Category', 'Question Count'];
const CSV_KEYS_FORM: (keyof FormBuilderItem | 'questionCount')[] = ['id', 'formName', 'status', 'departmentName', 'categoryName', 'questionCount'];
...
function exportFormsToCsv(filename: string, rows: FormBuilderItem[]) {
    ...
    const preparedRows = rows.map(row => ({
        ...row,
        departmentName: DEPARTMENT_OPTIONS.find(d => d.value === row.departmentName)?.label || row.departmentName,
        categoryName: CATEGORY_OPTIONS.find(c => c.value === row.categoryName)?.label || row.categoryName,
        status: FORM_STATUS_OPTIONS.find(s => s.value === row.status)?.label || row.status,
    ...
    const csvContent =
        ...
        '\n' +
        preparedRows.map((row: any) => CSV_KEYS_FORM.map(k => {
            let cell: any = row[k];
...
const ActionColumn = ({ onEdit, onDelete, onChangeStatus, onClone, onView }: {
    onEdit: () => void; onDelete: () => void; onChangeStatus: () => void; onClone: () => void; onView: () => void;
}) => {
    ...
    return (
        <div className="flex items-center justify-center gap-1.5">
            ...
            <Tooltip title="Edit Form"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-emerald-500 dark:hover:text-emerald-300')} role="button" onClick={onEdit}><TbPencil /></div></Tooltip>
            <Tooltip title="Clone Form"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-purple-500 dark:hover:text-purple-300')} role="button" onClick={onClone}><TbCopy /></div></Tooltip>
            <Tooltip title="Change Status"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-amber-500 dark:hover:text-amber-300')} role="button" onClick={onChangeStatus}><TbSwitchHorizontal /></div></Tooltip>
            <Tooltip title="Delete Form"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300')} role="button" onClick={onDelete}><TbTrash /></div></Tooltip>
        </div>
    );
};
...
const FormBuilderSearch = React.forwardRef<HTMLInputElement, FormBuilderSearchProps>(
    ({ onInputChange }, ref) => (
        <DebouceInput ref={ref} className="w-full" placeholder="Search forms by name or ID..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />
    )
);
FormBuilderSearch.displayName = 'FormBuilderSearch';

type FormBuilderTableToolsProps = { onSearchChange: (query: string) => void; onFilter: () => void; onExport: () => void; }
const FormBuilderTableTools = ({ onSearchChange, onFilter, onExport }: FormBuilderTableToolsProps) => (
...
const FormBuilder = () => {
    ...
    const formMethods = useForm<FormBuilderFormData>({
        ...
        defaultValues: {
            formName: '', status: formStatusValues[0], departmentName: departmentValues[0],
            categoryName: categoryValues[0], questions: [{ questionSectionTitle: '', questionText: '', questionType: questionTypeValues[0] }],
        },
        mode: 'onChange',
    });
    ...
    const openEditDrawer = (item: FormBuilderItem) => {
        setEditingItem(item);
        formMethods.reset({ // Reset the whole form with item data
            ...item,
            questions: item.questions.length > 0 ? item.questions : [{ questionSectionTitle: '', questionText: '', questionType: questionTypeValues[0] }]
        });
    ...
    const onSubmit = async (data: FormBuilderFormData, addAnother?: boolean) => {
        ...
        } catch (e: any) { toast.push(<Notification title={editingItem ? "Update Failed" : "Create Failed"} type="danger" duration={3000}>{e.message}</Notification>); }
    ...
    const handleDeleteSelected = async () => {
        if (selectedItems.length === 0) {
            toast.push(<Notification title="No Selection" type="info">Please select forms to delete.</Notification>);
            return;
        }
    ...
    const handleChangeStatus = async (item: FormBuilderItem) => {
        ...
        try {
            setFormsData(prev => prev.map(f => f.id === item.id ? { ...f, status: nextStatusValue } : f));
        ...
        } catch (e: any) { toast.push(<Notification title="Status Change Failed" type="danger" duration={3000}>{e.message}</Notification>); }
    ...
    const handleCloneForm = async (itemToClone: FormBuilderItem) => {
        ...
        } catch (e: any) { toast.push(<Notification title="Clone Failed" type="danger" duration={3000}>{e.message}</Notification>); }
    ...
    const handleSelectChange = useCallback( // This is for page size selector
        (value: number) => {
    ...
    const handleRowSelect = useCallback((checked: boolean, row: FormBuilderItem) => {
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterDepartment?.length) {
            ...
            processedData = processedData.filter(item => deptValues.includes(item.departmentName));
        }
        if (filterCriteria.filterCategory?.length) {
            const catValues = filterCriteria.filterCategory.map(c => c.value);
            processedData = processedData.filter(item => catValues.includes(item.categoryName));
        }
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter(item =>
                ...
                (DEPARTMENT_OPTIONS.find(d=>d.value === item.departmentName)?.label.toLowerCase() || '').includes(query) ||
                (CATEGORY_OPTIONS.find(c=>c.value === item.categoryName)?.label.toLowerCase() || '').includes(query)
            );
        }
        ...
        if (order && key) {
            processedData.sort((a, b) => {
                ...
                if (key === 'departmentName') { /* ... sort by label ... */ }
                else if (key === 'categoryName') { /* ... sort by label ... */ }
                else if (key === 'status') { /* ... sort by label ... */ }
    ...
    const renderDrawerForm = (currentFormMethods: typeof formMethods) => ( // Renamed parameter
        <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <FormItem label="Form Name" invalid={!!currentFormMethods.formState.errors.formName} errorMessage={currentFormMethods.formState.errors.formName?.message}>
                    <Controller name="formName" control={currentFormMethods.control} render={({ field }) => <Input {...field} prefix={<TbFileDescription/>} placeholder="e.g., Customer Satisfaction Survey" />} />
                </FormItem>
                <FormItem label="Status" invalid={!!currentFormMethods.formState.errors.status} errorMessage={currentFormMethods.formState.errors.status?.message}>
                    <Controller name="status" control={currentFormMethods.control} render={({ field }) => (
                        <Select placeholder="Select status" options={FORM_STATUS_OPTIONS} value={FORM_STATUS_OPTIONS.find(o=>o.value === field.value)} onChange={opt=>field.onChange(opt?.value)} />
                    )} />
                </FormItem>
                <FormItem label="Department Name" invalid={!!currentFormMethods.formState.errors.departmentName} errorMessage={currentFormMethods.formState.errors.departmentName?.message}>
                    <Controller name="departmentName" control={currentFormMethods.control} render={({ field }) => (
                        <Select placeholder="Select department" options={DEPARTMENT_OPTIONS} value={DEPARTMENT_OPTIONS.find(o=>o.value === field.value)} onChange={opt=>field.onChange(opt?.value)} />
                    )} />
                </FormItem>
                <FormItem label="Category Name" invalid={!!currentFormMethods.formState.errors.categoryName} errorMessage={currentFormMethods.formState.errors.categoryName?.message}>
                    <Controller name="categoryName" control={currentFormMethods.control} render={({ field }) => (
                        <Select placeholder="Select category" options={CATEGORY_OPTIONS} value={CATEGORY_OPTIONS.find(o=>o.value === field.value)} onChange={opt=>field.onChange(opt?.value)} />
                    )} />
                </FormItem>
            </div>
            ...
            {currentFormMethods.formState.errors.questions && !Array.isArray(currentFormMethods.formState.errors.questions) && (
                <p className="text-red-500 text-xs mb-2">{currentFormMethods.formState.errors.questions.message}</p>
            )}
                    ...
                    <Card key={field.id} bodyClass="p-4" className="relative bg-gray-50 dark:bg-gray-700/50 border dark:border-gray-600 rounded-lg">
                         {questionFields.length > 1 && (
                            <Tooltip title="Remove Question Section">
                                <Button shape="circle" size="xs" icon={<TbX/>} onClick={() => removeQuestion(index)}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <FormItem label={`Question Section Title ${index + 1} (Optional)`} className="md:col-span-2"
                                      invalid={!!currentFormMethods.formState.errors.questions?.[index]?.questionSectionTitle}
                                      errorMessage={currentFormMethods.formState.errors.questions?.[index]?.questionSectionTitle?.message}>
                                <Controller name={`questions.${index}.questionSectionTitle`} control={currentFormMethods.control}
                                            render={({ field: qField }) => <Input {...qField} placeholder="e.g., Personal Information" />} />
                            </FormItem>
                            <FormItem label={`Question ${index + 1}`} className="md:col-span-2"
                                      invalid={!!currentFormMethods.formState.errors.questions?.[index]?.questionText}
                                      errorMessage={currentFormMethods.formState.errors.questions?.[index]?.questionText?.message}>
                                <Controller name={`questions.${index}.questionText`} control={currentFormMethods.control}
                                            render={({ field: qField }) => <Textarea {...qField} rows={2} placeholder="Enter your question here..." />} />
                            </FormItem>
                            <FormItem label={`Question Type ${index + 1}`}
                                      invalid={!!currentFormMethods.formState.errors.questions?.[index]?.questionType}
                                      errorMessage={currentFormMethods.formState.errors.questions?.[index]?.questionType?.message}>
                                <Controller name={`questions.${index}.questionType`} control={currentFormMethods.control}
                                            render={({ field: qField }) => (
                                                <Select placeholder="Select type" options={QUESTION_TYPE_OPTIONS}
                                                        value={QUESTION_TYPE_OPTIONS.find(o=>o.value === qField.value)}
                                                        onChange={opt=>qField.onChange(opt?.value)} />
                                            )} />
                            </FormItem>
                             {/* Placeholder for question options: Implement if needed */}
                             {['checkbox', 'radio', 'select'].includes(formMethods.watch(`questions.${index}.questionType`)) && (
                                <div className="md:col-span-2 p-3 bg-gray-100 dark:bg-gray-800 rounded">
                                    <p className="text-xs text-gray-500 dark:text-gray-400">Dynamic options editor for Checkbox/Radio/Select types would go here. (Not implemented in this example).</p>
                                </div>
                            )}
                        </div>
                    </Card>
            ...
            <Button type="button" variant="dashed" icon={<TbPlus />} onClick={() => appendQuestion({ questionSectionTitle: '', questionText: '', questionType: questionTypeValues[0] })} className="mt-4 self-start">
    ...
    return (
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        <h3 className="mb-4 sm:mb-0 flex items-center gap-2"><TbFileDescription /> Form Builder</h3>
                        <Button variant="solid" icon={<TbPlus />} onClick={openAddDrawer}>Create New Form</Button>
                    </div>
                    <FormBuilderTableTools
                        onSearchChange={handleSearchChange}
                        onFilter={openFilterDrawer} onExport={function (): void {
                    ...
                    />
                     <div className="mt-4">
                        <DataTable
                            columns={columns}
                            data={pageData}
                            loading={masterLoadingStatus === 'loading' || isSubmitting || isDeleting || isCloning || isChangingStatus}
                            pagingData={{total, pageIndex: tableData.pageIndex as number, pageSize: tableData.pageSize as number}}
                            selectable
                            checkboxChecked={(row: FormBuilderItem) => selectedItems.some(selected => selected.id === row.id)}
            ...
            <Drawer
                ...
                footer={
                    <div className="flex justify-between w-full">
                        ...
                        <div className={editingItem || !isAddDrawerOpen ? "w-full text-right" : ""}> {/* Adjust based on add drawer visibility */}
                            ...
                            <Button size="sm" variant="solid" form="formBuilderForm" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>
                                {isSubmitting ? (editingItem ? 'Saving...' : 'Creating...') : (editingItem ? 'Save Changes' : 'Create Form')}
                            </Button>
                        </div>
                    </div>
                }
            >
                <Form id="formBuilderForm" onSubmit={formMethods.handleSubmit((data) => onSubmit(data, false))} className="flex flex-col gap-4">
                 ...
                 footer={
                     <div className="flex justify-between w-full">
                        ...
                        <div>
                            ...
                            <Button size="sm" variant="solid" form="filterFormBuilderForm" type="submit">Apply Filters</Button>
                        </div>
                    </div>
                }
            >
                <Form id="filterFormBuilderForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Filter by Status">
                        <Controller name="filterStatus" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Select status(es)..." options={FORM_STATUS_OPTIONS}
                     ...
                     <FormItem label="Filter by Category">
                        <Controller name="filterCategory" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Select category(es)..." options={CATEGORY_OPTIONS}
                                    value={field.value || []} onChange={val => field.onChange(val || [])} />
                        )} />
                    </FormItem>
                </Form>
            ...
            <ConfirmDialog
                isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Form"
