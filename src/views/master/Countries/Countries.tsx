...
export type CountryItem = {
    ...
    iso: string
    phonecode: string
    continent?: { // Optional continent object from backend
        id: string | number
        name: string
    }
}
...
const countryFormSchema = z.object({
    continent_id: z.union([z.string().min(1, "Continent is required."), z.number().min(1, "Continent is required.")]),
    name: z
        .string()
...
const CSV_HEADERS_COUNTRY = ['ID', 'Continent Name', 'Country Name', 'ISO Code', 'Phone Code']
const CSV_KEYS_COUNTRY_EXPORT = ['id', 'continentName', 'name', 'iso', 'phonecode'] // Adjusted for transformed data

type CountryExportItem = Omit<CountryItem, 'continent_id' | 'continent'> & { continentName?: string }
...
const Countries = () => {
    ...
    const continentOptions = useMemo(() => {
        ...
        return ContinentsData.map((continent: { id: string | number; name: string }) => ({
            value: continent.id,
    ...
    const openAddDrawer = () => {
        addFormMethods.reset({ continent_id: '', name: '', iso: '', phonecode: '' })
    ...
    const closeEditDrawer = () => {
        ...
        editFormMethods.reset({ continent_id: '', name: '', iso: '', phonecode: '' })
        ...
        try {
            ...
            toast.push(
                <Notification
                    ...
                    duration={2000}
                >
                    {validItemsToDelete.length} countr{validItemsToDelete.length > 1 ? 'ies' : 'y'} successfully
    ...
    const onClearFilters = () => {
        const defaultFilters = { filterContinentIds: [], filterNames: [], filterIsos: [] }
    ...
    const countryIsoOptions = useMemo(() => {
        ...
        const uniqueIsos = new Set(
            CountriesData.map((country) => country.iso),
        )
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (
            filterCriteria.filterIsos &&
            filterCriteria.filterIsos.length > 0
        ) {
            const selectedFilterIsos = filterCriteria.filterIsos.map((opt) =>
                opt.value.toLowerCase(),
            )
            processedData = processedData.filter((item: CountryItem) =>
                selectedFilterIsos.includes(
                    item.iso?.trim().toLowerCase() ?? '',
                ),
            )
        }
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter((item: CountryItem) => {
                ...
                const continentNameLower = item.continent?.name?.trim().toLowerCase() ?? ''
        ...
        if (
            order &&
            key &&
            (key === 'id' || key === 'name' || key === 'iso' || key === 'phonecode' || key === 'continent.name') &&
        ...
        <>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                <Select
                                    ...
                                    value={continentOptions.find(option => option.value === field.value) || null}
            ...
            >
                ...
                >
                     <FormItem
                        ...
                        errorMessage={editFormMethods.formState.errors.continent_id?.message}
                    >
                        <Controller
                            name="continent_id"
            ...
            >
                <p>
                    ...
                    <strong>{countryToDelete?.name} ({countryToDelete?.iso})</strong>"?
                </p>
            </ConfirmDialog>
        </>
    )
