...
export type CountryItem = { id: string | number; name: string; iso_code: string; phone_code: string; region: string | null; flag_path?: string | null; status: 'Active' | 'Inactive'; created_at?: string; updated_at?: string; updated_by_user?: { name: string; profile_pic_path?: string; roles: { display_name: string }[] } };
...
const Countries = () => {
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (activeFilters.regions?.length) { const regions = new Set(activeFilters.regions); processedData = processedData.filter(item => item.region && regions.has(item.region)); }
                            ...
                            <div className="grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3">
                                <div>
                                    ...
                                    <p className="text-sm font-semibold">
                                        {editingCountry.updated_by_user?.name ||
                                            'N/A'}
                                    </p>
                                    <p>
