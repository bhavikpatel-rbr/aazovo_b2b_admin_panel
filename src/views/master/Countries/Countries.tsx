...
type CountryExportItem = Omit<
    CountryItem,
    'continent_id' | 'continent' | 'created_at' | 'updated_at'
> & {
...
const Countries = () => {
    ...
    const onConfirmSingleDelete = async () => {
        ...
        try {
            await dispatch(
                deleteCountryAction({ id: countryToDelete.id }),
            ).unwrap()
            toast.push(
                <Notification
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (tableData.query) {
            ...
            processedData = processedData.filter(
                (item) =>
                    ...
                    (item.continent?.name?.trim().toLowerCase() ?? '').includes(
                        query,
                    ) ||
        ...
        if (
            ...
            [
                ...
                'updated_at',
                'updated_by_name',
            ].includes(key) && // Added status to sortable keys
            processedData.length > 0
        ) {
            processedData.sort((a, b) => {
                let aValue: any, bValue: any
                if (key === 'continent.name') {
                    aValue = a.continent?.name ?? ''
                    bValue = b.continent?.name ?? ''
                    ...
                    aValue = a[key as keyof CountryItem] ?? ''
                    bValue = b[key as keyof CountryItem] ?? ''
                }
