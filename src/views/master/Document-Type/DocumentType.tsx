    ...
    return (
        <>
            ...
            <Drawer title={isEditDrawerOpen ? "Edit Document Type" : "Add Document Type"} isOpen={isAddDrawerOpen || isEditDrawerOpen} onClose={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} onRequestClose={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} width={480} bodyClass="relative" footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} disabled={isSubmitting}>Cancel</Button><Button size="sm" variant="solid" form={isEditDrawerOpen ? "editDocTypeForm" : "addDocTypeForm"} type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>{isSubmitting ? (isEditDrawerOpen ? "Saving..." : "Adding...") : "Save"}</Button></div>}>
                ...
                {isEditDrawerOpen && editingDocType && (<div className="absolute bottom-4 right-4 left-4">
                    <div className="grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3">
                        <div><b className="font-semibold text-gray-900 dark:text-gray-100">Latest Update:</b><br /><p className="font-semibold">{editingDocType.updated_by_user?.name || "N/A"}</p><p>{editingDocType.updated_by_user?.roles?.[0]?.display_name || "N/A"}</p></div>
                        <div className="text-right"><b className="font-semibold text-gray-900 dark:text-gray-100"></b><br /><span className="font-semibold">Created:</span> <span>{editingDocType.created_at ? new Date(editingDocType.created_at).toLocaleString() : "N/A"}</span><br /><span className="font-semibold">Updated:</span> <span>{editingDocType.updated_at ? new Date(editingDocType.updated_at).toLocaleString() : "N/A"}</span></div>
                    </div>
                </div>)}
            </Drawer>
