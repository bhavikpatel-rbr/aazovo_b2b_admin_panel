...
export type DocumentItem = { id: string | number; name: string; document_type?: string | number; document_type_name?: string; status: 'Active' | 'Inactive'; created_at?: string; updated_at?: string; updated_by_user?: { name: string, profile_pic_path?: string, roles: { display_name: string }[] }; };
...
const ActiveFiltersDisplay = ({ filterData, onRemoveFilter, onClearAll, docTypeOptions }) => {
    const activeNames = filterData.names || []; const activeTypes = filterData.documentTypeIds || []; const activeStatuses = filterData.status || []; const hasActiveFilters = activeNames.length > 0 || activeTypes.length > 0 || activeStatuses.length > 0;
...
const DocumentTableTools = React.forwardRef(({ onSearchChange, onApplyFilters, onClearFilters, onExport, activeFilters, activeFilterCount, documentNameOptions, documentTypeOptions, columns, filteredColumns, setFilteredColumns, searchInputValue }, ref) => {
...
const Documents = () => {
    ...
    return (
        <>
            ...
            <Drawer title="Add Document" isOpen={isAddDrawerOpen} onClose={closeAddDrawer} onRequestClose={closeAddDrawer} width={460} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={closeAddDrawer} disabled={isSubmitting}>Cancel</Button><Button size="sm" variant="solid" form="addDocumentForm" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>{isSubmitting ? "Adding..." : "Save"}</Button></div>}>
                <Form id="addDocumentForm" onSubmit={formMethods.handleSubmit(onAddDocumentSubmit)} className="flex flex-col gap-4">
                    <FormItem label={<div>Document Name <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.name} errorMessage={formMethods.formState.errors.name?.message}><Controller name="name" control={formMethods.control} render={({ field }) => (<Input {...field} placeholder="Enter Document Name" />)} /></FormItem>
                    <FormItem label={<div>Document Type <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.document_type} errorMessage={formMethods.formState.errors.document_type?.message}><Controller name="document_type" control={formMethods.control} render={({ field }) => (<Select placeholder="Select Document Type" options={documentTypeOptionsForSelect} value={documentTypeOptionsForSelect.find(o => o.value === field.value) || null} onChange={(o) => field.onChange(o ? o.value : "")} />)} /></FormItem>
            ...
            <Drawer title="Edit Document" isOpen={isEditDrawerOpen} onClose={closeEditDrawer} onRequestClose={closeEditDrawer} width={480} bodyClass="relative" footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={closeEditDrawer} disabled={isSubmitting}>Cancel</Button><Button size="sm" variant="solid" form="editDocumentForm" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>{isSubmitting ? "Saving..." : "Save"}</Button></div>}>
                <Form id="editDocumentForm" onSubmit={formMethods.handleSubmit(onEditDocumentSubmit)} className="flex flex-col gap-4">
                    <FormItem label={<div>Document Name <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.name} errorMessage={formMethods.formState.errors.name?.message}><Controller name="name" control={formMethods.control} render={({ field }) => (<Input {...field} placeholder="Enter Document Name" />)} /></FormItem>
                    <FormItem label={<div>Document Type <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.document_type} errorMessage={formMethods.formState.errors.document_type?.message}><Controller name="document_type" control={formMethods.control} render={({ field }) => (<Select placeholder="Select Document Type" options={documentTypeOptionsForSelect} value={documentTypeOptionsForSelect.find(o => o.value === field.value) || null} onChange={(o) => field.onChange(o ? o.value : "")} />)} /></FormItem>
                    <FormItem label={<div>Status <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.status} errorMessage={formMethods.formState.errors.status?.message}><Controller name="status" control={formMethods.control} render={({ field }) => (<Select placeholder="Select Status" options={statusOptions} value={statusOptions.find(o => o.value === field.value) || null} onChange={(o) => field.onChange(o ? o.value : "")} />)} /></FormItem>
                </Form>
                {editingDocument && (<div className="absolute bottom-4 right-4 left-4"><div className="grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3"><div><b className="font-semibold text-gray-900 dark:text-gray-100">Latest Update:</b><br /><p className="font-semibold">{editingDocument.updated_by_user?.name  || "N/A"}</p><p>{editingDocument.updated_by_user?.roles[0]?.display_name || "N/A"}</p></div><div className="text-right"><span className="font-semibold">Created:</span> <span>{editingDocument.created_at ? new Date(editingDocument.created_at).toLocaleString() : "N/A"}</span><br /><span className="font-semibold">Updated:</span> <span>{editingDocument.updated_at ? new Date(editingDocument.updated_at).toLocaleString() : "N/A"}</span></div></div></div>)}
            </Drawer>
