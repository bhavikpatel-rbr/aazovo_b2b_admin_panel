...
import {
    ...
    TbBriefcase, TbMapPin, TbUsers, TbFileText, TbSwitchHorizontal, TbBuildingSkyscraper,
} from 'react-icons/tb';
...
const JOB_POST_STATUS_OPTIONS_FORM: { value: JobPostStatusForm, label: string }[] = [
...
const jobPostFormSchema = z.object({
    job_title: z.string().min(1, 'Job Title is required.').max(150, "Title too long"),
    ...
    vacancies: z.coerce.number().int().min(0, 'Vacancies must be a non-negative number.'),
...
const CSV_HEADERS_JOB = ['ID', 'Job Title', 'Status', 'Department ID', 'Description', 'Location', 'Experience', 'Vacancies', 'Created At'];
const CSV_KEYS_JOB: (keyof JobPostItem)[] = ['id', 'job_title', 'status', 'job_department_id', 'description', 'location', 'experience', 'vacancies', 'created_at'];
function exportJobPostsToCsv(filename: string, rows: JobPostItem[]) {
    ...
    const separator = ',';
    const csvContent = CSV_HEADERS_JOB.join(separator) + '\n' + rows.map((row: any) => CSV_KEYS_JOB.map(k => { let cell: any = row[k]; if (cell === null || cell === undefined) cell = ''; else cell = String(cell).replace(/"/g, '""'); if (String(cell).search(/("|,|\n)/g) >= 0) cell = `"${cell}"`; return cell; }).join(separator)).join('\n');
...
const JobPostsTable = ({
    ...
    onPaginationChange, onSelectChange, onSort,
    onRowSelect, onAllRowSelect
}: JobPostsTableProps) => (
     <DataTable
        selectable
        columns={columns}
...
const JobPostsSelectedFooter = ({ selectedItems, onDeleteSelected, isDeleting }: JobPostsSelectedFooterProps) => {
    ...
    return (
        ...
        >
            <p>Are you sure you want to delete the selected job post{selectedItems.length > 1 ? 's' : ''}?</p>
        </ConfirmDialog>
        </>
    );
};
...
const JobPostsListing = () => {
    ...
    const openEditDrawer = useCallback((item: JobPostItem) => {
        ...
        formMethods.reset({
            ...
            vacancies: typeof item.vacancies === 'string' ? parseInt(item.vacancies, 10) || 0 : Number(item.vacancies) || 0,
 ...
 console.log("data",data);
        ...
        try {
            if (editingItem) {
                await dispatch(editJobPostAction({ id: editingItem.id, ...apiPayload })).unwrap();
    ...
    const handleChangeStatus = useCallback(async (item: JobPostItem) => {
        ...
        const payload = { // Construct full payload as editJobPostAction likely expects it
            id: item.id,
            status: newStatus,
        ...
        try {
            ...
            toast.push(<Notification title="Status Changed" type="success" duration={2000}>{`Job Post status changed to ${newStatus}.`}</Notification>);
            dispatch(getJobPostsAction());
        } catch (error: any) {
    ...
    const handleDeleteSelected = useCallback(async () => { if (selectedItems.length === 0) return; setIsDeleting(true); const validItems = selectedItems.filter(item => item.id); if (validItems.length === 0) { setIsDeleting(false); return; } const idsToDelete = validItems.map(item => String(item.id)); try { await dispatch(deleteAllJobPostsAction({ ids: idsToDelete.join(',') })).unwrap(); toast.push(<Notification title="Deletion Successful" type="success" duration={2000}>{`${validItems.length} job post(s) deleted.`}</Notification>); setSelectedItems([]); dispatch(getJobPostsAction()); } catch (error: any) { toast.push(<Notification title="Deletion Failed" type="danger" duration={3000}>{error.message}</Notification>); console.error("Bulk Delete Error:", error); } finally { setIsDeleting(false); } }, [dispatch, selectedItems]);

    const openFilterDrawer = useCallback(() => { filterFormMethods.reset(filterCriteria); setIsFilterDrawerOpen(true); }, [filterFormMethods, filterCriteria]);
    const closeFilterDrawer = useCallback(() => setIsFilterDrawerOpen(false), []);
    const onApplyFiltersSubmit = useCallback((data: FilterFormData) => { setFilterCriteria({ filterStatus: data.filterStatus || [], filterDepartment: data.filterDepartment || [] }); setTableData(prev => ({ ...prev, pageIndex: 1 })); closeFilterDrawer(); }, [closeFilterDrawer]);
    const onClearFilters = useCallback(() => { const defaultFilters = { filterStatus: [], filterDepartment: [] }; filterFormMethods.reset(defaultFilters); setFilterCriteria(defaultFilters); setTableData(prev => ({ ...prev, pageIndex: 1 })); }, [filterFormMethods]);

    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        const sourceData: JobPostItem[] = Array.isArray(jobPostsData) ? jobPostsData : [];
        ...
        if (filterCriteria.filterDepartment?.length) { const v = filterCriteria.filterDepartment.map(d=>d.value); processedData = processedData.filter(item => v.includes(String(item.job_department_id)));}

        if (tableData.query && tableData.query.trim() !== '') {
            const q = tableData.query.toLowerCase().trim();
            processedData = processedData.filter(j => (j.job_title?.toLowerCase() ?? '').includes(q) || (j.description?.toLowerCase() ?? '').includes(q) || (j.location?.toLowerCase() ?? '').includes(q) || (j.experience?.toLowerCase() ?? '').includes(q) || String(j.id).toLowerCase().includes(q) );
        }
        const { order, key } = tableData.sort as OnSortParam;
        if (order && key) {
            processedData.sort((a, b) => {
                const aVal = a[key as keyof JobPostItem]; const bVal = b[key as keyof JobPostItem];
                ...
                if (key === 'vacancies') { const numA = typeof aVal === 'string' ? parseInt(aVal,10) || 0 : Number(aVal) || 0; const numB = typeof bVal === 'string' ? parseInt(bVal,10) || 0 : Number(bVal) || 0; return order === 'asc' ? numA - numB : numB - numA; }
    ...
    const handleExportData = useCallback(() => { const success = exportJobPostsToCsv('job_posts_export.csv', allFilteredAndSortedData); if (success) toast.push(<Notification title="Export Successful" type="success" duration={2000}>Data exported.</Notification>); }, [allFilteredAndSortedData]);
    ...
    const handleRowSelect = useCallback((checked: boolean, row: JobPostItem) => { setSelectedItems((prev) => { if (checked) return prev.some((item) => item.id === row.id) ? prev : [...prev, row]; return prev.filter((item) => item.id !== row.id); }); }, []);
    ...
    const columns: ColumnDef<JobPostItem>[] = useMemo(
        () => [
            { header: 'Status', accessorKey: 'status', size: 120, enableSorting: true,
              cell: props => { const statusVal = props.getValue<JobPostStatusApi>(); return <Tag className={classNames('capitalize whitespace-nowrap', jobStatusColor[statusVal] || 'bg-gray-100 text-gray-600')}>{statusVal}</Tag> }
            },
            { header: 'Department', accessorKey: 'job_department_id', size: 180, enableSorting: true,
              cell: props => { const deptId = String(props.getValue()); const department = departmentOptions.find(opt => opt.value === deptId); return department ? department.label : `ID: ${deptId}`; }
            },
            { header: 'Description', accessorKey: 'description', enableSorting: false, size: 250,
              cell: props => <Tooltip title={props.getValue<string>()}><span className="block whitespace-nowrap overflow-hidden text-ellipsis max-w-xs">{props.getValue<string>()}</span></Tooltip>
            },
            ...
            { header: 'Vacancies', accessorKey: 'vacancies', size: 100, enableSorting: true, meta: { cellClass: "text-center" } },
            {
    ...
    const renderDrawerForm = (currentFormMethods: typeof formMethods) => (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <FormItem label="Job Title" className="md:col-span-2" invalid={!!currentFormMethods.formState.errors.job_title} errorMessage={currentFormMethods.formState.errors.job_title?.message}>
             ...
             <FormItem label="Job Department" invalid={!!currentFormMethods.formState.errors.job_department_id} errorMessage={currentFormMethods.formState.errors.job_department_id?.message}>
            ...
            <FormItem label="Status" invalid={!!currentFormMethods.formState.errors.status} errorMessage={currentFormMethods.formState.errors.status?.message}>
                <Controller name="status" control={currentFormMethods.control} render={({ field }) => (
                    <Select placeholder="Select Status" options={JOB_POST_STATUS_OPTIONS_FORM}
                            value={JOB_POST_STATUS_OPTIONS_FORM.find(o=>o.value === field.value)}
                            onChange={opt=>field.onChange(opt?.value)} />
                )} />
            </FormItem>
            ...
            <FormItem label="Total Vacancies" invalid={!!currentFormMethods.formState.errors.vacancies} errorMessage={currentFormMethods.formState.errors.vacancies?.message}>
                <Controller name="vacancies" control={currentFormMethods.control} render={({ field }) => <Input {...field} type="number" prefix={<TbUsers/>} min={0} />} />
            </FormItem>
             <FormItem label="Description" className="md:col-span-2" invalid={!!currentFormMethods.formState.errors.description} errorMessage={currentFormMethods.formState.errors.description?.message}>
                <Controller name="description" control={currentFormMethods.control} render={({ field }) => <Textarea {...field} rows={5}  prefix={<TbFileText />} placeholder="Detailed job description, responsibilities, qualifications..." />} />
            </FormItem>
        </div>
    );
    ...
    return (
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        <h3 className="mb-4 sm:mb-0 flex items-center gap-2"><TbBriefcase /> Job Posts</h3>
            ...
            <Drawer
                ...
                width={700}
                footer={ <div className="text-right w-full"> <Button size="sm" className="mr-2" onClick={editingItem ? closeEditDrawer : closeAddDrawer} disabled={isSubmitting} type="button">Cancel</Button> <Button size="sm" variant="solid" form="jobPostForm" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}> {isSubmitting ? (editingItem ? 'Saving...' : 'Adding...') : (editingItem ? 'Save Changes' : 'Add Job Post')} </Button> </div> }
            >
                <Form id="jobPostForm" onSubmit={formMethods.handleSubmit(onSubmitHandler)} className="flex flex-col gap-4">
                    {renderDrawerForm(formMethods)}
                </Form>
            </Drawer>

            <Drawer
                title="Filter Job Posts" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer}
                footer={ <div className="flex justify-between w-full"> <Button size="sm" onClick={onClearFilters} type="button">Clear All</Button> <div> <Button size="sm" className="mr-2" onClick={closeFilterDrawer} type="button">Cancel</Button> <Button size="sm" variant="solid" form="filterJobPostForm" type="submit">Apply Filters</Button> </div> </div> }
            >
                <Form id="filterJobPostForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Filter by Status">
                        <Controller name="filterStatus" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Any Status" options={JOB_POST_STATUS_OPTIONS_FORM.map(s=>({value:s.value, label: s.label}))}
                     ...
                     <FormItem label="Filter by Department">
                        <Controller name="filterDepartment" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Any Department" options={departmentOptions}
                                    value={field.value || []} onChange={val => field.onChange(val || [])} />
                        )} />
                    </FormItem>
                </Form>
            ...
            >
                <p>Are you sure you want to delete the job post "<strong>{itemToDelete?.job_title || itemToDelete?.id}</strong>"?</p>
            </ConfirmDialog>
        </>
    );
};
