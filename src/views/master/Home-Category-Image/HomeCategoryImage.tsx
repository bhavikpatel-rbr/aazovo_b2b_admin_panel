...
// --- End HomeCategorySearch ---

// --- HomeCategoryTableTools Component ---
const HomeCategoryTableTools = ({
    ...
    const {control, handleSubmit} = useForm<HomeCategoryFilterSchema>()

    const exportFiltersSubmitHandler = (data : HomeCategoryFilterSchema) => {
...
const FormListTable = ({
    ...
    selectedForms: HomeCategoryItem[]
...
const FormListSelected = ({
    ...
    selectedForms: HomeCategoryItem[]
    setSelectedForms: React.Dispatch<React.SetStateAction<HomeCategoryItem[]>>
...
const HomeCategoryImage = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<HomeCategoryItem | null>(null);

    const openEditDrawer = (item: HomeCategoryItem) => {
    ...
    const { HomeCategoryImageData = [], status: masterLoadingStatus = 'idle' } =
        useSelector(masterSelector)
 // Use the provided dummy data
 const initialDummyForms: HomeCategoryItem[] = [
    { id: 'F001', name: 'User Registration', date: '12-12-2024', status: 'active' },
    { id: 'F002', name: 'Contact Us', date: '12-12-2024', status: 'active' },
    { id: 'F003', name: 'Product Feedback V1', date: '12-12-2024', status: 'inactive' },
    { id: 'F004', name: 'Support Ticket', date: '12-12-2024', status: 'active' },
    { id: 'F005', name: 'Newsletter Signup', date: '12-12-2024', status: 'active' },
    { id: 'F006', name: 'Job Application', date: '12-12-2024', status: 'inactive' },
    { id: 'F007', name: 'Event RSVP', date: '12-12-2024', status: 'active' },
    { id: 'F008', name: 'Password Reset', date: '12-12-2024', status: 'active' },
    { id: 'F009', name: 'Feature Request', date: '12-12-2024', status: 'inactive' },
    { id: 'F010', name: 'Beta Program Signup', date: '12-12-2024', status: 'active' },
    { id: 'F011', name: 'Onboarding Checklist', date: '12-12-2024', status: 'active' },
    {
        id: 'F012',
...
const [forms, setForms] = useState<HomeCategoryItem[]>(initialDummyForms); // Initialize with dummy data
...
const [selectedForms, setSelectedForms] = useState<HomeCategoryItem[]>([]);
    const [localIsLoading, setLocalIsLoading] = useState(false)
    // filterData state and handleApplyFilter removed

    console.log('Raw CountriesData from Redux:', HomeCategoryImageData)
    ...
    const { pageData, total, processedDataForCsv } = useMemo(() => {
        console.log(
            ...
            HomeCategoryImageData?.length ?? 0,
        )

        const sourceData: HomeCategoryItem[] = Array.isArray(HomeCategoryImageData)
            ? HomeCategoryImageData
            : []
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter((item: HomeCategoryItem) => {
        ...
        if (
            ...
            sortedData.sort((a, b) => {
                ...
                const aValue = String(a[key as keyof HomeCategoryItem] ?? '')
                const bValue = String(b[key as keyof HomeCategoryItem] ?? '')
    ...
    }, [HomeCategoryImageData, tableData]) // filterData removed from dependencies
    ...
    const handleRowSelect = useCallback((checked: boolean, row: HomeCategoryItem) => {
    ...
    const handleDelete = useCallback((formToDelete: HomeCategoryItem) => {
        console.log(
                        ...
                        <Input
                            ...
                            onChange={(e) =>
                                setSelectedItem((prev) => ({
                                    ...(prev || { id: '', name: '', date: '', status: 'active' }),
                                    name: e.target.value,
                                }))
                            }
                        />
