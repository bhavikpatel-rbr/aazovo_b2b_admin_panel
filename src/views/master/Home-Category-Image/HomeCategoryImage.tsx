...
const categoryNameValues = categoryNameOptionsConst.map(opt => opt.value) as [string, ...string[]];
...
const homeCategoryFormSchema = z.object({
    ...
    viewMoreLink: z.string().url('Invalid URL for "View More" link.').optional().or(z.literal('')),
    images: z.array(
        z.object({
            url: z.string().url('Each image must have a valid URL.').min(1, 'Image URL cannot be empty.'),
        })
    ).min(1, 'At least one image is required.').max(MAX_IMAGES_PER_CATEGORY, `You can add up to ${MAX_IMAGES_PER_CATEGORY} images.`),
...
const CSV_HEADERS_HOME_CAT = ['ID', 'Category Name', 'View More Link', 'Date', 'Status', 'Image Count', 'Image URLs'];
const CSV_KEYS_HOME_CAT: (keyof HomeCategoryImageItem | 'imageCount' | 'imageUrlsString')[] = ['id', 'categoryName', 'viewMoreLink', 'date', 'status', 'imageCount', 'imageUrlsString'];

function exportHomeCategoriesToCsv(filename: string, rows: HomeCategoryImageItem[]) {
    if (!rows || !rows.length) {
    ...
    const csvContent =
        ...
        preparedRows.map((row: any) => CSV_KEYS_HOME_CAT.map(k => {
            let cell: any = row[k];
...
const HomeCategorySearch = React.forwardRef<HTMLInputElement, HomeCategorySearchProps>(
    ({ onInputChange }, ref) => (
        <DebouceInput ref={ref} className="w-full" placeholder="Search categories by name..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />
    )
);
...
const HomeCategoryTableTools = ({ onSearchChange, onFilter, onExport }: HomeCategoryTableToolsProps) => (
...
const HomeCategories = () => {
    const [homeCategoriesData, setHomeCategoriesData] = useState<HomeCategoryImageItem[]>(initialDummyHomeCategories);
    const [isAddDrawerOpen, setIsAddDrawerOpen] = useState(false);
    const [isEditDrawerOpen, setIsEditDrawerOpen] = useState(false);
    const [editingHomeCategory, setEditingHomeCategory] = useState<HomeCategoryImageItem | null>(null);
    const [itemToDelete, setItemToDelete] = useState<HomeCategoryImageItem | null>(null);
    ...
    const [filterCriteria, setFilterCriteria] = useState<{ filterStatus?: any[], filterCategoryName?: any[] }>({});
    ...
    const filterForm = useForm<{ filterStatus?: any[], filterCategoryName?: any[] }>({ defaultValues: {} });

    // --- CRUD Handlers ---
    const openAddDrawer = () => {
        formMethods.reset({ // Reset with one empty image field
            categoryName: categoryNameValues[0],
            viewMoreLink: '',
            images: [{ url: '' }],
            date: new Date().toISOString().split('T')[0],
    ...
    const openEditDrawer = (item: HomeCategoryImageItem) => {
        ...
        formMethods.reset({ // Map item data to form data structure
    ...
    const closeEditDrawer = () => { setIsEditDrawerOpen(false); setEditingHomeCategory(null); };
    ...
    const onSubmit = async (data: HomeCategoryFormData) => {
        ...
        try {
            if (editingHomeCategory) { // Edit mode
                const updatedItem: HomeCategoryImageItem = { ...editingHomeCategory, ...data };
                setHomeCategoriesData(prev => prev.map(item => item.id === updatedItem.id ? updatedItem : item));
                toast.push(<Notification title="Home Category Updated" type="success">Category updated.</Notification>);
                closeEditDrawer();
            } else { // Add mode
                const newItem: HomeCategoryImageItem = { ...data, id: `HCAT${Date.now()}` };
                setHomeCategoriesData(prev => [...prev, newItem]);
                toast.push(<Notification title="Home Category Added" type="success">Category added.</Notification>);
                closeAddDrawer();
            }
        } catch (error: any) {
            toast.push(<Notification title={editingHomeCategory ? "Failed to Update" : "Failed to Add"} type="danger">{error?.message || 'Operation failed.'}</Notification>);
        } finally {
    ...
    const onConfirmSingleDelete = async () => {
        ...
        try {
            setHomeCategoriesData(prev => prev.filter(item => item.id !== itemToDelete!.id));
            toast.push(<Notification title="Home Category Deleted" type="success">{`Category "${categoryNameOptionsConst.find(c => c.value === itemToDelete.categoryName)?.label}" deleted.`}</Notification>);
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            setHomeCategoriesData(prev => prev.filter(item => !idsToDelete.includes(item.id)));
    ...
    const onApplyFiltersSubmit = (data: { filterStatus?: any[], filterCategoryName?: any[] }) => {
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (filterCriteria.filterCategoryName && filterCriteria.filterCategoryName.length > 0) {
            const selectedCats = filterCriteria.filterCategoryName.map((opt: any) => opt.value);
            processedData = processedData.filter(item => selectedCats.includes(item.categoryName));
        }

        if (tableData.query && tableData.query.trim() !== '') {
            const query = tableData.query.toLowerCase().trim();
            processedData = processedData.filter(item => {
                const catLabel = categoryNameOptionsConst.find(c=>c.value === item.categoryName)?.label.toLowerCase() || '';
                return catLabel.includes(query) || String(item.id).toLowerCase().includes(query);
            });
        }

        const { order, key } = tableData.sort as OnSortParam;
        if (order && key && ['id', 'categoryName', 'date', 'status'].includes(key)) {
             processedData.sort((a, b) => {
                const aVal = key === 'categoryName' ? (categoryNameOptionsConst.find(c=>c.value === a.categoryName)?.label || '') : a[key as keyof HomeCategoryImageItem];
                const bVal = key === 'categoryName' ? (categoryNameOptionsConst.find(c=>c.value === b.categoryName)?.label || '') : b[key as keyof HomeCategoryImageItem];
    ...
    const handleExportData = () => {
        const success = exportHomeCategoriesToCsv('home_categories.csv', allFilteredAndSortedData);
    ...
    const handleRowSelect = useCallback((checked: boolean, row: HomeCategoryImageItem) => {
        setSelectedItems(prev => {
            if (checked) return prev.some(item => item.id === row.id) ? prev : [...prev, row];
            return prev.filter(item => item.id !== row.id);
        });
    }, []);
    const handleAllRowSelect = useCallback((checked: boolean, currentRows: Row<HomeCategoryImageItem>[]) => {
    ...
    const columns: ColumnDef<HomeCategoryImageItem>[] = useMemo(
        () => [
            ...
            {
                header: 'Category Name', accessorKey: 'categoryName', enableSorting: true, size: 200,
                cell: (props) => {
                    const cat = categoryNameOptionsConst.find(c => c.value === props.row.original.categoryName);
                    return <span className="font-semibold">{cat ? cat.label : props.row.original.categoryName}</span>;
                }
            },
            {
                ...
                cell: (props) => (
                    <div className="flex items-center gap-1">
                        {props.row.original.images.slice(0, 3).map((img, idx) => ( // Show up to 3 previews
                            <Avatar key={idx} size={30} shape="square" src={img.url || undefined} icon={<TbPhoto />} />
                        ))}
                        {props.row.original.images.length > 3 && <span className="text-xs">+{props.row.original.images.length - 3}</span>}
                        {props.row.original.images.length === 0 && <span className="text-xs text-gray-400">No images</span>}
                    </div>
                )
            },
            { header: 'Date', accessorKey: 'date', enableSorting: true, size: 120 },
            {
                ...
                cell: (props) => <Tag className={classNames('rounded-md capitalize font-semibold border-0', statusColorsConst[props.row.original.status])}>{props.row.original.status}</Tag>,
            },
            {
    ...
    const renderDrawerForm = () => (
        <>
            <FormItem label="Category Name" invalid={!!formMethods.formState.errors.categoryName} errorMessage={formMethods.formState.errors.categoryName?.message}>
                <Controller name="categoryName" control={formMethods.control} render={({ field }) => (
                    <Select placeholder="Select category" options={categoryNameOptionsConst} value={categoryNameOptionsConst.find(o => o.value === field.value)} onChange={opt => field.onChange(opt?.value)} />
                )} />
            </FormItem>
            <FormItem label="View More Link (Optional)" invalid={!!formMethods.formState.errors.viewMoreLink} errorMessage={formMethods.formState.errors.viewMoreLink?.message}>
                <Controller name="viewMoreLink" control={formMethods.control} render={({ field }) => (
                    <Input {...field} type="url" placeholder="https://example.com/category/view-all" />
                )} />
            </FormItem>

            <FormItem label={`Images (Up to ${MAX_IMAGES_PER_CATEGORY})`} invalid={!!formMethods.formState.errors.images} errorMessage={ formMethods.formState.errors.images?.message || (formMethods.formState.errors.images as any)?.root?.message}>
                {imageFields.map((field, index) => (
                    <div key={field.id} className="flex items-center gap-2 mb-2">
                        ...
                        {imageFields.length > 1 && ( // Don't allow removing the last one directly, add one if needed
                            <Button size="xs" shape="circle" variant="plain" icon={<TbX />} onClick={() => removeImage(index)} aria-label="Remove image" />
                        )}
                         {formMethods.formState.errors.images?.[index]?.url && (
                            <p className="text-red-500 text-xs mt-1">{formMethods.formState.errors.images?.[index]?.url?.message}</p>
                        )}
                    </div>
                ))}
                {imageFields.length < MAX_IMAGES_PER_CATEGORY && (
                    <Button size="sm" type="button" variant="dashed" onClick={() => appendImage({ url: '' })}>
                        Add Another Image
                    </Button>
                )}
        ...
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        <h3 className="mb-4 sm:mb-0 flex items-center gap-2"><TbCategory /> Manage Home Categories</h3>
                    ...
                    <div className="mt-4">
                        <DataTable
                            ...
                            selectable
                            checkboxChecked={(row: HomeCategoryImageItem) => selectedItems.some(selected => selected.id === row.id)}
                 ...
                 <div className="flex items-center justify-between w-full px-4 sm:px-8">
                    <span className="flex items-center gap-2">
                        ...
                        <span className="font-semibold flex items-center gap-1 text-sm sm:text-base">
                            ...
                            <span>
                                Categor{selectedItems.length > 1 ? 'ies' : 'y'}{' '}
                                selected
                            </span>
                        </span>
                    </span>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button size="sm" variant="solid" form="homeCategoryFormAdd" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>
                            {isSubmitting ? 'Adding...' : 'Add Category'}
                        </Button>
                    </div>
                }
            >
                <Form id="homeCategoryFormAdd" onSubmit={formMethods.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    {renderDrawerForm()}
                </Form>
            </Drawer>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button size="sm" variant="solid" form="homeCategoryFormEdit" type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>
                            {isSubmitting ? 'Saving...' : 'Save Changes'}
                        </Button>
                    </div>
                }
            >
                <Form id="homeCategoryFormEdit" onSubmit={formMethods.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    {renderDrawerForm()}
                </Form>
            </Drawer>
            ...
            >
                <Form id="filterForm" onSubmit={filterForm.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    ...
                    <FormItem label="Filter by Category Name">
                        <Controller name="filterCategoryName" control={filterForm.control} render={({ field }) => (
                            <Select isMulti placeholder="Select category name(s)..." options={categoryNameOptionsConst} value={field.value || []} onChange={val => field.onChange(val || [])} />
                        )} />
                    </FormItem>
                </Form>
            </Drawer>
            ...
            >
                <p>
                    ...
                    <strong>{itemToDelete ? (categoryNameOptionsConst.find(c => c.value === itemToDelete.categoryName)?.label || itemToDelete.categoryName) : ''}</strong>
