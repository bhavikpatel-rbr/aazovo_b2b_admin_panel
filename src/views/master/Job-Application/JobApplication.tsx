...
const ApplicationDetailDialog = ({ isOpen, onClose, application }: { isOpen: boolean; onClose: () => void; application: JobApplicationItem | null }) => { /* ... existing code ... */
    ...
    return (
        <Dialog isOpen={isOpen} onClose={onClose} onRequestClose={onClose} width={700}>
            ...
            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <p><strong>Status:</strong> <Tag className={`${applicationStatusColor[application.status]} text-white capitalize`}>{application.status.replace(/_/g, ' ')}</Tag></p>
                <p><strong>Job Applied For:</strong> {application.jobTitle ?? 'General Application'}</p>
                <p><strong>Job ID:</strong> {application.jobId ?? 'N/A'}</p>
                <p><strong>Department:</strong> {application.department}</p>
                <p><strong>Applicant Name:</strong> {application.name}</p>
                <p><strong>Email:</strong> {application.email}</p>
                <p><strong>Mobile:</strong> {application.mobileNo ?? 'N/A'}</p>
                <p><strong>Work Experience:</strong> {application.workExperience}</p>
                <p><strong>Application Date:</strong> {application.applicationDate.toLocaleString()}</p>
                <p><strong>Job Application Link:</strong> {application.jobApplicationLink ? <a href={application.jobApplicationLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{application.jobApplicationLink}</a> : 'Not Added'}</p>
            </div>
            <div className="mt-4">
                <h6 className="mb-2 font-semibold">Cover Letter / Requirement:</h6>
                <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap bg-gray-50 dark:bg-gray-700 p-3 rounded-md max-h-60 overflow-y-auto">{application.coverLetter ?? application.requirement ?? 'N/A'}</p>
            </div>
            <div className="mt-4">
                <h6 className="mb-2 font-semibold">Notes:</h6>
                <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap bg-gray-50 dark:bg-gray-700 p-3 rounded-md max-h-40 overflow-y-auto">{application.notes ?? 'No notes added.'}</p>
            </div>
            {application.resumeUrl && (<div className="mt-4"><a href={application.resumeUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Resume</a></div>)}
            <div className="text-right mt-6"><Button variant="solid" onClick={onClose}>Close</Button></div>
        </Dialog>
    )
};
const ScheduleInterviewDialog = ({ isOpen, onClose, application }: { isOpen: boolean; onClose: () => void; application: JobApplicationItem | null }) => { /* ... existing code ... */
    ...
    const handleSchedule = () => {
        if (!application || !interviewDate || !interviewer) {
            toast.push(<Notification title="Missing Information" type="warning">Please select date/time and interviewer.</Notification>)
            return
        }
        setIsScheduling(true)
        // Simulate API call
        setTimeout(() => {
            toast.push(<Notification title="Interview Scheduled" type="success">{`Interview scheduled for ${application.name}.`}</Notification>)
            setIsScheduling(false)
            onClose()
        }, 1000)
    }
    React.useEffect(() => { if (!isOpen) { setInterviewDate(null); setInterviewer(''); setNotes(''); } }, [isOpen])
    if (!application) return null
    return (
        <Dialog isOpen={isOpen} onClose={onClose} onRequestClose={onClose} width={500}>
            ...
            <p className="mb-4 text-sm">Applied for: {application.jobTitle ?? 'General Application'}</p>
            <FormContainer>
                <FormItem label="Interview Date & Time" className="mb-4">
                    <DatePicker placeholder="Select date and time" value={interviewDate} onChange={(date) => setInterviewDate(date)} inputFormat="dd/MM/yyyy hh:mm a" inputPrefix={<HiOutlineCalendar className="text-lg" />} showTimeInput />
                </FormItem>
                <FormItem label="Interviewer(s)" className="mb-4"><Input placeholder="Enter interviewer names" value={interviewer} onChange={(e) => setInterviewer(e.target.value)} /></FormItem>
                <FormItem label="Notes (Optional)"><Input textArea placeholder="Any specific notes..." value={notes} onChange={(e) => setNotes(e.target.value)} /></FormItem>
            </FormContainer>
            <div className="text-right mt-6"><Button size="sm" className="mr-2" onClick={onClose} disabled={isScheduling}>Cancel</Button><Button size="sm" variant="solid" onClick={handleSchedule} loading={isScheduling}>Schedule</Button></div>
        </Dialog>
    )
};
const AddJobLinkDialog = ({ isOpen, onClose, application, onLinkSubmit }: { isOpen: boolean; onClose: () => void; application: JobApplicationItem | null; onLinkSubmit: (applicationId: string, link: string) => void }) => { /* ... existing code ... */
    const [jobLink, setJobLink] = useState('')
    const [isSubmittingLink, setIsSubmittingLink] = useState(false) // Renamed to avoid conflict

    const handleSubmitLink = () => { // Renamed to avoid conflict
        if (!application || !jobLink) {
            toast.push(<Notification title="Missing Link" type="warning">Please enter the Job Application Link.</Notification>)
            return
        }
        setIsSubmittingLink(true)
        setTimeout(() => {
            onLinkSubmit(application.id, jobLink)
            toast.push(<Notification title="Link Added" type="success">Job Application Link updated.</Notification>)
            setIsSubmittingLink(false)
            onClose()
        }, 500)
    }
    React.useEffect(() => {
        if (isOpen && application?.jobApplicationLink) setJobLink(application.jobApplicationLink)
    ...
    return (
        <Dialog isOpen={isOpen} onClose={onClose} onRequestClose={onClose} width={500}>
            ...
            <FormContainer>
                <FormItem label="Job Application Link"><Input placeholder="https://example.com/apply/123" value={jobLink} onChange={(e) => setJobLink(e.target.value)} /></FormItem>
            </FormContainer>
            <div className="text-right mt-6"><Button size="sm" className="mr-2" onClick={onClose} disabled={isSubmittingLink}>Cancel</Button><Button size="sm" variant="solid" onClick={handleSubmitLink} loading={isSubmittingLink}>Save Link</Button></div>
        </Dialog>
    )
};
...
const JobApplicationListing = () => {
    ...
    const [currentItem, setCurrentItem] = useState<JobApplicationItem | null>(null)

    // Drawer States
    const [isAddDrawerOpen, setIsAddDrawerOpen] = useState(false);
    const [isEditDrawerOpen, setIsEditDrawerOpen] = useState(false);
    const [isFilterDrawerOpen, setIsFilterDrawerOpen] = useState(false);
    const [editingApplication, setEditingApplication] = useState<JobApplicationItem | null>(null); // For edit drawer
    ...
    const addFormMethods = useForm<ApplicationFormData>({
        ...
        defaultValues: { // Sensible defaults
            name: '', email: '', mobileNo: null, department: '', jobId: null, jobTitle: '',
            workExperience: '', applicationDate: new Date(), status: 'new',
            resumeUrl: null, coverLetter: null, notes: null, jobApplicationLink: null,
        },
        mode: 'onChange',
    });
    ...
    // --- Handlers ---
    const handleSetTableData = useCallback((data: Partial<TableQueries>) => { // Made data partial
    ...
    const handleAllRowSelect = useCallback(
        (checked: boolean, currentRows: Row<JobApplicationItem>[]) => { // Use currentRows from DataTable
    ...
    const handleActualDelete = useCallback( // Renamed from handleDelete for clarity
    ...
    // Drawer Handlers
    const openAddDrawer = () => {
        addFormMethods.reset({ // Reset with default or empty values
            name: '', email: '', mobileNo: null, department: '', jobId: null, jobTitle: '',
            workExperience: '', applicationDate: new Date(), status: 'new',
            resumeUrl: null, coverLetter: null, notes: null, jobApplicationLink: null,
        });
    ...
    const onAddApplicationSubmit = (data: ApplicationFormData) => {
        ...
        toast.push(<Notification title="Application Added" type="success" duration={2000}>Application for {data.name} added.</Notification>);
    ...
    const onEditApplicationSubmit = (data: ApplicationFormData) => {
        ...
        setApplications(prev => prev.map(app => app.id === editingApplication.id ? updatedApplication : app));
        toast.push(<Notification title="Application Updated" type="success" duration={2000}>Application for {data.name} updated.</Notification>);
        setIsSubmittingDrawer(false);
        closeEditDrawer();
    };
    ...
    const onClearFilters = () => {
        const defaultFilters: FilterFormData = { filterStatus: [], filterDepartment: [] };
    ...
    return (
        <Container className="h-full">
            ...
            <ApplicationDetailDialog isOpen={detailViewOpen} onClose={handleCloseDetailView} application={currentItem} />
            <ScheduleInterviewDialog isOpen={scheduleInterviewOpen} onClose={handleCloseScheduleInterview} application={currentItem} />
            <AddJobLinkDialog isOpen={addJobLinkOpen} onClose={handleCloseAddJobLink} application={currentItem} onLinkSubmit={handleSubmitJobLink} />
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        <Button size="sm" className="mr-2" onClick={closeAddDrawer} disabled={isSubmittingDrawer}>Cancel</Button>
                        <Button size="sm" variant="solid" form="addApplicationForm" type="submit" loading={isSubmittingDrawer} disabled={!addFormMethods.formState.isValid || isSubmittingDrawer}>
                            {isSubmittingDrawer ? 'Adding...' : 'Add Application'}
                        </Button>
                    </div>
                }
            >
                <Form id="addApplicationForm" onSubmit={addFormMethods.handleSubmit(onAddApplicationSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Applicant Name" invalid={!!addFormMethods.formState.errors.name} errorMessage={addFormMethods.formState.errors.name?.message}>
                        <Controller name="name" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="Enter applicant's full name" />} />
                    </FormItem>
                    <FormItem label="Email" invalid={!!addFormMethods.formState.errors.email} errorMessage={addFormMethods.formState.errors.email?.message}>
                        <Controller name="email" control={addFormMethods.control} render={({ field }) => <Input {...field} type="email" placeholder="applicant@example.com" />} />
                    </FormItem>
                    <FormItem label="Mobile No (Optional)" invalid={!!addFormMethods.formState.errors.mobileNo} errorMessage={addFormMethods.formState.errors.mobileNo?.message}>
                        <Controller name="mobileNo" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="+1-555-1234" />} />
                    </FormItem>
                    <FormItem label="Department" invalid={!!addFormMethods.formState.errors.department} errorMessage={addFormMethods.formState.errors.department?.message}>
                        <Controller name="department" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="e.g., Engineering, Marketing" />} />
                    </FormItem>
                     <FormItem label="Job Title (Optional if general)" invalid={!!addFormMethods.formState.errors.jobTitle} errorMessage={addFormMethods.formState.errors.jobTitle?.message}>
                        <Controller name="jobTitle" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="e.g., Software Engineer" />} />
                    </FormItem>
                    <FormItem label="Job ID (Optional)" invalid={!!addFormMethods.formState.errors.jobId} errorMessage={addFormMethods.formState.errors.jobId?.message}>
                        <Controller name="jobId" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="e.g., JP001" />} />
                    </FormItem>
                    <FormItem label="Work Experience" invalid={!!addFormMethods.formState.errors.workExperience} errorMessage={addFormMethods.formState.errors.workExperience?.message}>
                        <Controller name="workExperience" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="e.g., 3 Years in React, None" />} />
                    </FormItem>
                    <FormItem label="Application Date" invalid={!!addFormMethods.formState.errors.applicationDate} errorMessage={addFormMethods.formState.errors.applicationDate?.message as string}>
                        <Controller name="applicationDate" control={addFormMethods.control} render={({ field }) => <DatePicker {...field} placeholder="Select date" inputFormat="dd/MM/yyyy" />} />
                    </FormItem>
                    <FormItem label="Status" invalid={!!addFormMethods.formState.errors.status} errorMessage={addFormMethods.formState.errors.status?.message}>
                        <Controller name="status" control={addFormMethods.control} render={({ field }) => (
                            <Select placeholder="Select status" options={applicationStatusOptions} value={applicationStatusOptions.find(opt => opt.value === field.value)} onChange={opt => field.onChange(opt?.value)} />
                        )} />
                    </FormItem>
                    <FormItem label="Resume URL (Optional)" invalid={!!addFormMethods.formState.errors.resumeUrl} errorMessage={addFormMethods.formState.errors.resumeUrl?.message}>
                        <Controller name="resumeUrl" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="https://example.com/resume.pdf" />} />
                    </FormItem>
                    <FormItem label="Job Application Link (Optional)" invalid={!!addFormMethods.formState.errors.jobApplicationLink} errorMessage={addFormMethods.formState.errors.jobApplicationLink?.message}>
                        <Controller name="jobApplicationLink" control={addFormMethods.control} render={({ field }) => <Input {...field} placeholder="https://platform.com/apply/xyz" />} />
                    </FormItem>
                    <FormItem label="Cover Letter / Notes (Optional)" invalid={!!addFormMethods.formState.errors.coverLetter} errorMessage={addFormMethods.formState.errors.coverLetter?.message}>
                        <Controller name="coverLetter" control={addFormMethods.control} render={({ field }) => <Input {...field} textArea rows={3} placeholder="Enter cover letter or additional notes" />} />
                    </FormItem>
                </Form>
            </Drawer>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        <Button size="sm" className="mr-2" onClick={closeEditDrawer} disabled={isSubmittingDrawer}>Cancel</Button>
                        <Button size="sm" variant="solid" form="editApplicationForm" type="submit" loading={isSubmittingDrawer} disabled={!editFormMethods.formState.isValid || isSubmittingDrawer}>
            ...
            >
                {editingApplication && (
                    <Form id="editApplicationForm" onSubmit={editFormMethods.handleSubmit(onEditApplicationSubmit)} className="flex flex-col gap-4">
                        <FormItem label="Applicant Name" invalid={!!editFormMethods.formState.errors.name} errorMessage={editFormMethods.formState.errors.name?.message}>
                            <Controller name="name" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Email" invalid={!!editFormMethods.formState.errors.email} errorMessage={editFormMethods.formState.errors.email?.message}>
                            <Controller name="email" control={editFormMethods.control} render={({ field }) => <Input {...field} type="email" />} />
                        </FormItem>
                        <FormItem label="Mobile No (Optional)" invalid={!!editFormMethods.formState.errors.mobileNo} errorMessage={editFormMethods.formState.errors.mobileNo?.message}>
                            <Controller name="mobileNo" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Department" invalid={!!editFormMethods.formState.errors.department} errorMessage={editFormMethods.formState.errors.department?.message}>
                            <Controller name="department" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Job Title (Optional)" invalid={!!editFormMethods.formState.errors.jobTitle} errorMessage={editFormMethods.formState.errors.jobTitle?.message}>
                            <Controller name="jobTitle" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Job ID (Optional)" invalid={!!editFormMethods.formState.errors.jobId} errorMessage={editFormMethods.formState.errors.jobId?.message}>
                            <Controller name="jobId" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Work Experience" invalid={!!editFormMethods.formState.errors.workExperience} errorMessage={editFormMethods.formState.errors.workExperience?.message}>
                            <Controller name="workExperience" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Application Date" invalid={!!editFormMethods.formState.errors.applicationDate} errorMessage={editFormMethods.formState.errors.applicationDate?.message as string}>
                            <Controller name="applicationDate" control={editFormMethods.control} render={({ field }) => <DatePicker {...field} inputFormat="dd/MM/yyyy" />} />
                        </FormItem>
                        ...
                        <FormItem label="Resume URL (Optional)" invalid={!!editFormMethods.formState.errors.resumeUrl} errorMessage={editFormMethods.formState.errors.resumeUrl?.message}>
                            <Controller name="resumeUrl" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                         <FormItem label="Job Application Link (Optional)" invalid={!!editFormMethods.formState.errors.jobApplicationLink} errorMessage={editFormMethods.formState.errors.jobApplicationLink?.message}>
                            <Controller name="jobApplicationLink" control={editFormMethods.control} render={({ field }) => <Input {...field} />} />
                        </FormItem>
                        <FormItem label="Cover Letter / Notes (Optional)" invalid={!!editFormMethods.formState.errors.coverLetter} errorMessage={editFormMethods.formState.errors.coverLetter?.message}>
                            <Controller name="coverLetter" control={editFormMethods.control} render={({ field }) => <Input {...field} textArea rows={3} />} />
                        </FormItem>
                    </Form>
                )}
            </Drawer>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        <Button size="sm" className="mr-2" onClick={onClearFilters}>Clear All</Button>
                        <Button size="sm" variant="solid" form="filterApplicationForm" type="submit">Apply Filters</Button>
                    </div>
                }
            >
                <Form id="filterApplicationForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Filter by Status">
                        <Controller name="filterStatus" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Select status..." options={applicationStatusOptions} value={field.value || []} onChange={val => field.onChange(val || [])} />
                        )} />
                    </FormItem>
                    <FormItem label="Filter by Department">
                        <Controller name="filterDepartment" control={filterFormMethods.control} render={({ field }) => (
                            <Select isMulti placeholder="Select departments..." options={departmentOptionsForFilter} value={field.value || []} onChange={val => field.onChange(val || [])} />
                        )} />
                    </FormItem>
