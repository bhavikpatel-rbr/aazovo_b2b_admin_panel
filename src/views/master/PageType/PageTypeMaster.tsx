...
const memberTypeFormSchema = z.object({ name: z.string().min(1, 'Page type name is required.').max(100, 'Name cannot exceed 100 characters.'), status: z.enum(['Active', 'Inactive'], { required_error: 'Status is required.' }), });
...
function exportToCsvMemberType(filename: string, rows: MemberTypeItem[]) {
    ...
    const CSV_HEADERS = ["ID", "Page Type Name", "Status", "Updated By", "Updated Role", "Updated At"];
    const preparedRows = (rows || []).map(row => ({ id: row.id, name: row.name, status: row.status, updated_by_name: row.updated_by_user?.name || " ", updated_by_role: row.updated_by_user?.roles?.[0]?.display_name || " ", updated_at: row.updated_at ? new Date(row.updated_at).toLocaleString() : " ", }));
        ...
        <div className="md:flex items-center justify-between w-full gap-2">
            ...
            <Drawer title="Filters" isOpen={isFilterDrawerOpen} onClose={() => setIsFilterDrawerOpen(false)} footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={onDrawerClear}>Clear</Button><Button size="sm" variant="solid" type="submit" form="filterMemberTypeForm">Apply</Button></div>}>
                <Form id="filterMemberTypeForm" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    <FormItem label="Page Type Name"><Controller name="names" control={control} render={({ field }) => (<Select isMulti placeholder="Select names..." options={memberNameOptions} value={memberNameOptions.filter(o => (field.value || []).includes(o.value))} onChange={(val) => field.onChange((val || []).map(v => v.value))} />)} /></FormItem>
...
const MemberTypeMaster = () => {
    ...
    const { PageData: MemberTypeDatas = [] } = useSelector(masterSelector, shallowEqual);
    const MemberTypeData = MemberTypeDatas?.data || []

    console.log(MemberTypeData, "MemberTypeData");
    ...
    const columns: ColumnDef<MemberTypeItem>[] = useMemo(() => [
        { header: "Page Type", accessorKey: "name", enableSorting: true, size: 360 },
        {
            header: "Updated Info",
    ...
    const onAddMemberTypeSubmit = async (data: MemberTypeFormData) => {
        ...
        try {
            ...
            toast.push(<Notification title="Failed to Add Page Type" type="danger">{error.message || "An unexpected error occurred."}</Notification>);
        } finally {
    ...
    const onEditMemberTypeSubmit = async (data: MemberTypeFormData) => {
        ...
        try {
            ...
            toast.push(<Notification title="Failed to Update Page Type" type="danger">{error.message || "An unexpected error occurred."}</Notification>);
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const onConfirmSingleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            toast.push(<Notification title="Page Type Deleted" type="success">{`"${typeToDelete.name}" was successfully deleted.`}</Notification>);
            refreshData();
        } catch (error: any) {
            toast.push(<Notification title="Failed to Delete Page Type" type="danger">{error.message || "An unexpected error occurred."}</Notification>);
        } finally {
            setIsDeleting(false);
    ...
    const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
        ...
        try {
            await dispatch(submitExportReasonAction({ reason: data.reason, module: "Page Type", file_name: fileName })).unwrap();
        ...
        <>
            <Container className="h-auto">
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    <div className="lg:flex items-center justify-between mb-4">
                        <h3 className="mb-4 lg:mb-0">Page Types</h3>
                        <Button variant="solid" icon={<TbPlus />} onClick={openAddDrawer} className="w-full sm:w-auto mt-2 sm:mt-0">Add Page Type</Button>
                    </div>
            ...
            <Drawer title={isEditDrawerOpen ? "Edit Page Type" : "Add Page Type"} isOpen={isAddDrawerOpen || isEditDrawerOpen} onClose={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} onRequestClose={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} width={480} bodyClass="relative" footer={<div className="text-right w-full"><Button size="sm" className="mr-2" onClick={isEditDrawerOpen ? closeEditDrawer : closeAddDrawer} disabled={isSubmitting}>Cancel</Button><Button size="sm" variant="solid" form={isEditDrawerOpen ? "editMemberTypeForm" : "addMemberTypeForm"} type="submit" loading={isSubmitting} disabled={!formMethods.formState.isValid || isSubmitting}>{isSubmitting ? (isEditDrawerOpen ? "Saving..." : "Adding...") : "Save"}</Button></div>}>
                <Form id={isEditDrawerOpen ? "editMemberTypeForm" : "addMemberTypeForm"} onSubmit={formMethods.handleSubmit(isEditDrawerOpen ? onEditMemberTypeSubmit : onAddMemberTypeSubmit)} className="flex flex-col gap-4">
                    <FormItem label={<div>Page Type Name <span className="text-red-500">*</span></div>} invalid={!!formMethods.formState.errors.name} errorMessage={formMethods.formState.errors.name?.message}><Controller name="name" control={formMethods.control} render={({ field }) => (<Input {...field} placeholder="Enter Page Type Name" />)} /></FormItem>
                    ...
                    <div className=" grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3">
                        <div>
                            ...
                            <p className="text-sm font-semibold">
                                {editingMemberType.updated_by_user?.name || " "}
                            </p>
                            <p>
