...
// --- TrendingImageSearch Component ---
type TrendingImageSearchProps = {
    // Renamed component
    onInputChange: (value: string) => void
    ref?: Ref<HTMLInputElement>
}
const TrendingImageSearch = React.forwardRef<
    HTMLInputElement,
    TrendingImageSearchProps
>(({ onInputChange }, ref) => {
    return (
        <DebouceInput
            ref={ref}
...
TrendingImageSearch.displayName = 'TrendingImageSearch'
// --- End TrendingImageSearch ---

// --- TrendingImageTableTools Component ---
const TrendingImageTableTools = ({
    // Renamed component
    onSearchChange,
}: {
    ...
    type TrendingImageFilterSchema = {
        userRole : String,
        exportFrom : String,
        exportDate : Date
    }
    ...
    const {control, handleSubmit} = useForm<TrendingImageFilterSchema>()

    const exportFiltersSubmitHandler = (data : TrendingImageFilterSchema) => {
    ...
    return (
        <div className="flex items-center w-full gap-2">
            <div className="flex-grow">
                <TrendingImageSearch onInputChange={onSearchChange} />
            </div>
...
const FormListTable = ({
    ...
    columns: ColumnDef<TrendingImageItem>[]
    data: TrendingImageItem[]
    loading: boolean
    pagingData: { total: number; pageIndex: number; pageSize: number }
    selectedForms: TrendingImageItem[]
    onPaginationChange: (page: number) => void
    onSelectChange: (value: number) => void
    onSort: (sort: OnSortParam) => void
    onRowSelect: (checked: boolean, row: TrendingImageItem) => void
    onAllRowSelect: (checked: boolean, rows: Row<TrendingImageItem>[]) => void
}) => {
    return (
        <DataTable
            selectable
...
const FormListActionTools = ({
    ...
    allFormsData: TrendingImageItem[];
...
const FormListSelected = ({
    ...
    selectedForms: TrendingImageItem[]
    setSelectedForms: React.Dispatch<React.SetStateAction<TrendingImageItem[]>>
...
const TrendingImage = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<TrendingImageItem | null>(null);

    const openEditDrawer = (item: TrendingImageItem) => {
    ...
    const { TrendingImageData = [], status: masterLoadingStatus = 'idle' } =
        useSelector(masterSelector)
 // Use the provided dummy data
// --- Initial Dummy Data ---
const initialDummyForms: TrendingImageItem[] = [
...
const [forms, setForms] = useState<TrendingImageItem[]>(initialDummyForms); // Initialize with dummy data
...
const [selectedForms, setSelectedForms] = useState<TrendingImageItem[]>([]);
    const [localIsLoading, setLocalIsLoading] = useState(false)
    // filterData state and handleApplyFilter removed

    console.log('Raw CountriesData from Redux:', TrendingImageData)
    ...
    const { pageData, total, processedDataForCsv } = useMemo(() => {
        console.log(
            ...
            TrendingImageData?.length ?? 0,
        )

        const sourceData: TrendingImageItem[] = Array.isArray(TrendingImageData)
            ? TrendingImageData
            : []
        let processedData: TrendingImageItem[] = cloneDeep(sourceData)
        ...
        if (tableData.query && tableData.query.trim() !== '') {
            ...
            processedData = processedData.filter((item: TrendingImageItem) => {
                const itemNameLower = item.pageName?.trim().toLowerCase() ?? ''
                const continentNameLower = item.pageName?.trim().toLowerCase() ?? ''
        ...
        if (
            ...
            sortedData.sort((a, b) => {
                ...
                const aValue = String(a[key as keyof TrendingImageItem] ?? '')
                const bValue = String(b[key as keyof TrendingImageItem] ?? '')
    ...
    }, [TrendingImageData, tableData]) // filterData removed from dependencies
    ...
    const handleRowSelect = useCallback((checked: boolean, row: TrendingImageItem) => {
    ...
    const handleAllRowSelect = useCallback(
        (checked: boolean, currentRows: Row<TrendingImageItem>[]) => {
    ...
    const handleDelete = useCallback((formToDelete: TrendingImageItem) => {
        console.log(
            'Delete item (needs Redux action):',
            formToDelete.id,
            formToDelete.pageName,
        )
        setSelectedForms((prevSelected) =>
        ...
        toast.push(
            <Notification title="Delete Action" type="warning">
                Delete action for "{formToDelete.pageName}" (ID: {formToDelete.id}).
    ...
    const columns: ColumnDef<TrendingImageItem>[] = useMemo(
        () => [
            {
                header: 'ID',
                accessorKey: 'id',
            ...
            },
            {
                ...
                accessorKey: 'name',
                enableSorting: true,
                cell: (props) => <span>{props.row.original.pageName}</span>,
            },
            {
                header: 'Date',
                accessorKey: 'date',
            ...
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full">
                    <div className="lg:flex items-center justify-between mb-4">
                        <h5 className="mb-4 lg:mb-0">Trending Images</h5>
                        <FormListActionTools
                            allFormsData={forms}
            ...
            >
                <Form>
                    <FormItem label="Page Name">
                        <Input
                            value={selectedItem?.pageName || ''}
                            onChange={(e) =>
                                setSelectedItem((prev) => ({
                                    ...(prev || { id: '', pageName: '', date: '', status: 'active' }),
                    ...
                    footer={
                        <div className="text-right w-full">
                            ...
                            <Button
                                size="sm"
                                variant="solid"
                                onClick={() => console.log('Document Added')}
                            >
                                Add
                            </Button>
                        </div>
                ...
                >
                    <Form
                        onSubmit={(e) => {
                            ...
                            const newItem: TrendingImageItem = {
                                id: `${forms.length + 1}`,
                                pageName: formData.get('pageName') as string,
                                date: formData.get('date') as string,
                                status: formData.get('status') as 'active' | 'inactive',
                            };
                            console.log('New Document:', newItem);
                            // handleAdd(newItem);
                        }}
                    >
                        <FormItem label="Page Name">
                            <Input name="pageName" placeholder="Enter Page Name" />
                        </FormItem>
                        <FormItem label="Date">
