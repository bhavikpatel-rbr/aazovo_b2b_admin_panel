...
const Currency = () => {
  ...
  */ // --- End Delete Logic ---
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (tableData.query) {
            ...
            processedData = processedData.filter(
                (item) =>
                    (item.currency_code?.trim().toLowerCase() ?? '').includes(
                        query,
                    ) ||
                    (item.currency_symbol?.trim().toLowerCase() ?? '').includes(
                        query,
                    ) ||
    ...
    const handleConfirmExportWithReason = async (
        ...
        const fileName = `currencies_export_${timestamp}.csv`
        try {
            await dispatch(
                            ...
                            <div className="grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-semibold">
                                        {editingCurrency.updated_by_user
                                            ?.name || 'N/A'}
                                    </p>
                                    <p>
                                        {editingCurrency.updated_by_user
