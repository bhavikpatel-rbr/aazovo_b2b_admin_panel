...
const CSV_CAROUSEL_HEADERS = ['ID', 'Image Path (Server)', 'Link', 'Date Created', 'Image Full URL'];
const CSV_CAROUSEL_KEYS: (keyof TrendingCarouselItemData | string)[] = ['id', 'images', 'links', 'created_at', 'images_full_path'];
...
function exportCarouselItemsToCsv(filename: string, rows: TrendingCarouselItemData[]) {
    ...
    const csvContent =
        ...
        rows.map(row => CSV_CAROUSEL_KEYS.map(k => {
...
const ActionColumn = ({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) => {
    ...
    return (
        <div className="flex items-center justify-center gap-3">
            <Tooltip title="Edit"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400')} role="button" tabIndex={0} onClick={onEdit} onKeyDown={(e) => e.key === 'Enter' && onEdit()}><TbPencil /></div></Tooltip>
            <Tooltip title="Delete"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-600 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400')} role="button" tabIndex={0} onClick={onDelete} onKeyDown={(e) => e.key === 'Enter' && onDelete()}><TbTrash /></div></Tooltip>
        </div>
    );
};
...
type ItemTableToolsProps = { onSearchChange: (query: string) => void; onExport: () => void; searchPlaceholder: string }
const ItemTableTools = ({ onSearchChange, onExport, searchPlaceholder }: ItemTableToolsProps) => (
...
const TrendingCarouselSelectedFooter = ({ selectedItems, onDeleteSelected, disabled }: TrendingCarouselSelectedFooterProps) => {
    ...
    return (
        <>
            ...
            >
                <div className="flex items-center justify-between w-full px-4 sm:px-8"> {/* Ensure full width and padding */}
            ...
            <ConfirmDialog
                ...
                type="danger"
                title={`Delete ${selectedItems.length} Carousel Item${selectedItems.length > 1 ? 's' : ''}`}
...
console.log("trendingCarouselData",trendingCarouselData);
    ...
    useEffect(() => {
        return () => { // Cleanup image preview URL on unmount or when URL changes
    ...
    const openEditDrawer = useCallback((item: TrendingCarouselItemData) => {
        setEditingItem(item);
    ...
    const handleDeleteClick = useCallback((item: TrendingCarouselItemData) => {
        if (item.id === undefined || item.id === null) { // ID check
    ...
    const handleDeleteSelected = async () => {
        ...
        if (validItemsToDelete.length === 0) {
            toast.push(<Notification title="No Valid Items" type="info">No valid items to delete (all selected items were missing IDs).</Notification>);
            setIsDeleting(false);
            return;
        }
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (order && key && processedData.length > 0 && processedData[0].hasOwnProperty(key)) {
            processedData.sort((a, b) => {
                ...
                if (key === 'created_at' || key === 'updated_at') { // Date comparison
    ...
    const handleExportData = () => {
        const success = exportCarouselItemsToCsv('trending_carousel_items.csv', allFilteredAndSortedData);
        if (success) toast.push(<Notification title="Export Successful" type="success" duration={2000}>Data exported to CSV.</Notification>);
    };
    ...
    const handleAllRowSelect = useCallback((checked: boolean, currentRows: Row<TrendingCarouselItemData>[]) => {
        ...
        if (checked) {
            setSelectedItems(prev => {
                ...
                const newToAdd = originals.filter(r => r.id !== undefined && !prevIds.has(r.id)); // Ensure ID exists
                return [...prev, ...newToAdd];
            });
        } else {
            const currentIds = new Set(originals.map(r => r.id).filter(id => id !== undefined)); // Ensure IDs exist
    ...
    const columns: ColumnDef<TrendingCarouselItemData>[] = useMemo(() => [
        ...
        },
        {
            ...
            cell: (props) =>
                ...
                <a href={props.row.original.links} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 hover:underline truncate block max-w-xs">
                    {props.row.original.links}
                </a> : <span className="text-gray-500">No Link</span>,
        },
        {
            header: 'Date Created',
    ...
    ], [openEditDrawer, handleDeleteClick]); // Dependencies for callbacks
    ...
    return (
        <>
            ...
            >
                 <Form id="addCarouselItemForm" onSubmit={formMethods.handleSubmit(onAddItemSubmit)} className="flex flex-col gap-y-6"> {/* Use gap-y for vertical spacing */}
                    <FormItem
                        label="Image"
                    ...
                    >
                        ...
                        />
                         {imagePreviewUrl && (
                            <Avatar src={imagePreviewUrl} size={100} shape="square" className="mt-2 border border-gray-200 dark:border-gray-600" />
                        )}
                ...
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button
                            ...
                            disabled={
                                isSubmitting ||
                                (!formMethods.formState.isDirty && !imagePreviewUrl && !formMethods.formState.errors.imageFile) || // Disable if no changes and no new image unless there's an image file error
            ...
            >
                <Form id="editCarouselItemForm" onSubmit={formMethods.handleSubmit(onEditItemSubmit)} className="flex flex-col gap-y-6">
                    <FormItem label="Current Image">
                        {editingItem?.images_full_path ? (
                             <Avatar src={editingItem.images_full_path} size={100} shape="square" className="mt-1 border border-gray-200 dark:border-gray-600" />
                        ) : (
            ...
            >
                 <p>
                    ...
                    {itemToDelete && ` (ID: ${itemToDelete.id})`} This action cannot be undone.
                </p>
            </ConfirmDialog>
        </>
    );
