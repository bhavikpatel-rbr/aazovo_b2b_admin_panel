...
const PaymentTerms = () => {
  ...
  */
    ...
    const paymentTermNameOptions = useMemo(() => {
        ...
        const uniqueNames = new Set(
            PaymentTermsData.map((term) => term.term_name),
        )
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (tableData.query) {
            ...
            processedData = processedData.filter(
                (item) =>
                    (item.term_name?.trim().toLowerCase() ?? '').includes(
                        query,
                    ) ||
  ...
  */
    ...
    const isLoading =
        masterLoadingStatus === 'pending' || isSubmitting || isDeleting
                                ...
                                <div className="grid grid-cols-2 text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded mt-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-semibold">
                                            {editingPaymentTerm.updated_by_user
                                                ?.name || 'N/A'}
                                        </p>
                                        <p>
                                            {editingPaymentTerm.updated_by_user
