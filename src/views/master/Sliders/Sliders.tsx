...
import { getSlidersAction , addSliderAction,
    editSliderAction,
    deleteSliderAction,
...
const sliderFormSchema = z.object({
    ...
    slider_color: z.string().regex(/^#([0-9A-Fa-f]{3,4}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$|^[a-zA-Z]+$/, 'Invalid color (hex/name), e.g., #FF0000 or red').optional().nullable().or(z.literal('')),
})
...
const SLIDER_IMAGE_BASE_URL = import.meta.env.VITE_API_URL_STORAGE || 'https://your-api-domain.com/storage/';
...
const CSV_HEADERS_SLIDER = ['ID', /*'Title',*/ 'Subtitle', 'Button Text', 'Image URL', 'Display Page', 'Link', 'Source', 'Status', 'Domain IDs', 'Slider Color', 'Created At', 'Updated At'] // Removed 'Title'
type SliderCsvItem = Omit<SliderItem, 'image' | 'title'> & { imageUrl: string | null }; // Removed 'title'
...
function exportToCsvSlider(filename: string, rows: SliderItem[]) {
    ...
    const transformedRows: SliderCsvItem[] = rows.map(item => ({
        id: item.id, /*title: item.title,*/ subtitle: item.subtitle, buttonText: item.buttonText, // Removed title
        imageUrl: item.imageFullPath, displayPage: item.displayPage, link: item.link,
        source: item.source, status: item.status, domainIds: item.domainIds, sliderColor: item.sliderColor,
        createdAt: new Date(item.createdAt).toLocaleString(),
        updatedAt: new Date(item.updatedAt).toLocaleString(),
    }));
    const csvKeys: (keyof SliderCsvItem)[] = ['id', /*'title',*/ 'subtitle', 'buttonText', 'imageUrl', 'displayPage', 'link', 'source', 'status', 'domainIds', 'sliderColor', 'createdAt', 'updatedAt']; // Removed 'title'
    const separator = ',';
    const csvContent = CSV_HEADERS_SLIDER.join(separator) + '\n' + transformedRows.map(row =>
        csvKeys.map(k => {
            let cellValue = row[k];
...
const ActionColumn = ({ onEdit, onClone, onChangeStatus, onDelete }: {
    ...
    return (
        <div className="flex items-center justify-center gap-2">
            <Tooltip title="Clone Slider"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400')} role="button" tabIndex={0} onClick={onClone}><TbCopy /></div></Tooltip>
            ...
            <Tooltip title="Edit Slider"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400')} role="button" tabIndex={0} onClick={onEdit}><TbPencil /></div></Tooltip>
            <Tooltip title="Delete Slider"><div className={classNames(iconButtonClass, hoverBgClass, 'text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400')} role="button" tabIndex={0} onClick={onDelete}><TbTrash /></div></Tooltip>
        </div>
    );
};
...
const SlidersSearch = React.forwardRef<HTMLInputElement, SlidersSearchProps>(
    ({ onInputChange }, ref) => (<DebouceInput ref={ref} className="w-full" placeholder="Search sliders (title, id)..." suffix={<TbSearch className="text-lg" />} onChange={(e) => onInputChange(e.target.value)} />)
);
SlidersSearch.displayName = 'SlidersSearch';

// --- SlidersTableTools Component ---
const SlidersTableTools = ({ onSearchChange, onFilter, onExport, onImport }: {
...
type SlidersTableProps = {
    columns: ColumnDef<SliderItem>[]; data: SliderItem[]; loading: boolean;
    pagingData: { total: number; pageIndex: number; pageSize: number }; selectedItems: SliderItem[];
    onPaginationChange: (page: number) => void; onSelectChange: (value: number) => void; onSort: (sort: OnSortParam) => void;
    onRowSelect: (checked: boolean, row: SliderItem) => void; onAllRowSelect: (checked: boolean, rows: Row<SliderItem>[]) => void;
}
...
type SlidersSelectedFooterProps = { selectedItems: SliderItem[]; onDeleteSelected: () => void; }
const SlidersSelectedFooter = ({ selectedItems, onDeleteSelected }: SlidersSelectedFooterProps) => {
    ...
    return (
        <>
            ...
            <ConfirmDialog isOpen={deleteOpen} type="danger" title={`Delete ${selectedItems.length} Slider${selectedItems.length > 1 ? 's' : ''}`}
...
const Sliders = () => {
    ...
    const [sliderForStatusChange, setSliderForStatusChange] = useState<SliderItem | null>(null);

    const [importDialogOpen, setImportDialogOpen] = useState(false);
    const [filterCriteria, setFilterCriteria] = useState<FilterFormData>({
        // filterTitles: [], // Removed
        filterStatuses: [], filterDisplayPages: [], filterSources: [],
    });
    ...
    const defaultSliderFormValues: SliderFormData = {
        title: '', subtitle: null, button_text: null, image: null,
        display_page: displayPageOptionsConst[0]?.value || '',
        link: null, source: sourceOptionsConst[0]?.value || 'both', status: 'Active',
        domain_ids: null, slider_color: '#FFFFFF',
    };
    ...
    const onConfirmChangeStatus = async () => {
        ...
        try {
            ...
            // dispatch(getSlidersAction());
            toast.push(<Notification title="Status Update (Mock)" type="info" duration={2000}>Status update for "{sliderForStatusChange.title}" (functionality to be implemented).</Notification>);
        } catch (error: any) {
            toast.push(<Notification title="Status Update Failed" type="danger">{error.message || 'Could not update status.'}</Notification>);
        } finally {
    ...
    const handleClone = (slider: SliderItem) => {
        ...
        setAddFormPreviewUrl(null);
        setIsAddDrawerOpen(true);
        toast.push(<Notification title="Clone Slider" type="info">Cloning "{slider.title}". Review, upload image, and save.</Notification>);
    };
    ...
    const onClearFilters = () => {
        const defaultFilters = { /*filterTitles: [],*/ filterStatuses: [], filterDisplayPages: [], filterSources: [] }; // Removed
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
        ...
        if (order && key) {
            processedData.sort((a, b) => {
                ...
                else if (typeof aValue === 'number' && typeof bValue === 'number') { /* default sort */ }
    ...
    const handleExportData = () => { const success = exportToCsvSlider('sliders_export.csv', allFilteredAndSortedData); if (success) toast.push(<Notification title="Export Successful" type="success">Data exported.</Notification>); };
    ...
    const columns: ColumnDef<SliderItem>[] = useMemo(() => [
        { header: 'ID', accessorKey: 'id', enableSorting: true, size: 80, meta: { tdClass: "text-center", thClass: "text-center"} },
        {
            ...
            cell: props => {
                ...
                return (
                    <Avatar size={40} shape="circle" src={imageFullPath || undefined} icon={<TbPhoto />}
        ...
        { header: 'Display Page', accessorKey: 'displayPage', enableSorting: true,
          cell: props => displayPageOptionsConst.find(p => p.value === props.row.original.displayPage)?.label || props.row.original.displayPage
        },
        { header: 'Source', accessorKey: 'source', enableSorting: true,
          cell: props => sourceOptionsConst.find(s => s.value === props.row.original.source)?.label || props.row.original.source
        },
        ...
        {
          header: 'Actions', id: 'action', size: 160, meta: { HeaderClass: 'text-center' },
          cell: (props) => (
            <ActionColumn
    ...
    const renderFormFields = (formMethods: typeof addFormMethods | typeof editFormMethods, isEditMode: boolean, currentSlider?: SliderItem | null) => (
        <>
            ...
            <FormItem label="Subtitle (Optional)" invalid={!!formMethods.formState.errors.subtitle} errorMessage={formMethods.formState.errors.subtitle?.message}>
            ...
            <FormItem label="Button Text (Optional)" invalid={!!formMethods.formState.errors.button_text} errorMessage={formMethods.formState.errors.button_text?.message}>
            ...
            {isEditMode && currentSlider?.imageFullPath && !editFormPreviewUrl && (
                <FormItem label="Current Image">
                    <Avatar size={80} src={currentSlider.imageFullPath} shape="square" icon={<TbPhoto />} />
                </FormItem>
            )}
            <FormItem label={isEditMode ? "New Image (Optional)" : "Image"} invalid={!!formMethods.formState.errors.image} errorMessage={formMethods.formState.errors.image?.message as string} isRequired={!isEditMode}>
                <Controller name="image" control={formMethods.control} render={({ field: { onChange, onBlur, name, ref } }) => (
            ...
            <FormItem label="Display Page" invalid={!!formMethods.formState.errors.display_page} errorMessage={formMethods.formState.errors.display_page?.message} isRequired>
                <Controller name="display_page" control={formMethods.control} render={({ field }) => (
                    <UiSelect options={displayPageOptionsConst} value={displayPageOptionsConst.find(opt => opt.value === field.value)}
                              onChange={opt => field.onChange(opt ? opt.value : undefined)} placeholder="Select display page" />
                )} />
            </FormItem>
            <FormItem label="Link (Optional, include http/https)" invalid={!!formMethods.formState.errors.link} errorMessage={formMethods.formState.errors.link?.message}>
                <Controller name="link" control={formMethods.control} render={({ field }) => <Input {...field} value={field.value ?? ''} type="url" placeholder="https://example.com/target-page" />} />
            </FormItem>
            <FormItem label="Source" invalid={!!formMethods.formState.errors.source} errorMessage={formMethods.formState.errors.source?.message} isRequired>
                <Controller name="source" control={formMethods.control} render={({ field }) => (
                    <UiSelect options={sourceOptionsConst} value={sourceOptionsConst.find(opt => opt.value === field.value)}
                              onChange={opt => field.onChange(opt ? opt.value : undefined)} placeholder="Select source" />
                )} />
            </FormItem>
            <FormItem label="Status" invalid={!!formMethods.formState.errors.status} errorMessage={formMethods.formState.errors.status?.message} isRequired>
                <Controller name="status" control={formMethods.control} render={({ field }) => (
                    <UiSelect options={apiStatusOptions} value={apiStatusOptions.find(opt => opt.value === field.value)}
                              onChange={opt => field.onChange(opt ? opt.value : undefined)} placeholder="Select status" />
                )} />
            </FormItem>
            <FormItem label="Domain IDs (Optional, comma-separated)" invalid={!!formMethods.formState.errors.domain_ids} errorMessage={formMethods.formState.errors.domain_ids?.message}>
                <Controller name="domain_ids" control={formMethods.control} render={({ field }) => <Input {...field} value={field.value ?? ''} placeholder="e.g., 1,2,3" />} />
            </FormItem>
            <FormItem label="Slider Color (Optional)" invalid={!!formMethods.formState.errors.slider_color} errorMessage={formMethods.formState.errors.slider_color?.message}>
                <div className="flex items-center gap-2">
                    <Controller name="slider_color" control={formMethods.control}
                        render={({ field }) => <Input {...field} value={field.value ?? '#FFFFFF'} type="color" className="w-12 h-10 p-1" />} />
                    <Controller name="slider_color" control={formMethods.control}
                        render={({ field }) => <Input {...field} value={field.value ?? ''} type="text" placeholder="#RRGGBB or color name" className="flex-grow" />} />
                </div>
            </FormItem>
        </>
    );
    ...
    return (
        <>
            <Container className="h-full">
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                        ...
                        <Button variant="solid" icon={<TbPlus />} onClick={openAddDrawer}>Add New Slider</Button>
                    </div>
                    <SlidersTableTools onSearchChange={handleSearchChange} onFilter={openFilterDrawer} onExport={handleExportData} onImport={handleImportData} />
                    <div className="mt-4 flex-grow overflow-y-auto">
                        <SlidersTable columns={columns} data={pageData} loading={tableLoading}
            ...
            <SlidersSelectedFooter selectedItems={selectedItems} onDeleteSelected={handleDeleteSelected} />
            ...
            <Drawer title="Add New Slider" isOpen={isAddDrawerOpen} onClose={closeAddDrawer} onRequestClose={closeAddDrawer} width={600}
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button size="sm" variant="solid" form="addSliderForm" type="submit" loading={isSubmitting} disabled={!addFormMethods.formState.isValid || isSubmitting}>
            ...
            <Drawer title="Edit Slider" isOpen={isEditDrawerOpen} onClose={closeEditDrawer} onRequestClose={closeEditDrawer} width={600}
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button size="sm" variant="solid" form="editSliderForm" type="submit" loading={isSubmitting} disabled={!editFormMethods.formState.isValid || isSubmitting}>
                            {isSubmitting ? 'Saving...' : 'Save Changes'}
                        </Button>
                    </div>
                }>
                <Form id="editSliderForm" onSubmit={editFormMethods.handleSubmit(onEditSliderSubmit)} className="flex flex-col gap-4">
                    {renderFormFields(editFormMethods, true, editingSlider)}
                </Form>
            </Drawer>

            <Drawer title="Filter Sliders" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer}
                footer={
                    <div className="flex justify-between w-full">
                        ...
                        <div>
                            ...
                            <Button size="sm" variant="solid" form="filterSliderForm" type="submit">Apply Filters</Button>
                        </div>
                    </div>
                }>
                <Form id="filterSliderForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    ...
                    <FormItem label="Filter by Status(es)">
                        <Controller name="filterStatuses" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select statuses..." options={uiStatusOptions}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </FormItem>
            ...
            <ConfirmDialog isOpen={statusChangeConfirmOpen} type="warning" title="Change Slider Status"
                onClose={() => { setStatusChangeConfirmOpen(false); setSliderForStatusChange(null); }}
                onRequestClose={() => { setStatusChangeConfirmOpen(false); setSliderForStatusChange(null); }}
                onCancel={() => { setStatusChangeConfirmOpen(false); setSliderForStatusChange(null); }}
                onConfirm={onConfirmChangeStatus} loading={isProcessing}>
                <p>Are you sure you want to change the status for "<strong>{sliderForStatusChange?.title}</strong>" to <strong>{sliderForStatusChange?.status === 'active' ? 'Inactive' : 'Active'}</strong>?</p>
            </ConfirmDialog>

            <Drawer title="Import Sliders" isOpen={importDialogOpen} onClose={() => setImportDialogOpen(false)} onRequestClose={() => setImportDialogOpen(false)}>
                 <div className="p-4">
                    <p className="mb-4">Upload a CSV file to import sliders. Ensure the CSV format matches the export structure.</p>
                    <Input type="file" accept=".csv" className="mt-2" onChange={(e) => {
                        if (e.target.files && e.target.files[0]) {
                            toast.push(<Notification title="Import" type="info">File selected: {e.target.files[0].name}. Import processing to be implemented.</Notification>)}}} />
            ...
            <Dialog isOpen={isImageViewerOpen} onClose={closeImageViewer} onRequestClose={closeImageViewer} shouldCloseOnOverlayClick={true} shouldCloseOnEsc={true} width={600}>
                <div className="flex justify-center items-center p-4">
                    {imageToView ? (<img src={imageToView} alt="Slider Image Full View" style={{ maxWidth: '100%', maxHeight: '80vh', objectFit: 'contain' }} />) : (<p>No image to display.</p>)}
                </div>
            </Dialog>
        </>
    );
