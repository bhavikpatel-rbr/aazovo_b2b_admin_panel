...
import React, { useState, useMemo, useCallback, Ref, useEffect } from 'react'; // Added React, useEffect, Ref
...
import { useForm as useReactHookForm } from 'react-hook-form'; // Added useForm
...
const continentOptions = [
    { value: 'Asia', label: 'Asia' }, { value: 'Africa', label: 'Africa' }, { value: 'North America', label: 'North America' },
    { value: 'South America', label: 'South America' }, { value: 'Antarctica', label: 'Antarctica' },
    { value: 'Europe', label: 'Europe' }, { value: 'Australia', label: 'Australia' },
];
const businessTypeOptions = [ // Example
    { value: 'Manufacturer', label: 'Manufacturer' }, { value: 'Retailer', label: 'Retailer' }, { value: 'Service', label: 'Service' },
];
const stateOptions = [ // Example
    { value: 'NY', label: 'New York' }, { value: 'CA', label: 'California' }, { value: 'KA', label: 'Karnataka' },
];
const cityOptions = [ // Example
    { value: 'New York City', label: 'New York City' }, { value: 'Los Angeles', label: 'Los Angeles' }, { value: 'Bengaluru', label: 'Bengaluru' },
];
const interestedForOptions = [
    { value: 'Buy', label: 'Buy' }, { value: 'Sell', label: 'Sell' }, { value: 'Both', label: 'Both' },
];
const kycStatusOptions = [
    { value: 'Verified', label: 'Verified' }, { value: 'Pending', label: 'Pending' }, { value: 'Not Submitted', label: 'Not Submitted' },
];
...
const initialDummyForms: FormItem[] = [
    ...
    {
        id: "1", member_name: "John Doe", member_contact_number: "+1234567890", member_email_id: "john.doe@example.com",
        member_photo: "https://example.com/photo1.jpg", member_photo_upload: "Uploaded at 2025-05-17T10:00:00Z",
        member_role: "Director", member_status: "active", member_join_date: "2023-01-01", profile_completion: 85.5,
        success_score: 75, trust_score: 80, activity_score: 90, associated_brands: ["Brand A", "Brand B"],
        business_category: ["Automotive", "Electronics"], interested_in: "Both", company_id: "COMP12345",
        company_name: "Acme Corporation", membership_stats: "12/43", member_location: "USA / New York / NY", kyc_status: "Verified",
    },
    {
        id: "2", member_name: "Jane Smith", member_contact_number: "+1987654321", member_email_id: "jane.smith@example.com",
        member_photo: "https://example.com/photo2.jpg", member_photo_upload: "Uploaded at 2025-05-16T11:00:00Z",
        member_role: "Manager", member_status: "inactive", member_join_date: "2022-06-15", profile_completion: 72.0,
        success_score: 60, trust_score: 70, activity_score: 65, associated_brands: ["Brand C"],
        business_category: ["Healthcare"], interested_in: "Sell", company_id: "COMP67890",
        company_name: "Beta Enterprises", membership_stats: "8/30", member_location: "UK / London", kyc_status: "Pending",
    },
     {
        id: "3", member_name: "Alice Johnson", member_contact_number: "+1123456789", member_email_id: "alice.johnson@example.com",
        member_photo: "https://example.com/photo3.jpg", member_photo_upload: "Uploaded at 2025-05-15T09:30:00Z",
        member_role: "Engineer", member_status: "active", member_join_date: "2021-09-10", profile_completion: 92.0,
        success_score: 88, trust_score: 95, activity_score: 98, associated_brands: ["Brand D", "Brand E"],
        business_category: ["IT Services", "FinTech"], interested_in: "Buy", company_id: "COMP24680",
        company_name: "Gamma Innovations", membership_stats: "20/50", member_location: "India / Bengaluru / KA", kyc_status: "Verified",
      }
];
// --- End Dummy Data ---
...
const FormListTable = () => {
    ...
    const { pageData, total } = useMemo(() => {
        ...
        if (filterCriteria.filterState && filterCriteria.filterState.length > 0) {
            ...
            filteredData = filteredData.filter(form =>
                selectedStates.some(state => form.member_location?.toLowerCase().includes(` / ${state}`.toLowerCase())) // More specific state check
            );
        }
         if (filterCriteria.filterCity && filterCriteria.filterCity.length > 0) {
            ...
            filteredData = filteredData.filter(form =>
                selectedCities.some(city => form.member_location?.toLowerCase().includes(` / ${city} / `.toLowerCase())) // More specific city check
            );
        }
    ...
    const columns: ColumnDef<FormItem>[] = useMemo(() => [
        ...
        { 
            header: 'Member', accessorKey: 'member_name', size: 180,
            cell : (props) => (
                <div className="flex items-center">
                    ...
                    <div className="ml-2 rtl:mr-2 text-xs">
                        <span className="font-semibold"> {props.row.original.id} | {props.row.original.member_name}</span>
        ...
        { 
            header : "Company", accessorKey : "company_name", size: 200,
            cell : (props) => (
                <div className="ml-2 rtl:mr-2 text-xs">
        ...
        {
            header: 'Status', accessorKey: 'member_status', size: 140,
            cell: (props) => {
                ...
                return (
                    <div className="flex flex-col text-xs">
                        ...
                        <span className="mt-0.5"><b>Joined:</b> {new Date(member_join_date).toLocaleDateString()}</span>
                    </div>
                )
            },
        },
        { 
            header: 'Profile', accessorKey: 'profile_completion', size: 220,
            cell : (props) => (
                <div className='text-xs flex flex-col'>
                    <b>{props.row.original.membership_stats}</b> {/* Assuming membership_stats is like "INS - Premium" */}
                    ...
                    <Tooltip title={`Profile: ${props.row.original.profile_completion}%`}>
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                            <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${props.row.original.profile_completion}%` }}></div>
                        </div>
                    </Tooltip>
                </div>
            )
        ...
        {
            header: 'Preferences', accessorKey: 'associated_brands', size:300, // Changed accessorKey
            cell: props => (
        ...
        {
            header: 'Ratio', accessorKey: 'trust_score', size:110,
            cell: props => (
                <div className='flex flex-col gap-1'>
    ...
    const handleSetTableData = useCallback((data: Partial<TableQueries>) => { // Changed data to Partial
        setTableData(prev => ({ ...prev, ...data }));
        if (selectedForms.length > 0 && !data.pageIndex) { // Clear selection if not just a page change
            setSelectedForms([]);
        }
    ...
    return (
        <> {/* Added Fragment */}
            ...
            <DataTable
                ...
                checkboxChecked={(row) => selectedForms.some((selected) => selected.id === row.id)}
            ...
            >
                <UiForm id="filterMemberForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-4">
                    <UiFormItem label="Status">
                        <Controller name="filterStatus" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select status..." options={memberStatusOptions}
                    ...
                    <UiFormItem label="Continent">
                        <Controller name="filterContinent" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select continent..." options={continentOptions}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="Business Type">
                        <Controller name="filterBusinessType" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select business type..." options={businessTypeOptions}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="State">
                        <Controller name="filterState" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select state..." options={stateOptions} // Ideally dynamic
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="City">
                        <Controller name="filterCity" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select city..." options={cityOptions} // Ideally dynamic
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="Interested For">
                        <Controller name="filterInterestedFor" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select interest..." options={interestedForOptions}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="Interested Category">
                        <Controller name="filterInterestedCategory" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select category..." options={businessTypeOptions} // Assuming same as business type for example
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                    <UiFormItem label="Brand"> {/* Assuming associated_brands are the source */}
                        <Controller name="filterBrand" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select brand..."
                                                        options={forms.flatMap(f => f.associated_brands).filter((v, i, a) => a.indexOf(v) === i).map(b => ({value: b, label: b}))}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
                     <UiFormItem label="KYC Verified">
                        <Controller name="filterKycVerified" control={filterFormMethods.control}
                            render={({ field }) => <UiSelect isMulti placeholder="Select KYC status..." options={kycStatusOptions}
                                                        value={field.value || []} onChange={val => field.onChange(val || [])} />} />
                    </UiFormItem>
