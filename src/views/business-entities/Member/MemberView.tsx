...
// Icons
import { TbArrowLeft, TbPencil, TbUserCircle } from "react-icons/tb";
...
const renderBoolean = (value?: boolean | null | string) => {
    if (value === true || value === '1') return <Tag className="bg-emerald-100 text-emerald-600">Yes</Tag>;
    if (value === false || value === '0') return <Tag className="bg-red-100 text-red-600">No</Tag>;
    return <span className="text-gray-400 dark:text-gray-500">N/A</span>;
};
        ...
        const fetchMember = async () => {
            ...
            try {
                ...
                if (response) {
                    ...
                    toast.push(<Notification type="danger" title="Fetch Error">Failed to load member data.</Notification>);
                    navigate('/business-entities/member');
                }
            } catch (error: any) {
                toast.push(<Notification type="danger" title="Fetch Error">{error?.message || 'An error occurred.'}</Notification>);
                navigate('/business-entities/member');
            } finally {
                setIsLoading(false);
            }
        ...
        return (
            <Container>
                <Card className="text-center p-8">
                    ...
                    <Button className="mt-4" onClick={() => navigate("/business-entities/member")}>
                        Back to List
                    </Button>
                </Card>
            </Container>
            ...
            <AdaptiveCard>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                            <h4 className="font-bold">{memberData.name || 'N/A'}</h4>
                            <p className="text-gray-500">{memberData.email || 'No email'}</p>
                        </div>
                        <Tag className={`${statusColorMap[currentStatus] || statusColorMap["unregistered"]} capitalize`}>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button 
                            variant="solid" 
                            icon={<TbPencil />}
                            onClick={() => navigate(`/business-entities/member-edit/${id}`)}
                        >
                            Edit Member
                        </Button>
                    </div>
                ...
                <div className="mt-8 flex flex-col gap-6">
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1">
                            <DetailItem label="Member ID" value={memberData.id} />
                    ...
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1">
                            <DetailItem label="Primary Number" value={`${memberData.number_code || ''} ${memberData.number || ''}`} />
                            <DetailItem label="WhatsApp Number" value={memberData.whatsApp_no} />
                            <DetailItem label="Alternate Number" value={`${memberData.alternate_contact_number_code || ''} ${memberData.alternate_contact_number || ''}`} />
                            <DetailItem label="Alternate Email" value={memberData.alternate_email} />
                            <DetailItem label="Landline Number" value={memberData.landline_number} />
                            <DetailItem label="Fax Number" value={memberData.fax_number} />
                        </div>
                    </Card>
                    ...
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1">
                            <DetailItem label="Temp. Company Name" value={memberData.company_temp} />
                            <DetailItem label="Actual Company Name" value={memberData.company_actual} />
                            <DetailItem label="Business Type" value={memberData.business_type} />
                            <DetailItem label="Business Opportunity" value={memberData.business_opportunity} />
                            <DetailItem label="Member Grade" value={memberData.member_grade} />
                            <DetailItem label="Dealing in Bulk" value={memberData.dealing_in_bulk} />
                            <DetailItem label="Relationship Manager" value={memberData.relationship_manager?.name} />
                        </div>
                    </Card>
                    ...
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1">
                            ...
                            <DetailItem label="Pincode" value={memberData.pincode} />
                            <DetailItem label="Country" value={memberData.country?.name} />
                            <DetailItem label="Continent" value={memberData.continent?.name} />
                        </div>
                    </Card>
                    ...
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-1">
                             ...
                             <DetailItem label="Facebook" value={renderLink(memberData.facebook_profile)} />
                             <DetailItem label="Instagram" value={renderLink(memberData.instagram_handle, `@${memberData.instagram_handle}`)} />
                             <DetailItem label="Skype ID" value={memberData.skype_id} />
                             <DetailItem label="WeChat ID" value={memberData.wechat_id} />
                        </div>
                    </Card>
                    ...
                    <Card bordered>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-1">
                            <DetailItem label="Product Upload" value={renderBoolean(memberData.product_upload_permission)} />
                            <DetailItem label="Wall Enquiry" value={renderBoolean(memberData.wall_enquiry_permission)} />
                            <DetailItem label="Enquiry Permission" value={renderBoolean(memberData.enquiry_permission)} />
                            <DetailItem label="Trade Inquiry" value={renderBoolean(memberData.trade_inquiry_allowed)} />
                        </div>
                    </Card>

                    {memberData.dynamic_member_profiles?.length > 0 && (
                        <Card bordered>
                            <h5 className="mb-4">Dynamic Member Profiles</h5>
                                ...
                                {memberData.dynamic_member_profiles.map((profile: any, index: number) => (
                                    <div key={profile.id || index} className="p-4 border rounded-md dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                                            ...
                                            <DetailItem label="Categories" value={<ListAsTags list={profile.category_names} />} />
                                            <DetailItem label="Sub-categories" value={<ListAsTags list={profile.sub_category_names} />} />
                                        </div>
                                    </div>
                                ))}
                            </div>
