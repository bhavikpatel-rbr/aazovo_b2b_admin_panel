...
const MemberModals: React.FC<MemberModalsProps> = ({ modalState, onClose }) => {
  ...
  const renderModalContent = () => {
    switch (type) {
      ...
      default:
        return (
          <GenericActionDialog type={type} member={member} onClose={onClose} />
        );
    }
  };
...
const MemberListProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <MemberListContext.Provider
      value={{ memberList, setMemberList, selectedMembers, setSelectedMembers }}
    >
      {children}
    </MemberListContext.Provider>
  );
...
const FormListSearch: React.FC<FormListSearchProps> = ({
  onInputChange,
  value,
}) => {
  return (
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      >
        ...
        >
          {" "}
          <TbUserSearch size={18} />{" "}
          <span className="text-xs">View Brand Profile</span>{" "}
        </Dropdown.Item>
...
const ActiveFiltersDisplay = ({
  ...
  const filterKeyToLabelMap: Record<string, string> = {
    ...
    filterBusinessOpportunity: "Opportunity",
    filterContinent: "Continent",
    filterCountry: "Country",
    filterState: "State",
    filterCity: "City",
    filterInterestedFor: "Interest",
    filterBrand: "Brand",
    memberGrade: "Grade",
  };
  ...
  const activeFiltersList = Object.entries(filterData).flatMap(
    ([key, value]) => {
      ...
      return (value as { value: string; label: string }[]).map(
      ...
      {activeFiltersList.map((filter) => (
        ...
        >
          ...
          <TbX
            ...
            onClick={() =>
              onRemoveFilter(filter.key as keyof FilterFormData, filter.value)
            }
          />
        </Tag>
      ))}
...
const FormListTable = ({
  ...
  filterCriteria: FilterFormData;
  setFilterCriteria: React.Dispatch<React.SetStateAction<FilterFormData>>;
}) => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  ...
  const { activeFilterCount, pageData, total, allFilteredAndSortedData } =
    useMemo(() => {
      ...
      if (filterBusinessOpportunity?.length) {
        ...
        processedData = processedData.filter(
          (item) =>
            ...
            values.includes(item.business_opportunity)
        );
      }
...
const Member = () => {
  ...
  const handleCardClick = useCallback(
    (status: "Active" | "Disabled" | "Unregistered" | "All") => {
      const newCriteria: FilterFormData = {};
      if (status !== "All") {
        const statusOption = memberStatusOptions.find(
          ...
          <AdaptiveCard>
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-4 mb-4 gap-2">
                <Tooltip title="Click to show all Member">
                  <div
                ...
                <Tooltip title="Click to filter by Disabled status">
                  <div
                ...
                <Tooltip title="Click to filter by Unregistered status">
                  <div
                    onClick={() => handleCardClick("Unregistered")}
                    className="cursor-pointer"
                  >
                    <Card
                      bodyClass="flex gap-2 p-2"
