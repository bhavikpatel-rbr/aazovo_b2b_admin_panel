...
const processApiDataToInquiryItems = (apiData: ApiInquiryItem[]): InquiryItem[] => {
  ...
  return apiData.map(apiItem => ({
    ...
    inquiry_description: apiItem.requirements || apiItem.inquiry_description || 'N/A', // Prioritize requirements
...
const InquiryListProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (masterLoadingStatus === "idle") {
      ...
      const deptsFromApi = Array.isArray(departmentsData) ? departmentsData : [];
...
const InquiryActionColumn = ({
  rowData, onViewDetail, onDeleteItem, onEdit, onShare, onChangeItemStatus,
}: {
  rowData: InquiryItem;
  ...
  const handleChangeStatus = () => onChangeItemStatus && onChangeItemStatus(rowData.id, rowData.status);
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      {onShare && <Tooltip title="Share"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-orange-600 dark:text-gray-400 dark:hover:text-orange-400" role="button" onClick={handleShare}><TbShare /></div></Tooltip>}
      <Tooltip title="Delete"><div className="text-xl cursor-pointer select-none text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400" role="button" onClick={() => onDeleteItem(rowData)}><TbTrash /></div></Tooltip>
      <Tooltip title="More">
...
const InquiryListTable = () => {
  ...
  const onConfirmSingleDelete = async () => {
    ...
    try {
      ...
      setSelectedInquiries(prev => prev.filter(si => si.id !== itemToDelete.id));
    } catch (error: any) {
  ...
  const { pageData, total, allFilteredAndSortedData } = useMemo(() => {
    ...
    if (filterCriteria.filterDepartment?.length) {
      ...
      data = data.filter(item => item.department && selected.includes(item.department));
    }
    ...
    if (filterCriteria.filterInquiryDate?.[0] || filterCriteria.filterInquiryDate?.[1]) {
      data = data.filter(item => checkDateRange(item.inquiry_date, filterCriteria.filterInquiryDate));
    }
    if (filterCriteria.filterResponseDate?.[0] || filterCriteria.filterResponseDate?.[1]) {
      data = data.filter(item => checkDateRange(item.response_date, filterCriteria.filterResponseDate));
    }
    if (filterCriteria.filterResolutionDate?.[0] || filterCriteria.filterResolutionDate?.[1]) {
      data = data.filter(item => checkDateRange(item.resolution_date, filterCriteria.filterResolutionDate));
    }
    if (filterCriteria.filterFollowUpDate?.[0] || filterCriteria.filterFollowUpDate?.[1]) {
      data = data.filter(item => checkDateRange(item.follow_up_date, filterCriteria.filterFollowUpDate));
    }
    ...
    if (order && key) {
      data.sort((a, b) => {
        ...
        if (['inquiry_date', 'response_date', 'resolution_date', 'follow_up_date'].includes(key)) {
          const dateA = aVal && aVal !== 'N/A' ? new Date(aVal as string).getTime() : (order === 'asc' ? Infinity : -Infinity);
          const dateB = bVal && bVal !== 'N/A' ? new Date(bVal as string).getTime() : (order === 'asc' ? Infinity : -Infinity);
          if (isNaN(dateA)) return 1; if (isNaN(dateB)) return -1; // Push NaN to end
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
        ...
        <div className="flex gap-2">
          ...
          {allFilteredAndSortedData.length > 0 ? (
            <CSVLink data={allFilteredAndSortedData} headers={csvHeaders} filename="inquiries_export.csv" onClick={handleExport}>
      ...
      />
      <Drawer title="Inquiry Filters" isOpen={isFilterDrawerOpen} onClose={closeFilterDrawer} onRequestClose={closeFilterDrawer} width={480}
        ...
        <UiForm id="filterInquiryForm" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)}>
          <div className="sm:grid grid-cols-2 gap-x-4 gap-y-2">
            ...
            <UiFormItem label="Resolution Date Range" className="col-span-2"><Controller name="filterResolutionDate" control={filterFormMethods.control} render={({ field }) => <DatePickerRange placeholder="Select Resolution Dates" value={field.value as [Date | null, Date | null]} onChange={field.onChange} />} /></UiFormItem>
            <UiFormItem label="Follow-up Date Range" className="col-span-2"><Controller name="filterFollowUpDate" control={filterFormMethods.control} render={({ field }) => <DatePickerRange placeholder="Select Follow-up Dates" value={field.value as [Date | null, Date | null]} onChange={field.onChange} />} /></UiFormItem>
          </div>
        </UiForm>
      </Drawer>
      <ConfirmDialog isOpen={singleDeleteConfirmOpen} type="danger" title="Delete Inquiry"
  ...
  return (
    <>
      <StickyFooter className="flex items-center justify-between py-4 bg-white dark:bg-gray-800" stickyClass="-mx-4 sm:-mx-8 border-t border-gray-200 dark:border-gray-700 px-8">
        <div className="container mx-auto"><div className="flex items-center justify-between">
          ...
          <div className="flex items-center"><Button size="sm" className="ltr:mr-3 rtl:ml-3" type="button" customColorClass={() => "border-red-500 ring-1 ring-red-500 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30"} onClick={handleDelete} loading={isBulkDeleting}>Delete</Button><Button size="sm" variant="solid" onClick={() => setSendMessageDialogOpen(true)}>Message</Button></div>
        </div></div>
      </StickyFooter>
      <ConfirmDialog isOpen={deleteConfirmationOpen} type="danger" title="Remove Inquiries" onClose={handleCancelDelete} onRequestClose={handleCancelDelete} onCancel={handleCancelDelete} onConfirm={handleConfirmDelete} loading={isBulkDeleting}><p>Are you sure you want to remove these inquiries? This action can't be undone.</p></ConfirmDialog>
      <Dialog isOpen={sendMessageDialogOpen} onRequestClose={() => setSendMessageDialogOpen(false)} onClose={() => setSendMessageDialogOpen(false)}>
        <h5 className="mb-2">Send Message</h5><p>Send message regarding the following inquiries:</p>
