...
// --- UPDATED: PartnerItem Type to include all document fields ---
...
function exportToCsv(filename: string, rows: PartnerItem[]) {
  ...
  const csvContent = [
    ...
    ...transformedRows.map((row) =>
      PARTNER_CSV_HEADERS.map((header) =>
        JSON.stringify(row[headerToKeyMap[header]] ?? "")
      ).join(",")
    ),
  ].join("\n");
...
export const getPartnerStatusClass = (
  statusValue?: PartnerItem["status"]
): string => {
  ...
  const partnerStatusColors: Record<string, string> = {
    ...
    verified:
      "border border-blue-300 bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-300",
    pending:
...
const PartnerListProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const getAllUserDataOptionsData = useMemo(
    () =>
  ...
  return (
    ...
    >
      ...
      <div className="p-3 flex flex-col flex-grow">
        <p
          className="font-semibold truncate flex-grow text-sm"
          title={document.name}
        >
          {document.name}
        </p>
...
const KycDocumentsTab: React.FC<{ partner: PartnerItem }> = ({ partner }) => {
  ...
  const kycDocs = useMemo((): DocumentRecord[] => {
    ...
    addDoc(
      "GST Certificate",
      partner.gst_certificate_file,
      partner.gst_certificate_verified
    );
    addDoc(
      "Office Photo",
      partner.office_photo_file,
      partner.office_photo_verified
    );
    addDoc(
      "Cancel Cheque",
      partner.cancel_cheque_file,
      partner.cancel_cheque_verified
    );
    addDoc(
      "Visiting Card",
      partner.visiting_card_file,
      partner.visiting_card_verified
    );
    addDoc(
      "Authority Letter",
      partner.authority_letter_file,
      partner.authority_letter_verified
    );
    addDoc("Agreement", partner.agreement_file, partner.agreement_verified);
    addDoc(
      "Other Document",
      partner.other_document_file,
      partner.other_document_verified
    );
  ...
  const handleNext = () =>
    setViewerState((prev) => ({
      ...prev,
      index: Math.min(prev.index + 1, kycDocs.length - 1),
    }));
  const handlePrev = () =>
    setViewerState((prev) => ({
      ...prev,
  ...
  if (kycDocs.length === 0) {
    return <NoDataMessage message="No KYC documents are available." />;
  }
...
const BillingDocumentsTab: React.FC<{ partner: PartnerItem }> = ({
  partner,
}) => {
  const [viewerState, setViewerState] = useState({ isOpen: false, index: 0 });
  ...
  const billingDocs = useMemo(
    (): DocumentRecord[] => {
      return (
        partner.billing_documents?.map((doc, index) => ({
          name: `Billing Document ${index + 1}`,
          url: doc.document,
  ...
  const handleNext = () =>
    setViewerState((prev) => ({
      ...prev,
      index: Math.min(prev.index + 1, billingDocs.length - 1),
    }));
  const handlePrev = () =>
    setViewerState((prev) => ({
      ...prev,
  ...
  if (billingDocs.length === 0) {
    return <NoDataMessage message="No billing documents found." />;
  }
...
const PartnerDocumentsModal: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
  const [activeTab, setActiveTab] = useState("kyc");
  const hasBillingDocs = (partner.billing_documents || []).length > 0;
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      width={1200}
      contentClassName="flex flex-col overflow-y-auto max-h-[60vh] pr-4"
    >
      ...
      <div className="mt-6 flex-grow overflow-y-auto pr-2">
        {activeTab === "kyc" && <KycDocumentsTab partner={partner} />}
        {activeTab === "billing" && hasBillingDocs && (
          <BillingDocumentsTab partner={partner} />
        )}
      </div>
...
export interface ModalState {
  isOpen: boolean;
  type: ModalType | null;
  data: PartnerItem | null;
}

const ViewPartnerDetailDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
      ...
      <div className="max-h-[80vh] overflow-y-auto pr-4">
        ...
        <Card className="mb-4" bordered>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
            ...
            <div className="mb-3">
              ...
              <span className="text-gray-600 dark:text-gray-400">
                <Tag
                  className={`${getPartnerStatusClass(partner.status)} capitalize`}
                >
                  {partner.status}
                </Tag>
              </span>
        ...
        <Card className="mb-4" bordered>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
            ...
            {renderDetailItem(
              "Primary Contact",
              `${partner.primary_contact_number_code} ${partner.primary_contact_number}`
            )}
...
const AddPartnerActivityDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
  user: any;
}> = ({ partner, onClose, user }) => {
  const dispatch = useAppDispatch();
      ...
      <UiForm onSubmit={handleSubmit(onAddActivity)}>
        ...
        >
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Input {...field} placeholder="e.g., Followed up with partner" />
            )}
          />
        </UiFormItem>
        <UiFormItem
...
const AddPartnerScheduleDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
  const dispatch = useAppDispatch();
  ...
  const {
    ...
    defaultValues: {
      event_title: `Meeting with ${partner.partner_name}`,
    ...
    try {
      ...
      toast.push(
        <Notification
          ...
          children={`Successfully scheduled event for ${partner.partner_name}.`}
        />
      );
      onClose();
    } catch (error: any) {
...
const AddPartnerNotificationDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
  userOptions: SelectOption[];
}> = ({ partner, onClose, userOptions }) => {
  const dispatch = useAppDispatch();
  const [isLoading, setIsLoading] = useState(false);
  const {
    control,
...
const AssignPartnerTaskDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
  userOptions: SelectOption[];
}> = ({ partner, onClose, userOptions }) => {
  ...
  const {
    ...
    defaultValues: {
      task_title: `Follow up with ${partner.partner_name}`,
      assign_to: [],
      priority: "Medium",
      due_date: null,
      description: `Follow up with partner ${partner.partner_name} (${partner.primary_email_id}).`,
    },
    mode: "onChange",
  });
...
const ViewPartnerMembersDialog: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => (
  <Dialog
...
const ViewPartnerDataDialog: React.FC<{
  title: string;
  message: string;
  onClose: () => void;
...
const SendPartnerEmailAction: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
  useEffect(() => {
    if (!partner.primary_email_id) {
      toast.push(
        <Notification
          type="warning"
          title="Missing Email"
          children="Primary email is not available for this partner."
        />
      );
      onClose();
      return;
...
const SendPartnerWhatsAppAction: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
  useEffect(() => {
    ...
    if (!phone || !countryCode) {
      toast.push(
        <Notification
          ...
          children="Primary contact number is not available for this partner."
        />
      );
      onClose();
      return;
...
const PartnerAlertModal: React.FC<{
  partner: PartnerItem;
  onClose: () => void;
}> = ({ partner, onClose }) => {
  // --- State and Hooks ---
  ...
  const fetchAlerts = useCallback(() => {
    setIsFetching(true);
    dispatch(
      getAlertsAction({ module_id: partner.id, module_name: "Partner" })
    )
      .unwrap()
...
const PartnerModals: React.FC<{
  ...
  switch (type) {
    ...
    case "members":
      return <ViewPartnerMembersDialog partner={partner} onClose={onClose} />;
    case "alert":
      return <PartnerAlertModal partner={partner} onClose={onClose} />;
    case "transaction":
      return (
        <ViewPartnerDataDialog
          title={`Transactions for ${partner.partner_name}`}
          message="No transactions found for this partner."
          onClose={onClose}
        />
      );
    case "document":
      return <PartnerDocumentsModal partner={partner} onClose={onClose} />;
    case "activity":
      return (
        <AddPartnerActivityDialog
          partner={partner}
          onClose={onClose}
          user={userData}
        />
...
// --- Child Components ---
const PartnerListSearch: React.FC<{
    ...
    <div className="flex items-center justify-center gap-1">
      ...
      <Tooltip title="View">
        <div
          ...
          role="button"
          onClick={() => navigate(`/business-entities/partner-view/${rowData.id}`)}
        >
          <TbEye />
        </div>
      </Tooltip>
      ...
      >
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("email", rowData))}
          className="flex items-center gap-2"
        >
          <TbMail /> Send Email
        </Dropdown.Item>
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("whatsapp", rowData))}
          className="flex items-center gap-2"
        >
        ...
        <Dropdown.Item
          onClick={(e) =>
            handleAction(e, () => onOpenModal("notification", rowData))
          }
          className="flex items-center gap-2"
        >
        ...
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("schedule", rowData))}
          className="flex items-center gap-2"
        >
          <TbCalendarEvent /> Add Schedule
        </Dropdown.Item>
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("task", rowData))}
          className="flex items-center gap-2"
        >
          <TbUser /> Assign Task
        </Dropdown.Item>
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("activity", rowData))}
          className="flex items-center gap-2"
        >
          <TbTagStarred size={18} /> Add to active
        </Dropdown.Item>
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("alert", rowData))}
          className="flex items-center gap-2"
        >
          <TbAlarm /> View Remarks
        </Dropdown.Item>
        <Dropdown.Item
          onClick={(e) => handleAction(e, () => onOpenModal("document", rowData))}
          className="flex items-center gap-2"
        >
...
const ActiveFiltersDisplay = ({
  ...
  const filterKeyToLabelMap: Record<string, string> = {
    filterStatus: "Status",
    filterIndustryExpertise: "Expertise",
      ...
      {activeFiltersList.map((filter) => (
        ...
        >
          ...
          <TbX
            ...
            onClick={() =>
              onRemoveFilter(
                filter.key as keyof PartnerFilterFormData,
                filter.value
              )
            }
          />
...
const PartnerListTable = () => {
  ...
  const [tableData, setTableData] =
    useState<TableQueries>(initialState.tableData);
  ...
  const handleOpenModal = (type: ModalType, partnerData: PartnerItem) =>
  ...
  const onConfirmSingleDelete = useCallback(async () => {
    ...
    try {
      ...
      toast.push(
        <Notification title="Partner Deleted" type="success">
          {`Partner "${itemToDelete.partner_name}" has been deleted.`}
        </Notification>
      );
      dispatch(getpartnerAction());
      ...
      toast.push(
        <Notification title="Delete Failed" type="danger">
          {error?.message || "Could not delete partner."}
        </Notification>
      );
    } finally {
      setIsDeleting(false);
  ...
  const handleRemoveFilter = (
    key: keyof PartnerFilterFormData,
  ...
  const statusOptions = useMemo(
    () =>
      Array.from(new Set(partnerList.map((c) => c.status)))
        .filter(Boolean)
  ...
  const industryExpertiseOptions = useMemo(
    () =>
      Array.from(new Set(partnerList.map((c) => c.industrial_expertise)))
        .filter(Boolean)
  ...
  const { pageData, total, allFilteredAndSortedData, activeFilterCount } =
    useMemo(() => {
      ...
      if (
        ...
        const selected = filterCriteria.filterStatus.map((s) =>
          s.value.toLowerCase()
        );
        filteredData = filteredData.filter(
          (p) => p.status && selected.includes(p.status.toLowerCase())
        );
      }
      if (
        filterCriteria.filterIndustryExpertise &&
        filterCriteria.filterIndustryExpertise.length > 0
      ) {
        const selected = filterCriteria.filterIndustryExpertise.map((s) => s.value);
        filteredData = filteredData.filter(
      ...
      if (
        ...
        filteredData = filteredData.filter(
          (p) => p.continent && selected.includes(p.continent.name)
        );
      }
      if (
        ...
        filteredData = filteredData.filter(
          (p) => p.country && selected.includes(p.country.name)
        );
      }
      ...
      if (
        ...
        const selected = filterCriteria.filterKycVerified.map(
          (k) => k.value === "Yes"
        );
        filteredData = filteredData.filter((p) =>
          selected.includes(p.kyc_verified)
        );
      }
      if (
      ...
      count +=
        (filterCriteria.filterIndustryExpertise?.length ?? 0) > 0 ? 1 : 0; // --- ADDED ---
      ...
      if (order && key) {
        filteredData.sort((a, b) => {
          const av = (a[key as keyof PartnerItem] ?? "") as any;
          const bv = (b[key as keyof PartnerItem] ?? "") as any;
      ...
      const pI = tableData.pageIndex as number,
        pS = tableData.pageSize as number;
      return {
  ...
  const handleRowSelect = (checked: boolean, row: PartnerItem) => {
    setSelectedPartners((prev) =>
      checked ? [...prev, row] : prev.filter((p) => p.id !== row.id)
    );
  };
  ...
  const skeletonData: PartnerItem[] = useMemo(
    () =>
      Array.from({ length: tableData.pageSize as number }).map((_, i) => ({
        ...
        authority_letter_file: null,
        authority_letter_verified: false,
        visiting_card_file: null,
        visiting_card_verified: false,
        cancel_cheque_file: null,
            ...
            <div className="flex flex-col">
              ...
              {industrial_expertise && (
                <span className="text-xs mt-1">
                  <b>Expertise:</b> {industrial_expertise}
                </span>
              )}
              {join_us_as && (
                <span className="text-xs mt-1">
                  <b>Joined As:</b> {join_us_as}
                </span>
              )}
          ...
          return (
            <div className="flex flex-col gap-1 text-[11px]">
              {row.original.gst_number && (
                <div>
                  <b>GST:</b>{" "}
                  <span className="break-all">{row.original.gst_number}</span>
                </div>
              )}
              {row.original.pan_number && (
                <div>
                  <b>PAN:</b>{" "}
                  <span className="break-all">{row.original.pan_number}</span>
                </div>
              )}
              <Tag
                className={`${getPartnerStatusClass(
                  row.original.status
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col gap-1.5 text-xs">
              <span>
                <b>Teams:</b> {row.original.teams_count || 0}
              </span>
              <div className="flex gap-1 items-center">
                <b>KYC Verified:</b>
                <Tooltip
                  title={`KYC: ${row.original.kyc_verified ? "Yes" : "No"}`}
                >
                  {row.original.kyc_verified ? (
              ...
              <Tooltip
                title={`Profile Completion ${row.original.profile_completion}%`}
              >
                <div className="h-2.5 w-full rounded-full bg-gray-300">
                  <div
                    className="rounded-full h-2.5 bg-blue-500"
                    style={{ width: `${row.original.profile_completion}%` }}
                  ></div>
                </div>
              </Tooltip>
            </div>
        ...
        cell: (props) => {
          ...
          return (
            <PartnerActionColumn
              ...
              onEdit={(id) => navigate(`/business-entities/partner-edit/${id}`)}
  ...
  const stateOptions = useMemo(
    () =>
      Array.from(new Set(partnerList.map((c) => c.state)))
        .filter(Boolean)
  ...
  const cityOptions = useMemo(
    () =>
      Array.from(new Set(partnerList.map((c) => c.city)))
        .filter(Boolean)
  ...
  const CardSkeleton = () => (
    <div
      className={classNames(cardClass, "border-gray-200 dark:border-gray-700")}
    >
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 mb-4 gap-2">
        {isLoading ? (
          ...
          <>
            <Tooltip title="Click to show all partners">
              <div onClick={onClearFilters}>
                ...
                >
                  ...
                  <div className="flex flex-col gap-0">
                    <b className="text-sm">{partnerCount?.total ?? 0}</b>
                    <span className="text-[9px] font-semibold">Total</span>
                  </div>
                </Card>
              </div>
            </Tooltip>
            <Tooltip title="Click to filter by Active status">
              <div onClick={() => handleCardClick("status", "Active")}>
                <Card
                  bodyClass={cardBodyClass}
                ...
                >
                  ...
                  <div className="flex flex-col gap-0">
                    <b className="text-sm">{partnerCount?.active ?? 0}</b>
                    <span className="text-[9px] font-semibold">Active</span>
                  </div>
                </Card>
              </div>
            </Tooltip>
            <Tooltip title="Click to filter by Disable status">
              <div onClick={() => handleCardClick("status", "Disabled")}>
                <Card
                  bodyClass={cardBodyClass}
                ...
                >
                  ...
                  <div className="flex flex-col gap-0">
                    <b className="text-sm">{partnerCount?.disabled ?? 0}</b>
                    <span className="text-[9px] font-semibold">Disabled</span>
                  </div>
                </Card>
              </div>
            </Tooltip>
            <Tooltip title="Click to filter by KYC Verified">
              <div onClick={() => handleCardClick("kyc", "Yes")}>
                <Card
                  bodyClass={cardBodyClass}
                ...
                >
                  ...
                  <div className="flex flex-col gap-0">
                    <b className="text-sm">{partnerCount?.verified ?? 0}</b>
                    <span className="text-[9px] font-semibold">Verified</span>
                  </div>
                </Card>
              </div>
            </Tooltip>
            <Tooltip title="Click to filter by KYC Unverified">
              <div onClick={() => handleCardClick("kyc", "No")}>
                <Card
                  bodyClass={cardBodyClass}
                ...
                >
                  ...
                  <div className="flex flex-col gap-0">
                    <b className="text-sm">
                      {partnerCount?.non_verified ?? 0}
                    </b>
                    <span className="text-[9px] font-semibold">Unverified</span>
                  </div>
                </Card>
              </div>
            </Tooltip>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={!allFilteredAndSortedData.length}
          >
            Export
          </Button>
        </div>
        ...
        >
          ...
          <div className="h-full overflow-y-auto">
            <div className="sm:grid grid-cols-2 gap-x-4 gap-y-2">
              ...
              <UiFormItem label="Industry Expertise">
                <Controller
                  name="filterIndustryExpertise"
                  control={filterFormMethods.control}
                  render={({ field }) => (
                    <UiSelect
                      isMulti
      ...
      >
        <p>
          ...
          <strong>{itemToDelete?.partner_name}</strong>"? This action cannot be
          undone.
        </p>
      </ConfirmDialog>
    </>
        ...
        <div className="my-4">
          ...
          >
            {selectedPartners.map((p) => (
              <Tooltip key={p.id} title={p.partner_name}>
                <Avatar
                  src={p.partner_logo || undefined}
                  icon={<TbUserCircle />}
                />
              </Tooltip>
            ))}
