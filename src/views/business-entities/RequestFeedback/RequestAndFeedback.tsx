...
import { Drawer, Form, FormItem, Input, Select, DatePicker, Tag, Tooltip } from '@/components/ui';
...
// Types
import type { OnSortParam, ColumnDef, Row, CellContext } from '@/components/shared/DataTable'; // Added CellContext
import type { TableQueries } from '@/@types/common';
import Textarea from '@/views/ui-components/forms/Input/Textarea';

// --- Define Item Type ---
export type RequestStatus = 'Open' | 'In Progress' | 'Resolved' | 'Closed' | 'Pending Review' | string;
export type RequestSource = 'Website Form' | 'Email' | 'Phone Call' | 'In-App Feedback' | 'Support Ticket' | string;
...
const statusColor: Record<RequestStatus, string> = {
    ...
    'In Progress': 'bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-100',
    ...
    'Pending Review': 'bg-purple-100 text-purple-600 dark:bg-purple-500/20 dark:text-purple-100',
};
...
type RequestFeedbackEditFormData = z.infer<typeof requestFeedbackEditFormSchema>;
...
const CSV_HEADERS = ['ID', 'Name', 'Email', 'Phone', 'From', 'Status', 'Date', 'Message', 'Last Updated', 'Assigned To'];
const CSV_KEYS: (keyof RequestFeedbackItem)[] = [
    'id', 'name', 'email', 'phone', 'from', 'status', 'date', 'message', 'lastUpdated', 'assignedTo'
];
...
const initialDummyRequests: RequestFeedbackItem[] = [
    { id: 1, name: 'John Doe', email: 'john.doe@example.com', phone: '555-1234', from: 'Website Form', message: 'The website is a bit slow on mobile devices. Otherwise, great content!', status: 'Open', date: dayjs().subtract(2, 'day').toDate(), lastUpdated: dayjs().subtract(1, 'hour').toDate() },
    { id: 2, name: 'Jane Smith', email: 'jane.smith@example.com', from: 'Email', message: 'I have a feature request: can you add dark mode?', status: 'Pending Review', date: dayjs().subtract(5, 'day').toDate() },
    { id: 3, name: 'Alice Brown', email: 'alice.b@example.com', phone: '555-5678', from: 'Support Ticket', message: 'Login issue resolved. Thanks for the quick help!', status: 'Resolved', date: dayjs().subtract(1, 'week').toDate(), lastUpdated: dayjs().subtract(2, 'day').toDate(), assignedTo: 'Support Team B' },
    { id: 4, name: 'Bob Green', email: 'bob.g@example.com', from: 'In-App Feedback', message: 'The new UI is fantastic!', status: 'Closed', date: dayjs().subtract(3, 'day').toDate(), lastUpdated: dayjs().subtract(3, 'day').toDate() },
    { id: 5, name: 'Charlie Black', email: 'charlie.b@example.com', phone: '555-8765', from: 'Phone Call', message: 'User reported a billing discrepancy. Needs investigation.', status: 'In Progress', date: dayjs().subtract(1, 'day').toDate(), assignedTo: 'Billing Dept' },
    { id: 6, name: 'Diana White', email: 'diana.w@example.com', from: 'Website Form', message: 'Found a typo on the pricing page under the "Enterprise" plan.', status: 'Open', date: dayjs().toDate() },
    { id: 7, name: 'Edward Grey', email: 'ed.grey@example.com', from: 'Email', message: 'How do I reset my password? The link seems broken.', status: 'In Progress', date: dayjs().subtract(4, 'hour').toDate(), assignedTo: 'Tech Support' },
    { id: 8, name: 'Fiona Purple', email: 'fiona.p@example.com', phone: '555-4321', from: 'Support Ticket', message: 'Request for API documentation update regarding new endpoints.', status: 'Pending Review', date: dayjs().subtract(6, 'day').toDate() },
];


// --- Main RequestAndFeedback Component ---
const RequestAndFeedback = () => {
    const [allRequests, setAllRequests] = useState<RequestFeedbackItem[]>(initialDummyRequests);
        ...
        try {
            switch (action.type) {
                ...
                case 'requests/delete':
                    setAllRequests((prev) => prev.filter((item) => item.id !== action.payload.id));
                    break;
                case 'requests/deleteAll':
                    const idsToDelete = new Set(action.payload.ids.split(','));
                    setAllRequests((prev) => prev.filter((item) => !idsToDelete.has(String(item.id))));
                    break;
    ...
    const [currentItem, setCurrentItem] = useState<RequestFeedbackItem | null>(null);
    ...
    const openViewEditDrawer = useCallback((item: RequestFeedbackItem) => {
        setCurrentItem(item);
    ...
    const onViewEditSubmit = useCallback(async (data: RequestFeedbackEditFormData) => {
        ...
        try {
            ...
            toast.push(<Notification title="Request Updated" type="success">Status changed to {data.status}.</Notification>);
            closeViewEditDrawer();
        } catch (error: any) {
            toast.push(<Notification title="Update Failed" type="danger">{error.message || 'Could not update request.'}</Notification>);
        } finally {
    ...
    const onConfirmSingleDelete = useCallback(async () => {
        ...
        try {
            await dispatchSimulated({ type: 'requests/delete', payload: { id: itemToDelete.id } }).unwrap();
            toast.push(<Notification title="Deleted" type="success">Request from "{itemToDelete.name}" deleted.</Notification>);
            setSelectedItems((prev) => prev.filter((item) => item.id !== itemToDelete.id)); // Corrected: use itemToDelete.id
        } catch (error: any) {
    ...
    const handleDeleteSelected = useCallback(async () => {
        ...
        try {
            await dispatchSimulated({ type: 'requests/deleteAll', payload: { ids: idsToDelete } }).unwrap();
            toast.push(<Notification title="Bulk Delete Successful" type="success">{selectedItems.length} items deleted.</Notification>);
            setSelectedItems([]);
        } catch (error: any) {
            toast.push(<Notification title="Bulk Delete Failed" type="danger">{error.message || 'Could not delete selected items.'}</Notification>);
        } finally {
            setIsDeleting(false);
        }
    ...
    const availableStatuses = useMemo(() => {
        ...
        return Array.from(unique).sort().map(s => ({ value: s, label: s }));
    }, [allRequests]);
    ...
    const { pageData, total, allFilteredAndSortedData } = useMemo((): ProcessedDataType => {
        ...
        if (filterCriteria.filterFrom && filterCriteria.filterFrom.length > 0) {
            const fromSources = new Set(filterCriteria.filterFrom.map(opt => opt.value));
            processedData = processedData.filter(item => fromSources.has(item.from));
        }
    ...
    const handleExportData = useCallback(() => {
        exportRequestsToCsv('requests_feedback_export.csv', allFilteredAndSortedData);
    ...
    const handleRowSelect = useCallback((checked: boolean, row: RequestFeedbackItem) => {
    ...
    const handleAllRowSelect = useCallback((checked: boolean, currentRows: Row<RequestFeedbackItem>[]) => {
        ...
        if (checked) {
            setSelectedItems((prev) => {
                ...
                const newToAdd = currentPageOriginals.filter(r => !prevIds.has(r.id));
    ...
    const columns: ColumnDef<RequestFeedbackItem>[] = useMemo(
        () => [
            { 
                ...
                enableSorting: true, 
                size: 150,
                cell: (props: CellContext<RequestFeedbackItem, unknown>) => ( // Explicitly type props
                    <div>
                        <div>{props.row.original.name}</div>
                        <div className="text-xs text-gray-500">{props.row.original.email}</div>
                         {props.row.original.phone && <div className="text-xs text-gray-500">P: {props.row.original.phone}</div>}
                    </div>
                )
            },
            { header: 'From', accessorKey: 'from', enableSorting: true, size: 150 },
            {
                ...
                enableSorting: true, 
                size: 150,
                cell: (props: CellContext<RequestFeedbackItem, unknown>) => dayjs(props.row.original.date).format('YYYY-MM-DD HH:mm'),
            },
            {
                header: 'Status', 
                accessorKey: 'status', 
                ...
                cell: (props: CellContext<RequestFeedbackItem, unknown>) => {
                    ...
                    const colorClass = statusColor[status] || 'bg-gray-100 text-gray-600 dark:bg-gray-600/20 dark:text-gray-100';
                    return (
                        <Tag className={`${colorClass} capitalize`}>
                            {status}
                        </Tag>
                    );
                },
                ...
                cell: (props: CellContext<RequestFeedbackItem, unknown>) => (
                    <div className="flex items-center justify-center gap-2">
                        <Tooltip title="View/Edit Status">
                            <Button shape="circle" variant="plain" size="sm" icon={<TbPencil />} onClick={() => openViewEditDrawer(props.row.original)} />
                        </Tooltip>
                        <Tooltip title="Delete">
                            <Button shape="circle" variant="plain" size="sm" icon={<TbTrash />} onClick={() => handleDeleteClick(props.row.original)} className="text-red-500 hover:text-red-700"/>
                        </Tooltip>
                    </div>
                ),
            },
    ...
    return (
        <>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        <Button size="sm" className="mr-2" onClick={closeViewEditDrawer} disabled={isSubmitting}>
                            Cancel
                        </Button>
                        <Button
                            size="sm"
            ...
            >
                {currentItem && (
                    <div className="flex flex-col gap-4">
                        ...
                        {currentItem.phone && <div><strong>Phone:</strong> {currentItem.phone}</div>}
                        <div><strong>From:</strong> {currentItem.from}</div>
                        <div><strong>Date:</strong> {dayjs(currentItem.date).format('YYYY-MM-DD HH:mm:ss')}</div>
                         {currentItem.lastUpdated && <div><strong>Last Updated:</strong> {dayjs(currentItem.lastUpdated).format('YYYY-MM-DD HH:mm:ss')}</div>}
                        {currentItem.assignedTo && <div><strong>Assigned To:</strong> {currentItem.assignedTo}</div>}
                        ...
                        >
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Select
                                            ...
                                            value={availableStatuses.find(opt => opt.value === field.value)}
                            ...
                            <FormItem label="Internal Notes (Optional)">
                                 <Controller
                                    ...
                                    control={editFormMethods.control}
                                    render={({ field }) => (
                                        <Textarea {...field} rows={3} placeholder="Add internal notes about this request or status change..." />
                                    )}
                                />
                            </FormItem>
                        </Form>
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full">
                        ...
                        <Button size="sm" variant="solid" form="filterRequestForm" type="submit">Apply</Button>
                    </div>
                }
            >
                <Form
            ...
            >
                <p>Are you sure you want to delete the item from "<strong>{itemToDelete?.name}</strong>"?</p>
            </ConfirmDialog>
        </>
    );
};
