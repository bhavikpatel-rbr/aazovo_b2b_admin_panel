...
const UploadFile = () => { const [isOpen, setIsOpen] = useState(false); const [isUploading, setUploading] = useState(false); const [files, setFiles] = useState<File[]>([]); const handleUpload = async () => { setUploading(true); await sleep(500); setUploading(false); setIsOpen(false); toast.push(<Notification title="Successfully uploaded" type="success" />, { placement: "top-center", }); }; return <><Dialog isOpen={isOpen} onClose={() => setIsOpen(false)}><h4>Upload Files</h4><Upload draggable className="mt-6" onChange={setFiles} onFileRemove={setFiles}><div className="my-4 text-center"><UploadMedia height={150} width={200} /><p className="font-semibold">Drop files here, or <span className="text-blue-500">browse</span></p></div></Upload><div className="mt-4"><Button block loading={isUploading} variant="solid" disabled={files.length === 0} onClick={handleUpload}>Upload</Button></div></Dialog></>; };
                ...
                {directories.length > 0 ? (
                    <div className="flex items-center gap-2">
                        <h3 className="flex items-center gap-2 text-base sm:text-2xl">
                            <span className="hover:text-primary-600 cursor-pointer" role="button" onClick={onEntryClick}>File Manager</span>
                            {/* FIX: Removed the conditional logic to make all breadcrumbs clickable */}
                            {directories.map((dir) => (
                                <Fragment key={dir.id}>
                                    <TbChevronRight className="text-lg" />
                                    <span className="hover:text-primary-600 cursor-pointer" role="button" onClick={() => onDirectoryClick(dir.id)}>
                                        {dir.label}
                                    </span>
                                </Fragment>
                            ))}
...
const FileManager = () => {
  ...
  const {
    layout, fileList, setFileList, setDeleteDialog, setInviteDialog, setRenameDialog, openedDirectoryId, setOpenedDirectoryId, setDirectories, setSelectedFile,
  } = useFileManagerStore();
      ...
      <div className="mt-6">
        {isLoading ? (
          layout === "grid" ? (
            ...
            <Table>
              <Table.THead><Table.Tr><Table.Th>File</Table.Th><Table.Th>Type</Table.Th><Table.Th></Table.Th></Table.Tr></Table.THead>
