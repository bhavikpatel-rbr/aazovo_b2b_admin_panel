...
// --- MOCKED UTILITIES & COMPONENTS ---
const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));
const fileSizeUnit = (bytes: number) => {
...
const EllipsisButton = (props: { onClick: (e: MouseEvent) => void }) => (
  <Button
    {...props}
    shape="circle"
    variant="plain"
...
const FileIcon = ({ type }: { type: string }) => {
  // Use custom Folder icon for directories, emoji for others
  const iconMap: Record<string, React.ReactNode> = {
    ...
    png: <FileImage height={24} width={24} />,
    jpeg: <FileImage height={24} width={24} />,
    jpg: <FileImage height={24} width={24} />,
    default: <FileNotFound height={24} width={24} />,
  };
  return (
    <span style={{ fontSize: "24px" }}>{iconMap[type] || iconMap.default}</span>
  );
};
const UploadMedia = ({ width, height }: { width: number; height: number }) => (
...
const MediaSkeleton = ({ avatarProps }: { avatarProps: any }) => (
  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
    ...
    <div style={{ flex: 1 }}>
      ...
      ></div>
      <div
        style={{ height: "0.8em", width: "60%", backgroundColor: "#e0e0e0" }}
      ></div>
    </div>
  </div>
);
      ...
      <tr key={i}>
        {[...Array(columns)].map((_, j) => (
          <td key={j} style={{ padding: "16px" }}>
            {avatarInColumns.includes(j) ? (
              <MediaSkeleton avatarProps={{ width: 30, height: 30 }} />
            ) : (
              <div style={{ height: "1em", backgroundColor: "#e0e0e0" }}></div>
            )}
          </td>
        ))}
      </tr>
...
const transformApiData = (apiData: any) => {
  ...
  Object.entries(apiData).forEach(([moduleKey, items]) => {
    ...
    const moduleName =
      moduleKey.charAt(0).toUpperCase() + moduleKey.slice(1).replace(/_/g, " ");
    const moduleFolder = createFolder(`module-${moduleKey}`, moduleName);
    
    // FIX 1: The breadcrumb for the module folder should be itself, not a hardcoded root.
    ...
    items.forEach((item: any) => {
      ...
      // FIX 2: The breadcrumb for the item folder is the parent's path plus the item folder itself.
      addEntry(moduleFolder.id, itemFolder, [
        ...(fileSystemData[moduleFolder.id]?.directory || []),
        { id: itemFolder.id, label: itemFolder.name },
      ]);
      ...
      const processDocs = (
        ...
        const docFiles = Object.values(docObj)
          .flatMap((doc: any) => {
            if (typeof doc === "string") return createFile(doc, folderId);
            if (Array.isArray(doc))
              return doc.map((subDoc) => createFile(subDoc, folderId));
            return [];
          })
        ...
        if (docFiles.length > 0) {
          ...
          // FIX 3: The breadcrumb for the doc folder is the parent's path plus the doc folder itself.
          addEntry(itemFolder.id, docFolder, [
            ...(fileSystemData[itemFolder.id]?.directory || []),
            { id: docFolder.id, label: docFolder.name },
          ]);
          docFiles.forEach((file) => addEntry(docFolder.id, file, []));
        }
      };

      if (item.all_documents)
        processDocs(
          item.all_documents,
          "All Documents",
          `docs-${moduleKey}-${item.id}`
        );
      if (item.attachments)
        processDocs(
          { attachments: item.attachments },
          "Attachments",
          `attach-${moduleKey}-${item.id}`
        );
      if (item.images)
        processDocs(item.images, "Images", `images-${moduleKey}-${item.id}`);
      if (item.icons)
        processDocs(item.icons, "Icons", `icons-${moduleKey}-${item.id}`);

      const nestedArrays = Object.entries(item).filter(
        ([key, value]) =>
          Array.isArray(value) &&
          !["attachments", "images", "icons"].includes(key)
      );
      nestedArrays.forEach(([key, value]) => {
        const arrayFolderName =
          key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, " ");
        const arrayFolder = createFolder(
          `array-${moduleKey}-${item.id}-${key}`,
          arrayFolderName
        );

        // FIX 4: The breadcrumb for the nested array folder is the parent's path plus the array folder itself.
        addEntry(itemFolder.id, arrayFolder, [
          ...(fileSystemData[itemFolder.id]?.directory || []),
          { id: arrayFolder.id, label: arrayFolder.name },
        ]);

        (value as any[]).forEach((subItem, index) => {
          const docPath =
            subItem.upload_certificate_path ||
            subItem.photo_upload_path ||
            subItem.document ||
            subItem.icon_url ||
            subItem.attachment_url ||
            (subItem.path && subItem.name ? subItem.path : null) ||
            subItem.attachments;
          const docName =
            subItem.certificate_name ||
            subItem.document_name ||
            subItem.name ||
            subItem.title ||
            `Sub-item ${subItem.id || index}`;
          if (docPath) {
            const file = createFile(
              docPath,
              `file-${key}-${subItem.id || index}`
            );
            if (file) {
              addEntry(
                arrayFolder.id,
                { ...file, name: `${docName}.${getFileExtension(file.name)}` },
                []
              );
            }
          }
...
const FileType = ({ type }: { type: string }) => {
  const getFileTypeString = (t: string) => {
    switch (t) {
      case "pdf":
        return "PDF";
      case "doc":
...
const FileRow = (
  props: BaseFileItemProps & { fileType: string; size: number; name: string }
) => {
    ...
    <Table.Tr>
      <Table.Td width="70%">
        ...
        >
          ...
          <div className="font-bold heading-text group-hover:text-primary-600">
            {name}
          </div>
        </div>
      </Table.Td>
...
const FileList = ({ fileList, layout, ...rest }: any) => {
  const folders = useMemo(
    () => fileList.filter((f: File) => f.fileType === "directory"),
    [fileList]
  );
  const files = useMemo(
    () => fileList.filter((f: File) => f.fileType !== "directory"),
  ...
  const render = (list: Files, isFolder?: boolean) =>
    layout === "grid" ? (
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mt-4 gap-4 lg:gap-6">
        {list.map((file) => (
          <FileSegment
            key={file.id}
            {...file}
            onClick={() => rest.onClick(file.id)}
            onDownload={rest.onDownload}
            onShare={() => rest.onShare(file.id)}
            onDelete={() => rest.onDelete(file.id)}
            onRename={() => rest.onRename(file.id)}
            {...(isFolder ? { onOpen: () => rest.onOpen(file.id) } : {})}
          />
        ))}
      </div>
    ) : (
...
const FileDetails = ({ onShare }: { onShare: (id: string) => void }) => {
  const { selectedFile, setSelectedFile, fileList } = useFileManagerStore();
  const file = useMemo(
    () => fileList.find((f) => selectedFile === f.id),
        ...
        <div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 flex flex-col gap-4">
              {file.size > 0 && <span>Size: {fileSizeUnit(file.size)}</span>}
              <span>
                Type: <FileType type={file.fileType} />
              </span>
              <span>
                Created: {dayjs.unix(file.uploadDate).format("MMM DD, YYYY")}
              </span>
            </div>
          </div>
          <div className="mt-10">
...
const FileManagerDeleteDialog = () => {
  ...
  const handleClose = () => setDeleteDialog({ id: "", open: false });
  const handleConfirm = () => {
...
const FileManagerInviteDialog = () => {
  ...
  const handleClose = () => setInviteDialog({ id: "", open: false });
  const handleInvite = async () => {
    setInviting(true);
    await sleep(500);
    toast.push(
      <Notification type="success" title="Invitation sent!"></Notification>,
      { placement: "top-end" }
    );
    setInviting(false);
  };
  return (
    <Dialog isOpen={inviteDialog.open} onClose={handleClose}>
      <h4 className="mb-4">Share this file</h4>
      <Input
        placeholder="Email"
        // suffix={
...
const FileManagerRenameDialog = () => {
  ...
  const handleClose = () => setRenameDialog({ id: "", open: false });
  const handleSubmit = () => {
    renameFile({ id: renameDialog.id, fileName: newName });
    handleClose();
  };
  return (
    <Dialog isOpen={renameDialog.open} onClose={handleClose}>
      <h4 className="mb-4">Rename</h4>
      <DebouceInput
        placeholder="New name"
        onChange={(e) => setNewName(e.target.value)}
      />
      <div className="mt-4 text-right">
        <Button size="sm" className="mr-2" onClick={handleClose}>
          Cancel
        </Button>
        <Button
          variant="solid"
...
const UploadFile = () => {
  ...
  const handleUpload = async () => {
    ...
    setUploading(false);
    setIsOpen(false);
    toast.push(<Notification title="Successfully uploaded" type="success" />, {
        ...
        {directories.length > 0 ? (
          <div className="flex items-center gap-2">
            <h3 className="flex items-center gap-2 text-base sm:text-2xl">
              ...
              {/* FIX: Removed the conditional logic to make all breadcrumbs clickable */}
              {directories.map((dir) => (
                <Fragment key={dir.id}>
                  ...
                  <h5
                    // className="hover:text-primary-600 cursor-pointer"
                    role="button"
                    onClick={() => onDirectoryClick(dir.id)}
                  >
                    {dir.label}
                  </h5>
                </Fragment>
...
const FileManager = () => {
  ...
  const { allDocuments, status: masterLoadingStatus } =
  ...
  const handleDownload = (srcUrl: string, fileName?: string) => {
    ...
    link.href = srcUrl;
    link.download = fileName || 'downloaded_file'; // fallback name if not provided
  ...
  return (
    <div className="p-4">
      <FileManagerHeader
        ...
        onDirectoryClick={handleNavigate}
      />
      <div className="mt-6">
        {isLoading ? (
          layout === "grid" ? (
            <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mt-4 gap-4 lg:gap-6">
              {[...Array(8)].map((_, i) => (
                <FileSegment key={i} loading={isLoading} />
              ))}
            </div>
          ) : (
            <Table>
              ...
              <TableRowSkeleton avatarInColumns={[0]} columns={3} rows={8} />
            </Table>
          )
        ) : (
          <FileList
