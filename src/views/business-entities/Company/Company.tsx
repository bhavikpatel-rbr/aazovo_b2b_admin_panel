...
const AddCompanyScheduleDialog: React.FC<{ company: CompanyItem; onClose: () => void; onSubmit: (data: ScheduleFormData) => void; isLoading: boolean; }> = ({ company, onClose, onSubmit, isLoading }) => {
    const { control, handleSubmit, formState: { errors, isValid } } = useForm<ScheduleFormData>({
        resolver: zodResolver(scheduleSchema),
        defaultValues: { event_title: `Meeting with ${company.company_name}`, date_time: null as any, notes: `Regarding company ${company.company_name} (${company.company_code}).`, remind_from: null, event_type: undefined },
        mode: "onChange",
    });
...
const AddActivityDialog: React.FC<{ company: CompanyItem; onClose: () => void; user: any; }> = ({ company, onClose, user }) => {
    const dispatch = useAppDispatch();
