...
export type MergedFormItem = {
    ...
    status: 'Active' | 'Pending' | 'Inactive' | 'Verified' | 'active' | 'inactive' 
    progress: number
...
const FormListTable = () => {
    ...
    const handleEdit = (form: MergedFormItem) => { console.log('Edit:', form.id) }
    ...
    const handleCloneForm = (form: MergedFormItem) => {
        ...
        const clonedForm: MergedFormItem = { ...form, id: newId, name: `${form.name} (Clone)`, status: 'inactive' }
        setForms((prev) => [clonedForm, ...prev])
    }
    const handleChangeStatus = (form: MergedFormItem) => {
        let newStatus: MergedFormItem['status'] = 'inactive';
        if (form.status === 'Active' || form.status === 'active') newStatus = 'inactive';
        else if (form.status === 'Inactive' || form.status === 'inactive') newStatus = 'active';
        
        setForms((currentForms) =>
            currentForms.map((f) => (f.id === form.id ? { ...f, status: newStatus } : f)),
        )
    }
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="text-xs">
                                <span className="font-semibold">Category:</span> {Array.isArray(category) ? category.join(', ') : category}
                            </span>
                            <span className="text-xs">
                    ...
                    return (
                        <div className="flex flex-col gap-1.5 text-xs">
                            <span className="flex flex-wrap gap-1 ">
                                ...
                                {noOfMember}/{total_members || 43} ({member_participation !== undefined ? `${member_participation}%` : '34%'})
                            </span>
                            <div className="flex gap-1 items-center">
                                <Tooltip title="KYC Verification : 48% (Placeholder)" className="text-xs">
                                    <div className="border border-gray-300 dark:border-gray-600 rounded-md py-1 px-1.5 flex items-center gap-1">
                                        <MdCheckCircle className="text-green-500 text-lg" />
                                        <span>13/27</span>
                                    </div>
                                </Tooltip>
                                <Tooltip title="Enable Billing (Placeholder)" className="text-xs">
                                    <MdCancel className="text-red-500 text-lg" />
                                </Tooltip>
                            </div>
                            <Tooltip className="text-xs" title={`Profile Completion ${progress}%`}>
    ...
    const handlePaginationChange = useCallback((page: number) => {
        const newTableData = cloneDeep(tableData); newTableData.pageIndex = page; handleSetTableData(newTableData)
    }, [tableData, handleSetTableData])

    const handleSelectChange = useCallback((value: number) => {
        const newTableData = cloneDeep(tableData); newTableData.pageSize = Number(value); newTableData.pageIndex = 1; handleSetTableData(newTableData)
    }, [tableData, handleSetTableData])

    const handleSort = useCallback((sort: OnSortParam) => {
        const newTableData = cloneDeep(tableData); newTableData.sort = sort; handleSetTableData(newTableData)
    }, [tableData, handleSetTableData])

    const handleRowSelect = useCallback((checked: boolean, row: MergedFormItem) => {
        setSelectedForms((prev) =>
            checked ? (prev.some((f) => f.id === row.id) ? prev : [...prev, row]) : prev.filter((f) => f.id !== row.id)
        )
    }, [])

    const handleAllRowSelect = useCallback((checked: boolean, rows: Row<MergedFormItem>[]) => {
        if (checked) setSelectedForms(rows.map((row) => row.original)); else setSelectedForms([])
    }, [])
