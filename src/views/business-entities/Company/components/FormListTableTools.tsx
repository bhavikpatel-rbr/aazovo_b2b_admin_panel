// src/views/your-module/FormListTableTools.tsx
...
import { Button, DatePicker, Drawer, Form as UiForm, FormItem as UiFormItem, Select as UiSelect } from '@/components/ui';
...
export type CustomerListItem = { // Example - replace with your actual item type
    id: string;
...
const filterFormSchema = z.object({
    ...
    filterBusinessType: z.array(z.object({ value: z.string(), label: z.string() })).optional(), // For 'Manufacture', 'Supplier'
    filterCompanyType: z.array(z.object({ value: z.string(), label: z.string() })).optional(), // For 'Private Limited'
    ...
    filterCreatedDate: z.array(z.date().nullable()).optional().default([null, null]), // For DatePickerRange [from, to]
});
...
const businessTypeOptions = [ // For 'Manufacture', 'Supplier'
    { value: "Manufacture", label: "Manufacture" }, { value: "Supplier", label: "Supplier" },
    { value: "Service", label: "Service Provider" }, // Add more as needed
];
const companyTypeOptions = [ // For 'Private Limited' etc.
    { value: "Private Limited", label: "Private Limited" },
    { value: "Charitable Trust", label: "Charitable Trust" },
    { value: "LLP", label: "LLP" }, { value: "Partnership", label: "Partnership" },
];
const continentOptionsList = [ /* Your continent options */
    { value: 'Asia', label: 'Asia' }, { value: 'Europe', label: 'Europe' }, // etc.
];
const countryOptionsList = [ /* Your country options, ideally dynamic */
    { value: 'India', label: 'India' }, { value: 'USA', label: 'USA' }, // etc.
];
const stateOptionsList = [ /* Your state options, ideally dynamic */
    { value: 'Gujarat', label: 'Gujarat' }, { value: 'New York', label: 'New York' }, // etc.
];
const cityOptionsList = [ /* Your city options, ideally dynamic */
    { value: 'Ahmedabad', label: 'Ahmedabad' }, { value: 'New York City', label: 'New York City' }, // etc.
];
...
const FormListTableTools = () => {
    ...
    const { tableData, setTableData, allCustomers, setFilterCriteria: applyFiltersToContext } = useCustomerList();
    ...
    const [localFilterCriteria, setLocalFilterCriteria] = useState<FilterFormData>({});
    ...
    const handleInputChange = (val: string) => {
        ...
        if ((typeof val === 'string' && val.length > 1) || (typeof val === 'string' && val.length === 0) ) {
            setTableData(newTableData);
        }
    };
    ...
    const dynamicBrandOptions = useMemo(() => {
        ...
        return Array.from(new Set(allBrands)).map(brand => ({ value: brand, label: brand }));
    }, [customersData]);

    const dynamicCategoryOptions = useMemo(() => {
        if (!customersData || customersData.length === 0) return [];
        const allCategories = customersData.flatMap(item => item.categories || []);
        return Array.from(new Set(allCategories)).map(cat => ({ value: cat, label: cat }));
    }, [customersData]);

    const dynamicSubCategoryOptions = useMemo(() => {
        if (!customersData || customersData.length === 0) return [];
        const allSubCategories = customersData.flatMap(item => item.sub_categories || []);
        return Array.from(new Set(allSubCategories)).map(subCat => ({ value: subCat, label: subCat }));
    }, [customersData]);
    // --- End Dynamic options ---
    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <FormListSearch onInputChange={(e) => handleInputChange(e.target.value)} />
            ...
            >
                <div className="p-4 overflow-y-auto h-[calc(100vh-130px)]"> {/* Scrollable content area */}
                    <UiForm id="filterFormInTools" onSubmit={filterFormMethods.handleSubmit(onApplyFiltersSubmit)} className="flex flex-col gap-y-5">
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='Status'>
                                <Controller name="filterStatus" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select Status" options={statusOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='Business Type'> {/* Manufacture, Supplier */}
                        ...
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='Continent'>
                                <Controller name="filterContinent" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select Continent" options={continentOptionsList}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='Country'>
                                 <Controller name="filterCountry" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select Country" options={countryOptionsList}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                        </div>
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='State'>
                                <Controller name="filterState" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select State" options={stateOptionsList}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='City'>
                                <Controller name="filterCity" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select City" options={cityOptionsList}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                        </div>
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='Interested For'>
                                <Controller name="filterInterestedFor" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Interested For" options={interestedForOptionsList}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='Brand'>
                                <Controller name="filterBrand" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Select Brand" options={dynamicBrandOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                        </div>
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='Category'>
                                <Controller name="filterCategory" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Category" options={dynamicCategoryOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='Sub Category'>
                                <Controller name="filterSubCategory" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Sub Category" options={dynamicSubCategoryOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                        </div>
                        <div className='md:grid grid-cols-2 gap-x-4'>
                            <UiFormItem label='KYC Verified'>
                                <Controller name="filterKycVerified" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="KYC Verified" options={kycOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                            <UiFormItem label='Enable Billing'>
                                 <Controller name="filterEnableBilling" control={filterFormMethods.control}
                                    render={({ field }) => <UiSelect isMulti placeholder="Enable Billing" options={billingOptions}
                                                                value={field.value || []} onChange={val => field.onChange(val || [])} />}
                                />
                            </UiFormItem>
                        </div>
                         <UiFormItem label='Company Type'> {/* e.g. Private Limited */}
                            <Controller name="filterCompanyType" control={filterFormMethods.control}
                                render={({ field }) => <UiSelect isMulti placeholder="Select Company Type" options={companyTypeOptions}
                                                            value={field.value || []} onChange={val => field.onChange(val || [])} />}
                            />
                        </UiFormItem>
