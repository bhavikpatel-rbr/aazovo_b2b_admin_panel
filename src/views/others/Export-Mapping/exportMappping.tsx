            ...
            >
                <div className="px-1">
                    ...
                    <p className="mb-2 mt-1">
                        <span className="font-semibold text-black dark:text-white">Module:{' '}</span>
                    ...
                    <Card className="!mt-8 bg-gray-100 dark:bg-gray-700 border-none">
                        ...
                        <p className="mt-2">
                            ...
                            <span>
                                {!isNaN(data.exportDate.getTime())
                                    ? data.exportDate.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                                    : 'Invalid Date'}
                            </span>
                        </p>
                        <p className="mt-1">
                        ...
                        {data.reason && (
                            <>
                                <h6 className="text-sm font-semibold text-black dark:text-white mt-2">Reason:</h6>
    ...
    const toggleColumn = (checked: boolean, colHeader: string) => {
        if (checked) {
            setFilteredColumns((currentCols) => {
                const newVisibleHeaders = [...currentCols.map((c) => c.header as string), colHeader]
                return columns.filter((c) => newVisibleHeaders.includes(c.header as string))
            })
        } else {
            setFilteredColumns((currentCols) => currentCols.filter((c) => c.header !== colHeader))
        }
    }
    ...
    return (
        <div className="md:flex items-center justify-between w-full gap-2">
                        ...
                        {columns.map((col) => col.header && (
                            <div key={col.header as string} className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md py-1.5 px-2">
                                <Checkbox
                                    ...
                                    onChange={(checked) => toggleColumn(checked, col.header as string)}
                                />
                                {col.header}
                            </div>
                        ))}
            ...
            <Drawer
                ...
                footer={
                    <div className="text-right w-full ">
                        <Button size="sm" className="mr-2" onClick={handleClearFormInDrawer} type="button">Clear</Button>
                        <Button size="sm" variant="solid" type="submit" form="filterForm">Apply</Button>
                    </div>
                }
            >
                <Form id="filterForm" size="sm" onSubmit={handleSubmit(exportFiltersSubmitHandler)}>
                    <FormItem label="User Role">
                        <Controller control={control} name="userRole" render={({ field }) => (
                            <Select isMulti placeholder="Select roles" options={userRoles} value={userRoles.filter((option) => field.value?.includes(option.value))} onChange={(selected) => field.onChange(selected ? selected.map((opt) => opt.value) : [])} isLoading={!isDataReady && userRoles.length === 0} />
                        )} />
                    </FormItem>
                    <FormItem label="Exported From (Module)">
                        <Controller control={control} name="exportFrom" render={({ field }) => (
                            <Select isMulti placeholder="Select modules" options={exportFromOptions} value={exportFromOptions.filter((option) => field.value?.includes(option.value))} onChange={(selected) => field.onChange(selected ? selected.map((opt) => opt.value) : [])} isLoading={!isDataReady && exportFromOptions.length === 0} />
                        )} />
                    </FormItem>
                    <FormItem label="File Type (Extension)">
                        <Controller control={control} name="fileExtensions" render={({ field }) => (
                            <Select isMulti placeholder="Select file types" options={fileExtensionsOptions} value={fileExtensionsOptions.filter((option) => field.value?.includes(option.value))} onChange={(selected) => field.onChange(selected ? selected.map((opt) => opt.value) : [])} />
                        )} />
                    </FormItem>
                    <FormItem label="Export Date Range">
                        <Controller control={control} name="exportDate" render={({ field }) => (
                            <DatePicker.DatePickerRange placeholder="Select date range" value={field.value ?? undefined} onChange={(dateRange) => field.onChange(dateRange)} inputFormat="MM/DD/YYYY" />
                        )} />
                    </FormItem>
                </Form>
            </Drawer>
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 mb-4  border-b border-gray-200 dark:border-gray-700 pb-4">
            ...
            {activeRoles.map((role) => (
                <Tag key={`role-${role}`} prefix className="bg-gray-100 text-gray-600 dark:bg-gray-600 dark:text-gray-100 border border-gray-300 dark:border-gray-500">
                    Role: {role}
                    <TbX className="ml-1 h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => onRemoveFilter('userRole', role)} />
                </Tag>
            ))}
            {activeModules.map((module) => (
                <Tag key={`module-${module}`} prefix className="bg-gray-100 text-gray-600 dark:bg-gray-600 dark:text-gray-100 border border-gray-300 dark:border-gray-500">
                    Module: {module}
                    <TbX className="ml-1 h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => onRemoveFilter('exportFrom', module)} />
                </Tag>
            ))}
            {activeExtensions.map((ext) => (
                <Tag key={`ext-${ext}`} prefix className="bg-gray-100 text-gray-600 dark:bg-gray-600 dark:text-gray-100 border border-gray-300 dark:border-gray-500">
                    Type: {ext}
                    <TbX className="ml-1 h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => onRemoveFilter('fileExtensions', ext)} />
                </Tag>
            ))}
            {activeDateRange && (activeDateRange[0] || activeDateRange[1]) && (
                <Tag key="date-range-filter" prefix className="bg-gray-100 text-gray-600 dark:bg-gray-600 dark:text-gray-100 border border-gray-300 dark:border-gray-500">
                    Date: {formatDate(activeDateRange[0])} - {formatDate(activeDateRange[1])}
                    <TbX className="ml-1 h-3 w-3 cursor-pointer hover:text-red-500" onClick={() => onRemoveFilter('exportDate', '')} />
                </Tag>
            )}
...
const ExportMapping = () => {
    ...
    const toolsRef = useRef<{ resetForm: () => void } | null>(null);
    ...
    const activeFilterCount = useMemo(() => {
        let count = 0
        if (activeFilters.userRole && activeFilters.userRole.length > 0) count++
        if (activeFilters.exportFrom && activeFilters.exportFrom.length > 0) count++
        if (activeFilters.fileExtensions && activeFilters.fileExtensions.length > 0) count++
        if (activeFilters.exportDate && (activeFilters.exportDate[0] || activeFilters.exportDate[1])) count++
        return count
    }, [activeFilters])
    ...
    const handleRemoveFilter = useCallback((key: keyof ExportMappingFilterSchema, value: string) => {
        setActiveFilters((prev) => {
            ...
            if (newValues.length > 0) { (newFilters as any)[key] = newValues } else { delete newFilters[key] }
            return newFilters
        })
        handleSetTableData({ pageIndex: 1 })
    }, [handleSetTableData])
    ...
    const handleConfirmExportWithReason = async (data: ExportReasonFormData) => {
        ...
        try {
            ...
            const dataToExport = allFilteredAndSortedData.map((item) => ({
                id: item.id, userName: item.userName, userRole: item.userRole, exportFrom: item.exportFrom,
                fileName: item.fileName, reason: item.reason || '',
                exportDate: !isNaN(item.exportDate.getTime()) ? item.exportDate.toISOString() : 'Invalid Date',
            }))
    ...
    const csvHeaders = useMemo(() => [
        { label: 'Record ID', key: 'id' }, { label: 'User Name', key: 'userName' },
        { label: 'User Role', key: 'userRole' }, { label: 'Exported From Module', key: 'exportFrom' },
        { label: 'File Name', key: 'fileName' }, { label: 'Reason', key: 'reason' },
    ...
    const columns: ColumnDef<ExportMappingItem>[] = useMemo(() => [
            ...
            cell: (props) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <Avatar src={profile_pic_path || userIconPlaceholder} size="sm" shape="circle" className="cursor-pointer hover:ring-2 hover:ring-indigo-500" onClick={() => openImageViewer(profile_pic_path || userIconPlaceholder)} icon={<TbUserCircle />} />
                        <div>
                ...
                return (
                    <div className="flex flex-col">
                        ...
                        <Tooltip title={fileName} placement="top">
                            <span className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[180px] block">{fileName || 'N/A'}</span>
                        </Tooltip>
                    </div>
                )
            },
        },
        {
            ...
            cell: (props) => (<Tooltip title={props.row.original.reason || ''} placement="top"><span className="truncate block max-w-[230px] text-justify">{props.row.original.reason || '–'}</span></Tooltip>),
        },
        {
            header: 'Date', accessorKey: 'exportDate', enableSorting: true, size: 220,
            cell: (props) => {
                const date = new Date(props.row.original.exportDate)
                return (<span className="text-sm">{!isNaN(date.getTime()) ? `${date.getDate()} ${date.toLocaleString('en-US', { month: 'short' })} ${date.getFullYear()}, ${date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}` : 'Invalid Date'}</span>)
            },
        },
        {
        ...
        <>
                ...
                <AdaptiveCard className="h-full" bodyClass="h-full flex flex-col">
                    ...
                    {(activeFilterCount > 0 || tableData.query) && isDataReady && (
                        <div className="mb-4 text-sm text-gray-600 dark:text-gray-300">
                            Found <strong>{total}</strong> matching record(s).
                        </div>
                    )}
            ...
            <Dialog isOpen={isImageViewerOpen} onClose={closeImageViewer} onRequestClose={closeImageViewer} shouldCloseOnOverlayClick={true} shouldCloseOnEsc={true} width={600}>
                <div className="flex justify-center items-center p-4">
                    {imageToView ? (<img src={imageToView} style={{ maxWidth: "100%", maxHeight: "80vh", objectFit: "contain" }} />) : (<p>No image to display.</p>)}
                </div>
            </Dialog>
            <ConfirmDialog
