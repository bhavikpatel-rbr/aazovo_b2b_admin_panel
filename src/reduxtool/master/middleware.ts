...
export const submitExportReasonAction = createAsyncThunk<any, any>("auth/addpartner",
  async (data, { rejectWithValue, dispatch }) => {
    try {
      const response: AxiosResponse<any> = await submitExportAsync(data)
      if (response?.data?.status === true) {
